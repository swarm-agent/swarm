...
export function SwarmToolSidebar({
  ...
  return (
    <aside className={layoutClassName ?? 'flex min-h-0 w-full shrink-0 flex-1 flex-col overflow-hidden py-2 font-mono text-[12px] text-[var(--app-text-muted)] lg:mr-5 lg:w-[276px] lg:flex-none lg:border-r lg:border-[var(--app-border)] lg:py-5 lg:pl-3 lg:pr-4'}>
      ...
      {workspaceSlug && <a className="mb-3 block px-2 py-2 hover:bg-[var(--app-surface-hover)] focus-visible:outline-2" href={`/${encodeURIComponent(workspaceSlug)}/automations`}>Automations</a>}
      ...
      <div className={`${prioritizeChildren ? 'order-5' : ''} mt-4 shrink-0 border-y border-[var(--app-border)] py-3`}>
      ...
      {children ? <div className={`${prioritizeChildren ? 'order-1' : ''} mt-3 shrink-0`}><div className={childrenClassName ?? 'hidden lg:contents'}>{children}</div></div> : null}
      </div>
      {actions.length > 0 ? (
        <div className={`${prioritizeChildren ? 'order-6' : ''} ${compactSelectedSession ? 'mt-3' : 'mt-4'} shrink-0 flex flex-col gap-1 border-b border-[var(--app-border)] pb-3`}>
          {actions.map((action) => (
