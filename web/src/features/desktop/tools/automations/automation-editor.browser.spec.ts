...
test('configuration uses accessible inputs and refuses stale draft submission', { timeout: 30000 }, async () => {
  ...
  try {
    ...
    assert.equal(saved.schedule.timezone, undefined)
    ...
    // Routine daily edits use wall-time controls, never hand-authored JSON.
    await page.getByLabel('Trigger', { exact: true }).selectOption('cron')
    await page.getByLabel('Timezone (IANA)', { exact: true }).fill('America/New_York')
    await page.getByLabel('Daily wall time').fill('09:30')
    await page.getByRole('button', { name: 'Save configuration' }).click()
    await page.waitForFunction(() => (window as any).calls.length === 2)
    assert.deepEqual(await page.evaluate(() => (window as any).calls[1].schedule), { kind: 'cron', timezone: 'America/New_York', expression: '30 9 * * *', missed_policy: 'skip', overlap_policy: 'independent' })
    await page.getByLabel('Trigger', { exact: true }).selectOption('interval')
    await page.getByLabel('Interval seconds', { exact: true }).fill('1')
    await page.getByRole('button', { name: 'Save configuration' }).click()
    ...
    assert.equal(await page.evaluate(() => (window as any).calls[2].schedule.timezone), undefined)
    ...
    assert.equal(await page.evaluate(() => (window as any).calls.length), 3)
