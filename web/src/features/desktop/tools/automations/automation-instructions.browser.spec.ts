...
test('instruction editor refuses foreign source and pins only approved results', { timeout: 30000 }, async () => {
  const bundle = await build({ stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
    ...
    window.pins=[];window.calls=[];window.approved=false;window.digest='stale';window.raw={id:'old',info:{goal:'Old objective'},checkpoints:[{id:'check',tasks:['Retained task']}]};
  ...
  ` }, bundle: true, write: false, platform: 'browser', format: 'iife', jsx: 'automatic', logLevel: 'silent', plugins: [{ name: 'boundaries', setup(b) {
    ...
    b.onLoad({ filter: /.*/, namespace: 'fixture' }, args => ({ contents: args.path === 'api' ? `export const requestJson=async(url,init)=>{window.calls.push({url,init});if(!init)return {document_sha256:window.digest,plan:{id:'old',version:2,document:window.raw}};const body=JSON.parse(init.body);return {document_sha256:'digest',plan:{id:body.plan_id,version:1,document:body.document,status:window.approved?'approved':'pending',approval_state:window.approved?'approved':'pending'}};};` : args.path === 'automation-plan-approval' ? `export const requireAutomationDocumentDigest=value=> {if(value!=='digest')throw Error('Missing digest');return value};` : `export const automationControl='';` }))
  } }] })
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Edit executable instructions' }).click()
    await page.getByRole('alert').filter({ hasText: 'Missing digest' }).waitFor()
    assert.equal(await page.evaluate(() => (window as any).pins.length), 0)
    assert.equal(await page.evaluate(() => (window as any).calls.filter((call: any) => call.init).length), 0)
    await page.evaluate(() => { (window as any).digest = 'digest' })
    await page.getByRole('button', { name: 'Edit executable instructions' }).click()
    await page.getByRole('textbox', { name: 'Plan summary' }).fill('New objective')
    await page.getByRole('textbox', { name: 'Checkpoint 1 objective' }).fill('Execute changed instructions')
    ...
    assert.deepEqual(saved.document.checkpoints, [{ id: 'check', tasks: ['Retained task'], objective: 'Execute changed instructions' }])
