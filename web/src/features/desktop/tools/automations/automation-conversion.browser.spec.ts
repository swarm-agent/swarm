...
test('typed conversion preserves intent and separates approval from enable', { timeout: 30000 }, async () => {
  const bundle = await build({ stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
    ...
    createRoot(window.document.getElementById('root')).render(<><StructuredPlanReviewView document={document}/><AutomationPlanApproval document={document} parentSessionId='parent'/><div>Retained artifact</div></>);
