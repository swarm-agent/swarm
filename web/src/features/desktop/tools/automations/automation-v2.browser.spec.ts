...
test('V2 card edits exact snapshots and reconciles accepted discovery without reload', { timeout: 45000 }, async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Deploy Worker', exact: true }).waitFor()
    ...
    assert.equal(await page.getByRole('button', { name: 'Deploy Worker', exact: true }).count(), 0)
    ...
    await page.getByRole('button', { name: 'Save schedule changes' }).click()
    await page.getByRole('button', { name: 'Deploy Worker', exact: true }).waitFor().catch(async error => { console.error((await page.locator('body').innerText()).slice(-2500)); throw error })
    ...
    await page.getByRole('button', { name: 'Deploy Worker', exact: true }).dblclick()
    await page.getByRole('button', { name: 'Worker deployed' }).waitFor()
    // Requirement: acceptance discloses scheduling, never an immediate run.
    await page.getByRole('status', { name: 'Worker handoff' }).getByText('Deployment did not start a run.', { exact: false }).waitFor()
    ...
    const details = page.getByRole('region', { name: 'Worker details', exact: true })
