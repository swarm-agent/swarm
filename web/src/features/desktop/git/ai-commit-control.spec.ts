...
test('AI Commit split control exposes generation and an accessible post-commit Action menu', async () => {
  ...
  assert.match(source, /className="fixed z-\[100\][^\n]*style=\{\{ bottom: menuPosition\.bottom, right: menuPosition\.right \}\} data-menu-direction="up"/)
  assert.match(source, /bottom: Math\.max\(8, window\.innerHeight - bounds\.top \+ 8\)/)
  assert.doesNotMatch(source, /top: bounds\.bottom \+ 8/)
  ...
  assert.match(source, /actions\.length > 5 && 'max-h-\[240px\] overflow-y-auto/)
  assert.match(source, /data-action-list-scroll=\{actions\.length > 5 \? 'conditional' : undefined\}/)
  assert.match(source, /title=\{actionOptionsPreview\(action\)\}>\{actionOptionsPreview\(action\)\}<\/span>/)
  assert.doesNotMatch(source, /actionsOnly \? 'Run' : selectedAction\?\.id === action\.id \? 'Selected' : 'Select'/)
  assert.doesNotMatch(source, /action\.description/)
})

test('Action menu opens upward as a compact name-and-option list and scrolls after five entries', async () => {
  const source = await readFile(controlURL, 'utf8')

  assert.match(source, /data-menu-direction="up"/)
  assert.match(source, /bottom: Math\.max\(8, window\.innerHeight - bounds\.top \+ 8\)/)
  assert.match(source, /actions\.length > 5 && 'max-h-\[240px\] overflow-y-auto/)
  assert.match(source, /<span className="flex min-w-0 flex-1 flex-col items-start justify-center leading-tight">/)
  assert.match(source, /<strong className="w-full truncate text-left font-medium">\{action\.name\}<\/strong>/)
  assert.match(source, /function actionOptionsPreview\(action: WorkspaceAction\): string/)
  assert.match(source, /const options = \[\.\.\.action\.arguments\]/)
  assert.match(source, /options\.push\(\.\.\.input\.arguments, `<\$\{input\.label\}>`\)/)
  assert.match(source, /return options\.length > 0 \? options\.join\(' '\) : 'No options'/)
  assert.match(source, /<span className="mt-1[^\n]*title=\{actionOptionsPreview\(action\)}>\{actionOptionsPreview\(action\)\}<\/span>/)
  assert.doesNotMatch(source, /actionOptionsPreview[\s\S]*action\.entrypoint/)
  assert.doesNotMatch(source, /actionsOnly \? 'Run' : selectedAction\?\.id === action\.id \? 'Selected' : 'Select'/)
  assert.doesNotMatch(source, /action\.description/)
})
