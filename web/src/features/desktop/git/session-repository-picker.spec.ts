...
test('repository picker renders grouped retained identity and honest stale/error state', () => {
  ...
  for (const text of ['Project', '/project/worker', 'worker', 'failed', 'Retained', 'Authorization expired', 'Git status could not be updated', 'Refresh failed', 'Selected repository is not', 'Git repositories']) assert.ok(html.includes(text), text)
