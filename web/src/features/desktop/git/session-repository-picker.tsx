...
export function SessionRepositoryPicker({ inventory, onSelect, onRefresh, onLoadMore }: {
  ...
  return <div data-testid="session-repository-inventory" className="mb-2 min-h-0 shrink-0 text-xs">
    ...
    <div data-testid="repository-scroll-region" className="mb-2 max-h-80 overflow-y-auto overflow-x-hidden border-b border-[var(--app-border)] pb-2">
      {[...groups].map(([key, rows]) => <fieldset key={key} className="my-2 min-w-0 border-t border-[var(--app-border)]">
        ...
        {rows.map(row => <button type="button" key={repositoryKey(row)} aria-label={`${row.kind} · ${row.branch || row.status?.branch || 'No branch'} · ${row.workspace_path}`} aria-pressed={inventory.selectedKey === repositoryKey(row)} onClick={() => onSelect(repositoryKey(row))} className="my-1 block w-full rounded border border-[var(--app-border)] p-2 text-left aria-pressed:border-[var(--app-primary)]">
          <span className="flex flex-wrap items-center justify-between gap-1 font-medium"><span className="min-w-0 break-all">{row.branch || row.status?.branch || 'No branch'}</span><span className="text-[10px] text-[var(--app-text-muted)]">{row.kind === 'parent' ? 'Session worktree' : row.kind === 'source' ? 'Source checkout' : row.kind === 'worker' ? 'Worker worktree' : 'Repository lane'}</span></span>
          <span title={row.workspace_path} className="block truncate text-[10px] text-[var(--app-text-muted)]">{row.workspace_path}</span>
          <span className="block break-words text-[10px] text-[var(--app-text-muted)]">{row.active ? 'Active execution lane' : row.retained ? 'Retained lane · Inspection' : 'Inspection'}{row.lifecycle ? ` · ${row.lifecycle}` : ''}</span>
          {row.program_id ? <span className="block break-all text-[10px] text-[var(--app-text-muted)]">Program {row.program_id}{row.job_id ? ` · Job ${row.job_id}` : ''}</span> : null}
