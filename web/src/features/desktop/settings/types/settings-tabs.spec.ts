import assert from 'node:assert/strict'
import test from 'node:test'

import { SETTINGS_TABS, normalizeSettingsTabID } from './settings-tabs'

test('account settings is the first/default settings tab', () => {
  assert.equal(SETTINGS_TABS[0], 'account')
  assert.equal(SETTINGS_TABS[3], 'behavior')
  assert.equal(normalizeSettingsTabID(undefined), 'account')
  assert.equal(normalizeSettingsTabID('not-a-tab'), 'account')
  assert.equal(normalizeSettingsTabID('behavior'), 'behavior')
  assert.equal(normalizeSettingsTabID('notifications'), 'notifications')
  assert.equal(normalizeSettingsTabID('media'), 'media')
  assert.equal(normalizeSettingsTabID('images'), 'media')
  assert.equal(normalizeSettingsTabID('quick-actions'), 'shortcuts')
  assert.equal(normalizeSettingsTabID('quick_actions'), 'shortcuts')
  assert.equal(normalizeSettingsTabID('models'), 'actions')
  assert.equal(normalizeSettingsTabID('actions'), 'actions')
  assert.equal(SETTINGS_TABS.includes('models' as never), false)
  assert.equal(normalizeSettingsTabID('tailscale'), 'tailscale')
  assert.equal(SETTINGS_TABS.includes('agents' as never), false)
  assert.equal(normalizeSettingsTabID('agents'), 'account')
  assert.equal(SETTINGS_TABS.includes('tailscale'), true)
  assert.equal(SETTINGS_TABS.includes('cloud'), true)
  assert.equal(normalizeSettingsTabID('cloud'), 'cloud')
  assert.equal(normalizeSettingsTabID('s3'), 'cloud')
  assert.equal(normalizeSettingsTabID('gcp'), 'cloud')
  assert.equal(normalizeSettingsTabID('storage'), 'cloud')
  assert.equal(SETTINGS_TABS.includes('media'), true)
  assert.equal(SETTINGS_TABS.includes('images' as never), false)
})
