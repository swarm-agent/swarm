...
test('behavior settings is the first/default settings tab', () => {
  assert.equal(SETTINGS_TABS[0], 'behavior')
  assert.equal(normalizeSettingsTabID(undefined), 'behavior')
  assert.equal(normalizeSettingsTabID('not-a-tab'), 'behavior')
