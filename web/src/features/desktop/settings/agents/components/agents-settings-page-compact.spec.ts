...
assert.match(source, /System Agents[\s\S]*Swarm[\s\S]*Compact[\s\S]*Explorer[\s\S]*Clone/, 'agent settings must list compiled agents in a dedicated system-agent section')
assert.match(source, /System primary[\s\S]*Immutable built-in primary/, 'agent settings must identify Swarm as the immutable system primary')
assert.match(source, /Clone[\s\S]*Inherits the parent session provider, model, thinking, and service tier/, 'agent settings must explain Clone model and priority inheritance')
