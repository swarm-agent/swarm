...
import { BASH_APPROVAL_PROFILES, normalizeBashApprovalProfile, sessionMutationDecision, type PermissionRule } from './permissions-settings-page'
...
test('Bash approvals expose exactly four mutually exclusive profiles with safe reads recommended', () => {
  ...
  assert.deepEqual(BASH_APPROVAL_PROFILES.map((profile) => profile.description), [
    'Use granular rules and the existing default Bash behavior.',
...
test('Bash approvals render immediately below global permissions as disabled radio cards while OFF', () => {
  ...
  const granularIndex = settingsSource.indexOf('Granular permission rules', bashIndex)
  assert(globalIndex >= 0 && bashIndex > globalIndex && granularIndex > bashIndex)
...
test('Bash profile guidance explains granular rule precedence', () => {
  assert.match(settingsSource, /Granular rules stay active in every profile/)
  assert.match(settingsSource, /Deny rules override profile auto-approvals/)
  assert.match(settingsSource, /Allow and Ask rules apply when the profile leaves an operation undecided/)
  assert.match(settingsSource, /Profile safety prompts still win over ordinary Allow rules/)
  assert.match(settingsSource, /Allow every read intentionally does not stop critical reads/)
})

test('granular controls remain present and disabled without mutation while permissions are OFF', () => {
