...
import { currentCheckoutCommitCandidate, reviewWorktreeReasonLabel, selectableDoneIDs, selectedArchiveCandidates } from './needs-review-cleanup-panel'
...
describe('needs review cleanup panel helpers', () => {
  ...
  it('only exposes done candidates for archive selection', () => {
    const result: ReviewWorktreesResponse = { ok: true, target_detection: '', comparison: '', retained: [candidate({ session_id: 'keep' })], done: [candidate({ session_id: 'done', classification: 'done', reason: 'clean_and_integrated', worktree_branch: 'agent/done', worktree_path: '/worktrees/done', done_at: 10, archive_after: 3_600_010 })], archived_session_ids: [], recently_archived: [], grace_period_ms: 3_600_000, checkout_dirty: true, checkout_dirty_count: 2, blocked_by_checkout_count: 1, complete: true }
    expect(selectableDoneIDs(result)).toEqual(['done'])
    expect(selectedArchiveCandidates(result, new Set(['keep', 'done']))).toEqual([expect.objectContaining({ session_id: 'done', worktree_branch: 'agent/done', worktree_path: '/worktrees/done' })])
  })
