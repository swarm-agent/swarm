...
test('Git sidebar recognizes the missing-Git rev-parse failure and builds an install request', () => {
  ...
  const prompt = buildInstallGitPrompt(error)
  assert.match(prompt, /Install Git on this machine, then retry the Git feature/)
  assert.match(prompt, /detect the Linux distribution/)
  assert.match(prompt, /request any required Bash or privilege permission/)
  assert.match(prompt, /verify `git --version`/)
  assert.match(prompt, /workspace itself is still usable/)
...
// Purpose: guard DesktopAppPage's integration-help wiring against creating/sending a
// repair session. This narrow source-contract check proves callback/prop wiring only;
// interactive navigation, focus and draft delivery still require a browser test.
test('plan Git sidebar renders session commits and drafts integration help in the existing session', async () => {
  const source = await readFile(new URL('./desktop-app-page.tsx', import.meta.url), 'utf8')
  const panelStart = source.indexOf('const planSidebarGitPanel =')
  const panelEnd = source.indexOf('const focusedSidebarContent =', panelStart)
  const panelSource = source.slice(panelStart, panelEnd)
  ...
  const helpHandlerSource = source.slice(source.indexOf('const handleAskSwarmForGitIntegrationHelp'), source.indexOf('const gitSidebarError'))
  assert.match(helpHandlerSource, /modal\.integrationComplete \|\| !integrationError/)
  assert.match(helpHandlerSource, /openIntegrationHelpDraft\(modal\.sessionId, modal\.workspacePath, buildGitSidebarIntegrationHelpPrompt\(modal, integrationError\)\)/)
  assert.match(helpHandlerSource, /Could not ask Swarm for integration help/)
  assert.match(helpHandlerSource, /Integration error added to the existing session/)
  const draftHandlerSource = source.slice(source.indexOf('const openIntegrationHelpDraft'), source.indexOf('const reviewFixAvailable'))
  assert.match(draftHandlerSource, /if \(!sessionId\) throw new Error/)
  assert.match(draftHandlerSource, /await navigate\(\{ to: '\/\$workspaceSlug\/\$sessionId', params: \{ workspaceSlug, sessionId \} \}\)/)
  assert.match(draftHandlerSource, /setIntegrationHelpDraft\(\{ id: \+\+integrationHelpRequestIdRef\.current, sessionId, draft: prompt, append: true \}\)/)
  assert.match(draftHandlerSource, /setComposerFocusSignal/)
  assert.match(source, /composerDraftRequest=\{integrationHelpDraft\?\.sessionId === routeSessionId \? integrationHelpDraft : undefined\}/)
  assert.match(source, /onComposerDraftRequestHandled=\{handleIntegrationHelpDraftHandled\}/)
  const paneSource = await readFile(new URL('../chat/components/desktop-v3-existing-conversation-pane.tsx', import.meta.url), 'utf8')
  const composerSource = paneSource.slice(paneSource.indexOf('export function DesktopV3ExistingConversationComposer'), paneSource.indexOf('export function DesktopV3ExistingConversationPane('))
  assert.match(composerSource, /draftRequest\.id === handledDraftRequestRef\.current/)
  assert.match(composerSource, /draftRequest\.append && current\.trim\(\)/)
  assert.ok(composerSource.includes('`${current}\\n\\n${draftRequest.draft}`'))
  assert.match(composerSource, /onDraftRequestHandled\?\.\(draftRequest\.id\)/)
  assert.doesNotMatch(composerSource, /onSubmit\(/)
  assert.match(source, /current\?\.id === id \? undefined : current/)
  const reviewHandlerSource = source.slice(source.indexOf('const handleAskSwarmToFixReviewIntegration'), source.indexOf('  useEffect(() => {', source.indexOf('const handleAskSwarmToFixReviewIntegration')))
  assert.match(reviewHandlerSource, /openIntegrationHelpDraft\(failure\.candidate\.session_id, topWorkspacePath, buildReviewWorktreeFixPrompt\(failure, topWorkspacePath\)\)/)
