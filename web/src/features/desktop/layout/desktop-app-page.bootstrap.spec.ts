...
test('DesktopAppPage leaves Desktop V3 runtime ownership to the root provider while route selectors only select sessions', async () => {
  ...
  assert.match(source, /if \(!uiSettingsQuery\.data\) \{[\s\S]*New-session settings are still loading/)
  assert.match(source, /const configuredMode = normalizeDefaultNewSessionMode\(uiSettingsQuery\.data\.chat\?\.default_new_session_mode\)/)
  assert.match(source, /createDesktopV3CreateOnlySessionOperation\(\{[\s\S]*mode: configuredMode/)
