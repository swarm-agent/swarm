...
test('DesktopAppPage leaves Desktop V3 runtime ownership to the root provider while route selectors only select sessions', async () => {
  ...
  assert.doesNotMatch(source, /selectAndHydrateDesktopV3Session\(sessionId\)[\s\S]{0,200}requireDesktopV3RealtimeControllerReady/)
  assert.doesNotMatch(source, /selectAndHydrateDesktopV3Session\(sessionId\)[\s\S]{0,200}ensureSessionConnected/)
