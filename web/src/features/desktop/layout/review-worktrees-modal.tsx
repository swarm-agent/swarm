...
export function reviewWorktreeReasonLabel(item: ReviewWorktreeCandidate): string {
  switch (item.reason) {
    case 'inspection_pending': return 'Checking Git status and integration…'
...
export function selectableReviewIDs(result: ReviewWorktreesResponse | null): string[] {
  return result?.retained.filter((item) => item.reason !== 'inspection_pending').map((item) => item.session_id) ?? []
}
...
export function selectedArchiveCandidates(result: ReviewWorktreesResponse | null, selected: Set<string>): ReviewWorktreeCandidate[] {
  return result?.retained.filter((item) => item.reason !== 'inspection_pending' && selected.has(item.session_id)) ?? []
}
...
export function ReviewWorktreesModal({ workspacePath, onClose, onAskSwarmFix, repairFixAvailable = false }: { workspacePath?: string; onClose: () => void; onAskSwarmFix?: (failure: ReviewWorktreeIntegrationFailure) => void | Promise<void>; repairFixAvailable?: boolean }) {
  ...
  const integrationAnchorRef = useRef<HTMLDivElement | null>(null)
  const integrationPopoutRef = useRef<HTMLDivElement | null>(null)
  const [integrationPopoutStyle, setIntegrationPopoutStyle] = useState<CSSProperties>({ visibility: 'hidden' })
  const refresh = useCallback(async (automatic = false): Promise<ReviewWorktreesResponse | null> => {
    ...
    setError('')
    try {
      const next = await reviewDesktopV3Worktrees({ workspacePath, automatic, graceHours: 1, signal: controller.signal })
      ...
      if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : 'Could not inspect review worktrees.')
      return null
    } finally {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const catalog = await reviewDesktopV3Worktrees({ workspacePath, catalogOnly: true, signal: controller.signal })
        if (controller.signal.aborted) return
        setResult(catalog)
        const next = await reviewDesktopV3Worktrees({ workspacePath, graceHours: 1, signal: controller.signal })
        if (controller.signal.aborted) return
        setResult(next)
      } catch (cause) {
        if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : 'Could not inspect review worktrees.')
  ...
  const commitReviewChanges = async (integrateAfterCommit: boolean, archiveAfterIntegration = false) => {
    ...
    try {
      ...
      if (commitAndIntegrate) {
        const refreshed = await reviewDesktopV3Worktrees({ workspacePath, sessionIds: [candidate.session_id], graceHours: 1 })
  ...
  useEffect(() => {
    if ((!activeCommitBatchID && activeReviewCommitJobs.length === 0) || loading || integrating || committing) return
    const timer = window.setTimeout(() => { void refresh(false) }, 1500)
    return () => window.clearTimeout(timer)
  }, [activeCommitBatchID, activeReviewCommitJobs.length, loading, integrating, committing, result, refresh])
  useEffect(() => {
  ...
  const integrateWorktree = async (archiveAfterIntegration = integrationArchiveAfter) => {
    if (!integrateCandidate?.integrate_eligible || integrating) return
    // A pre-mutation read must not overwrite the post-integration refresh.
    reviewRequestRef.current?.abort()
    reviewRequestRef.current = null
    setLoading(false)
    setIntegrating(true)
    ...
    try {
      ...
      // Git has completed; a full-list refresh is not part of integration.
      void refresh(false)
    } catch (cause) {
  ...
  const closeIntegrationReview = useCallback(() => {
  ...
  const positionIntegrationPopout = useCallback(() => {
    const anchor = integrationAnchorRef.current
    const popout = integrationPopoutRef.current
    ...
    setIntegrationPopoutStyle({ left, top, width, maxHeight, visibility: 'visible' })
  }, [])
  useLayoutEffect(() => {
    if (!integrateCandidate) return
    positionIntegrationPopout()
    const frame = window.requestAnimationFrame(positionIntegrationPopout)
    const observer = typeof ResizeObserver === 'undefined' ? null : new ResizeObserver(positionIntegrationPopout)
    if (integrationPopoutRef.current) observer?.observe(integrationPopoutRef.current)
    window.addEventListener('resize', positionIntegrationPopout)
    window.addEventListener('scroll', positionIntegrationPopout, true)
    return () => {
      window.cancelAnimationFrame(frame)
      observer?.disconnect()
      window.removeEventListener('resize', positionIntegrationPopout)
      window.removeEventListener('scroll', positionIntegrationPopout, true)
    }
  }, [integrateCandidate, integrationArchiveError, integrationFailure, integrationSucceeded, positionIntegrationPopout, showIntegrationError])
  useEffect(() => {
    if (!integrateCandidate) return
    const dismissOnOutsidePointer = (event: PointerEvent) => {
      const target = event.target as Node | null
      if (target && (integrationAnchorRef.current?.contains(target) || integrationPopoutRef.current?.contains(target))) return
      closeIntegrationReview()
    }
    const dismissOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') closeIntegrationReview()
    }
  ...
  }, [closeIntegrationReview, integrateCandidate])
  const askSwarmToFixIntegration = () => {
    if (!integrationFailure || !onAskSwarmFix) return
      ...
      <DialogPanel className="w-[min(1120px,calc(100vw-32px))] p-0 shadow-2xl max-sm:h-[100dvh] max-sm:max-h-none max-sm:w-full max-sm:rounded-none max-sm:border-0">
        ...
        <div className="shrink-0 border-b border-[var(--app-border)] px-5 py-3 max-sm:px-[calc(var(--app-safe-area-right)+1rem)] max-sm:pl-[calc(var(--app-safe-area-left)+1rem)]">
          <div className="flex flex-wrap items-center gap-2 text-xs max-sm:grid max-sm:grid-cols-2">
            <button type="button" className="inline-flex min-h-11 touch-manipulation items-center justify-center gap-1.5 rounded-md border border-[var(--app-border)] px-2.5 py-1.5" disabled={loading || integrating || committing || archiving} onClick={() => void refresh(false)}>{loading ? <LoaderCircle size={13} className="animate-spin" /> : <RefreshCcw size={13} />} Recheck worktrees</button>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5 max-sm:px-[calc(var(--app-safe-area-right)+1rem)] max-sm:pb-[calc(var(--app-safe-area-bottom)+1rem)] max-sm:pl-[calc(var(--app-safe-area-left)+1rem)] [-webkit-overflow-scrolling:touch]">
          ...
          {loading ? <p className="mt-3 flex items-center gap-2 text-xs text-[var(--app-text-muted)]" role="status"><LoaderCircle size={13} className="animate-spin" />{result ? 'Checking worktrees. Actions become available after verification.' : 'Loading review sessions…'}</p> : null}
          ...
          <CandidatePile title="Keep in review" icon={<ShieldAlert size={14} />} items={result?.retained ?? []} selected={selected} onToggle={toggleSelected} selectable={reviewingSelection} onCommit={(item) => void openCommitReview(item)} renderIntegrationAction={renderIntegrationAction} />
          ...
          {commitCandidate ? (
            <section className="mt-5 rounded-xl border border-[var(--app-warning)] bg-[var(--app-surface-subtle)] p-4" aria-label={commitCandidate.current_checkout ? 'Commit current checkout review' : 'Commit worktree review'}>
              ...
              <p className="mt-1 text-xs text-[var(--app-text-subtle)]">{commitCandidate.current_checkout ? <>This stages all {commitFiles.length} shown changes and creates one commit in <strong>{commitCandidate.worktree_branch || 'the current branch'}</strong>. It does not archive anything until the commit succeeds and Swarm rechecks the waiting sessions.</> : <>This stages all {commitFiles.length} shown changes and creates one commit in <strong>{commitCandidate.worktree_branch || 'the worktree branch'}</strong>. Choose <strong>Commit only</strong> to leave it there for review, or <strong>Commit and integrate</strong> to safely cherry-pick it into <strong>{commitCandidate.target_branch || 'the target branch'}</strong> in the same action. If either step of the combined action fails, Ask Swarm for Help opens the complete error in the existing session’s message box for you to review and send.</>}</p>
...
function CandidatePile({ title, icon, items, selected, onToggle, selectable, onCommit, renderIntegrationAction, expanded, onExpandedChange }: { title: string; icon: ReactNode; items: ReviewWorktreeCandidate[]; selected: Set<string>; onToggle: (id: string) => void; selectable: boolean; onCommit?: (item: ReviewWorktreeCandidate) => void; renderIntegrationAction?: (item: ReviewWorktreeCandidate) => ReactNode; expanded?: boolean; onExpandedChange?: (expanded: boolean) => void }) {
  const contents = <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{items.length === 0 ? <p className="col-span-full rounded-lg bg-[var(--app-surface-subtle)] p-3 text-xs text-[var(--app-text-muted)]">None</p> : items.map((item) => <WorktreeCard key={item.session_id} item={item} selected={selected} onToggle={onToggle} selectable={selectable} onCommit={onCommit} renderIntegrationAction={renderIntegrationAction} />)}</div>
