...
test('shared sidebar keeps the info row first and Automations directly below Studio', () => {
  ...
  const media = sidebar.indexOf('aria-label="Open Media"')
  const automation = sidebar.indexOf('aria-label="Open Workers"')
  const environments = sidebar.indexOf('aria-label="Open Environments"')
  const usage = sidebar.indexOf('aria-label="Open Usage & Analytics"')
  assert.ok(info >= 0 && info < studio && studio < media && media < automation && automation < environments && environments < usage)
  assert.doesNotMatch(sidebar, /aria-label="Open Desktop quick actions"/)
