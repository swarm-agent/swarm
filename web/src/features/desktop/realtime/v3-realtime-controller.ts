...
import { desktopEnvironments } from '../runtime/desktop-environments-runtime'
...
export class DesktopV3RealtimeControllerRuntime implements DesktopV3RealtimeController {
    ...
    this.transport = new DesktopV3RealtimeTransport({
      ...
      onStatus: ({ status, reason }) => {
        ...
        if (status === 'open') {
          ...
          desktopEnvironments.invalidate()
        }
        const mappedStatus = mapTransportStatus(status)
        desktopEnvironments.onRealtimeStatus(mappedStatus)
        this.dispatch({
  ...
  private async handleFrame(frame: RealtimeMessage): Promise<void> {
    try {
      ...
      desktopEnvironments.acceptFrame(frame)
