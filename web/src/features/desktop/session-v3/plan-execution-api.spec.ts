...
test('startDesktopPlanAutomatic calls the dedicated start-automatic lifecycle endpoint only', async () => {
  ...
  try {
    await startDesktopPlanAutomatic('session-1', 'plan-1', { checkpointId: 'cp-final', executionGranularity: 'run_through', continuationPolicy: 'automatic', continueAutomatically: true })
  } finally {
    globalThis.fetch = originalFetch
  }
  ...
  assert.deepEqual(calls, [{
    ...
    body: { checkpoint_id: 'cp-final', execution_granularity: 'run_through', continuation_policy: 'automatic', continue_automatically: true },
  }])
})
...
test('startDesktopPlanCheckpointed calls the dedicated start-checkpointed lifecycle endpoint only', async () => {
  ...
  assert.deepEqual(calls, [{
    ...
    body: { checkpoint_id: 'cp-2', execution_granularity: 'checkpointed', continuation_policy: 'review_each_checkpoint', continue_automatically: false },
  }])
})
