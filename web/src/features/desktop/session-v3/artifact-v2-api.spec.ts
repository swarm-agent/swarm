...
import { normalizeDesktopV3ArtifactV2CatalogItem, normalizeDesktopV3ArtifactV2Studio } from './artifact-v2-api'
...
test('Artifact V2 catalog preserves working invalid state and bounded diagnostic without V1 fields', () => {
  const item = normalizeDesktopV3ArtifactV2CatalogItem({ schema_version: 1, read_only: true, working, projection })
...
test('Artifact V2 Studio requires explicit read-only authority and rejects mismatched catalog state', () => {
  assert.equal(normalizeDesktopV3ArtifactV2CatalogItem({ schema_version: 1, read_only: true, working, projection: { ...projection, artifact_id: 'foreign' } }), null)
  assert.equal(normalizeDesktopV3ArtifactV2CatalogItem({ schema_version: 1, working, projection }), null)
  const payload = { schema_version: 1, read_only: true, working: { ...working, state: 'ready', latest_diagnostic: undefined }, projection: { ...projection, state: 'ready', latest_diagnostic: undefined }, parts: [{ id: 'hero', artifact_id: 'artv2-1', key: 'hero', label: 'Hero', media_class: 'text/html', order: 0 }, { id: 'footer', artifact_id: 'artv2-1', key: 'footer', label: 'Footer', media_class: 'text/html', order: 1 }], part_revisions: [], compositions: [], builds: [], validations: [], iterations: [], published_heads: [] }
  const studio = normalizeDesktopV3ArtifactV2Studio(payload)
  assert.equal(studio?.readOnly, true)
  assert.equal(normalizeDesktopV3ArtifactV2Studio({ ...payload, read_only: false }), null)
})
