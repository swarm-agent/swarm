import {
  ...
  SESSION_V3_REALTIME_STREAM_PATH,
...
export function openDesktopV3RealtimeTransportSocket(options: DesktopV3RealtimeTransportOpenSocketOptions): WebSocket {
  const endpointCursor = normalizeString(options.endpointCursor)
  if (!endpointCursor) {
    throw new Error('Desktop V3 realtime transport requires endpoint_cursor.')
  }
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
  const url = new URL(SESSION_V3_REALTIME_STREAM_PATH, `${protocol}//${window.location.host}`)
...
export class DesktopV3RealtimeTransport {
  ...
  registerSession(subscription: SessionV3RealtimeSubscriptionRequestWire): void {
    const entry = normalizeSessionSubscription(subscription, this.currentEndpointCursor())
    if (!entry) return
  ...
  private async handleMessage(raw: unknown, generation: number): Promise<void> {
    ...
    this.noteWorksetSessionFrame(frame)
    if (kind === 'cursor.error') {
      await this.requestRehydrate(frame.reason || frame.error || frame.error_code || 'V3 realtime cursor error', frame)
      return
    }
    if (kind === 'slow_consumer.reconnect_required') {
      await this.requestRehydrate(frame.reason || frame.error || 'V3 realtime slow consumer', frame)
      return
    }
    if (kind === 'auth.denied') {
  ...
  private noteWorksetSessionFrame(frame: SessionV3RealtimeFrameWire): void {
    const kind = frameKind(frame)
    ...
    this.sessions.set(sessionId, {
      ...
      endpoint_cursor: frameEndpointCursor(frame) || this.currentEndpointCursor(),
      autoDiscovered: true,
      updatedAt: this.now(),
    })
  }
