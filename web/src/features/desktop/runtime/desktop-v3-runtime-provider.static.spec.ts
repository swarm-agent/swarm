...
test('DesktopV3RuntimeProvider synchronously retains one realtime lease with delayed bootstrap', async () => {
  ...
  assert.match(source, /const realtimeReady = runtime\.realtimeLease\.ready\.catch[\s\S]*?await runtime\.bootstrapReady/)
  assert.match(source, /if \(isCancelled\(\)\) return\s*console\.error\('\[desktop-v3\] realtime startup failed after bootstrap render'/)
