...
test('desktop store V3 submit path applies committed V3 messages through canonical runtime', async () => {
  ...
  const submitPromptSource = source.slice(source.indexOf('submitPrompt: async'), source.indexOf('__testApplyRunStreamFrame'))

  assert.match(submitPromptSource, /clientRequestId = providedClientRequestId\?\.trim\(\) \|\| `desktop-v3-message:\$\{targetSessionId\}:\$\{submitStartedAt\}`/)
  assert.match(submitPromptSource, /runtime\.sendMessage\(\{ sessionId: targetSessionId, content: trimmedPrompt, clientRequestId \}\)/)
  ...
  assert.match(submitPromptSource, /const requestedSessionApi = sessionApi\?\.trim\(\)\.toLowerCase\(\) \|\| get\(\)\.sessions\[targetSessionId\]\?\.sessionApi\?\.trim\(\)\.toLowerCase\(\) \|\| 'v3'/)
  assert.match(submitPromptSource, /if \(requestedSessionApi !== 'v3'\)/)
  assert.match(submitPromptSource, /if \(compact\)/)
  assert.match(submitPromptSource, /runtime\.compactSession\(\{ sessionId: targetSessionId, note: trimmedPrompt, agentName, clientRequestId \}\)/)
  assert.match(submitPromptSource, /const result = await runtime\.sendMessage/)
  assert.match(submitPromptSource, /runtime\.sendMessage\(\{ sessionId: targetSessionId, content: trimmedPrompt, clientRequestId \}\)/)
  assert.match(submitPromptSource, /const message = result\.message/)
  assert.match(submitPromptSource, /const mutationEndpointCursor = realtimeOutbox\?\.endpoint_cursor\?\.trim\(\) \?\? ''/)
  assert.match(source, /applyV3RuntimeEnvelope\(createV3SnapshotEnvelope/)
  assert.doesNotMatch(submitPromptSource, /get\(\)\.closeRunStream\(targetSessionId\)/)
  assert.doesNotMatch(submitPromptSource, /await get\(\)\.connect\(\)/)
  assert.doesNotMatch(submitPromptSource, /desktopV3RealtimeEndpointCursor = mutationEndpointCursor/)
  assert.doesNotMatch(submitPromptSource, /requestScopedSessionWorkset\(targetSessionId, \{ force: true \}\)/)
  assert.doesNotMatch(source, /requireV3RealtimeController\(\)\.setEndpointCursor\(cursor\)/)
  ...
  assert.doesNotMatch(panelSourceAfterCompactFix, /sessionApi:\s*'v3',[\s\S]*compact:\s*true/)
  assert.doesNotMatch(panelSourceAfterCompactFix, /upsertDesktopDbRecord|desktopMessagesCollection|desktopSessionsCollection/)
})
