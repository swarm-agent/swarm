...
test('desktop store V3 submit path applies committed V3 messages through canonical runtime', async () => {
  ...
  assert.match(source, /runtime\.sendMessage\(\{ sessionId: targetSessionId, content: trimmedPrompt, clientRequestId \}\)/)
  ...
  assert.match(source, /const result = await runtime\.sendMessage/)
  assert.match(source, /const message = result\.message/)
  assert.match(source, /desktopV3RealtimeEndpointCursor = mutationEndpointCursor/)
