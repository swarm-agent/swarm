...
assert.match(source, /agent: profile\.name === COMPACT_AGENT_NAME \? 'compact' : profile\.name === CODER_AGENT_NAME \? 'coder' : 'explorer'[\s\S]*provider:[\s\S]*model:[\s\S]*thinking:/, 'Compact, Explorer, and Coder setup must save only supported preference fields')
