...
assert.match(source, /agent: profile\.name === COMPACT_AGENT_NAME \? 'compact' : profile\.name === CODER_AGENT_NAME \? 'coder' : 'finder'[\s\S]*provider:[\s\S]*model:[\s\S]*thinking:/, 'Compact, Finder, and Coder setup must save only supported preference fields')
