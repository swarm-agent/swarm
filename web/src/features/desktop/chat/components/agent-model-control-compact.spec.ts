...
assert.match(source, /agent: profile\.name === CODER_AGENT_NAME \? 'coder' : 'explorer'[\s\S]*provider:[\s\S]*model:[\s\S]*thinking:/, 'Explorer and Coder setup must save only supported preference fields')
