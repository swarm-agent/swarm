...
test('production composer keeps style 3 with comfortably sized controls and no temporary comparison code', () => {
  ...
  assert.match(composerSource, /min-w-0 overflow-hidden bg-transparent px-4 py-3 text-\[11px\]/)
  assert.doesNotMatch(composerSource, /border-y border-\[var\(--app-border-strong\)\]/)
  ...
  assert.doesNotMatch(composerSource, /SendHorizontal/)
  assert.equal(composerSource.match(/<ArrowUp size=\{22\} strokeWidth=\{2\.25\} className="shrink-0" \/>/g)?.length, 2)
  assert.match(composerSource, /dictationEnabled[\s\S]*rounded-lg border-0 bg-transparent text-\[var\(--app-primary\)\][\s\S]*rounded-lg border-0 bg-transparent text-\[var\(--app-text-muted\)\]/)
  assert.equal(composerSource.match(/className="h-10 w-10 shrink-0 rounded-lg border/g)?.length, 2)
  assert.match(composerSource, /grid-cols-\[minmax\(0,1fr\)_48px_36px_40px\]/)
