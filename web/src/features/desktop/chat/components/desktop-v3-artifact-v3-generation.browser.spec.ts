...
// Named options must survive normalization and start the revision-bound player
// on click, including cross-artifact remounts and re-clicking a paused option.
...
test('generation navigation and shared-playhead scene reference remain intent-only', { timeout: 30_000 }, async () => {
  ...
  const member = (artifact: string, index: number, wave = 'wave') => ({ label: artifact === 'failed' ? '' : `Design ${artifact}`, wave_id: wave, index, count: 3, artifact_id: artifact, turn_id: 'turn', candidate_id: `candidate-${index}`, commit_oid: artifact === 'failed' ? '' : (artifact === 'one' ? 'a' : 'b').repeat(40), status: artifact === 'failed' ? 'failed' : 'ready' })
  ...
  try {
    ...
    await nav.getByRole('button', { name: 'Design two Option 2 · ready' }).click()
    ...
    assert.equal(await nav.getByRole('button', { name: 'Option 3 Option 3 · failed' }).isDisabled(), true)
    ...
    await frame.getByText('1000', { exact: true }).waitFor()
...
assert.equal(staged.pending_request, undefined)
    ...
    await page.getByText('Animation seek failed or timed out. Reload the preview to retry.', { exact: true }).waitFor()
