...
test('new Desktop chat uses only the routed controller and endpoint before activation', async () => {
  ...
  assert.match(source, /const initialCommandStarting = Boolean\(initialCommandPrompt\)[\s\S]*const activationPending = initialCommandStarting[\s\S]*routedState\.phase === 'routing'[\s\S]*routedState\.phase === 'resolved'[\s\S]*const pendingState = routedState\.phase === 'failed' \? routedState\.phase : activationPending \? 'routing' : 'draft'/)
...
test('routed pending and failure stay local and retry the persisted controller identity', async () => {
  ...
  assert.match(composerSource, /planModeRequested: newSessionCommand\?\.planModeRequested \?\? mode === 'plan'/)
...
test('all successful direct starts retain the new-chat surface until destination activation', async () => {
  ...
  assert.match(source, /const pendingState = routedState\.phase === 'failed' \? routedState\.phase : activationPending \? 'routing' : 'draft'/)
  ...
  assert.match(source, /pendingPrompt=\{routedState\.prompt\}/)
