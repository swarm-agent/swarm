...
test('new Desktop chat uses only the routed controller and endpoint before activation', async () => {
  ...
  assert.match(source, /const initialCommandStarting = Boolean\(initialCommandPrompt\)[\s\S]*const activationPending = initialCommandStarting[\s\S]*routedState\.phase === 'routing'[\s\S]*routedState\.phase === 'resolved'/)
...
test('routed pending and failure stay local and retry the persisted controller identity', async () => {
  ...
  assert.match(paneSource, /startPresentation=\{!durable \? \{ workspaceName: workspace\.workspaceName, runStatus: headerStatus \}/)
  assert.match(paneSource, /const headerStatus:[\s\S]*activationPending[\s\S]*'Starting…'[\s\S]*'Start failed'/)
  assert.match(paneSource, /<DesktopV3RoutedPendingShell[\s\S]*state="draft"/)
  assert.doesNotMatch(paneSource, /pendingPrompt=|startPath="router"/)
...
test('all successful direct starts retain the new-chat surface until destination activation', async () => {
  ...
  assert.match(source, /<DesktopV3ExistingConversationPane/)
  assert.match(source, /const \[startedHere\] = useState\(\(\) => !props\.sessionId\)/)
  assert.match(source, /<DesktopV3AgenticComposer[\s\S]*disabled=\{activationPending\}[\s\S]*busy=\{activationPending\}/)
  assert.match(source, /content: routedState\.prompt/)
  assert.match(source, /firstMessageIdentity=\{firstMessageIdentityRef\.current\}/)
