...
export function DesktopV3ExistingConversationPane({
  ...
  const refreshSessionArtifacts = useCallback(async () => {
    ...
    try {
      const [catalog, artifactV2Catalog] = await Promise.all([
  ...
  const hasSessionArtifacts = sessionArtifactV2.length > 0 || sessionArtifacts.length > 0;
  ...
  useEffect(() => {
    ...
    setSidebarView((current) => desktopV3NextSessionSidebarView({
      ...
      artifactCount: sessionArtifactV2.length + sessionArtifacts.length,
  ...
  }, [hasPendingVisualSwarm, normalizedSessionId, sessionArtifactV2.length, sessionArtifacts.length, showPlanSidebar, stablePlanDocument]);
          ...
          >
            {showPlanSidebar && hasSessionArtifacts ? (
              <div className={cn("shrink-0 border-b border-l border-[var(--app-border)]/60 bg-[var(--app-surface)]", planSidebarDisplayMode === "thin" ? "grid gap-1 p-1.5" : "grid grid-cols-2 gap-1 p-2")} role="tablist" aria-label="Session sidebar view" data-session-sidebar-toggle>
                ...
                <button type="button" role="tab" aria-selected={activeSidebarView === "artifacts"} aria-label={`Show ${sessionArtifactV2.length + sessionArtifacts.length} session artifacts`} title="Artifacts" onClick={() => setSidebarView("artifacts")} className={cn("inline-flex min-h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs font-semibold transition", activeSidebarView === "artifacts" ? "bg-[var(--app-surface-active)] text-[var(--app-text)]" : "text-[var(--app-text-muted)] hover:bg-[var(--app-surface-hover)]", planSidebarDisplayMode === "thin" && "px-0")}><GalleryHorizontal size={14} aria-hidden="true" />{planSidebarDisplayMode !== "thin" ? `Artifacts ${sessionArtifactV2.length + sessionArtifacts.length}` : null}</button>
              </div>
            ) : null}
