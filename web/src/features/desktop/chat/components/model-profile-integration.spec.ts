...
test('agent setup favorites switch both the canonical Swarm default and current chat', () => {
  ...
  assert.match(controlSource, /applyFavorite\(profile/)
  assert.match(controlSource, /deleteModelProfile\(profile\.profileId\)/)
  ...
  assert.match(controlSource, /saveSwarmAgentModelSettings\(\{[\s\S]*action: \{[\s\S]*provider: profile\.provider[\s\S]*plan: settings\.swarm\.plan/)
  ...
  assert.doesNotMatch(controlSource, /setDefaultModelProfile|actionFavoriteId|planFavoriteId/)
