...
test('/task always uses the dedicated background Router session API', async () => {
  ...
  assert.match(backgroundCase, /postDesktopV3BackgroundRouterSessionStart\(\{[\s\S]*?input: request/)
  assert.match(backgroundCase, /plan_mode_requested: mode === 'plan'/)
  assert.doesNotMatch(backgroundCase, /createWorkspaceAITask|\/v1\/workspace\/todos|aiTasks\.mergeItems|onRoutedSubmit/)
...
test('/task in a pending routed session takes background precedence without consuming the pending session', async () => {
  ...
  assert.match(composer, /command\.action\.kind === 'start-background-router-session'[\s\S]*?if \(routedNewSession\) \{[\s\S]*?void handleSubmitClick\(\)/)
