...
test('artifact gallery selection actions attach opaque references without artifact bytes', async () => {
  ...
  assert.match(gallery, /desktopV3ArtifactMessageSelection\(artifact, 'select'\)/)
  assert.match(gallery, /desktopV3ArtifactMessageSelection\(selected, 'use'\)/)
  ...
  assert.match(composer, /artifacts\.map\(\(\{ label, description, selection \}\) => \(\{ \.\.\.selection, label, description, action: 'select' \}\)\)/)
  ...
  assert.match(api, /desktopV3ArtifactVariantLabel/)
  assert.match(api, /desktopV3ArtifactVariantDescription/)
