...
test('agent setup keeps default session mode and an understandable model policy button without summary cards', () => {
  ...
  assert.match(source, /Agent model policy[\s\S]*ModelPolicyButton/)
  assert.doesNotMatch(source, /function ModelPolicyChoices/)
