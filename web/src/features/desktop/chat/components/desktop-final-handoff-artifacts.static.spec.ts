...
test('final handoff gallery uses authenticated opaque artifact routes and an isolated scripted HTML preview', async () => {
  ...
  assert.match(pane, /data-artifact-gallery-page/)
  assert.match(pane, /fixed inset-0.*h-\[100dvh\].*w-\[100dvw\]/)
  assert.match(pane, /Back to conversation/)
  assert.match(pane, /aria-label="Previous artifact"/)
  assert.match(pane, /aria-label="Next artifact"/)
  assert.doesNotMatch(pane, /<Dialog role="dialog"[^>]*Final handoff artifacts/)
  assert.doesNotMatch(pane, /data-artifact-gallery-fullscreen/)
  ...
  assert.match(pane, /fetchDesktopV3ArtifactBundle\(sessionId, artifact\.artifactId\)/)
  assert.match(pane, /Download bundle/)
