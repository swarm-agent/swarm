...
test('Desktop V3 composer opens task actions from a borderless plus trigger and primes without submitting', async () => {
  ...
  assert.match(actions, /aria-label=\{view === 'task' \? 'Task type' : view === 'actions' \? 'Workspace Actions' : view === 'skills' \? 'Workspace Skills' : 'Composer actions'\}/)
  ...
  assert.ok((actions.match(/role="menuitem"/g) ?? []).length >= 5)
...
test('Desktop V3 composer exposes only backend-projected media and preserves durable references', async () => {
  ...
  assert.match(pane, /const admitted = admission\.capability/)
  assert.match(pane, /const declaredMIME = mimeType \|\| \(fileType \? \(admitted\.mime_types \?\? \[\]\)\.find/)
  assert.match(pane, /if \(!declaredMIME\) throw new Error/)
...
test('Desktop V3 composer keeps new-session Worktree before Plan and omits resolved Worktree clutter', async () => {
  ...
  assert.match(plan, /data-testid="desktop-composer-plan-toggle"/)
  assert.match(plan, /border-\[var\(--app-border-accent\)\]/)
