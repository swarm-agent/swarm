...
test('plan modal keeps recovery hidden until opened and shows the full active checkpoint under the title', () => {
  const base = planRecord()
  base.document.checkpoints[0].title = 'Build UI with a very long active checkpoint title that should wrap instead of truncating underneath the plan title'
  ...
  const activeTitleIndex = markup.indexOf('Build UI with a very long active checkpoint title that should wrap instead of truncating underneath the plan title')
  ...
  assert(activeIndex > titleIndex, 'expected active checkpoint count under modal title')
  assert(activeTitleIndex > activeIndex, 'expected active checkpoint title under modal title')
  assert(recoveryButtonIndex > titleIndex, 'expected recovery entry in the header controls')
  assert(copyIndex > recoveryButtonIndex, 'expected recovery entry directly left of copy')
  assert(documentIndex > copyIndex, 'expected plan details after header controls')
