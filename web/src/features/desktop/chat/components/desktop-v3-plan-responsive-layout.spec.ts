...
test('plan execution adapts its desktop sidebar and shifts above the composer below 1300px', async () => {
  ...
  const composerIndex = paneSource.indexOf('<DesktopV3ExistingConversationComposer', compactRegionIndex)
  ...
  assert.match(compactRegion, /<IsolatedPlanExecutionSidebar|<DesktopPlanExecutionSidebar/)
  ...
  assert.match(paneSource, /policyMode !== "automatic"/)
  assert.match(paneSource, />\s*Review each\s*</)
})
