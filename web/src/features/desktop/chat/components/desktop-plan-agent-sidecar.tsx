      ...
      <aside className={embedded
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          ...
          <div className="flex min-w-0 flex-col gap-5">
            {renderItems.map((item, index) => <DesktopV3RenderItemView key={`${item.type}:${"id" in item ? item.id : item.type === "pending-user" ? index : item.message.id}`} item={item} thinkingTagsEnabled timerNow={Date.now()} index={index} />)}
          </div>
