...
test('artifact sidebar renders authorized thumbnail previews and opens full gallery selection', async () => {
  ...
  assert.match(previewThumbnail, /interactivePreviewRequested/)
  assert.match(previewThumbnail, /previewMotionAllowed/)
  ...
  assert.match(sidebar, /const livePreviewKey = \(requestedArtifact\?[\s\S]*\|\| fallbackLivePreviewKey/)
  assert.match(sidebar, /sidebarArtifactNeedsExclusiveLivePreview/)
  assert.match(sidebar, /artifact\.mediaType === 'text\/html' && !artifact\.animationProfile/)
  assert.match(sidebar, /useDesktopV3ArtifactPreviewVisibility<HTMLSpanElement>\(live\)/)
  assert.match(sidebar, /!sidebarArtifactNeedsMotionPermission\(artifact\) \|\| previewMotionAllowed/)
  assert.match(sidebar, /preload="metadata"/)
  assert.doesNotMatch(sidebar, /autoPlay loop/)
  assert.doesNotMatch(sidebar, /<DesktopV3ArtifactThumbnail artifact=\{artifact\} live \/>/)
