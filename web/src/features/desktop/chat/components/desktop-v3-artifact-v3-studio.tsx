...
import { artifactV3SelectionProtocol, nativeArtifactPlaybackEvent, type NativeArtifactPlaybackState, nativeArtifactPreviewSelectionEvent, toggleNativeArtifactPart } from '../../session-v3/artifact-v3-preview-selection'
...
function NativeArtifactStudio({ artifact, open, onOpenChange, onIterate, onRefresh, onRepairDraft, onNavigate, autoplay }: DesktopV3ArtifactV3StudioProps & { autoplay: { current: string } }) {
  ...
  useEffect(() => {
    ...
    const resize = () => setPreviewScale(Math.min(element.clientWidth / 1920, element.clientHeight / 1080))
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelectedRevisionRef((current) => {
        if (explicitlySelectedRevision.current && current) return current
  ...
  const previewOption = (revisionRef: string) => {
    ...
    if (revisionRef === selectedRevisionRef && playback && !playback.error) {
      autoplay.current = ''
      sendPlayback('play')
    } else {
      autoplay.current = `${artifact?.artifactId}:${revisionRef}`
  ...
  const navigateMember = (member: NonNullable<typeof group>['members'][number]) => {
    ...
    if (member.artifactId === artifact.artifactId) {
      previewOption(revisionRef)
    } else if (onNavigate) {
      autoplay.current = `${member.artifactId}:${revisionRef}`
      writeNativeArtifactNavigation({ artifactId: member.artifactId, revisionRef, waveId: group.waveId })
      onNavigate({ ...artifact, artifactId: member.artifactId, artifactRef: member.artifactId, label: member.label?.trim() || `Option ${member.index}`, head: null })
    }
  }
  ...
  const visibleParts = selectedRevision?.parts ?? []
  const revisionUnavailable = Boolean(studio && selectedRevisionRef && (!selectedRevision || selectedRevision.status !== 'ready' || !selectedRevision.parts))
  const canAttach = Boolean(selectedRevision?.status === 'ready' && selectedRevision.parts && !loading)
  ...
  useEffect(() => {
    setPreviewURL('')
    if (!artifact || !selectedRevisionRef || !open || revisionUnavailable) { setPreviewLoading(false); return undefined }
    const controller = new AbortController()
  ...
  }, [artifact?.artifactId, artifact?.ownerSessionId, open, selectedRevisionRef, revisionUnavailable])
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      const player = nativeArtifactPlaybackEvent(event, previewRef.current?.contentWindow, selectedRevisionRef, playbackCommand.current)
      if (player) {
        if (player === 'error') setError('Animation runtime failed to initialize.')
        else {
          setPlayback(player)
  ...
  const focusPart = (partId: string) => {
    ...
    const temporal = visibleParts.find((part) => part.id === partId)?.temporal
    if (temporal && playback) sendPlayback('seek', Math.min(playback.durationMs, temporal.startMs))
  }

  const iterate = async (partIds: readonly string[], referenceOnly = false) => {
    if (!studio || !onIterate || !canAttach || !selectedRevision) return
    try {
      setBusy('iterate')
      setError('')
      explicitlySelectedRevision.current = true
      const selection = desktopV3NativeArtifactIterationSelection(studio, partIds, selectedRevisionRef)
      await onIterate(referenceOnly ? { ...selection, action: 'select', description: 'Style/example reference only. Do not revise or select the source unless separately requested.' } : selection)
      close()
    } catch (cause) {
