...
cleanup() { rm -f -- "$scratch/go.mod" "$scratch/go.sum" "$scratch/pnpm-lock.yaml" "$scratch/root.mod" "$scratch/root.sum"; rmdir -- "$scratch"; }
trap cleanup EXIT
...
timeout --signal=TERM --kill-after=10s 600s systemd-nspawn --quiet --settings=no --register=no \
  ...
  --bind-ro="$scratch:/manifests" /bin/bash -ec '
    ...
    cp /manifests/pnpm-lock.yaml /cache-manifests/web/
    cd /cache-manifests
    go mod download
    cd /cache-manifests/go
    go mod download
    cd /cache-manifests/web
    pnpm fetch --ignore-scripts --network-concurrency=4
