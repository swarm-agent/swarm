...
function countEvent(events, sid, eventType){ return (events||[]).filter(e=>e.session_id===sid && (!eventType || e.event_type===eventType || e.event?.event_type===eventType)).length; }
