...
try {
 if(stage==='prepare'){
  ...
  assert(job.sources.every(src=>src.session_id===s.id));assert(!(await mem()).entries.some(e=>e.id===job.proposal.id));check('scoped provider learning waits for review')
  const approved=await post({action:'approve',job_id:state.jobID});assert.equal(approved.status,'completed')
  const d=await mem();assert(d.entries.some(e=>e.id===job.proposal.id));assert.equal(d.entries.find(e=>e.id===state.id).content,state.entry.content);check('approved learning persists without rewriting pinned rule')
  await save()
 } else if(stage==='cleanup'){
  restoreClient();const before=await mem();
  await post({action:'settings',settings:state.originalSettings})
  for(const entry of (await mem()).entries||[]){if(entry.id===state.id||entry.id===state.proposal?.id)await post({action:'forget',entry_id:entry.id})}
  const d=await mem();assert(!d.settings.automation_enabled);assert(!(d.entries||[]).some(e=>e.id===state.id||e.id===state.proposal?.id));assert(!JSON.stringify(d.history).includes(state.entry.content));check('pause and permanent forget redact fixture history')
  await save()
