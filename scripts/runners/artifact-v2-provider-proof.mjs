...
async function runSingle(index, topology, assignment) {
  ...
  assert(catalog.length === 1 && published.length === 1, `single ${index} produced total=${catalog.length} published=${published.length}; expected one task call and one published V2 artifact`)
