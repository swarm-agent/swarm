...
try {
  ...
  try {
    ...
    assert(watermarkCreate.session?.id, `watermark probe create did not return session id: ${JSON.stringify(watermarkCreate)}`);
    const watermarkDiscovery = (await apiJSON('POST', '/v3/sessions:discover', token, { global: true, recent: { limit: 20 } }, 'sessions.discover.watermark_probe')).body;
    const watermarkCursor = String(watermarkDiscovery.snapshot_endpoint_cursor || '');
    assert(isV3Cursor(watermarkCursor), `watermark discovery cursor is not v3c1: ${watermarkCursor}`);
    const watermarkCursorPayload = decodeCursorPayload(watermarkCursor);
    const watermarkSeq = Number(watermarkCursorPayload.after_endpoint_seq || 0);
    assert(Number.isSafeInteger(watermarkSeq) && watermarkSeq > currentHead, `watermark probe did not advance endpoint head after ${currentHead}: ${JSON.stringify(watermarkCursorPayload)}`);
