...
PY`]); result.candidate.diagnostics_env=env.stdout; assert(env.stdout.includes('SWARM_V3_DIAGNOSTICS=1') && env.stdout.includes('SWARM_PROVIDER_API_DIAGNOSTICS=1'),`diagnostics env not enabled on running swarmd: ${env.stdout}`); result.gates.diagnostics_enabled=true;
  ...
  events=await fetchEvents(token,sessionID,'after_a2'); let d2=requestDiag(events,result.ids.codex_a2_run_id); assertBoundary('codex_a2',d2,a2,{boundary:'session_turn',native:true,fresh:false,previousEqualsCurrent:true}); result.gates.codex_same_lineage=true;
  ...
  const usageEvents=diagnosticEvents(events,result.ids.fireworks_run_id,'session.diagnostic.provider.usage').concat(diagnosticEvents(events,checkpointRunID,'session.diagnostic.provider.usage'));
  ...
  result.diagnostics.fireworks_usage={usage_event_count:usageEvents.length, stream_chunk_usage_seen:usageBodies.some(b=>b.includes('"usage"') && !b.includes('"usage":null')), assistant_usage:f.metadata?.usage||cp.metadata?.usage||null};
