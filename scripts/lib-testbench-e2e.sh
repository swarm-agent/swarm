...
swarm_testbench_env_key_allowed() {
  case "$1" in
    SWARM_LOCAL_TESTBENCH_ROOT|SWARM_LOCAL_TESTBENCH_SLOTS|SWARM_LOCAL_TESTBENCH_CPUS|SWARM_LOCAL_TESTBENCH_MEMORY_MB|SWARM_LOCAL_TESTBENCH_DISK_MB|SWARM_LOCAL_TESTBENCH_TASKS|SWARM_LOCAL_TESTBENCH_TOTAL_CPUS|SWARM_LOCAL_TESTBENCH_TOTAL_MEMORY_MB|SWARM_LOCAL_TESTBENCH_TOTAL_DISK_MB|SWARM_LOCAL_TESTBENCH_LEASE_SECONDS|SWARM_LOCAL_TESTBENCH_IDLE_SECONDS|SWARM_LOCAL_TESTBENCH_LOCK_TIMEOUT|SWARM_LOCAL_TESTBENCH_QUEUE_TIMEOUT|SWARM_LOCAL_TESTBENCH_BASE_IMAGE|SWARM_LOCAL_TESTBENCH_BASE_SHA256|SWARM_LOCAL_TESTBENCH_PORT_BASE|SWARM_LOCAL_TESTBENCH_DEADLINE|SWARM_TESTBENCH_GENERATION|SWARM_PRIMARY_SSH|SWARM_TESTBENCH_TARGET|SWARM_TESTBENCH_LOCAL_DESKTOP_PORT|SWARM_REMOTE_DESKTOP_PORT|SWARM_TESTBENCH_LOCAL_API_PORT|SWARM_TESTBENCH_REMOTE_API_PORT|SWARM_TESTBENCH_REVERSE_LOCAL_PORT|SWARM_TESTBENCH_REVERSE_REMOTE_PORT|SWARM_TESTBENCH_PROVIDER|SWARM_TESTBENCH_MODEL|SWARM_TESTBENCH_THINKING|SWARM_TESTBENCH_ACTION_MODEL|SWARM_TESTBENCH_ACTION_THINKING|SWARM_TESTBENCH_PLAN_MODEL|SWARM_TESTBENCH_PLAN_THINKING|SWARM_TESTBENCH_CODER_MODEL|SWARM_TESTBENCH_CODER_THINKING|SWARM_TESTBENCH_DESIGNER_MODEL|SWARM_TESTBENCH_DESIGNER_THINKING|SWARM_TESTBENCH_LINKED_WORKSPACE_PATH)
      return 0
      ;;
    *)
      return 1
...
swarm_testbench_load_env() {
  ...
  SWARM_TESTBENCH_TARGET="${SWARM_TESTBENCH_TARGET:-container}"
  SWARM_TESTBENCH_LOCAL_DESKTOP_PORT="${SWARM_TESTBENCH_LOCAL_DESKTOP_PORT:-15655}"
  SWARM_REMOTE_DESKTOP_PORT="${SWARM_REMOTE_DESKTOP_PORT:-5655}"
  SWARM_TESTBENCH_LOCAL_API_PORT="${SWARM_TESTBENCH_LOCAL_API_PORT:-17881}"
  SWARM_TESTBENCH_REMOTE_API_PORT="${SWARM_TESTBENCH_REMOTE_API_PORT:-7881}"
  ...
  export SWARM_PRIMARY_SSH SWARM_TESTBENCH_TARGET SWARM_TESTBENCH_LOCAL_DESKTOP_PORT SWARM_REMOTE_DESKTOP_PORT
...
swarm_testbench_validate_env() {
  if [[ "${SWARM_TESTBENCH_TARGET}" == local ]]; then
    [[ "${SWARM_TESTBENCH_PROVIDER}" == codex ]] || { swarm_testbench_fail 'local candidates require Codex'; return 1; }
    local key
    for key in SWARM_TESTBENCH_MODEL SWARM_TESTBENCH_ACTION_MODEL SWARM_TESTBENCH_PLAN_MODEL SWARM_TESTBENCH_CODER_MODEL SWARM_TESTBENCH_DESIGNER_MODEL; do
      [[ "${!key}" == gpt-5.6-luna ]] || { swarm_testbench_fail "${key} must be gpt-5.6-luna"; return 1; }
    done
    for key in SWARM_TESTBENCH_THINKING SWARM_TESTBENCH_ACTION_THINKING SWARM_TESTBENCH_PLAN_THINKING SWARM_TESTBENCH_CODER_THINKING SWARM_TESTBENCH_DESIGNER_THINKING; do
      [[ "${!key}" == medium ]] || { swarm_testbench_fail "${key} must be medium"; return 1; }
    done
    [[ -z "${SWARM_TESTBENCH_REVERSE_LOCAL_PORT}" && -z "${SWARM_TESTBENCH_REVERSE_REMOTE_PORT}" ]] || { swarm_testbench_fail 'local candidates do not use SSH reverse tunnels'; return 1; }
    return 0
  fi
  [[ "${SWARM_PRIMARY_SSH}" =~ ^[A-Za-z0-9._-]+$ ]] || { swarm_testbench_fail "SWARM_PRIMARY_SSH must be a configured SSH alias, not a hostname, command, or user@host value"; return 1; }
  [[ "${SWARM_TESTBENCH_TARGET}" == container ]] || { swarm_testbench_fail "SWARM_TESTBENCH_TARGET must be container; the retired host swarm.service testbench is not a supported live-test target"; return 1; }
  ...
  [[ "${SWARM_REMOTE_DESKTOP_PORT}" == 5655 && "${SWARM_TESTBENCH_REMOTE_API_PORT}" == 7881 ]] || { swarm_testbench_fail "container testbench remote ports are fixed at Desktop 5655 and API 7881; host ports 5555/7781 are retired for live candidate testing"; return 1; }
  [[ "${SWARM_TESTBENCH_LOCAL_DESKTOP_PORT}" != "${SWARM_TESTBENCH_LOCAL_API_PORT}" ]] || { swarm_testbench_fail "local Desktop and API ports must differ"; return 1; }
  [[ "${SWARM_TESTBENCH_PROVIDER}" == fireworks ]] || { swarm_testbench_fail "SWARM_TESTBENCH_PROVIDER must be fireworks; testbench live tests do not support Codex or another provider"; return 1; }
  ...
  for model_name in SWARM_TESTBENCH_MODEL SWARM_TESTBENCH_ACTION_MODEL SWARM_TESTBENCH_PLAN_MODEL SWARM_TESTBENCH_CODER_MODEL SWARM_TESTBENCH_DESIGNER_MODEL; do
    ...
    [[ "${model_value}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || { swarm_testbench_fail "${model_name} must be an explicit Fireworks catalog model ID, not a request path"; return 1; }
  done
  for thinking_name in SWARM_TESTBENCH_THINKING SWARM_TESTBENCH_ACTION_THINKING SWARM_TESTBENCH_PLAN_THINKING SWARM_TESTBENCH_CODER_THINKING SWARM_TESTBENCH_DESIGNER_THINKING; do
    ...
    [[ "${thinking_value}" =~ ^(off|low|medium|high|xhigh)$ ]] || { swarm_testbench_fail "${thinking_name} must be off, low, medium, high, or xhigh"; return 1; }
  done
  if [[ -n "${SWARM_TESTBENCH_REVERSE_LOCAL_PORT}" || -n "${SWARM_TESTBENCH_REVERSE_REMOTE_PORT}" ]]; then
    [[ -n "${SWARM_TESTBENCH_REVERSE_LOCAL_PORT}" && -n "${SWARM_TESTBENCH_REVERSE_REMOTE_PORT}" ]] || { swarm_testbench_fail "set both reverse-port variables or leave both empty"; return 1; }
