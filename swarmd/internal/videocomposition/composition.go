	...
	for id := range layouts {
		...
		visit = func(current string) error {
			...
			if layout.ExtendsLayoutID != "" {
				...
				if err := visit(layout.ExtendsLayoutID); err != nil { return err }
			}
			visiting[current] = false
			seen[current] = true
			return nil
		}
		if err := visit(id); err != nil { return err }
	}
	return nil
}
...
func ValidateLink(catalog *Catalog, link *Link, partDurationMs int64) error {
	...
	if partDurationMs <= 0 { return errors.New("composition requires a positive part duration") }
	if link.Detached {
	...
	for _, slot := range link.DetachedSlots {
		if _, duplicate := seen[slot.ID]; duplicate { return fmt.Errorf("duplicate detached composition slot %q", slot.ID) }
		seen[slot.ID] = struct{}{}
		if err := validateSlotForDuration(slot, partDurationMs); err != nil { return err }
	}
	seenOverrides := map[string]struct{}{}
	for _, override := range link.Overrides {
		if !stableIDPattern.MatchString(override.SlotID) { return errors.New("composition override has invalid slot_id") }
		if _, duplicate := seenOverrides[override.SlotID]; duplicate { return fmt.Errorf("duplicate composition override for slot %q", override.SlotID) }
		seenOverrides[override.SlotID] = struct{}{}
		if override.Source != nil && override.ClearSource { return fmt.Errorf("composition override %q cannot set and clear source", override.SlotID) }
	}
...
func Resolve(catalog *Catalog, link *Link, width, height int, partDurationMs int64) ([]ResolvedSlot, error) {
	if link == nil || link.Disabled { return nil, nil }
	if width <= 0 || height <= 0 || width%2 != 0 || height%2 != 0 { return nil, errors.New("composition output dimensions must be positive even pixels") }
	if err := ValidateCatalog(catalog); err != nil { return nil, err }
	var slots []Slot
	if link.Detached {
		slots = append(slots, link.DetachedSlots...)
	} else {
		if catalog == nil { return nil, errors.New("composition link requires a catalog") }
		layouts := make(map[string]Layout, len(catalog.Layouts))
		for _, layout := range catalog.Layouts { layouts[layout.ID] = layout }
		var err error
		slots, err = resolveLayout(layouts, link.LayoutID, nil)
		if err != nil { return nil, err }
	}
	indices := make(map[string]int, len(slots))
	for i := range slots { indices[slots[i].ID] = i }
	for _, override := range link.Overrides {
		index, ok := indices[override.SlotID]
		if !ok { return nil, fmt.Errorf("composition override references missing slot %q", override.SlotID) }
		applyOverride(&slots[index], override)
	}
	if len(slots) == 0 || len(slots) > MaxSlots { return nil, errors.New("resolved composition slot count is invalid") }
	resolved := make([]ResolvedSlot, len(slots))
	for i, slot := range slots {
		if err := validateSlotForDuration(slot, partDurationMs); err != nil { return nil, err }
		pixels, err := resolvePixels(slot, width, height)
		if err != nil { return nil, fmt.Errorf("composition slot %q: %w", slot.ID, err) }
		resolved[i] = ResolvedSlot{Slot: slot, Pixels: pixels}
	}
	sort.SliceStable(resolved, func(i, j int) bool {
		if resolved[i].ZIndex == resolved[j].ZIndex { return resolved[i].ID < resolved[j].ID }
...
func resolveLayout(layouts map[string]Layout, id string, stack []string) ([]Slot, error) {
	for _, current := range stack { if current == id { return nil, fmt.Errorf("composition layout inheritance cycle includes %q", id) } }
	layout, ok := layouts[id]
	if !ok { return nil, fmt.Errorf("composition layout %q is missing", id) }
	var slots []Slot
	if layout.ExtendsLayoutID != "" {
		var err error
	...
	for i := range slots { indices[slots[i].ID] = i }
	for _, slot := range layout.Slots {
		if i, exists := indices[slot.ID]; exists { slots[i] = slot } else { indices[slot.ID] = len(slots); slots = append(slots, slot) }
	}
	return slots, nil
}

func validateSlotForDuration(slot Slot, duration int64) error {
	if err := validateSlot(slot); err != nil { return err }
	if slot.Source != nil {
		s := slot.Source
		if s.TimelineStartMs < 0 || s.TimelineEndMs > duration { return fmt.Errorf("composition slot %q source timeline range exceeds part duration", slot.ID) }
	}
	return nil
}

func validateSlot(slot Slot) error {
	if !stableIDPattern.MatchString(slot.ID) { return errors.New("composition slot has invalid stable id") }
	if strings.TrimSpace(slot.Requirement) == "" || strings.TrimSpace(slot.Requirement) != slot.Requirement || len(slot.Requirement) > 1000 { return fmt.Errorf("composition slot %q has invalid requirement", slot.ID) }
	if err := validateRect(slot.Geometry); err != nil { return fmt.Errorf("composition slot %q: %w", slot.ID, err) }
	if slot.ZIndex < 0 || slot.ZIndex > 255 { return fmt.Errorf("composition slot %q z_index is outside 0..255", slot.ID) }
	if slot.Fit != FitContain && slot.Fit != FitCover { return fmt.Errorf("composition slot %q fit must be contain or cover", slot.ID) }
	if !unit(slot.AlignmentX) || !unit(slot.AlignmentY) { return fmt.Errorf("composition slot %q alignment is outside 0..1", slot.ID) }
	if !unit(slot.Crop.Top) || !unit(slot.Crop.Right) || !unit(slot.Crop.Bottom) || !unit(slot.Crop.Left) || slot.Crop.Left+slot.Crop.Right >= 1 || slot.Crop.Top+slot.Crop.Bottom >= 1 { return fmt.Errorf("composition slot %q crop is invalid", slot.ID) }
	if slot.Mask.Kind != MaskNone && slot.Mask.Kind != MaskRoundedRect && slot.Mask.Kind != MaskEllipse { return fmt.Errorf("composition slot %q mask is unsupported", slot.ID) }
	if slot.Mask.Kind == MaskRoundedRect && (slot.Mask.Radius < 0 || slot.Mask.Radius > .5) { return fmt.Errorf("composition slot %q mask radius is outside 0..0.5", slot.ID) }
	if slot.Mask.Kind != MaskRoundedRect && slot.Mask.Radius != 0 { return fmt.Errorf("composition slot %q mask radius requires rounded_rect", slot.ID) }
	if !finite(slot.AspectLock) || slot.AspectLock < 0 || slot.AspectLock > 10 { return fmt.Errorf("composition slot %q aspect_lock is invalid", slot.ID) }
	if slot.Source != nil {
		s := slot.Source
		if strings.TrimSpace(s.SourceRef) == "" || strings.TrimSpace(s.SourceRef) != s.SourceRef || len(s.SourceRef) > 256 || !strings.HasPrefix(s.MediaType, "video/") { return fmt.Errorf("composition slot %q requires an exact registered video source", slot.ID) }
		if s.SourceStartMs < 0 || s.SourceEndMs <= s.SourceStartMs || s.TimelineStartMs < 0 || s.TimelineEndMs <= s.TimelineStartMs { return fmt.Errorf("composition slot %q has invalid independent timing ranges", slot.ID) }
		if s.AudioPolicy != AudioMute && s.AudioPolicy != AudioInclude { return fmt.Errorf("composition slot %q requires explicit audio_policy", slot.ID) }
		if !finite(s.Gain) || s.Gain < 0 || s.Gain > 2 || (s.AudioPolicy == AudioMute && s.Gain != 0) { return fmt.Errorf("composition slot %q has invalid audio gain", slot.ID) }
	}
	return nil
}

func validateRect(rect NormalizedRect) error {
	if !finite(rect.X) || !finite(rect.Y) || !finite(rect.Width) || !finite(rect.Height) || rect.X < 0 || rect.Y < 0 || rect.Width <= 0 || rect.Height <= 0 || rect.X+rect.Width > 1 || rect.Y+rect.Height > 1 { return errors.New("normalized geometry must be finite and contained in 0..1") }
	return nil
}
...
func resolvePixels(slot Slot, width, height int) (PixelRect, error) {
	...
	if slot.AspectLock > 0 {
		...
		if float64(w)/float64(h) > target { w = evenFloor(float64(h)*target) } else { h = evenFloor(float64(w)/target) }
		x0 += evenFloor(float64((x1-x0)-w) * slot.AlignmentX)
		y0 += evenFloor(float64((y1-y0)-h) * slot.AlignmentY)
	}
	if w < 2 || h < 2 || x0 < 0 || y0 < 0 || x0+w > width || y0+h > height { return PixelRect{}, errors.New("geometry resolves outside safe even pixel bounds") }
	return PixelRect{X: x0, Y: y0, Width: w, Height: h}, nil
}

func applyOverride(slot *Slot, override SlotOverride) {
	if override.Requirement != nil { slot.Requirement = *override.Requirement }
	if override.Geometry != nil { slot.Geometry = *override.Geometry }
	if override.ZIndex != nil { slot.ZIndex = *override.ZIndex }
	if override.Fit != nil { slot.Fit = *override.Fit }
	if override.AlignmentX != nil { slot.AlignmentX = *override.AlignmentX }
	if override.AlignmentY != nil { slot.AlignmentY = *override.AlignmentY }
	if override.Crop != nil { slot.Crop = *override.Crop }
	if override.Mask != nil { slot.Mask = *override.Mask }
	if override.AspectLock != nil { slot.AspectLock = *override.AspectLock }
	if override.ClearSource { slot.Source = nil } else if override.Source != nil { slot.Source = override.Source }
}
