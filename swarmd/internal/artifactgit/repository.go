...
func Open(ctx context.Context, root, repositoryID string, limits Limits) (*Repository, error) {
	...
	if err := ensurePrivate(root, true); err != nil {
		return nil, err
	}
	...
	if _, err := os.Stat(path); errors.Is(err, os.ErrNotExist) {
		if _, err = r.raw(ctx, nil, "init", "--bare", "--initial-branch=official", path); err != nil {
			return nil, err
		}
	} else if err != nil {
		return nil, err
	}
	if err := ensurePrivate(path, true); err != nil {
		return nil, err
	}
	out, err := r.gitCmd(ctx, nil, "rev-parse", "--is-bare-repository")
	if err != nil || strings.TrimSpace(string(out)) != "true" {
		return nil, fmt.Errorf("%w: not a bare repository", ErrIntegrity)
	}
	return r, nil
}
...
func ensurePrivate(path string, directory bool) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("%w: symlink storage", ErrIntegrity)
	}
	if directory && !info.IsDir() {
		return fmt.Errorf("%w: storage is not directory", ErrIntegrity)
	}
...
func (r *Repository) Genesis(ctx context.Context, value Genesis) (string, error) {
	...
	if err := r.updateRef(ctx, "refs/heads/official", commit, strings.Repeat("0", len(commit))); err != nil {
		return "", err
	}
	return commit, nil
}
...
func (r *Repository) manifestFromGenesis(ctx context.Context, g Genesis) (Manifest, error) {
	...
	if g.Content != nil {
		p, err := r.storeBlob(ctx, "content", *g.Content)
		if err != nil {
			return m, err
		}
		m.Content = &p
	...
	if m.Content == nil && len(m.Parts) == 0 {
		return m, invalid("empty artifact")
	}
	return m, nil
}
...
func (r *Repository) storeBlob(ctx context.Context, id string, in BlobInput) (Part, error) {
	if !idPattern.MatchString(id) {
		return Part{}, invalid("part id")
	}
	if int64(len(in.Bytes)) > r.limits.MaxBlobBytes {
		return Part{}, ErrQuotaExceeded
	}
	out, err := r.gitCmd(ctx, in.Bytes, "hash-object", "-w", "--stdin")
	if err != nil {
		return Part{}, err
	}
...
func (r *Repository) commit(ctx context.Context, m Manifest, parents []string, message string) (string, error) {
	...
	body = append(body, '\n')
	manifestBlob, err := r.gitCmd(ctx, body, "hash-object", "-w", "--stdin")
	if err != nil {
		return "", err
	}
	...
	if m.Content != nil {
		fmt.Fprintf(&tree, "100644 blob %s\tcontent\n", m.Content.Blob)
	}
	if len(m.Parts) > 0 {
		var partsTree bytes.Buffer
		for _, p := range m.Parts {
			fmt.Fprintf(&partsTree, "100644 blob %s\t%s\n", p.Blob, p.ID)
		}
		partsTreeID, treeErr := r.gitCmd(ctx, partsTree.Bytes(), "mktree")
		if treeErr != nil {
			return "", treeErr
		}
		fmt.Fprintf(&tree, "040000 tree %s\tparts\n", strings.TrimSpace(string(partsTreeID)))
	}
	treeID, err := r.gitCmd(ctx, tree.Bytes(), "mktree")
	if err != nil {
		return "", err
	}
	args := []string{"commit-tree", strings.TrimSpace(string(treeID))}
	for _, p := range parents {
		if !objectPattern.MatchString(p) {
			return "", invalid("parent")
		}
		args = append(args, "-p", p)
	}
	args = append(args, "-m", boundedMessage(message))
	out, err := r.gitCmd(ctx, nil, args...)
	if err != nil {
		return "", err
	}
...
func boundedMessage(s string) string {
	...
	if s == "" {
		return "Swarm artifact revision"
	}
	if len(s) > 1024 {
		return s[:1024]
	}
...
func validateManifest(m Manifest, limits Limits) error {
	if m.Version != ManifestVersion || (m.Content == nil) == (len(m.Parts) == 0) {
		return fmt.Errorf("%w: invalid manifest shape", ErrIntegrity)
	}
	...
	for _, p := range all {
		if !idPattern.MatchString(p.ID) || !objectPattern.MatchString(p.Blob) || p.Size < 0 || p.Size > limits.MaxBlobBytes || seen[p.ID] {
			return fmt.Errorf("%w: invalid part", ErrIntegrity)
		}
		seen[p.ID] = true
		total += p.Size
	}
...
func (r *Repository) ReadCommit(ctx context.Context, id string) (Commit, error) {
	if !objectPattern.MatchString(id) {
		return Commit{}, invalid("commit")
	}
	typeOut, err := r.gitCmd(ctx, nil, "cat-file", "-t", id)
	...
	parents := strings.Fields(string(parentOut))
	data, err := r.gitCmd(ctx, nil, "show", id+":"+manifestPath)
	if err != nil {
		return Commit{}, err
	}
	var m Manifest
	if json.Unmarshal(data, &m) != nil {
		return Commit{}, ErrIntegrity
	}
	if err := validateManifest(m, r.limits); err != nil {
		return Commit{}, err
	}
...
func (r *Repository) ReadBlob(ctx context.Context, commit, partID string) ([]byte, error) {
	...
	p, ok := findPart(c.Manifest, partID)
	if !ok {
...
func (r *Repository) Candidate(ctx context.Context, req CandidateRequest) (string, error) {
	...
	base, err := r.ReadCommit(ctx, req.Base)
	if err != nil {
		return "", err
	}
	m := base.Manifest
	if req.Content != nil {
		if m.Content == nil {
			return "", invalid("content change on multipart artifact")
		}
		if m.Content.Locked {
			return "", ErrLockedPart
		}
		p, err := r.storeBlob(ctx, "content", *req.Content)
		if err != nil {
			return "", err
		}
		p.Locked = m.Content.Locked
		m.Content = &p
	}
	for id, ch := range req.Parts {
		if !idPattern.MatchString(id) {
		...
		if m.Parts[idx].Locked && len(ch.Bytes) > 0 {
			return "", ErrLockedPart
		}
		if len(ch.Bytes) > 0 {
			p, err := r.storeBlob(ctx, id, BlobInput{MediaType: ch.MediaType, Bytes: ch.Bytes})
			if err != nil {
				return "", err
			}
			p.Locked = m.Parts[idx].Locked
			m.Parts[idx] = p
		}
		if ch.Lock != nil {
	...
	ref := "refs/swarm/candidates/" + req.ID
	if old, e := r.ref(ctx, ref); e == nil {
		if old == commit {
			return commit, nil
		}
		return "", ErrConflict
...
func (r *Repository) Merge(ctx context.Context, req MergeRequest) (string, error) {
	if !idPattern.MatchString(req.ID) || len(req.Parents) < 2 {
		return "", invalid("merge")
	}
	commits := make(map[string]Commit, len(req.Parents))
	for _, id := range req.Parents {
		c, e := r.ReadCommit(ctx, id)
		if e != nil {
			return "", e
		}
		commits[id] = c
	}
	base := commits[req.Parents[0]].Manifest
	for partID, sel := range req.Selections {
		c, ok := commits[sel.Commit]
		if !ok {
			return "", invalid("selection parent")
		}
		sourceID := sel.PartID
		if sourceID == "" {
			sourceID = partID
		}
		p, ok := findPart(c.Manifest, sourceID)
		if !ok {
			return "", ErrNotFound
		}
		current, ok := findPart(base, partID)
		if !ok {
			return "", ErrNotFound
		}
		if current.Locked && current.Blob != p.Blob {
			return "", ErrLockedPart
		}
		p.ID = partID
		...
		if base.Content != nil {
			...
			for i := range base.Parts {
				if base.Parts[i].ID == partID {
					base.Parts[i] = p
				}
			}
		}
	}
	commit, err := r.commit(ctx, base, req.Parents, req.Message)
	if err != nil {
		return "", err
	}
	ref := "refs/swarm/candidates/" + req.ID
...
func (r *Repository) AdvanceOfficial(ctx context.Context, expected, next, transactionID string) (string, error) {
	...
	if _, e := r.ReadCommit(ctx, next); e != nil {
		return "", e
	}
	tx := "refs/swarm/transactions/" + transactionID
	if old, e := r.ref(ctx, tx); e == nil {
		if old == next {
			return old, nil
		}
		return "", ErrTransactionReuse
	}
	input := fmt.Sprintf("start\nupdate refs/heads/official %s %s\ncreate %s %s\nprepare\ncommit\n", next, expected, tx, next)
	if _, err := r.gitCmd(ctx, []byte(input), "update-ref", "--stdin"); err != nil {
		return "", fmt.Errorf("%w: %v", ErrConflict, err)
	}
	return next, nil
}

func (r *Repository) ref(ctx context.Context, name string) (string, error) {
	out, err := r.gitCmd(ctx, nil, "rev-parse", "--verify", name+"^{commit}")
	if err != nil {
		return "", ErrNotFound
	}
	return strings.TrimSpace(string(out)), nil
}
func (r *Repository) updateRef(ctx context.Context, name, next, old string) error {
	_, err := r.gitCmd(ctx, nil, "update-ref", name, next, old)
	return err
}
...
func (r *Repository) ListRefs(ctx context.Context, prefix string) ([]Ref, error) {
	...
	if !allowed[prefix] {
		return nil, invalid("ref prefix")
	}
	out, err := r.gitCmd(ctx, nil, "for-each-ref", "--format=%(refname) %(objectname)", prefix)
	if err != nil {
		return nil, err
	}
	...
	for _, line := range lines {
		...
		refs = append(refs, Ref{Name: f[0], Commit: f[1]})
	}
	if len(refs) > r.limits.MaxRefs {
...
func (r *Repository) IntegrityCheck(ctx context.Context) error {
	_, err := r.gitCmd(ctx, nil, "fsck", "--full", "--strict")
	return err
}
func (r *Repository) Bundle(ctx context.Context, dst string) error {
	...
	_, err := r.gitCmd(ctx, nil, "bundle", "create", dst, "--all")
	if err == nil {
...
func (r *Repository) Materialize(ctx context.Context, commit, dst string) error {
	...
	if c.Manifest.Content != nil {
		b, e := r.ReadBlob(ctx, commit, "content")
		if e != nil {
			return e
		}
		return os.WriteFile(filepath.Join(dst, "content"), b, 0o600)
	}
	for _, p := range c.Manifest.Parts {
		b, e := r.ReadBlob(ctx, commit, p.ID)
		if e != nil {
			return e
		}
		if e = os.WriteFile(filepath.Join(dst, p.ID), b, 0o600); e != nil {
			return e
		}
	}
	return nil
