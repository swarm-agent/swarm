...
func (f *automationOutboxFake) AckAutomationDelivery(store.AutomationDelivery) error {
	f.ack++
	return nil
}

type automationNotificationsFake struct {
	err     error
	record  store.NotificationRecord
	account string
}

func (f *automationNotificationsFake) UpsertSystemNotificationForAccount(a string, r store.NotificationRecord) (store.NotificationRecord, bool, error) {
	f.account, f.record = a, r
	return r, true, f.err
}
...
func TestAutomationDeliveryFailureAndRedaction(t *testing.T) {
	...
	if err := s.Deliver(context.Background(), r); err == nil || err.Error() != "automation notification persistence failed" {
	...
	n.err = nil
	if err := s.Deliver(context.Background(), r); err != nil {
		t.Fatal(err)
	}
	if o.ack != 1 || n.account != r.Scope.AccountID || n.record.ID != r.NotificationID() || n.record.Body != "Automation failed. Open the execution session for details." || n.record.ActionURL != "" {
		t.Fatal("delivery identity/redaction mismatch")
	}
}

type automationAckFailure struct {
	automationOutboxFake
	fail bool
}

func (f *automationAckFailure) AckAutomationDelivery(d store.AutomationDelivery) error {
	if f.fail {
		return errors.New("ack interrupted")
	}
...
func TestAutomationDeliveryAckRecovery(t *testing.T) {
	...
	if err := s.Deliver(context.Background(), ref); err == nil || o.ack != 0 {
	...
	if err := s.Deliver(context.Background(), ref); err != nil || o.ack != 1 || n.record.ID != id {
