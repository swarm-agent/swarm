...
func (b *recoveryOutput) Write(p []byte) (int, error) {
	if len(p) > recoveryByteLimit-b.Len() { return 0, errors.New("recovery Git output exceeds byte limit") }
...
func recoveryGit(path string, input []byte, args ...string) ([]byte, error) {
	...
	for _, value := range os.Environ() {
		if !strings.HasPrefix(value, "GIT_") { cmd.Env = append(cmd.Env, value) }
	}
	...
	if err := cmd.Run(); err != nil { return nil, fmt.Errorf("recovery git %s: %w: %s", args[0], err, stderr.String()) }
	return out.Bytes(), nil
}

func recoveryExactPath(path string) error {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path { return errors.New("recovery requires a clean absolute path") }
	resolved, err := filepath.EvalSymlinks(path)
	if err != nil { return err }
	if resolved != path { return errors.New("recovery refuses symlink path components") }
	return nil
}
...
func recoveryIdentity(repository, candidate string) (RecoveryIdentity, error) {
	...
	for _, path := range []string{repository, candidate} {
		if err := recoveryExactPath(path); err != nil { return id, err }
	}
	resolve := func(path, flag string) (string, error) {
		...
		if err := recoveryExactPath(value); err != nil { return "", err }
		return value, nil
	}
	for _, path := range []string{repository, candidate} {
		if err := recoveryExactPath(filepath.Join(path, ".git")); err != nil { return id, err }
	}
	common, err := resolve(repository, "--git-common-dir")
	if err != nil { return id, err }
	top, err := resolve(candidate, "--show-toplevel")
	if err != nil || top != candidate { return id, errors.New("recovery candidate is not an exact checkout root") }
	candidateCommon, err := resolve(candidate, "--git-common-dir")
	if err != nil || candidateCommon != common { return id, errors.New("recovery candidate common directory mismatch") }
	admin, err := resolve(candidate, "--absolute-git-dir")
	if err != nil { return id, err }
	if admin != common && !pathWithinRoot(filepath.Join(common, "worktrees"), admin) { return id, errors.New("recovery admin directory outside repository authority") }
	out, err := recoveryGit(repository, nil, "worktree", "list", "--porcelain", "-z")
	if err != nil { return id, err }
	registered := false
	for _, field := range bytes.Split(out, []byte{0}) {
		if string(field) == "worktree "+candidate { registered = true }
	}
	if !registered { return id, errors.New("recovery candidate is not Git registered") }
	if admin != common {
		back, err := recoveryRead(admin, "gitdir")
		if err != nil || strings.TrimSuffix(string(back.Data), "\n") != filepath.Join(candidate, ".git") { return id, errors.New("recovery admin backlink mismatch") }
	}
...
func DiscoverRecoveryWorktrees(repository string) ([]RecoveryIdentity, error) {
	if err := recoveryExactPath(repository); err != nil { return nil, err }
	out, err := recoveryGit(repository, nil, "worktree", "list", "--porcelain", "-z")
	if err != nil { return nil, err }
	var result []RecoveryIdentity
	for _, field := range bytes.Split(out, []byte{0}) {
		if !bytes.HasPrefix(field, []byte("worktree ")) { continue }
		if len(result) >= 128 { return nil, errors.New("recovery inventory exceeds 128 worktrees") }
...
func recoveryName(name string) error {
	if name == "" || filepath.IsAbs(name) || filepath.ToSlash(filepath.Clean(name)) != name || name == "." || name == ".." || strings.HasPrefix(name, "../") || strings.ContainsAny(name, "\x00\\") { return errors.New("recovery requires exact relative file names") }
	for _, part := range strings.Split(name, "/") {
		if strings.EqualFold(part, ".git") { return errors.New("recovery refuses Git administration paths") }
	}
	return nil
}
...
func recoveryRead(root, name string) (recoveryFile, error) {
	...
	if err := recoveryName(name); err != nil { return f, err }
	...
	for current := path; current != root; current = filepath.Dir(current) {
		...
		if st.Mode()&os.ModeSymlink != 0 { return f, errors.New("recovery refuses symlinks") }
	}
	st, err := os.Lstat(path)
	if os.IsNotExist(err) { return f, nil }
	if err != nil { return f, err }
	if !st.Mode().IsRegular() { return f, errors.New("recovery refuses special files and submodules") }
	if st.Size() > recoveryByteLimit { return f, errors.New("recovery file exceeds byte limit") }
	...
	if err != nil || !os.SameFile(st, opened) { return f, errors.New("recovery file identity changed") }
	f.Data, err = io.ReadAll(io.LimitReader(file, recoveryByteLimit+1))
	if err != nil { return f, err }
	if len(f.Data) > recoveryByteLimit { return f, errors.New("recovery file exceeds byte limit") }
	f.Mode = st.Mode()
	return f, nil
}
...
func inspectRecovery(repository, candidate string) (RecoveryIdentity, error) {
	...
	for _, args := range [][]string{{"ls-files", "--stage", "-z"}, {"diff", "--binary", "--no-ext-diff", "--no-textconv", "--no-renames", "--cached", "HEAD", "--"}, {"diff", "--binary", "--no-ext-diff", "--no-textconv", "--no-renames", "--"}} {
		...
		if args[0] == "ls-files" {
			for _, entry := range bytes.Split(out, []byte{0}) {
				...
				if (!strings.HasPrefix(header, "100644 ") && !strings.HasPrefix(header, "100755 ")) || !strings.HasSuffix(header, " 0") { return id, errors.New("recovery refuses unmerged index, symlinks and submodules") }
			}
		}
		h.Write(out)
	}
	...
	if len(names) > recoveryFileLimit+1 { return id, errors.New("recovery file count exceeds limit") }
	total := 0
	for _, name := range names {
		...
		if err != nil { return id, err }
		total += len(f.Data)
		if total > recoveryByteLimit { return id, errors.New("recovery snapshot exceeds byte limit") }
		fmt.Fprintf(h, "%q:%d:%d:", f.Name, f.Mode, len(f.Data))
		h.Write(f.Data)
	}
	end, err := recoveryIdentity(repository, candidate)
	if err != nil || end != id { return id, errors.New("recovery source identity drift") }
...
func SnapshotRecovery(repository string, expected RecoveryIdentity, selection RecoverySelection) (*RecoverySnapshot, error) {
	if len(selection.Commits) != 0 || len(selection.Patch) != 0 { return nil, errors.New("exact commit and arbitrary patch recovery are unsupported; select exact working files explicitly") }
	if len(selection.Files) == 0 || len(selection.Files) > recoveryFileLimit { return nil, errors.New("recovery requires bounded explicit file selection") }
	before, err := inspectRecovery(repository, expected.Path)
	if err != nil { return nil, err }
	if before != expected { return nil, errors.New("stale recovery source") }
	...
	for i, name := range s.selection {
		if err := recoveryName(name); err != nil { return nil, err }
		if i > 0 && name == s.selection[i-1] { return nil, errors.New("duplicate recovery selection") }
	}
	for _, layer := range []struct{ cached bool; target *[]byte }{{true, &s.staged}, {false, &s.unstaged}} {
		args := []string{"diff", "--binary", "--no-ext-diff", "--no-textconv", "--no-renames"}
		if layer.cached { args = append(args, "--cached", "HEAD") }
		args = append(args, "--")
	...
	for _, name := range bytes.Split(out, []byte{0}) { untracked[string(name)] = true }
	for _, name := range s.selection {
		...
		if err == nil && string(tracked) != name+"\x00" { return nil, errors.New("recovery selection must name a file, not a directory") }
		if err != nil || len(tracked) == 0 {
			// A staged deletion no longer appears in the index.
			base, baseErr := recoveryGit(expected.Path, nil, "ls-tree", "-z", "HEAD", "--", name)
			if baseErr != nil || len(base) == 0 || (!bytes.HasPrefix(base, []byte("100644 ")) && !bytes.HasPrefix(base, []byte("100755 "))) || !bytes.HasSuffix(base, []byte("\t"+name+"\x00")) { return nil, fmt.Errorf("selection %q is not an exact tracked or nonignored untracked file", name) }
		}
	}
	after, err := inspectRecovery(repository, expected.Path)
	if err != nil || after != before { return nil, errors.New("recovery source changed during snapshot") }
	return s, nil
}
...
func (s *Service) CopyRecovery(snapshot *RecoverySnapshot, nameSeed, branch string) (result RecoveryResult, err error) {
	if snapshot == nil { return result, errors.New("recovery snapshot required") }
	result.Source = snapshot.Identity
	defer func() { if err != nil { result.Diagnostic = err.Error() } }()
	current, err := inspectRecovery(snapshot.repository, snapshot.Identity.Path)
	if err != nil { return result, err }
	if current != snapshot.Identity { return result, errors.New("recovery source changed before allocation") }
	...
	for _, layer := range []struct{ data []byte; cached bool }{{snapshot.staged, true}, {snapshot.unstaged, false}} {
		if len(layer.data) == 0 { continue }
		args := []string{"apply", "--binary", "--whitespace=nowarn"}
		if layer.cached { args = append(args, "--index") }
		if _, err = recoveryGit(dest, layer.data, append(args, "--check", "-")...); err != nil { return result, err }
		if _, err = recoveryGit(dest, layer.data, append(args, "-")...); err != nil { return result, err }
	}
	...
	for _, f := range snapshot.untracked {
		...
		if err = destRoot.MkdirAll(filepath.Dir(name), 0700); err != nil { return result, err }
		var file *os.File
	...
	current, err = inspectRecovery(snapshot.repository, snapshot.Identity.Path)
	if err != nil { return result, err }
	if current != snapshot.Identity { return result, errors.New("recovery source changed during import; allocation retained, publication forbidden") }
...
func ValidateRecoveryIdentity(repository string, expected RecoveryIdentity) error {
	...
	if err != nil { return err }
	if actual != expected { return errors.New("stale recovery identity") }
	return nil
}
