...
- Managed output uses only artifact_v2_author. Start with inspect_context. For a new artifact, declare real independently revisioned parts, then write parts strictly one at a time. After every successful write_part, use the returned composition_head_revision for the next write; never substitute the Working Artifact revision and never send another part's revision as expected_base_revision_id (omit that field for a part's first write). If a stale-revision error occurs, call inspect_context once and retry with its exact composition_head_revision and that target part's current_revision_id. Then request the server-owned build and validation, repair invalid exact revisions when diagnostics are repairable, and submit_candidate immediately after the exact composition is ready. The server owns destination identity, output policy, animation profile, compiler/runtime shell, renderer controls, validation status, lineage, and candidate slot; never attempt to supply or override them. Part bytes persist before validation, so a failed validation is a visible repairable composition rather than a lost publication attempt. Repairs must name the exact prior part revision and composition-head revision and preserve every untouched part byte-for-byte. Do not call manage_artifact for managed output.
- Workspace output uses write and edit only within the concrete declared owned scope. Never call artifact_v2_author or manage_artifact for workspace output.
- Exact output requirements and animation profile are trusted server context returned by inspect_context. Obey them, but do not author host runtime protocols or claim renderer/pixel success without exact evidence returned by validation.
...
On a tool failure, preserve the exact safe diagnostic and retry only when a new part revision can correct it. Never classify raw errors, allocate another destination, fall back to legacy publication, or investigate product internals. Use only the locked tools; do not run commands, use Git, orchestrate agents, request user interaction, or change plans, sessions, settings, permissions, agents, themes, skills, or todos.`)
}
...
func designerAgentToolContract(managed bool) *pebblestore.AgentToolContract {
	return &pebblestore.AgentToolContract{Preset: "custom", Tools: map[string]pebblestore.AgentToolConfig{
		...
		"write": {Enabled: pebblestore.BoolPtr(!managed)}, "edit": {Enabled: pebblestore.BoolPtr(!managed)}, "artifact_v2_author": {Enabled: pebblestore.BoolPtr(managed)}, "manage_artifact": {Enabled: pebblestore.BoolPtr(false)},
