...
func (r *Registry) OwnedSession(sessionID, accountScopeID, userID string) (pebblestore.SessionSnapshot, error) {
	if r == nil || r.resolver == nil { return pebblestore.SessionSnapshot{}, errors.New("artifact registry session resolver is not configured") }
	session, ok, err := r.resolver.GetSession(strings.TrimSpace(sessionID)); if err != nil { return pebblestore.SessionSnapshot{}, err }
	if !ok { return pebblestore.SessionSnapshot{}, fmt.Errorf("session %q was not found", sessionID) }
	if session.AccountScopeID != strings.TrimSpace(accountScopeID) || session.UserID != strings.TrimSpace(userID) { return pebblestore.SessionSnapshot{}, errors.New("artifact session ownership does not match") }
	return session, nil
}
...
func (r *Registry) Repository(ctx context.Context, repositoryID string) (*artifactgit.Repository, error) {
	...
	if strings.TrimSpace(r.repositoryRoot) == "" { return nil, errors.New("artifact Git repository root is not configured") }
	...
	repo, err := artifactgit.Open(ctx, r.repositoryRoot, repositoryID, limits); if err != nil { return nil, err }
...
func (r *Registry) RunMaintenance(limit int) (MaintenanceReport, error) {
	...
	tombstones, err := r.resolver.ListPendingSessionArtifactCleanups(limit); if err != nil { return report, err }
	...
	remaining := limit-report.SessionsVisited; if remaining <= 0 { return report, nil }
	sessions, err := r.resolver.ListSessions(remaining); if err != nil { return report, err }
	for _, session := range sessions {
		repaired, err := r.resolver.RepairSessionArtifactCollections(session.ID); if err != nil { return report, err }
		report.SessionsVisited++; report.CollectionsRepaired += repaired.CollectionsRepaired
	}
	return report, nil
}
