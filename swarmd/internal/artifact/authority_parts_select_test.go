...
func TestAuthoritySelectsAndLocksExactPartRevisionsAtomically(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if selected.Composition == nil || selected.Composition.Parent == nil || !selected.Composition.Parts[0].Locked || !selected.Composition.Parts[1].Locked { t.Fatalf("selected=%#v", selected) }
	if selected.Composition.Parts[0].Revision != source.Composition.Parts[0].Revision || selected.Composition.Parts[1].Revision != source.Composition.Parts[1].Revision { t.Fatal("selection copied or changed exact part revisions") }
	...
	if err != nil || replayed.ID != selected.ID { t.Fatalf("idempotent replay=%#v err=%v", replayed, err) }
}
...
func TestAuthorityPartSelectionRejectsDuplicateAndStaleRevisionIdentity(t *testing.T) {
	...
	if _, err := authority.SelectPartRevisions(context.Background(), principal, base); err == nil || !strings.Contains(err.Error(), "duplicate") { t.Fatalf("duplicate error=%v", err) }
	base.Choices = []PartRevisionChoiceInput{{PartID: "hero", Revision: ref, RevisionEventSeq: seq + 1}}
	if _, err := authority.SelectPartRevisions(context.Background(), principal, base); err == nil || !strings.Contains(err.Error(), "stale") { t.Fatalf("stale error=%v", err) }
}
