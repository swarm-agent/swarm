...
func removeDirectoryTree(root string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 || !info.IsDir() {
		return errors.New("artifact removal target is not a real directory")
	}
	...
	for _, entry := range entries {
		...
		if current.Mode()&os.ModeSymlink != 0 {
			return errors.New("artifact removal target contains a symlink")
		}
		if current.IsDir() {
		...
		if !current.Mode().IsRegular() {
			return errors.New("artifact removal target contains a special file")
		}
