...
func (a *Authority) publishGitVariant(ctx context.Context, principal Principal, input CreateInput, variant *pebblestore.SessionArtifactVariant, body []byte) error {
	...
	if input.SourceSessionID != "" {
		...
		if source.RepositoryID == "" || source.CommitOID == "" { return errors.New("source artifact is missing exact Git identity") }
		chainID, variant.RepositoryID, base = source.ArtifactChainID, source.RepositoryID, source.CommitOID
	}
	if variant.RepositoryID == "" { variant.RepositoryID = chainID }
	...
	if base == "" {
		...
		if err == nil && input.AutoAccept { _, err = repo.AdvanceOfficial(ctx, base, variant.CommitOID, artifactGitID("tx", input.RequestID)) }
	}
	if err != nil { return fmt.Errorf("publish artifact Git commit: %w", err) }
...
func (a *Authority) readGitVariant(ctx context.Context, variant pebblestore.SessionArtifactVariant, maxBytes int64) ([]byte, error) {
	if variant.RepositoryID == "" || variant.CommitOID == "" { return nil, errors.New("artifact is missing exact Git identity") }
	repo, err := a.repository(ctx, variant.RepositoryID)
	if err != nil { return nil, err }
	body, err := repo.ReadBlob(ctx, variant.CommitOID, "content")
	if err != nil { return nil, err }
	if maxBytes <= 0 || int64(len(body)) > maxBytes { return nil, errors.New("artifact exceeds read byte bound") }
	return body, nil
}
...
func (a *Authority) attachGitProjection(ctx context.Context, principal Principal, requestID, kind string, mutation *pebblestore.V3ArtifactMutation) error {
	...
	if variant == nil && mutation.Selection != nil {
		...
		if err != nil { return err }
		if !ok { return errors.New("selected artifact Git projection was not found") }
		variant = &selected
	}
	if variant == nil || variant.RepositoryID == "" || variant.CommitOID == "" || variant.ArtifactChainID == "" { return errors.New("artifact mutation is missing exact Git identity") }
	parents := append([]string(nil), variant.ParentCommitOIDs...)
	expected := ""
	if len(parents) > 0 { expected = parents[0] }
	...
	if kind == pebblestore.V3SessionMutationSelectArtifact {
		if variant.AutoAccept {
			...
			if err != nil { return err }
			if !ok { return errors.New("artifact Git chain projection was not found") }
			expected = chain.OfficialCommitOID
			repo, err := a.repository(ctx, variant.RepositoryID)
			if err != nil { return err }
			if _, err = repo.AdvanceOfficial(ctx, expected, variant.CommitOID, artifactGitID("tx", requestID)); err != nil { return err }
			state, resulting = "committed", variant.CommitOID
		}
	}
