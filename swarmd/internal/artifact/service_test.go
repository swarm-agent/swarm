...
func TestStatelessMaterializationVerifiesGitProjection(t *testing.T) {
	...
	workspace := t.TempDir()
	if _, err := MaterializeBytes(context.Background(), Limits{}, variant, body, workspace, "out/note.txt", false); err != nil { t.Fatal(err) }
	got, err := os.ReadFile(filepath.Join(workspace, "out", "note.txt")); if err != nil || string(got) != string(body) { t.Fatalf("materialized=%q err=%v", got, err) }
	if _, err := MaterializeBytes(context.Background(), Limits{}, variant, []byte("wrong"), workspace, "bad.txt", false); err == nil { t.Fatal("accepted bytes that do not match Git projection") }
}
