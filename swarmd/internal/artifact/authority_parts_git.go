...
func gitConstruction(in pebblestore.SessionArtifactConstruction) artifactgit.Construction {
	...
	for _, entry := range in.Entries { out.Entries = append(out.Entries, artifactgit.ConstructionEntry{PartID: entry.PartID, Path: entry.Path}) }
	return out
}
...
func (a *Authority) publishGitInitialComposition(ctx context.Context, input CreateInitialCompositionInput, variant *pebblestore.SessionArtifactVariant, composition *pebblestore.SessionArtifactComposition, revisions []pebblestore.SessionArtifactPartRevision) ([]pebblestore.SessionArtifactPartRevision, error) {
	...
	for _, part := range input.Parts { parts[part.Definition.ID] = artifactgit.BlobInput{MediaType: part.MediaType, Bytes: part.Body} }
	...
	for index := range revisions {
		...
		for slotIndex := range composition.Parts {
			if composition.Parts[slotIndex].PartID == revisions[index].PartID { composition.Parts[slotIndex].Revision = revisions[index].Reference() }
		}
	}
...
func (a *Authority) publishGitReplacement(ctx context.Context, input PublishPartReplacementsInput, source pebblestore.SessionArtifactVariant, variant *pebblestore.SessionArtifactVariant, composition *pebblestore.SessionArtifactComposition, revisions []pebblestore.SessionArtifactPartRevision) ([]pebblestore.SessionArtifactPartRevision, error) {
	if source.RepositoryID == "" || source.CommitOID == "" { return nil, errors.New("multipart source is missing exact Git identity") }
	repo, err := a.repository(ctx, source.RepositoryID)
	if err != nil { return nil, err }
	changes := make(map[string]artifactgit.PartChange, len(input.Replacements))
	for _, replacement := range input.Replacements { locked := replacement.Locked; changes[replacement.PartDefinition.ID] = artifactgit.PartChange{MediaType: replacement.MediaType, Bytes: replacement.Body, Lock: &locked} }
	candidateID := artifactGitID("candidate", input.ArtifactStepID, input.VariantID)
	commitOID, err := repo.Candidate(ctx, artifactgit.CandidateRequest{ID: candidateID, Base: source.CommitOID, Parts: changes, Message: "artifact multipart candidate"})
	if err != nil { return nil, err }
	commit, err := repo.ReadCommit(ctx, commitOID)
	if err != nil { return nil, err }
	...
	for index := range revisions { changed[revisions[index].PartID] = index }
	for slotIndex := range composition.Parts {
		...
		if composition.Parts[slotIndex].Revision.BlobOID != blobOID { return nil, artifactgit.ErrIntegrity }
	}
	if input.AutoAccept { if _, err = repo.AdvanceOfficial(ctx, source.CommitOID, commitOID, artifactGitID("tx", input.RequestID)); err != nil { return nil, err } }
	return revisions, nil
}
...
func (a *Authority) readGitPartRevision(ctx context.Context, revision pebblestore.SessionArtifactPartRevision, maxBytes int64) ([]byte, error) {
	if revision.RepositoryID == "" || revision.CommitOID == "" || revision.BlobOID == "" { return nil, errors.New("artifact part revision is missing exact Git identity") }
	repo, err := a.repository(ctx, revision.RepositoryID)
	if err != nil { return nil, err }
	actual, err := repo.ReadBlobOID(ctx, revision.CommitOID, revision.PartID)
	if err != nil { return nil, err }
	if actual != revision.BlobOID { return nil, artifactgit.ErrIntegrity }
	body, err := repo.ReadBlob(ctx, revision.CommitOID, revision.PartID)
	if err != nil { return nil, err }
	if maxBytes <= 0 || int64(len(body)) > maxBytes { return nil, errors.New("artifact part exceeds read byte bound") }
	return body, nil
}
