...
func (a *Authority) publishGitPartSelection(ctx context.Context, input SelectPartRevisionsInput, source pebblestore.SessionArtifactVariant, variant *pebblestore.SessionArtifactVariant, composition *pebblestore.SessionArtifactComposition) error {
	if source.RepositoryID == "" || source.CommitOID == "" {
		return errors.New("part selection source is missing exact Git identity")
	}
	...
	if !seenParents[source.CommitOID] {
		parents = append(parents, source.CommitOID)
		seenParents[source.CommitOID] = true
	}
	selections := make(map[string]artifactgit.Selection, len(input.Choices))
	for _, choice := range input.Choices {
		if choice.Revision.RepositoryID != source.RepositoryID || choice.Revision.CommitOID == "" {
			return errors.New("part selection crosses artifact Git repositories")
		}
		if _, readErr := repo.ReadCommit(ctx, choice.Revision.CommitOID); readErr != nil {
			return readErr
		}
		actualBlob, blobErr := repo.ReadBlobOID(ctx, choice.Revision.CommitOID, choice.Revision.PartID)
		if blobErr != nil || actualBlob != choice.Revision.BlobOID {
			return artifactgit.ErrIntegrity
		}
		if !seenParents[choice.Revision.CommitOID] {
			parents = append(parents, choice.Revision.CommitOID)
			seenParents[choice.Revision.CommitOID] = true
		}
	...
	if len(parents) == 1 {
		...
		for _, choice := range input.Choices {
			locked := choice.Locked
			changes[choice.PartID] = artifactgit.PartChange{Lock: &locked}
		}
