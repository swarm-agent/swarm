...
func artifactByteLimit(limits Limits, mediaType, kind string) int64 {
	if isVideoMediaTypeOrKind(mediaType, kind) {
		return limits.MaxVideoArtifactBytes
	}
...
func canonicalArtifactBytes(ctx context.Context, limits Limits, variant pebblestore.SessionArtifactVariant, body []byte) ([]byte, string, int64, error) {
	...
	if strings.TrimSpace(variant.Filename) == "" || filepath.Base(variant.Filename) != variant.Filename || strings.ContainsAny(variant.Filename, `/\\`) {
		return nil, "", 0, errors.New("artifact filename is unsafe")
	}
	if strings.TrimSpace(variant.MediaType) == "" {
		return nil, "", 0, errors.New("artifact media type is required")
	}
	limit := limits.MaxArtifactBytes
	if isVideoMediaTypeOrKind(variant.MediaType, variant.Presentation.Kind) {
		limit = limits.MaxVideoArtifactBytes
	}
	...
	if variant.MediaType == "application/zip" || variant.Presentation.Kind == "package" {
		if variant.MediaType != "application/zip" {
			return nil, "", 0, errors.New("artifact package has an invalid media type")
		}
		if _, _, err := readPackageBytes(limits, body, "", limits.MaxPackageEntryBytes); err != nil {
			return nil, "", 0, err
		}
	}
	if int64(len(body)) > limits.MaxSessionBytes {
		return nil, "", 0, ErrQuotaExceeded
	}
...
func readArtifactFile(ctx context.Context, limits Limits, variant pebblestore.SessionArtifactVariant, sourcePath string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("inspect artifact import: %w", err)
	}
	if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
		return nil, errors.New("artifact import source must be a regular non-symlink file")
	}
	resolved, err := filepath.EvalSymlinks(absolute)
	if err != nil || filepath.Clean(resolved) != absolute {
		return nil, errors.New("artifact import source path contains a symlink")
	}
	limit := normalizeLimits(limits).MaxArtifactBytes
	if isVideoMediaTypeOrKind(variant.MediaType, variant.Presentation.Kind) {
		limit = normalizeLimits(limits).MaxVideoArtifactBytes
	}
	if info.Size() > limit {
	...
	if int64(len(body)) != info.Size() || int64(len(body)) > limit {
		return nil, errors.New("artifact import changed while reading")
	}
	return body, ctx.Err()
}
...
func canonicalPackageEntries(ctx context.Context, limits Limits, entries []PackageEntry) ([]byte, error) {
	...
	for _, item := range copyEntries {
		...
		if name == "" || name != item.Name || len(name) > 1024 || strings.Contains(name, "\\") || !safePackageEntryName(name) || seen[strings.ToLower(name)] {
			_ = archive.Close()
			return nil, errors.New("artifact package contains an unsafe or duplicate entry")
		}
		seen[strings.ToLower(name)] = true
		expanded += int64(len(item.Data))
		if int64(len(item.Data)) > limits.MaxPackageEntryBytes || expanded > limits.MaxPackageBytes {
			_ = archive.Close()
			return nil, ErrQuotaExceeded
		}
	...
	if int64(out.Len()) > limits.MaxArtifactBytes {
		return nil, ErrQuotaExceeded
	}
...
func packageDirectoryEntries(sourceDir string, limits Limits) ([]PackageEntry, error) {
	limits = normalizeLimits(limits)
	absolute, err := filepath.Abs(strings.TrimSpace(sourceDir))
	if err != nil {
		return nil, err
	}
	absolute = filepath.Clean(absolute)
	...
	err = filepath.WalkDir(absolute, func(path string, entry os.DirEntry, walkErr error) error {
		...
		total += item.Size()
		if len(entries)+1 > limits.MaxPackageFiles || item.Size() > limits.MaxPackageEntryBytes || total > limits.MaxPackageBytes {
		...
		data, readErr := io.ReadAll(io.LimitReader(file, limits.MaxPackageEntryBytes+1))
		closeErr := file.Close()
		if readErr != nil {
			return readErr
		}
		...
		if int64(len(data)) != item.Size() {
			return errors.New("artifact package file changed while reading")
		}
		entries = append(entries, PackageEntry{Name: filepath.ToSlash(rel), Data: data})
		return nil
	})
	return entries, err
}
...
func readPackageBytes(limits Limits, body []byte, entryName string, maxBytes int64) ([]PackageManifestEntry, []byte, error) {
	limits = normalizeLimits(limits)
	if maxBytes <= 0 {
		return nil, nil, errors.New("artifact package read limit is required")
	}
	trimmed := strings.TrimSpace(entryName)
	if entryName != trimmed || strings.Contains(trimmed, "\\") || (trimmed != "" && !safePackageEntryName(trimmed)) {
		return nil, nil, errors.New("artifact package entry name is unsafe")
	}
	archive, err := zip.NewReader(bytes.NewReader(body), int64(len(body)))
	if err != nil {
		return nil, nil, errors.New("artifact package is not a valid zip archive")
	}
	if len(archive.File) == 0 || len(archive.File) > limits.MaxPackageFiles {
	...
	for _, entry := range archive.File {
		if entry.FileInfo().IsDir() || entry.Mode()&os.ModeSymlink != 0 || !entry.Mode().IsRegular() || !safePackageEntryName(entry.Name) {
			return nil, nil, errors.New("artifact package contains an unsafe entry")
		}
		folded := strings.ToLower(entry.Name)
		if names[folded] {
			return nil, nil, errors.New("artifact package contains duplicate or ambiguous entries")
		}
		names[folded] = true
		size := int64(entry.UncompressedSize64)
		total += size
		if size > limits.MaxPackageEntryBytes || total > limits.MaxPackageBytes {
			return nil, nil, ErrQuotaExceeded
		}
	...
	if selected == nil {
		return nil, nil, fmt.Errorf("artifact package entry %q was not found", trimmed)
	}
	...
	if err != nil || int64(len(data)) > maxBytes || int64(len(data)) != int64(selected.UncompressedSize64) {
		return nil, nil, ErrQuotaExceeded
	}
	return nil, data, nil
}
