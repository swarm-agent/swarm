...
func (i ImportVariantInput) SourceReference() pebblestore.SessionArtifactSelectionReference {
	if strings.TrimSpace(i.Source.SessionID) != "" {
...
func (a *Authority) validateAndFetchSource(ctx context.Context, ownedPrincipal Principal, ref pebblestore.SessionArtifactSelectionReference) (validatedSource, error) {
	...
	if _, err := a.registry.OwnedSession(ref.SessionID, ownedPrincipal.AccountScopeID, ownedPrincipal.UserID); err != nil {
	...
	if sourceVariant.PartGraphState == pebblestore.SessionArtifactGraphAuthoritative && sourceVariant.Composition != nil {
		...
		for _, slot := range comp.Parts {
			...
			partBytes, readErr := a.readGitPartRevision(ctx, rev, a.registry.limits.MaxVideoArtifactBytes)
			if readErr != nil {
				return validatedSource{}, fmt.Errorf("read source artifact part %q: %w", partID, readErr)
			}
			digest := sha256.Sum256(partBytes)
			if hex.EncodeToString(digest[:]) != rev.DigestSHA256 {
		...
		body, readErr := a.readGitVariant(ctx, sourceVariant, a.registry.limits.MaxVideoArtifactBytes)
		if readErr != nil {
			return validatedSource{}, fmt.Errorf("read source artifact: %w", readErr)
		}
		digest := sha256.Sum256(body)
		if hex.EncodeToString(digest[:]) != sourceVariant.DigestSHA256 {
...
func (a *Authority) importVariant(ctx context.Context, principal Principal, input ImportVariantInput) (pebblestore.SessionArtifactVariant, error) {
	...
	if destCollectionID == "" {
		destCollectionID = source.variant.CollectionID
		if destCollectionID == "" {
			destCollectionID = "imported"
		}
	}

	destVariantID := strings.TrimSpace(input.VariantID)
	if destVariantID == "" {
		if ownedPrincipal.SessionID == ref.SessionID && destCollectionID == source.variant.CollectionID {
			destVariantID = "imported-" + source.variant.ID
		} else {
			destVariantID = source.variant.ID
		}
	}
	...
	if existing, exists, getErr := a.metadata.GetSessionArtifactVariant(ownedPrincipal.AccountScopeID, ownedPrincipal.SessionID, destCollectionID, destVariantID); getErr != nil {
		...
		if existing.Status == pebblestore.SessionArtifactStatusReady {
			...
			sameDigest := existing.DigestSHA256 == source.variant.DigestSHA256 && existing.Size == source.variant.Size
		...
		if existing.Status != pebblestore.SessionArtifactStatusStaging {
...
func (a *Authority) importMonolithicVariant(ctx context.Context, principal Principal, requestID string, collection pebblestore.SessionArtifactCollection, variantID, chainID, stepID string, lineage pebblestore.SessionArtifactLineage, source validatedSource) (pebblestore.SessionArtifactVariant, error) {
	...
	existing, existingOK, err := a.metadata.GetSessionArtifactVariant(principal.AccountScopeID, principal.SessionID, collection.ID, variant.ID)
	if err != nil {
		return pebblestore.SessionArtifactVariant{}, err
	}
	existingStaging := existingOK && existing.Status == pebblestore.SessionArtifactStatusStaging

	if !existingStaging {
		if _, err := a.mutate(principal, requestID+":stage", pebblestore.V3SessionMutationCreateArtifact, collection, &variant, nil); err != nil {
			return pebblestore.SessionArtifactVariant{}, err
		}
	}

	variant.Status = pebblestore.SessionArtifactStatusReady
	if _, err := a.mutate(principal, requestID+":ready:"+variant.DigestSHA256, pebblestore.V3SessionMutationFinalizeArtifact, collection, &variant, nil); err != nil {
...
func (a *Authority) importCompositionVariant(ctx context.Context, principal Principal, requestID string, collection pebblestore.SessionArtifactCollection, variantID, chainID, stepID string, lineage pebblestore.SessionArtifactLineage, source validatedSource) (pebblestore.SessionArtifactVariant, error) {
	...
	result, err := a.mutateArtifact(principal, requestID+":composition", mutationKind, collection, &variant, nil, destDefs, destRevs, &destComposition)
	if err != nil {
	...
	finalized, err := a.mutateArtifact(principal, requestID+":composition-ready:"+created.DigestSHA256, pebblestore.V3SessionMutationFinalizeArtifact, collection, &created, nil, nil, nil, nil)
	if err != nil {
		return pebblestore.SessionArtifactVariant{}, fmt.Errorf("finalize imported artifact composition: %w", err)
	}
