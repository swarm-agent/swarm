...
1. Propose the trigger worker using manage_workers action=propose with schedule={"kind":"trigger"}. Instruct the user to review and click 'Accept' on the proposal card.
2. Once accepted, the worker ID (av2_...) is permanent and acts as a reusable template. It can be triggered repeatedly to spawn independent execution occurrences.
3. Mint a scoped API deploy token via POST /v3/auth/tokens (or client.auth.createScopedToken({ name, scopes: ["automations:trigger"], worker_id })) with scopes ["automations:trigger"]. Tokens are prefixed with 'swk_' and must be stored securely.
4. Trigger on-demand runs via POST /v3/automations/v2/trigger with {"worker_id": "av2_...", "context": { ... }} or @swarm/sdk client.automations.trigger({ worker_id: "av2_...", context: { ... } }).
5. Each triggered run executes in an isolated worktree and automatically delivers its report and artifacts to the user's Agent Mailbox (/v3/deliverables).
