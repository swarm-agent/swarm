...
func directArtifactV3ProjectManifest(project map[string][]byte) (pebblestore.ArtifactV3Manifest, error) {
 ...
 total := 0
 for _, body := range project { total += len(body); if total > manageArtifactMaxCreateBytes { return manifest, ErrArtifactV3AuthorQuota } }
 if json.Unmarshal(project[pebblestore.ArtifactV3ManifestFilename], &manifest) != nil || manifest.Entrypoint == "" || len(project[manifest.Entrypoint]) == 0 { return manifest, ErrArtifactV3AuthorInvalid }
 return manifest, nil
}
...
func (r *Runtime) discoverDirectArtifactV3(ctx context.Context, scope WorkspaceScope, principal artifact.Principal, args map[string]any) (map[string]any, error) {
 if r.artifactV3Author == nil || scope.SessionID != principal.SessionID { return nil, ErrArtifactV3AuthorInvalid }
 discovery, ok := r.artifactV3Author.repository.(ArtifactV3NativeDiscovery)
 if !ok { return nil, errors.New("native Artifact V3 discovery unavailable") }
 ...
 if id == "" { return nil, ErrArtifactV3AuthorInvalid }
...
func (r *Runtime) selectDirectArtifactV3(ctx context.Context, scope WorkspaceScope, principal artifact.Principal, callID string, args map[string]any) (map[string]any, error) {
 if r.artifactV3Author == nil || scope.SessionID != principal.SessionID { return nil, ErrArtifactV3AuthorUnauthorized }
 if err := requireOnlyArtifactV3Fields(args, "action", "artifact_id", "turn_id", "candidate_id", "expected_head", "expected_turn_revision"); err != nil { return nil, err }
 ...
 if err != nil || !supplied || seq <= 0 || id == "" || turn == "" || candidate == "" || head == "" { return nil, ErrArtifactV3AuthorInvalid }
 selector, ok := r.artifactV3Author.repository.(ArtifactV3NativeSelector)
 if !ok { return nil, errors.New("native Artifact V3 selection unavailable") }
