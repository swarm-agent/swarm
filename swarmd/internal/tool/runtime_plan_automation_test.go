...
type parentEditPlans struct {
	automationToolPlans
	sessions map[string]store.SessionSnapshot
	plan     store.SessionPlanSnapshot
}

func (p parentEditPlans) GetPlanRevision(string, string, int) (store.SessionPlanSnapshot, bool, error) {
	return p.plan, true, nil
}
func (p parentEditPlans) GetSession(id string) (store.SessionSnapshot, bool, error) {
	s, ok := p.sessions[id]
	return s, ok, nil
}
...
func TestRestrictedParentAutomationEdit(t *testing.T) {
	for _, scenario := range []string{"valid", "foreign", "stale", "ordinary", "self-approve"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "foreign" {
				intent.Scope.AccountID = "foreign"
			}
			if scenario == "stale" {
				intent.DefinitionRevision = 1
			}
			if scenario == "self-approve" {
				intent.Definition.Authorization.Mode = "approved_policy"
			}
			...
			if scenario != "valid" {
				if err == nil || repo.writes != 0 || definition.Schedule.Expression != "0 9 * * *" {
					t.Fatal("rejection wrote state", err)
				}
				return
			}
			if err != nil {
			...
			if result["applied"] != false || proposal["path"] != "/v3/automations" || proposal["method"] != "POST" || body["action"] != "save" || body["id"] != "auto" || body["workspace_id"] != "workspace" || body["expected_revision"] != float64(2) || body["mutation_id"] == "" || body["mutation_id"] == "edit" || proposed["session_id"] != "parent" || proposed["schedule"].(map[string]any)["expression"] != "0 18 * * *" {
				t.Fatal("incorrect save proposal", out)
			}
			if repo.writes != 0 || repo.record.Revision != 2 || repo.record.Definition.Schedule.Expression != "0 9 * * *" {
				t.Fatal("proposal mutated state", out)
			}
			var document store.SessionPlanDocument
			if err := json.Unmarshal([]byte(`{"title":"new instructions","checkpoints":[{"id":"cp-1","title":"work","tasks":["new work"],"acceptance_criteria":["done"]}]}`), &document); err != nil {
				t.Fatal(err)
			}
			...
			if err := json.Unmarshal([]byte(instruction), &result); err != nil {
				t.Fatal(err)
			}
			proposal = result["proposal"].(map[string]any)
			body = proposal["body"].(map[string]any)
			if result["applied"] != false || proposal["path"] != "/v3/sessions/parent/plans" || body["activate"] != false || body["approval_state"] != "pending" || repo.writes != 0 {
				t.Fatal("instruction draft granted authority", instruction)
			}
			intent.DefinitionRevision = 1
			if _, err := runtime.ProposeParentAutomationInstructions(context.Background(), scope, intent, "draft", 1, &document); err == nil || repo.writes != 0 {
				t.Fatal("stale instruction proposal admitted")
			}
		})
	}
}
...
func TestTypedAutomationApprovalRouting(t *testing.T) {
	...
	for _, scenario := range []string{"valid", "foreign", "stale", "timing"} {
		...
		if scenario == "timing" {
			intent.Definition.Schedule.Expression = "0 9 * * *"
		}
		...
		if scenario == "valid" {
			...
			proposal := result["proposal"].(map[string]any)
			if result["applied"] != false || proposal["path"] != "/v3/automations/approve" || result["review_kind"] != "automation" {
				t.Fatal(out)
			}
		} else if err == nil {
			t.Fatal("accepted mismatched intent", scenario)
		}
		if repo.writes != 0 || d.Enabled || d.Authorization.ApprovalReference != "" {
			t.Fatal("routing minted authority")
		}
	}
}
