...
func TestManagedArtifactRequirementsAreInjectedAndOverridesRejected(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "must omit output_requirements") {
	...
	if authority.created.OutputRequirements == nil || authority.created.Presentation.Width != 1500 || authority.created.Presentation.Height != 500 || !strings.Contains(output, "output_requirements") {
...
func TestOldArtifactOutputRequirementsCompatibility(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"version":1,"id":"v","collection_id":"c","account_scope_id":"a","session_id":"s","status":"ready","created_at":1,"updated_at":1,"event_seq":1}`), &variant); err != nil {
		t.Fatal(err)
	}
	if variant.OutputRequirements != nil {
		t.Fatalf("legacy requirements = %#v", variant.OutputRequirements)
	}
}
