...
func TestNativeHandoffMultiFileManifest(t *testing.T) {
 ...
 if err != nil || got.Entrypoint != manifest.Entrypoint || got.Parts[0].Locator.Kind != "file" { t.Fatalf("manifest=%+v err=%v", got, err) }
 if project["assets/logo.bin"][1] != 255 { t.Fatal("asset changed") }
 delete(project, manifest.Entrypoint)
 if _, err := directArtifactV3ProjectManifest(project); err == nil { t.Fatal("missing entrypoint accepted") }
}
...
func TestNativeHandoffRejectsInvalidLastSibling(t *testing.T) {
 for _, last := range []any{
  ...
  if err == nil || result != nil { t.Fatalf("partial admission: %+v %v", result, err) }
 }
}
...
func TestNativeHandoffLastPartPreflightNoAllocation(t *testing.T) {
 ...
 if _, err := runtime.reviseDirectArtifactV3HTML(context.Background(), WorkspaceScope{SessionID: "session-1"}, principal, "call", args); err == nil { t.Fatal("invalid final Part accepted") }
 if len(repo.turns) != 0 || len(repo.submits) != 1 || len(repo.selected) != 0 { t.Fatal("preflight mutated state") }
}
...
func (r *nativeHandoffRepo) ResolveArtifactV3SelectedSource(_ context.Context, account, user, session, id, commit string, seq uint64) (pebblestore.ArtifactV3SelectedSource, error) {
 if account != "account-1" || user != "user-1" || session != "session-1" { return pebblestore.ArtifactV3SelectedSource{}, pebblestore.ErrArtifactV3Unauthorized }
...
func TestNativeHandoffDiscoveryTuple(t *testing.T) {
 ...
 if source["projection_seq"] != uint64(7) || source["commit_oid"] != strings.Repeat("a",40) || source["session_id"] != reference["session_id"] || source["artifact_id"] != reference["artifact_id"] || reference["revision_ref"] != "revision-"+source["commit_oid"].(string) { t.Fatalf("incomplete tuple: %+v", output) }
 principal.AccountScopeID = "foreign"
 if _, err := runtime.discoverDirectArtifactV3(context.Background(), WorkspaceScope{SessionID: "session-1"}, principal, args); err == nil { t.Fatal("foreign principal accepted") }
 if len(repo.turns) != 0 || len(repo.selected) != 0 { t.Fatal("discovery mutated state") }
}
...
func (r *nativeHandoffRepo) SelectArtifactV3Exact(_ context.Context, account, user, session, id, turn, candidate, head, request string, seq uint64) (ArtifactV3Revision, error) {
 if head != "revision-"+strings.Repeat("a",40) || seq != 7 { return ArtifactV3Revision{}, pebblestore.ErrArtifactV3Conflict }
...
func TestNativeHandoffExplicitStaleSelection(t *testing.T) {
 ...
 if _, err := runtime.selectDirectArtifactV3(context.Background(), scope, principal, "select", args); err == nil || len(repo.selected) != 0 { t.Fatal("stale selection mutated head") }
 args["expected_turn_revision"] = float64(7)
 if _, err := runtime.selectDirectArtifactV3(context.Background(), scope, principal, "select", args); err != nil || len(repo.selected) != 1 { t.Fatalf("explicit selection: %v", err) }
}
