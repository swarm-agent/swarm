...
func TestManageVideoInspectFramesReturnsOnlyReadyReferencesAndExactLineage(t *testing.T) {
	...
	if err := sessionStore.CreateSession(pebblestore.SessionSnapshot{ID: "studio", UserID: principal.UserID, AccountScopeID: principal.AccountScopeID, WorkspacePath: "/workspace", Mode: "auto", Metadata: map[string]any{"lineage_kind": "video_project"}}); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	var exact struct { ProjectID string `json:"project_id"`; RevisionID string `json:"revision_id"` }
	if err := json.Unmarshal([]byte(created), &exact); err != nil { t.Fatal(err) }
	...
	for _, required := range []string{`"action":"inspect_frames"`, `"revision_event_seq":17`, `"timestamp_ms":1000`, `"session_id":"studio"`, `"collection_id":"frames"`, `"variant_id":"frame-1"`, `"event_seq":23`} {
		if !strings.Contains(payload, required) { t.Fatalf("inspect_frames payload lacks %s: %s", required, payload) }
	}
