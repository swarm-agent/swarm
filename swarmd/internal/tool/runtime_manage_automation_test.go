...
func TestAutomationToolIngress(t *testing.T) {
	for _, args := range []map[string]any{
		...
		if _, err := decodeAutomationToolRequest(args); err == nil {
			t.Fatal("accepted forged/unbounded request", args)
		}
	}
	if _, err := (&Runtime{}).executeManageAutomation(context.Background(), WorkspaceScope{}, map[string]any{"action": "run"}); err == nil {
		t.Fatal("missing adapter reported success")
	}
	d := manageAutomationDefinition()
	if d.Name != "manage_automation" || d.Parameters["additionalProperties"] != false {
		t.Fatal("schema lost strict envelope")
	}
	...
	for _, forbidden := range []string{"role", "principal", "account_id", "approval_reference", "user_instructions"} {
		if _, ok := properties[forbidden]; ok {
			t.Fatal("schema exposes authority", forbidden)
		}
	}
}
...
func TestAutomationToolProposals(t *testing.T) {
	...
	if out["applied"] != false || body["action"] != "save" || body["expected_revision"] != uint64(2) || body["definition"].(store.AutomationDefinition).Enabled {
		t.Fatal("incorrect proposal", out)
	}
	if !repo.record.Definition.Enabled || repo.writes != 0 {
		t.Fatal("proposal mutated state")
	}
	req.ExpectedRevision = 1
	if _, err := r.automationManagement(ctx, p, scope, req); err == nil {
...
type automationToolRepo struct {
	record store.AutomationRecord
	writes int
}

func (f *automationToolRepo) ApplyAutomationMutation(store.AutomationMutation) (store.AutomationRecord, bool, error) {
	f.writes++
	return store.AutomationRecord{}, false, automation.ErrDenied
}
func (f *automationToolRepo) GetAutomationRecord(_ store.AutomationScope, _, kind, _ string, _ uint64) (store.AutomationRecord, bool, error) {
	return f.record, kind == "definition", nil
}
func (*automationToolRepo) SearchAutomationRecords(store.AutomationSearch) ([]store.AutomationRecord, string, error) {
	return nil, "", nil
}

type automationToolPlans struct{}

func (automationToolPlans) GetPlanRevision(string, string, int) (store.SessionPlanSnapshot, bool, error) {
	return store.SessionPlanSnapshot{}, false, nil
}

type automationToolAccess struct{}

func (automationToolAccess) Workspace(ctx context.Context, p automation.Principal, s store.AutomationScope, _ string) error {
	actual, err := automation.RuntimePrincipal(ctx)
	if err != nil || actual != p || p.AccountID != s.AccountID {
		return automation.ErrDenied
	}
	return nil
}
func (automationToolAccess) PlanSession(context.Context, automation.Principal, store.AutomationScope, string) error {
	return nil
}
func (automationToolAccess) OccurrenceSession(context.Context, automation.Principal, store.AutomationScope, string) error {
	return nil
}
func (automationToolAccess) Execution(context.Context, automation.Principal, store.AutomationScope, store.AutomationDefinition, string) error {
	return automation.ErrDenied
}
...
func TestAutomationToolDefinitionIsolation(t *testing.T) {
	...
	for _, scenario := range []string{"valid", "grant", "enabled", "cross-action", "locked", "unknown"} {
		t.Run(scenario, func(t *testing.T) {
			...
			switch scenario {
			case "grant":
				definition["authorization"].(map[string]any)["approval_reference"] = "forged"
			case "enabled":
				definition["enabled"] = true
			case "cross-action":
				args["action"] = "run"
			case "locked":
				args["user_instructions"] = map[string]string{"locked": "overwrite"}
			case "unknown":
...
func TestAutomationToolDispatch(t *testing.T) {
	...
	user, _ := automation.RuntimePrincipal(userCtx)
	agent, _ := automation.RuntimePrincipal(agentCtx)
	scope := store.AutomationScope{AccountID: "account", WorkspaceID: "workspace"}
	...
	domain, err := automation.New(db, automationToolApprovedPlans{}, policy, clock)
	if err != nil {
		t.Fatal(err)
	}
	...
	record, _, err := domain.SaveDefinition(userCtx, user, scope, "auto", "create", 0, d)
	if err != nil {
		t.Fatal(err)
	}
	d = *record.Definition
	digest, _ := automation.ApprovalPolicyDigest(d)
	grant, err := policy.ApproveUser(userCtx, automation.ApprovalRequest{Scope: scope, AutomationID: "auto", DefinitionRevision: record.Revision, PolicySHA256: digest})
	if err != nil {
		t.Fatal(err)
	}
	d.Enabled, d.Authorization.Mode, d.Authorization.ApprovalReference = true, "approved_policy", grant.ID
	record, _, err = domain.SaveDefinition(userCtx, user, scope, "auto", "enable", record.Revision, d)
	if err != nil {
		t.Fatal(err)
	}
	host := &automationToolHost{}
	execution, err := automation.NewExecutionService(domain, host, automation.RuntimeTriggers{})
	if err != nil {
		t.Fatal(err)
	}
	occ, err := execution.Admit(userCtx, user, scope, "auto", record.Revision, automation.Trigger{Kind: "manual", Identity: "manual", ScheduledAt: 100000})
	if err != nil {
		t.Fatal(err)
	}
	r := &Runtime{automations: domain}
	r.ConfigureAutomationExecution(execution, policy)
	...
	if _, err := r.automationManagement(agentCtx, agent, scope, req); err == nil || host.calls != 0 {
		t.Fatal("stale occurrence dispatched")
	}
	...
	if out["status"] != "dispatched" || host.calls != 1 || host.principal != agent || out["record"].(store.AutomationRecord).Occurrence.SessionID != "execution-session" {
		t.Fatal("dispatch lost identity or session", out)
	}
	...
	changed, _, err := db.ApplyAutomationMutation(store.AutomationMutation{Actor: "user", SubjectID: "owner", WrittenAt: 100000, MutationID: "alter-policy", ExpectedRevision: record.Revision, Record: store.AutomationRecord{Scope: scope, AutomationID: "auto", ID: "auto", Kind: "definition", Definition: &d}})
	if err != nil {
		t.Fatal(err)
	}
	req.ExpectedRevision = changed.Revision
	if _, err := r.automationManagement(agentCtx, agent, scope, req); err == nil || host.calls != 1 {
		t.Fatal("altered grant dispatched")
	}
}

type automationToolApprovedPlans struct{}

func (automationToolApprovedPlans) GetPlanRevision(string, string, int) (store.SessionPlanSnapshot, bool, error) {
	return store.SessionPlanSnapshot{ID: "plan", SessionID: "plan-session", AccountScopeID: "account", Version: 1, ApprovalState: "approved", Document: &store.SessionPlanDocument{}}, true, nil
}

type automationToolOwnership struct{}

func (automationToolOwnership) Workspace(context.Context, automation.Principal, store.AutomationScope, string) error {
	return nil
}
func (automationToolOwnership) PlanSession(context.Context, automation.Principal, store.AutomationScope, string) error {
	return nil
}
func (automationToolOwnership) OccurrenceSession(context.Context, automation.Principal, store.AutomationScope, string) error {
	return nil
}

type automationToolHost struct {
	calls     int
	principal automation.Principal
}

func (h *automationToolHost) Ensure(_ context.Context, p automation.Principal, _, _ store.AutomationRecord) (string, error) {
	h.calls++
	h.principal = p
