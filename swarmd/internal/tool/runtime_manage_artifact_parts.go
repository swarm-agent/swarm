...
func managedArtifactFocusedPartRun(ctx context.Context) (ArtifactRunContext, error) {
	...
	for index, definition := range run.SourcePartDefinitions {
		...
		if _, duplicate := seen[definition.ID]; duplicate { return ArtifactRunContext{}, errors.New("manage_artifact focused part selection contains a duplicate") }
...
func encodeManagedArtifactPart(body []byte, revision pebblestore.SessionArtifactPartRevision) map[string]any {
	...
	if strings.HasPrefix(revision.MediaType, "text/") || revision.MediaType == "application/json" || revision.MediaType == "application/javascript" || revision.MediaType == "image/svg+xml" { result["content"] = string(body) } else { result["content_base64"] = base64.StdEncoding.EncodeToString(body) }
	return result
}
...
func (r *Runtime) readManagedArtifactParts(ctx context.Context, principal artifact.Principal, args map[string]any) ([]map[string]any, error) {
	if err := rejectFocusedPartCallerAuthority(args, "max_bytes"); err != nil { return nil, err }
	run, err := managedArtifactFocusedPartRun(ctx); if err != nil { return nil, err }
	maxBytes := int64(asInt(args["max_bytes"], manageArtifactDefaultReadBytes))
	if maxBytes < 1 || maxBytes > manageArtifactMaxReadBytes { return nil, fmt.Errorf("manage_artifact read_parts max_bytes must be between 1 and %d", manageArtifactMaxReadBytes) }
	authority, ok := r.artifactAuthority.(*artifact.Authority); if !ok || authority == nil { return nil, errors.New("authoritative artifact part operations are unavailable") }
	...
	for _, selected := range run.SourcePartRevisions {
		...
		total += revision.Size
		if total > maxBytes { return nil, errors.New("selected artifact parts exceed the bounded read limit") }
		parts = append(parts, encodeManagedArtifactPart(body, revision))
	}
	if err := r.markFocusedPartRead(run); err != nil { return nil, err }
	return parts, nil
}
...
func (r *Runtime) readManagedArtifactPart(ctx context.Context, principal artifact.Principal, args map[string]any) (map[string]any, error) {
	...
	if len(run.SourcePartRevisions) != 1 { return nil, errors.New("manage_artifact read_part requires exactly one selected part; use read_parts") }
	maxBytes := int64(asInt(args["max_bytes"], manageArtifactDefaultReadBytes))
...
func parseManagedPartReplacement(raw map[string]any) ([]byte, string, string, bool, error) {
	text, hasText := raw["content"]; encoded, hasBase64 := raw["content_base64"]
	if hasText == hasBase64 { return nil, "", "", false, errors.New("each replacement requires exactly one of content or content_base64") }
	var body []byte
	if hasText { value, ok := text.(string); if !ok { return nil, "", "", false, errors.New("replacement content must be a string") }; body = []byte(value) } else { value, ok := encoded.(string); if !ok { return nil, "", "", false, errors.New("replacement content_base64 must be a string") }; var err error; body, err = base64.StdEncoding.Strict().DecodeString(value); if err != nil { return nil, "", "", false, errors.New("replacement content_base64 is invalid") } }
	if len(body) == 0 || len(body) > manageArtifactMaxCreateBytes { return nil, "", "", false, fmt.Errorf("replacement content must be between 1 and %d bytes", manageArtifactMaxCreateBytes) }
...
func (r *Runtime) publishManagedArtifactParts(ctx context.Context, principal artifact.Principal, callID, requestID string, args map[string]any) (pebblestore.SessionArtifactVariant, error) {
	...
	raw, ok := args["replacements"].([]any); if !ok || len(raw) != len(run.SourcePartDefinitions) { return pebblestore.SessionArtifactVariant{}, errors.New("manage_artifact publish_parts requires exactly one replacement for every selected part") }
	selected := make(map[string]int, len(run.SourcePartDefinitions)); for index, definition := range run.SourcePartDefinitions { selected[definition.ID] = index }
	replacements := make([]artifact.PartReplacementInput, 0, len(raw)); seen := map[string]struct{}{}
	for _, item := range raw {
		row, ok := item.(map[string]any); if !ok { return pebblestore.SessionArtifactVariant{}, errors.New("manage_artifact publish_parts replacements must be objects") }
		for key := range row { if key != "part_id" && key != "content" && key != "content_base64" && key != "media_type" && key != "filename" && key != "locked" { return pebblestore.SessionArtifactVariant{}, fmt.Errorf("manage_artifact publish_parts replacement contains unsupported field %q", key) } }
		partID := strings.TrimSpace(asString(row["part_id"])); index, exists := selected[partID]; if !exists { return pebblestore.SessionArtifactVariant{}, fmt.Errorf("replacement part %q is not in the authenticated selection", partID) }; if _, duplicate := seen[partID]; duplicate { return pebblestore.SessionArtifactVariant{}, fmt.Errorf("replacement part %q is duplicated", partID) }; seen[partID] = struct{}{}
		body, mediaType, filename, locked, parseErr := parseManagedPartReplacement(row); if parseErr != nil { return pebblestore.SessionArtifactVariant{}, parseErr }; if mediaType == "" { mediaType = run.SourcePartRevisions[index].MediaType }; if filename == "" { filename = "part-"+partID }
	...
	finish, err := r.beginFocusedPartPublish(run); if err != nil { return pebblestore.SessionArtifactVariant{}, err }; success := false; defer func(){ finish(success) }()
	authority, ok := r.artifactAuthority.(*artifact.Authority); if !ok || authority == nil { return pebblestore.SessionArtifactVariant{}, errors.New("authoritative artifact part operations are unavailable") }
	...
	if err != nil { return pebblestore.SessionArtifactVariant{}, err }; success = true; return variant, nil
}

func firstNonEmptyManagedPartID(values ...string) string { for _, value := range values { if strings.TrimSpace(value) != "" { return strings.TrimSpace(value) } }; return "part-turn" }
...
func (r *Runtime) publishManagedArtifactPart(ctx context.Context, principal artifact.Principal, callID, requestID string, args map[string]any) (pebblestore.SessionArtifactVariant, error) {
	...
	if len(run.SourcePartRevisions) != 1 { return pebblestore.SessionArtifactVariant{}, errors.New("manage_artifact publish_part requires exactly one selected part; use publish_parts") }
