...
func TestAutomationTimingEditReview(t *testing.T) {
	...
	if _, err := runtime.automationManagement(ctx, p, scope, req); err == nil { t.Fatal("stale edit accepted") }
	req.ExpectedRevision = 2
	foreign := scope
	foreign.AccountID = "foreign"
	if _, err := runtime.automationManagement(ctx, p, foreign, req); err == nil { t.Fatal("cross-account edit accepted") }
	if repo.writes != 0 || original.Schedule.Expression != "0 9 * * *" || !original.Enabled { t.Fatal("proposal mutated saved state") }
}
...
func TestAutomationTimingIngressFidelity(t *testing.T) {
	for _, tc := range []struct { expression, timezone string; valid bool }{
		...
		if (err == nil) != tc.valid { t.Fatalf("%q/%q: %v", tc.expression, tc.timezone, err) }
		if err == nil && req.Definition.Schedule.Expression != tc.expression { t.Fatal("timing approximated") }
	}
}
