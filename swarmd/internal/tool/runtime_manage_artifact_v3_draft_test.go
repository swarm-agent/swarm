...
func (f *primaryDraftRepo) LoadAuthorDraft(ctx context.Context, p ArtifactV3AuthorPrincipal, g ArtifactV3AuthorGrant) (ArtifactV3AuthorDraft, error) {
	...
	if !ok || trusted.ID != g.ID || trusted.ProducerRunID != p.ProducerRunID { return ArtifactV3AuthorDraft{}, ErrArtifactV3AuthorUnauthorized }
	if f.grants == nil { f.grants = map[string]ArtifactV3AuthorGrant{}; f.drafts = map[string]ArtifactV3AuthorDraft{} }
...
func (f *primaryDraftRepo) SaveAuthorDraft(ctx context.Context, p ArtifactV3AuthorPrincipal, g ArtifactV3AuthorGrant, d ArtifactV3AuthorDraft) (ArtifactV3AuthorDraft, error) {
	if _, err := f.LoadAuthorDraft(ctx, p, g); err != nil { return d, err }
	if d.Sequence != f.drafts[g.ID].Sequence { return d, ErrArtifactV3AuthorConflict }
	d.Sequence++
	f.drafts[g.ID] = d
	return d, nil
}
func (f *primaryDraftRepo) ResolveArtifactV3DirectDraft(_ context.Context, p ArtifactV3AuthorPrincipal, h ArtifactV3DraftHandle) (ArtifactV3AuthorGrant, error) {
	...
	if !ok || directArtifactV3Handle(g) != h || g.AccountScopeID != p.AccountScopeID || g.UserID != p.UserID || g.ProducerRunID != p.ProducerRunID { return ArtifactV3AuthorGrant{}, ErrArtifactV3AuthorUnauthorized }
	return g, nil
}
...
func TestPrimaryArtifactDraftRepairDispatch(t *testing.T) {
	...
	invoke := func(args map[string]any) (map[string]any, error) {
		t.Helper()
		b, err := json.Marshal(args); if err != nil { t.Fatal(err) }
	...
	if err != nil || created["status"] != "fixing" || len(repo.submits) != 0 { t.Fatalf("create=%v err=%v", created, err) }
	...
	if _, err = op(map[string]any{"action": "build_preview"}); !errors.Is(err, ErrArtifactV3AuthorNotReady) { t.Fatalf("unchanged retry: %v", err) }
	if builder.calls != 1 { t.Fatal("unchanged retry executed builder") }
	read, err := op(map[string]any{"action": "read_file", "path": "index.html"})
	if err != nil || read["result"].(map[string]any)["Content"] != html { t.Fatalf("retained read=%v err=%v", read, err) }
	for _, field := range []string{"grant_id", "candidate_id", "session_id"} {
		original := handle[field]; handle[field] = "foreign"
		if _, err = op(map[string]any{"action": "read_file", "path": "index.html"}); err == nil { t.Fatalf("accepted foreign %s", field) }
		handle[field] = original
	}
	grantID := handle["grant_id"].(string)
	grant := repo.grants[grantID]
	expired := grant; expired.ExpiresAt = 1; repo.grants[grantID] = expired
	if _, err = op(map[string]any{"action": "read_file", "path": "index.html"}); !errors.Is(err, ErrArtifactV3AuthorExpired) { t.Fatalf("expiry: %v", err) }
	repo.grants[grantID] = grant
	if _, err = op(map[string]any{"action": "delete_file", "path": "swarm-artifact.json"}); !errors.Is(err, ErrArtifactV3AuthorLocked) { t.Fatalf("manifest lock: %v", err) }
	if len(repo.submits) != 0 || string(repo.drafts[grantID].Project["index.html"]) != html { t.Fatal("rejection changed source or published") }
	if _, err = op(map[string]any{"action": "edit_file", "path": "index.html", "old_string": "Original", "new_string": "Corrected"}); err != nil { t.Fatal(err) }
	if _, err = op(map[string]any{"action": "finish_turn"}); !errors.Is(err, ErrArtifactV3AuthorNotReady) { t.Fatalf("stale finish: %v", err) }
	if _, err = op(map[string]any{"action": "build_preview"}); err != nil { t.Fatal(err) }
	finished, err := op(map[string]any{"action": "finish_turn"})
	if err != nil || finished["status"] != "ready" || len(repo.submits) != 1 { t.Fatalf("finish=%v err=%v", finished, err) }
	if string(repo.submits[0].Project["index.html"]) != strings.Replace(html, "Original", "Corrected", 1) { t.Fatal("unrelated source changed") }
	manifest := string(repo.submits[0].Project["swarm-artifact.json"])
	for _, id := range []string{"hero", "pricing", "footer"} { if !strings.Contains(manifest, `"id":"`+id+`"`) { t.Fatal("stable part lost") } }
	if len(repo.selected) != 0 { t.Fatal("unexpected selection") }
	...
	if err != nil || second["status"] != "editing" { t.Fatalf("second edit=%v err=%v", second, err) }
	...
	if _, err = op(map[string]any{"action": "build_preview"}); err != nil { t.Fatal(err) }
	finished, err = op(map[string]any{"action": "finish_turn"})
	if err != nil || finished["status"] != "awaiting_selection" || len(repo.submits) != 2 || len(repo.selected) != 0 { t.Fatalf("second finish=%v err=%v", finished, err) }
}
