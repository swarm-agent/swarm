...
func TestManageArtifactReviseV3TargetedAnimationPartPreservesSharedStage(t *testing.T) {
	repository := &directArtifactV3RepoFake{}
	author := NewArtifactV3AuthorService(t.TempDir(), repository, &artifactV3BuilderFake{}, &artifactV3PreviewerFake{})
	runtime := NewRuntime(1)
	runtime.SetArtifactV3AuthorService(author)
	...
	html := `<!doctype html><html><body><main id="stage"><canvas id="canvas"></canvas></main>` + manifest + `</body></html>`
	...
	if _, err := runtime.ExecuteForWorkspaceScopeWithRuntime(ctx, scope, Call{CallID: "create-stage", Name: "manage_artifact", Arguments: string(createArgs)}); err != nil {
		t.Fatal(err)
	}

	revisedHTML := strings.Replace(html, `<canvas id="canvas"></canvas>`, `<canvas id="canvas"></canvas><!-- updated part-2 vortex -->`, 1)
	reviseArgs, _ := json.Marshal(map[string]any{
		"action": "revise_v3",
		...
		"target_part_ids": []string{"part-2"},
		"content":         revisedHTML,
	})
	output, err := runtime.ExecuteForWorkspaceScopeWithRuntime(ctx, scope, Call{CallID: "revise-part-2", Name: "manage_artifact", Arguments: string(reviseArgs)})
	if err != nil {
		t.Fatalf("expected revise_v3 on targeted part-2 to succeed, got: %v", err)
	}
	if !strings.Contains(output, `"status":"awaiting_selection"`) || !strings.Contains(output, `"target_part_ids":["part-2"]`) {
