...
func TestArtifactV3HTTPDetailRevisionPreviewAndSelection(t *testing.T) {
	...
	for _, route := range []string{
		...
		if recorder.Code != http.StatusOK { t.Fatalf("GET %s status=%d body=%s", route, recorder.Code, recorder.Body.String()) }
		if strings.Contains(recorder.Body.String(), "collection_id") || strings.Contains(recorder.Body.String(), "variant_id") { t.Fatalf("legacy identity leaked from %s: %s", route, recorder.Body.String()) }
	}
	...
	if preview.Code != http.StatusOK || !strings.Contains(preview.Body.String(), "Artifact V3") || preview.Header().Get("Content-Security-Policy") == "" { t.Fatalf("preview status=%d headers=%v body=%s", preview.Code, preview.Header(), preview.Body.String()) }
	...
	if selected.Code != http.StatusOK || service.lastSelect.ExpectedHeadRef != "rev-root" || service.lastSelect.ExpectedTurnRevision != 2 { t.Fatalf("selection status=%d request=%+v body=%s", selected.Code, service.lastSelect, selected.Body.String()) }
}
...
func TestArtifactV3HTTPRejectsForeignSessionAndStaleSelection(t *testing.T) {
	...
	if service.calls != 0 { t.Fatalf("foreign request reached Artifact V3 service: %d", service.calls) }
	...
	if stale.Code != http.StatusConflict || !strings.Contains(stale.Body.String(), "artifact_v3_conflict") { t.Fatalf("stale status=%d body=%s", stale.Code, stale.Body.String()) }
	if service.selectionCount != 0 { t.Fatalf("stale service advanced head: %d", service.selectionCount) }
}
...
func TestArtifactV3ProjectionPublishesDurableReplayWithoutLegacyShape(t *testing.T) {
	...
	if err := server.PublishArtifactV3Projection(testPrincipal(), artifact, "artifact.v3.candidate.ready", "artifact-v3-event-1"); err != nil { t.Fatal(err) }
	events, err := server.sessions.ListSessionEvents("artifact-v3-api", 0, 100)
	if err != nil { t.Fatal(err) }
	var found bool
	for _, event := range events {
		if event.EventType != "artifact.v3.candidate.ready" { continue }
		found = true
		if strings.Contains(string(event.Payload), "collection_id") || strings.Contains(string(event.Payload), "variant_id") || strings.Contains(string(event.Payload), "artifact.v2") { t.Fatalf("legacy state in durable payload: %s", event.Payload) }
		var projection ArtifactV3ProjectionPayload
		if err := json.Unmarshal(event.Payload, &projection); err != nil || projection.Artifact.ID != artifact.ID { t.Fatalf("projection=%+v err=%v", projection, err) }
		record := sessionruntime.RealtimeOutboxRecord{SessionID: "artifact-v3-api", Event: event}
		if replayed, ok := ArtifactV3ProjectionPayloadFromRecord(record); !ok || replayed.Artifact.Head.CommitOID != artifact.Head.CommitOID { t.Fatalf("replayed=%+v ok=%t", replayed, ok) }
	}
	if !found { t.Fatal("native Artifact V3 durable event not found") }
}
...
func (f *artifactV3APIServiceFake) ListArtifacts(_ context.Context, _ ArtifactV3Principal, sessionID string, _ int) ([]ArtifactV3Artifact, error) { f.calls++; if sessionID != f.artifact.OwnerSessionID { return nil, pebblestore.ErrArtifactV3Unauthorized }; return []ArtifactV3Artifact{f.artifact}, nil }
func (f *artifactV3APIServiceFake) GetArtifact(_ context.Context, _ ArtifactV3Principal, sessionID, artifactID string) (ArtifactV3Artifact, error) { f.calls++; if sessionID != f.artifact.OwnerSessionID || artifactID != f.artifact.ID { return ArtifactV3Artifact{}, pebblestore.ErrArtifactV3NotFound }; return f.artifact, nil }
func (f *artifactV3APIServiceFake) ListRevisions(_ context.Context, _ ArtifactV3Principal, sessionID, artifactID, _ string, _ int) (ArtifactV3RevisionPage, error) { f.calls++; if sessionID != f.artifact.OwnerSessionID || artifactID != f.artifact.ID { return ArtifactV3RevisionPage{}, pebblestore.ErrArtifactV3NotFound }; return ArtifactV3RevisionPage{Revisions: f.artifact.Revisions}, nil }
func (f *artifactV3APIServiceFake) GetRevision(_ context.Context, _ ArtifactV3Principal, sessionID, artifactID, commit string) (ArtifactV3Revision, error) { f.calls++; if sessionID != f.artifact.OwnerSessionID || artifactID != f.artifact.ID || commit != f.artifact.Head.CommitOID { return ArtifactV3Revision{}, pebblestore.ErrArtifactV3NotFound }; return *f.artifact.Head, nil }
func (f *artifactV3APIServiceFake) OpenPreview(_ context.Context, _ ArtifactV3Principal, sessionID, artifactID, revision string) (ArtifactV3Preview, error) { f.calls++; if sessionID != f.artifact.OwnerSessionID || artifactID != f.artifact.ID || revision != "rev-root" { return ArtifactV3Preview{}, pebblestore.ErrArtifactV3NotFound }; return ArtifactV3Preview{RevisionRef: revision, CommitOID: f.artifact.Head.CommitOID, MediaType: "text/html; charset=utf-8", Body: []byte("<!doctype html><title>Artifact V3</title>"), ETag: `"artifact-v3-root"`}, nil }
func (f *artifactV3APIServiceFake) OpenTurn(_ context.Context, _ ArtifactV3Principal, req ArtifactV3OpenTurnRequest) (ArtifactV3Turn, error) { f.calls++; if req.ArtifactID != f.artifact.ID { return ArtifactV3Turn{}, pebblestore.ErrArtifactV3NotFound }; return f.artifact.Turns[0], nil }
func (f *artifactV3APIServiceFake) SelectCandidate(_ context.Context, _ ArtifactV3Principal, req ArtifactV3SelectCandidateRequest) (ArtifactV3SelectionResult, error) { f.calls++; f.lastSelect = req; if f.selectErr != nil { return ArtifactV3SelectionResult{}, f.selectErr }; if req.ExpectedHeadRef != f.artifact.Head.RevisionRef || req.ExpectedTurnRevision != f.artifact.Turns[0].Revision { return ArtifactV3SelectionResult{}, pebblestore.ErrArtifactV3Conflict }; candidate := f.artifact.Turns[0].Candidates[0]; if candidate.Revision == nil { return ArtifactV3SelectionResult{}, errors.New("missing candidate") }; f.selectionCount++; turn := f.artifact.Turns[0]; turn.Status, turn.SelectedCandidateID = "selected", candidate.CandidateID; return ArtifactV3SelectionResult{Head: *candidate.Revision, Turn: turn}, nil }
