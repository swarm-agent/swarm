...
func TestSessionsV3PlanModeDedicatedLifecycleEndpointsSuccess(t *testing.T) {
	...
	t.Run("start plan automatic queues backend run", func(t *testing.T) {
		...
		payload := postSessionsV3PlanModeTestJSON(t, server, created.ID, "/plan-mode/plans/plan-start-auto/start-automatic", `{"execution_granularity":"checkpointed"}`)
	...
	t.Run("start plan checkpointed queues backend run", func(t *testing.T) {
		...
		payload := postSessionsV3PlanModeTestJSON(t, server, created.ID, "/plan-mode/plans/plan-start-checkpointed/start-checkpointed", `{"execution_granularity":"checkpointed","continuation_policy":"review_each_checkpoint"}`)
