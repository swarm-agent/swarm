...
func (s *Server) handleSessionV3ArtifactPartSelection(w http.ResponseWriter, r *http.Request, principal identity.Principal, sessionID, artifactID string) {
	...
	if !found { writeSessionNotFound(w); return }
	if s.artifacts == nil { writeError(w, http.StatusInternalServerError, errors.New("artifact authority is not configured")); return }
	var req struct {
	...
	if req.ClientRequestID == "" || len(req.ClientRequestID) > 256 || req.EventSeq == 0 || len(req.Choices) == 0 || len(req.Choices) > pebblestore.SessionArtifactMaxParts { writeError(w, http.StatusBadRequest, errors.New("part selection requires client_request_id, exact source event, and bounded choices")); return }
	source, ok, err := s.sessions.GetSessionArtifactVariantByID(principal.AccountScopeID, sessionID, artifactID)
	if err != nil { writeError(w, http.StatusBadRequest, err); return }
	if !ok || source.Status != pebblestore.SessionArtifactStatusReady || source.EventSeq != req.EventSeq || source.Composition == nil || source.PartGraphState != pebblestore.SessionArtifactGraphAuthoritative { writeError(w, http.StatusConflict, errors.New("artifact part selection source is stale or not authoritative")); return }
	choices := make([]artifact.PartRevisionChoiceInput, 0, len(req.Choices))
	for _, choice := range req.Choices { choices = append(choices, artifact.PartRevisionChoiceInput{PartID: choice.PartID, Revision: choice.Revision, RevisionEventSeq: choice.RevisionEventSeq, Locked: choice.Locked}) }
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "stale") || strings.Contains(err.Error(), "mixed") || strings.Contains(err.Error(), "duplicate") { status = http.StatusConflict }
