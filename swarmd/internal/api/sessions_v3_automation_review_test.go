...
func (f *sidechatReviewFixture) Workspace(_ context.Context, p automation.Principal, scope store.AutomationScope, _ string) error {
	if scope != f.Scope || p.AccountID != f.Scope.AccountID {
	...
	for _, scenario := range []string{"valid", "stale", "identity", "foreign", "digest"} {
		t.Run(scenario, func(t *testing.T) {
			...
			case "identity":
				id = "other"
			case "foreign":
				f.plan.AccountScopeID = "foreign"
			case "digest":
				f.Definition.Plans[0].Plan.DocumentSHA256 = "forged"
			}
			...
			if scenario == "valid" {
				if w.Code != http.StatusOK || !found || side.Metadata["automation_review_id"] != "auto" || side.Metadata["automation_review_revision"] != "3" || side.Metadata["parent_session_id"] != parent.ID {
					t.Fatalf("review: %d %s %+v", w.Code, w.Body.String(), side)
				}
			} else if w.Code == http.StatusOK || found {
				t.Fatalf("invalid context mutated session: %d %+v", w.Code, side)
			}
		})
	}
}
