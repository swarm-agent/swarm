...
func (s *Server) ConfigureAutomations(domain *automation.Service, execution *automation.ExecutionService, cancel AutomationCancellation, events AutomationEventVerifier) {
	s.automations = &automationHTTPServices{domain: domain, execution: execution, cancel: cancel, events: events}
}
...
func decodeAutomationRequest(w http.ResponseWriter, r *http.Request, out *automationHTTPRequest) error {
	...
	if err := d.Decode(out); err != nil { return automation.ErrInvalid }
	if err := d.Decode(new(any)); err != io.EOF { return automation.ErrInvalid }
	return nil
}
...
func (s *Server) handleAutomations(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path != AutomationsPath { http.NotFound(w, r); return }
	...
	if r.Method == http.MethodGet {
		...
		if err != nil || limit < 1 || limit > 50 { automationHTTPError(w, automation.ErrInvalid); return }
		switch q.Get("action") {
		case "context":
			bundle, err := a.domain.Context(ctx, p, scope, q.Get("id"))
			if err != nil { automationHTTPError(w, err); return }
			writeJSON(w, http.StatusOK, map[string]any{"context": bundle})
		case "history", "get":
			before := uint64(0)
			if q.Has("before") { before, err = strconv.ParseUint(q.Get("before"), 10, 64) }
			if err != nil { automationHTTPError(w, automation.ErrInvalid); return }
			kind, recordID := q.Get("kind"), q.Get("record_id")
			if kind == "" { kind = "definition" }
			if recordID == "" { recordID = q.Get("id") }
			if q.Get("action") == "get" {
				limit = 1
				if q.Has("revision") {
					revision, parseErr := strconv.ParseUint(q.Get("revision"), 10, 64)
					if parseErr != nil || revision == 0 || revision == ^uint64(0) { automationHTTPError(w, automation.ErrInvalid); return }
					...
					if readErr != nil { automationHTTPError(w, readErr); return }
					if len(head) == 0 || revision > head[0].Revision { automationHTTPError(w, automation.ErrNotFound); return }
					if revision == head[0].Revision { writeJSON(w, http.StatusOK, map[string]any{"records": head, "next_before": uint64(0)}); return }
					before = revision + 1
				}
			}
		...
		case "", "search", "list":
			kind := q.Get("kind")
			if q.Get("action") == "list" { kind = "definition" }
	...
	switch req.Action {
	case "save":
		if req.Definition == nil { automationHTTPError(w, automation.ErrInvalid); return }
		record, fresh, err = a.domain.SaveDefinition(ctx, p, scope, req.ID, req.MutationID, req.ExpectedRevision, *req.Definition)
	case "context":
		if req.UserInstructions == nil { automationHTTPError(w, automation.ErrInvalid); return }
	...
	case "run", "event":
		if a.execution == nil || (req.Action == "event" && a.events == nil) {
			writeError(w, http.StatusServiceUnavailable, errors.New("automation execution unavailable")); return
		}
		...
		if req.Action == "event" {
			...
			if err := a.events.VerifyEvent(r, p, scope, req.ID, req.ExpectedRevision, trigger); err != nil { automationHTTPError(w, automation.ErrDenied); return }
		} else if req.Source != "" || req.Identity != "" { automationHTTPError(w, automation.ErrInvalid); return }
		...
		if err == nil {
			...
			writeJSON(w, http.StatusAccepted, map[string]any{"record": record}); return
		}
	case "cancel":
		if a.cancel == nil { writeError(w, http.StatusServiceUnavailable, errors.New("automation cancellation unavailable")); return }
		if req.ExpectedRevision == 0 || req.OccurrenceID == "" { automationHTTPError(w, automation.ErrInvalid); return }
		record, err = a.cancel.Cancel(ctx, p, scope, req.ID, req.OccurrenceID, req.ExpectedRevision, req.MutationID)
	default: err = automation.ErrInvalid
	}
	if err != nil { automationHTTPError(w, err); return }
