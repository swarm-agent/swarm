...
type ArtifactV3Service interface {
	...
	OpenPreview(context.Context, ArtifactV3Principal, string, string, string) (ArtifactV3Preview, error)
...
func (s *Server) handleSessionV3ArtifactsV3(w http.ResponseWriter, r *http.Request, principal identity.Principal, sessionID, tail string) {
	...
	case "preview":
		if len(segments) != 2 || (r.Method != http.MethodGet && r.Method != http.MethodHead) {
			if len(segments) != 2 { writeError(w, http.StatusBadRequest, errors.New("invalid artifact v3 preview path")) } else { methodNotAllowed(w) }
			return
		}
		...
		preview, err := s.artifactV3.OpenPreview(r.Context(), artifactV3APIPrincipal(principal), sessionID, artifactID, revisionRef)
		if err != nil {
			s.writeArtifactV3Error(w, err)
			return
		}
		...
		if strings.TrimSpace(preview.ETag) != "" { w.Header().Set("ETag", preview.ETag) }
		w.Header().Set("Content-Security-Policy", sessionsV3ArtifactPreviewHTMLCSP)
		if r.Method == http.MethodGet { _, _ = w.Write(preview.Body) }
	case "turns":
...
func (s *Server) handleArtifactV3Revisions(w http.ResponseWriter, r *http.Request, principal identity.Principal, sessionID, artifactID string, tail []string) {
	...
	if len(tail) == 0 {
		...
		if err != nil { s.writeArtifactV3Error(w, err); return }
...
func (s *Server) handleArtifactV3Turns(w http.ResponseWriter, r *http.Request, principal identity.Principal, sessionID, artifactID string, tail []string) {
	if len(tail) == 0 {
		...
		if err := validateArtifactV3RequestID(req.ClientRequestID); err != nil { writeError(w, http.StatusBadRequest, err); return }
...
func (s *Server) PublishArtifactV3Projection(principal identity.Principal, artifact ArtifactV3Artifact, eventType, requestID string) error {
	if s == nil || s.sessions == nil || !principal.Valid() { return identity.ErrPrincipalRequired }
	if artifact.ID == "" || artifact.OwnerSessionID == "" || artifact.OwnerSessionID != strings.TrimSpace(artifact.OwnerSessionID) { return pebblestore.ErrArtifactV3Invalid }
	if _, found, err := s.requireSessionV3Access(principal, artifact.OwnerSessionID); err != nil { return err } else if !found { return pebblestore.ErrArtifactV3Unauthorized }
	eventType = strings.TrimSpace(eventType)
	if !strings.HasPrefix(eventType, artifactV3EventPrefix) { eventType = artifactV3ProjectionEventType }
	...
	if requestID == "" { requestID = fmt.Sprintf("artifact-v3:%s:%d:%x", artifact.ID, artifact.Revision, sum[:8]) }
...
func ArtifactV3ProjectionPayloadFromRecord(record sessionruntime.RealtimeOutboxRecord) (ArtifactV3ProjectionPayload, bool) {
	if !strings.HasPrefix(strings.TrimSpace(record.Event.EventType), artifactV3EventPrefix) || len(record.Event.Payload) == 0 { return ArtifactV3ProjectionPayload{}, false }
	var payload ArtifactV3ProjectionPayload
	if err := json.Unmarshal(record.Event.Payload, &payload); err != nil { return ArtifactV3ProjectionPayload{}, false }
	if payload.SchemaVersion != 3 || payload.Artifact.ID == "" || payload.Artifact.OwnerSessionID != record.SessionID { return ArtifactV3ProjectionPayload{}, false }
	return payload, true
}
