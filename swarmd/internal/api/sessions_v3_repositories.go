...
func repositorySessionItems(row pebblestore.SessionRepositoryHistory, parent string) []sessionRepositoryItem {
	...
	if lifecycle == "" { lifecycle = sessionsV3MetadataString(owner.Metadata, "task_status") }
	if lifecycle == "" { lifecycle = "retained" }
	...
	for _, grant := range grants {
		if grant.Kind == pebblestore.WorkspaceGrantTemporary { continue }
		kind := "source"
		if grant.Path == owner.WorktreeRootPath && owner.WorktreeEnabled {
			kind = "parent"
			if owner.ID != parent { kind = "worker" }
		}
...
func (s *Server) authorizeRepositoryItem(principal identity.Principal, item sessionRepositoryItem) error {
	if !principal.Valid() || s.workspace == nil { return errors.New("repository authorization unavailable") }
	if !filepath.IsAbs(item.SourcePath) || !filepath.IsAbs(item.WorkspacePath) { return errors.New("repository identity is incomplete") }
	if item.WorkspaceID != "" {
		entry, found, err := s.workspace.GetByWorkspaceIDForPrincipal(principal, item.WorkspaceID)
		if err != nil { return err }
		if !found || entry.Path != item.SourcePath || entry.WorkspaceGeneration != item.WorkspaceGeneration { return errors.New("repository saved workspace identity is stale") }
	}
	owned, err := s.resolveAccountOwnedPath(principal, item.SourcePath)
	if err != nil { return err }
	if owned.WorkspacePath != item.SourcePath || owned.ResolvedPath != item.SourcePath { return errors.New("repository source must be an exact saved workspace root") }
	if item.WorkspaceID != "" && owned.Scope.WorkspaceID != item.WorkspaceID { return errors.New("repository saved identity mismatch") }
	if item.WorkspacePath == item.SourcePath {
		resolved, err := filepath.EvalSymlinks(item.WorkspacePath)
		if err != nil { return err }
		if resolved != item.WorkspacePath { return errors.New("repository path is a symlink alias") }
		return nil
	}
	...
	if !ok { return errors.New("repository lane validator unavailable") }
	if item.Kind == "lane" {
		digest := sha256.Sum256([]byte(item.SessionID + "\x00" + item.SourcePath))
		if err := validator.ValidateTaskRepositoryLane(item.SourcePath, item.WorkspacePath, "program-lane-" + hex.EncodeToString(digest[:12]), item.Branch); err != nil { return err }
	} else if err := validator.ValidateSessionRepositoryLane(item.SourcePath, item.WorkspacePath, item.SessionID, item.Branch); err != nil { return err }
...
func (s *Server) handleSessionV3Repositories(w http.ResponseWriter, r *http.Request, principal identity.Principal, sessionID string) {
	...
	for reads := 0; reads < limit && len(response.Items) < limit; reads++ {
		...
		var err error
		if cursor.Phase == "programs" { page, err = s.sessions.TaskProgramRepositoryHistory(query) } else { page, err = s.sessions.RepositoryHistory(query) }
		if err != nil {
		...
		if len(page.Sessions) > 0 {
			...
			if getErr != nil { writeError(w, http.StatusInternalServerError, getErr); return }
			for i := range items {
				if found {
		...
		if cursor.Offset > len(items) { writeError(w, http.StatusBadRequest, pebblestore.ErrRepositoryHistoryCursor); return }
		for cursor.Offset < len(items) && len(response.Items) < limit {
			item := items[cursor.Offset]
			item.Availability = "unavailable"
			if err := ctx.Err(); err != nil { writeError(w, http.StatusRequestTimeout, err); return }
			if err := s.authorizeRepositoryItem(principal, item); err != nil { item.Error = err.Error() } else {
				paths, err := gitstatus.ResolveWatchPaths(ctx, item.WorkspacePath)
				if err == nil && filepath.Clean(paths.RepoRoot) != filepath.Clean(item.WorkspacePath) { err = errors.New("repository is not an exact Git root") }
				if err != nil { item.Error = err.Error() } else {
					snapshot, err := boundedRepositorySnapshot(ctx, item.WorkspacePath, paths)
					if err != nil { item.Error = err.Error() } else {
						item.Availability = "available"
						if len(snapshot.Files) > 500 { snapshot.Files = snapshot.Files[:500]; item.FilesTruncated = true }
						item.Status = &snapshot
						if item.Branch == "" { item.Branch = snapshot.Branch }
					}
				}
			}
		...
		if cursor.Offset < len(items) { cursor.Context = contextID; break }
...
func (s *Server) selectedSessionRepository(principal identity.Principal, sessionID, path string) (sessionRepositoryItem, error) {
	if s == nil || s.sessions == nil { return sessionRepositoryItem{}, errors.New("session service unavailable") }
	owner, found, err := s.sessions.GetSession(sessionID)
	if err != nil { return sessionRepositoryItem{}, err }
	if !found || owner.AccountScopeID != principal.AccountScopeID || owner.UserID != principal.UserID { return sessionRepositoryItem{}, errors.New("session not found") }
	if path == "" {
		path = owner.WorkspacePath
		if owner.WorktreeEnabled { path = owner.WorktreeRootPath }
	}
	if !filepath.IsAbs(path) { return sessionRepositoryItem{}, errors.New("exact absolute repository selector required") }
...
func boundedRepositorySnapshot(parent context.Context, path string, paths gitstatus.WatchPaths) (gitstatus.Snapshot, error) {
	...
	if err != nil { return gitstatus.Snapshot{}, err }
