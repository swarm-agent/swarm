...
func TestAutomationPolicyDispatchRecovery(t *testing.T) {
	...
	for _, metadata := range []map[string]any{snapshot.Metadata, {}, {"automation_execution_policy": `{"allowed_tools":["write","task"]}`}} {
		...
		for _, name := range []string{"write", "task", "manage_sessions", "custom_tool"} {
			...
			if !errors.Is(err, errAutomationPolicy) || result.Output != "" || duration != 0 {
				t.Fatalf("dispatch escaped: %s %#v %v", name, result, err)
			}
		}
	}
	if err := (&Service{}).enforceAutomationTool("ordinary", "task"); err != nil {
		t.Fatal("ordinary session changed", err)
	}
	if err := (&Service{}).enforceAutomationTool("automation-missing", "read"); !errors.Is(err, errAutomationPolicy) {
...
func TestAutomationPolicyTargets(t *testing.T) {
	...
	if err := automationTargetPolicy(policy, metadata); err != nil {
		t.Fatal(err)
	}
	if !automationToolPermitted(&policy, "read") {
	...
	for _, name := range []string{"bash", "task", "manage_sessions", "custom", "webfetch"} {
		if automationToolPermitted(&policy, name) {
			t.Fatal("unbounded capability", name)
		}
	}
	policy.TargetIDs = []string{"remote"}
	if automationTargetPolicy(policy, metadata) == nil {
		t.Fatal("unknown target granted")
	}
	policy.TargetIDs = []string{"local"}
	metadata["swarm_v3_runtime_kind"] = "runner"
	if automationTargetPolicy(policy, metadata) == nil {
		t.Fatal("nonlocal target granted")
	}
}
