...
func planDocumentPatchFromArgs(args map[string]any) (*sessionruntime.PlanDocumentPatch, error) {
	...
	if planFinalHandoffArgsPresent(args) {
		...
		if value, ok := args["copyable_code_blocks"]; ok && value != nil {
			if err := unmarshalPlanToolArg(value, &handoff.CopyableCodeBlocks, "plan_manage copyable_code_blocks"); err != nil {
				return nil, err
			}
		}
...
func planFinalHandoffArgsPresent(args map[string]any) bool {
	for _, key := range []string{"handoff_title", "handoff_overview", "impact_bullets", "copyable_code_blocks", "suggested_prompts", "pull_request_url"} {
		if _, ok := args[key]; ok {
			return true
		}
	}
