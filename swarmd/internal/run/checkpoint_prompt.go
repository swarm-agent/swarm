...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	parts := []string{
		...
		"Do not call plan_manage update_checkpoint or structured document patches merely to record routine progress or summarize completed work. Keep typed subtask state durable while a multi-task checkpoint is underway: at a genuine boundary call complete_subtask for one task, or pass subtask_ids to atomically record every task completed since the last progress call. If work continues, that transition advances the next task and makes live client state visible. Do not call complete_subtask for discovery-only activity or for a single-step checkpoint. If new user feedback reaches this current run, route it by contract impact: inquiry/guidance only requires no plan mutation; a bounded same-deliverable refinement that preserves this checkpoint objective and acceptance criteria uses add_subtask and continues here without changing checkpoint identity or attempt history; feedback that invalidates the objective or acceptance criteria requires parent-owned restart_checkpoint; independently shippable work or a separate review/failure boundary requires a later parent-owned request_followup_checkpoint. Prefer the least disruptive valid route and do not classify by imperative wording alone. Never use add_subtask to clear blocked or failed state.",
	}
