...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"If user feedback asks for additional work on an active or completed plan, do not use complete_checkpoint to represent that new feedback; use plan_manage action=request_followup_checkpoint with exact change_request text instead. For broader scope rewrites use request_plan_revision, and for a separate replacement direction use request_new_plan. Use complete_checkpoint only when the current checkpoint acceptance criteria are met.",
