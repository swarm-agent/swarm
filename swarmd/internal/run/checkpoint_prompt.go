...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	parts := []string{
		...
		"Do not call plan_manage update_checkpoint or structured document patches merely to record routine progress or summarize completed work. For checkpoints with multiple typed subtasks, interleave implementation with complete_subtask immediately after each task is concretely complete; that durable transition advances the next task and makes live client state visible. Do not call complete_subtask for discovery-only activity or for a single-step checkpoint.",
	}
	if payload.FinalCheckpoint {
		parts = append(parts, "Final checkpoint handoff required: this is the last remaining checkpoint. Completing it will put the plan into final waiting_review/final-review state, not start another checkpoint. In the terminal plan_manage call, make report a substantive, skimmable durable handoff: clearly state what was accomplished or directly answer the user's request, identify important behavioral impact and remaining risks, and use short headings or lists when they improve legibility. Adapt the content to the work instead of forcing empty sections. Inside that full report include exactly one non-empty <swarm-handoff-summary>...</swarm-handoff-summary> block. Place it near the bottom of the report, after the substantive outcome, impact, and risk details, so the highlighted summary is easy to find; it should normally be the report's final section, with only an essential brief trailing note allowed after it. Put concise Markdown inside the block that captures the outcome or direct answer and the next action only when one is actually applicable. Do not label the block TLDR or place a second summary block anywhere in report, result, or validation. Keep the full report outside the tagged summary substantive; the tagged block is the stable client-highlighted 'At a glance' view, not a replacement for the report. Put changed files, validation, and result/next-action evidence in that same terminal plan_manage call as usual. Desktop V3 preserves the raw durable content and may parse the exact tag only for presentation. Do not save the compact user-facing handoff in the plan document; the backend records the user-facing handoff as a lifecycle system message from the terminal tool payload.")
	}
	parts = append(parts,
		...
		"Use complete_checkpoint only after all acceptance criteria are met and after each genuinely completed typed subtask in multi-task work has received its complete_subtask transition. complete_checkpoint is the single successful terminal checkpoint outcome; its atomic completion of any unresolved subtasks is a safety fallback, not a substitute for task-by-task progress updates. complete_checkpoint may continue to the next checkpoint only if backend execution policy allows it; the model must not start the next checkpoint manually.",
		...
		"Use mark_blocked only for a named external dependency, required input, or unavailable permission that cannot be obtained or worked around with the available tools. State the dependency and exact resolution required. Use mark_failed only for a nonrecoverable execution error after reasonable recovery attempts. Neither action is appropriate merely because more resolvable work was discovered; both stop execution instead of advancing.",
