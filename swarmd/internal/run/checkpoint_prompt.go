...
func (s *Service) buildPlanCheckpointRunInput(sessionID, runID string, options RunOptions) ([]map[string]any, bool, error) {
	...
	payload := checkpointRunPromptPayload{
		...
		RunKind:          runKindCheckpoint,
		ContextPolicy:    contextPolicyCheckpoint,
...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	parts := []string{
		...
		"This checkpoint runs inside the execution epoch and provider conversation that requested it. The normal same-epoch transcript remains authoritative context; this payload adds deterministic checkpoint ownership and lifecycle constraints.",
	...
	if payload.FinalCheckpoint {
		parts = append(parts, "Final checkpoint handoff required: this is the last remaining checkpoint. Completing it will put the plan into final waiting_review/final-review state, not start another checkpoint. The terminal plan_manage outcome is the single canonical user-visible completion. Do not emit an assistant text completion report before or after it. Before the terminal call, create every requested durable deliverable through its intended authority and include its role=deliverable reference in the terminal plan_manage artifacts argument instead of sending a separate assistant message. A managed artifact must be created directly with manage_artifact create/create_package and must not be staged in or duplicated into the workspace merely for submission; use a workspace path only when the requested deliverable is itself a workspace or repository file. In the terminal plan_manage call, keep report substantive and lossless, and author the compact structured handoff: handoff_overview is required and concise; handoff_title is optional; impact_bullets contains at most three short behavioral-impact items; copyable_code_blocks contains at most three optional display-only exact code or command payloads with an explicit client copy affordance; suggested_prompts contains at most three inert label/prompt objects; and pull_request_url may carry an optional public https://github.com/<owner>/<repository>/pull/<number> link when a real PR exists. Suggested prompts are ordinary future user chat messages only and must never be tool calls, shell commands, Git operations, or lifecycle mutations. Use suggested_prompts for concrete next steps when they are known. Otherwise, prefer relevant actions such as committing uncommitted changes, running focused tests that were skipped, or asking for clarification after a substantial handoff; do not use generic review as a default next step. Supply the single canonical recommendation separately with recommendation, where reason is the concise user-facing summary of what happened and action names a meaningful next step rather than merely saying review. Do not put handoff content inside XML-like tags or emit a swarm-handoff-summary marker. The backend persists the concise source fields on the checkpoint, derives schema-versioned lifecycle metadata, and joins report, result, changed_files, and validation as lossless details without duplicating that evidence in the handoff source fields.")
	}
	parts = append(parts, "Blocked checkpoint handoff required when using mark_blocked: make the blocker immediately understandable without forcing the user to read the full report. Include a short handoff_title that names the blocked outcome, a concise handoff_overview that identifies the external dependency/input/permission and why it prevents progress, and up to three impact_bullets led by the exact resolution required and any important safety/state note. When the exact resolution is code or a command, include it in copyable_code_blocks so clients expose a copy affordance. When useful, include up to three suggested_prompts as ordinary user messages for likely next steps, such as confirming that the dependency is resolved and asking Swarm to resume. Keep report, result, changed_files, and validation substantive and lossless; the client presents that evidence collapsed beneath the compact blocked handoff. Do not repeat the full report in the concise handoff fields.")
	parts = append(parts,
		"Review checkpoint handoff required when using mark_needs_review: present the review pause as an interactive structured handoff rather than a bare lifecycle stop. Include handoff_title, a concise handoff_overview explaining the judgment needed, up to three impact_bullets, and useful suggested_prompts that let the user ask questions or request changes in ordinary chat. Keep report, result, changed_files, and validation substantive and lossless; the client presents that evidence beneath the compact handoff. Do not tell the user that chat is blocked while review is pending.",
		...
		"When completing an automation execution checkpoint, select and return the appropriate structured closing_state in plan_manage complete_checkpoint (or complete_subtask with complete_checkpoint=true): routine_clean (routine maintenance or background check completed with no anomalies, warnings, or action needed), deliverable_ready (run created new deliverables or updated artifacts for user review; include role=deliverable in artifacts), attention_alert (run encountered actionable warnings, unexpected drift, threshold exceedances, or issues needing attention), or blocked (run cannot proceed due to external dependencies or permissions). Include a concise one-line summary (maximum 120 characters) describing the outcome for calm display or alert scanning.",
		...
		"Do not call start_session_checkpoint or transition_checkpoint_boundary from this checkpoint run; request_followup_checkpoint and its aliases are retired and rejected. The backend rejects session-checkpoint creation owned by an active provider-managed checkpoint run; do not call it speculatively and do not retry it after an error. The checkpoint payload is already the selected unit of work in the current conversation. If more work is discovered and remains part of this objective, incorporate it and keep working here. If a genuinely independent later checkpoint is required, record the proposed title, tasks, acceptance criteria, notes, and artifact inputs in the terminal result/next-action evidence and tell the user plainly that it was not created; a later parent-conversation turn must append it with transition_checkpoint_boundary, whose successful result assigns checkpoint ownership to that already-current run and continues without resetting the epoch or provider turn. Never use update_checkpoint, an artifact attachment, or assistant prose as a substitute for successful creation, and never claim a checkpoint was added unless the plan_manage result succeeded and returned the new checkpoint. When the original request already requires multiple AIs, ordered checkpoints, or explicit context-isolated stages, the parent should create a multi-checkpoint request_new_plan before the first checkpoint starts.",
		"Keep implementing until every acceptance criterion is met whenever the remaining gap is resolvable with the available tools. Discovering more work, a missing interface or API, scope growth, uncertainty, or an incomplete/failed first approach is implementation work to address, not a reason to pause. Safely adapt the implementation within this checkpoint when needed. Before declaring blocking or failure, inspect authoritative tool errors, durable plan/lifecycle state, required external input, and workspace/Git state. Try at most two materially distinct safe recovery paths when available; stop rather than repeat an equivalent operation after that bounded budget without new authoritative evidence or progress. Do not call mark_needs_review or mark_blocked merely because the original approach was insufficient.",
		...
		"Use mark_blocked only for a named external dependency, required input, or unavailable permission that remains impossible after bounded materially distinct recovery. State blocker code/message, authoritative evidence, completed scope, exact resolution requirement, lineage/run IDs, workspace/worktree, parent and child branches, base and HEAD commits, dirty state, and changed files when available, plus the compact blocked handoff fields described above. Dirty blocked work must remain recallable and must never be auto-committed or treated as successful. Use mark_failed only for a nonrecoverable execution error after bounded reasonable recovery. Neither action is appropriate merely because more resolvable work was discovered; both stop execution instead of advancing.",
