...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"The outcome tool call must include checkpoint_id, attempt_id when present, run_id, run_session_id, parent_session_id, report, changed_files, validation, and result/next-action evidence.",
		"If all acceptance criteria are met, use complete_checkpoint. If user/audit review is needed, use mark_needs_review. If external input is required, use mark_blocked. If the checkpoint cannot be completed because of an error, use mark_failed.",
		"Checkpoint payload:",
		string(raw),
	}, "\n\n")), nil
}
