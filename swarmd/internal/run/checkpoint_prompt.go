...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	if len(payload.Artifacts) > 0 {
		parts = append(parts, "Artifact references are workspace-relative metadata, not embedded file contents. Read only artifacts with role=input that are needed for this checkpoint, using the workspace file tools; do not bulk-read them. Create every role=deliverable artifact in the workspace before terminal completion and include its reference in the terminal plan_manage artifacts argument. Only concrete files produced for the deliverable belong there; never manufacture artifact references for tool-free Idea swarm answers or other prose-only work. Eligible viewable deliverables may appear as an authenticated final-handoff gallery. Do not emit a separate assistant completion report merely to deliver or link an artifact.")
	}
