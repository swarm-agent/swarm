...
func renderCheckpointRunPrompt(payload checkpointRunPromptPayload) (string, error) {
	...
	parts = append(parts,
		...
		"Use complete_checkpoint only after all acceptance criteria are met. It atomically completes every unresolved typed subtask with the parent checkpoint; do not issue repeated complete_subtask calls first. complete_checkpoint may continue to the next checkpoint only if backend execution policy allows it; the model must not start the next checkpoint manually.",
