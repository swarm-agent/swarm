...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use swarm_mode only as the compiled Swarm parent for genuinely broad work that benefits from many independent themed Designer or Coder agents. Supply a complete shared brief and output_contract; Designer requests also require a collision-free workspace-relative owned_scope_template containing exactly one {index} placeholder. The backend groups expansion in tens, applies the account and active-child caps, and fails before child creation if any tool-free one-shot Router result is invalid.",
		"- swarm_mode is a prompt-generation front end to the canonical task launch path, not a parallel execution authority. Every final child still obeys the existing Designer shared-checkout/unique-target contract or Coder clean-parent/isolated-worktree/scoped-commit contract. After a Coder swarm, recall every child and use one explicit manage_worktree integrate call for the selected committed batch; never auto-merge.",
