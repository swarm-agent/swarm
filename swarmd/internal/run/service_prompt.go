...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use task mode=regular for deliberate heavy work with explicit dependency-ready Finder/Coder/Designer launches. Use task mode=swarm when one parent brief should produce a coordinated wave. Choose swarm_strategy=explore (the backward-compatible default) when workers produce substitutes, alternatives, or independent trials; choose swarm_strategy=assembly when every worker owns a complementary production part required by one parent-integrated deliverable. Strategy follows output relationships, independently of whether workers are Coders or Designers. This is one task tool with one permission/settings path, not separate tooling. Never reinterpret a generic new-session request as delegation.",
		"- In Coder/Designer swarm mode, provide a complete shared prompt plus the strategy-specific contract. Explore uses count and optional themes/groups/output targets. Assembly uses count, exactly one declared assembly_part per worker with a distinct owned scope, and an integration_contract naming the parent-owned final deliverable. Router hydrates the base task into every specialized worker prompt before canonical task launch. Hydration failure fails the task call and must never degrade to regular delegation.",
		"- In Idea swarm mode, pass one exact question and count. Every tool-free one-shot Idea receives that same question directly; Router is not used. Idea is not available in regular task launches. Swarm mode is explicit and has no agent-count threshold.",
		...
		"- Assembly wave closure is parent-owned and is not satisfied merely because all child sessions reached terminal success. Wait for every child, inspect every handoff, resolve or explicitly exclude failed/incomplete parts against the integration_contract, incorporate the complete selected set, validate the assembled deliverable, and only then complete the assembly task/checkpoint or launch dependent work. Treat task outcome swarm_strategy and integration_required metadata as durable lifecycle facts, not transcript hints.",
		"- For a Coder Assembly wave, recall the durable task_call_id and inspect its child state summary, then call manage_worktree integrate exactly once with that task_call_id so the complete Coder wave is selected server-side. Use explicit session_ids only when intentionally integrating a reviewed subset. Never integrate children one by one, pass/reconstruct an integration manifest, or declare Assembly complete after child success alone. Missing, failed, dirty, stale, unauthorized, or conflicting children must remain errors; the atomic preflight leaves the parent unchanged.",
		"- After Coder Assembly integration, recall the same task_call_id again to verify integrated child state counts and the resulting parent HEAD. Keep the parent clean before completing the assembly boundary; every later dependent Coder wave/checkpoint must resolve its task base from that integrated parent HEAD, never from a pre-integration commit.",
		"- For a Designer Assembly wave, children already write distinct artifacts in the shared parent checkout. Inspect and reconcile every declared part there, wire them into the integration_contract's canonical deliverable, and validate the assembled result in the parent. Do not launch a later Coder wave while required Designer parts are unresolved or while their parent-checkout changes remain an uncommitted integration boundary.",
		...
		"- task (hydrated Explore swarm): {\"mode\":\"swarm\",\"swarm_strategy\":\"explore\",\"description\":\"Create landscape object variants\",\"prompt\":\"Create reusable low-poly landscape alternatives matching the existing art direction.\",\"agent_type\":\"designer\",\"count\":3,\"themes\":[\"vegetation\",\"rocks\",\"props\"],\"output_contract\":\"One reusable alternative per worker\",\"owned_scope_template\":\"web/src/objects/explore-{index}.tsx\"}",
		"- task (hydrated Assembly swarm): {\"mode\":\"swarm\",\"swarm_strategy\":\"assembly\",\"description\":\"Build settings feature\",\"prompt\":\"Build complementary production parts for the settings feature.\",\"agent_type\":\"coder\",\"count\":2,\"assembly_parts\":[{\"name\":\"Settings API\",\"instructions\":\"Implement the finalized backend contract.\",\"owned_scope\":[\"swarmd/internal/api/settings/**\"]},{\"name\":\"Settings UI\",\"instructions\":\"Implement the UI against the finalized contract.\",\"owned_scope\":[\"web/src/features/settings/**\"]}],\"integration_contract\":\"Parent recalls both committed handoffs, atomically integrates the complete batch, validates the assembled settings feature, and continues from the integrated clean HEAD.\"}",
...
func modeCapabilityInstructions(mode string, bypassPermissions bool, agentProfile pebblestore.AgentProfile) string {
	...
	if currentMode == sessionruntime.ModePlan {
		lines = append(lines,
			...
			"Plan-mode swarm-wave assessment: while creating the structured plan, consider whether distinct discovery alternatives belong in early Explore checkpoints and whether complementary Coder or Designer production scopes belong in later Assembly checkpoints. Do not launch workers during plan-only work; encode each approved wave and its parent-owned assembly boundary in ordinary ordered checkpoints, without adding another plan schema or executor.",
			...
			"For long work, prefer ordered checkpoints that can use Explore for early alternatives, Assembly for complementary implementation waves, explicit parent assembly/integration and validation tasks, and later dependent waves only after prior outcomes are incorporated. Each wave checkpoint must be self-contained for a fresh run and name strategy, worker parts or alternatives, owned scopes, integration contract when required, failure resolution, parent wiring/integration, validation, and the clean resulting boundary that later work depends on.",
		)
		if exitPlanModeEnabled {
			lines = append(lines,
