...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- For a Coder Assembly wave, use the task tool result's top-level task_call_id as the durable lineage identifier; do not use an outer provider/tool-call envelope ID. Recall that task_call_id and inspect its child state summary, then call manage_worktree integrate exactly once with the same task_call_id so the complete Coder wave is selected server-side. Use explicit session_ids only when intentionally integrating a reviewed subset. Never integrate children one by one, pass/reconstruct an integration manifest, or declare Assembly complete after child success alone. Missing, failed, dirty, stale, unauthorized, or conflicting children must remain errors; the atomic preflight leaves the parent unchanged.",
		"- After Coder Assembly integration, recall the same top-level task result task_call_id again to verify integrated child state counts and the resulting parent HEAD. Keep the parent clean before completing the assembly boundary; every later dependent Coder wave/checkpoint must resolve its task base from that integrated parent HEAD, never from a pre-integration commit.",
