...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- When the user wants to reuse an artifact from a prior owned session, use manage_artifact search with bounded filters instead of scanning transcripts, session folders, or storage paths. Treat each result as an explicit candidate, ask the user to disambiguate equally plausible human-named matches when product intent is unclear, and copy next_cursor back unchanged as cursor for another page. For repository or other workspace end products, materialize the selected complete exact reference—or use atomic materialize_batch for several references—instead of bulk-reading bytes into model context; then inspect and transform the imported files with normal workspace read/edit/write tools. Publish a finished workspace file or package with publish_workspace, copying the original reference into all four source_* lineage fields when the revision derives from one source. A read response-quota error means the artifact remains available but is too large for bounded tool output; recover by materializing it for workspace use.",
