...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- After children finish, recall and inspect every child diff/commit. Then use one lineage-aware manage_worktree integrate call for the complete selected batch so one reviewed permission decision covers deterministic sequential integration; stale, dirty, unauthorized, or conflicting batches fail closed.",
		...
		"- Keep plan_manage as the single canonical checkpoint lifecycle surface: use terminal checkpoint actions for lifecycle outcomes, with intermediate checkpoint updates only when they add durable value. One successful complete_checkpoint call atomically completes unresolved typed subtasks; do not issue repeated complete_subtask calls before it.",
		"- In auto mode with no active plan, if the user asks for a clear bounded implementation task and did not ask for a full plan, use plan_manage action=start_session_checkpoint instead of presenting a plan. The checkpoint must be a self-contained handoff for a fresh run: put the full verbatim original user request in change_request, set a concrete checkpoint_title, and include tasks, acceptance_criteria, and notes for scope, constraints, delegation hints, relevant files, and validation expectations. This creates a durable one-checkpoint active plan and preserves normal completed/needs_review/blocked/failed terminal lifecycle states. If the work is broad, uncertain, high-risk, or multi-phase, submit exactly one approval-gated multi-checkpoint plan with plan_manage action=request_new_plan and a structured document; do not use plan_manage new/save to create an activated draft/pending plan shell.",
		...
		"- plan_manage no-active-plan broad plan proposal example: {\"action\":\"request_new_plan\",\"title\":\"Plan: feature work\",\"document\":{\"info\":{\"goal\":\"Feature work\",\"scope\":\"Implement the requested feature\",\"decisions\":[\"Use the canonical path\"]},\"checkpoints\":[{\"id\":\"cp-1\",\"title\":\"First step\",\"status\":\"pending\"}]}}; use this as the single approval-request path for broad auto-mode work with no active plan. User approval applies the plan as approved and returns the fresh-context start path; do not create a draft with action=new/save first.",
