...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use task mode=regular for deliberate heavy work with explicit dependency-ready Finder/Coder/Designer launches. Use task mode=swarm for an Iteration Swarm: fast parallel alternatives or independent trials generated from one parent brief. Iteration Swarm is the launch-facing name; the backward-compatible internal strategy identifier remains explore. This is one task tool with one permission/settings path, not a separate tool. Never reinterpret a generic new-session request as delegation.",
		"- In Coder/Designer Iteration Swarm mode, provide a complete shared prompt, count, and optional themes/groups/output targets. Router hydrates the base task into every specialized worker prompt before canonical task launch. Hydration failure fails the task call and must never degrade to regular delegation. Explicit workspace Designer swarms require exactly one owned_scope_template containing exactly one {index} placeholder.",
		...
		"- Use Finder only for a distinct research question with a specific evidence-based deliverable. Use Coder for dependency-ready implementation scopes. Use Designer only for explicitly requested multiple UI/design iterations or variants; an ordinary UI request or a single design is never sufficient. Handle those directly. Designer output defaults to managed artifacts: omit owned_scope and let trusted orchestration inject one parent-session collection plus a unique opaque variant target for each child. Set output_mode=workspace only when the user explicitly needs checkout source output; every workspace Designer then requires one concrete non-overlapping workspace-relative owned_scope/output target.",
		"- Before eligible Designer delegation, inspect enough nearby product and code context to give every child a complete design brief, constraints, and relevant files. Batch dependency-ready variants in one wave. Managed Designers must use manage_artifact to create, update, and finalize only the server-injected opaque target and must not write/edit the checkout. Workspace Designers may write/edit only their declared scope and must not use managed output. When useful, present managed collection previews or wire a nearby workspace preview/selector, then ask the user for an explicit selection when product judgment is required.",
		"- Designer outputs remain parent-owned durable reusable artifacts, not disposable proposals. Managed variants stay in the parent-session collection; workspace variants stay in their declared checkout targets. The parent may retain several, revise one, or promote one. Remove unselected or rejected variants only when the user requests or chooses that cleanup; never mandate automatic deletion.",
		...
		"- When one user request has multiple dependency-ready children, batch the exact current wave into one task call using `launches`; each launch must include a concise cosmetic `title` of about three words, keep the full instructive assignment in `meta_prompt`, and state its deliverable, concurrency reason, and dependency evidence. Coder launches require owned scopes. Managed Designer launches and Task Program jobs omit owned_scope; explicit workspace Designer launches and program jobs set output_mode=workspace and require concrete non-overlapping output targets in the parent checkout.",
		"- Task Programs have no resume, redeploy, or continue action, and action=start with an existing program_id is rejected. If a program blocks or errors, inspect its status and durable blocker/completed-child context and take ownership of repairing the blocker before continuing. For integration_conflict, you must resolve the reported conflict first and verify the parent worktree is clean and consistent; do not start replacement work while the conflict remains unresolved. Preserve completed or integrated outputs, then intelligently author and start a new Task Program with a distinct program.id containing only the unfinished work. Never replay completed work or reconcile the old stored definition into another scheduler run.",
		...
		"- task (managed multi-variant Designer wave): {\"description\":\"Create two requested navigation variants\",\"prompt\":\"Create the requested reusable UI alternatives using the supplied product constraints.\",\"launches\":[{\"subagent_type\":\"designer\",\"title\":\"Compact Navigation Variant\",\"meta_prompt\":\"Create the compact navigation variant after inspecting nearby components.\",\"deliverable\":\"Reusable compact variant\",\"concurrency_reason\":\"Independent managed variant\",\"dependency_evidence\":\"Brief is finalized\"},{\"subagent_type\":\"designer\",\"title\":\"Spacious Navigation Variant\",\"meta_prompt\":\"Create the spacious navigation variant after inspecting nearby components.\",\"deliverable\":\"Reusable spacious variant\",\"concurrency_reason\":\"Independent managed variant\",\"dependency_evidence\":\"Brief is finalized\"}]}; the server allocates one parent-owned managed collection and injects a unique opaque target per Designer. Managed Designers use manage_artifact and cannot write/edit the checkout.",
		...
		"- task (managed hydrated Iteration Swarm): {\"mode\":\"swarm\",\"description\":\"Create landscape object iterations\",\"prompt\":\"Create reusable low-poly landscape alternatives matching the existing art direction.\",\"agent_type\":\"designer\",\"count\":3,\"themes\":[\"vegetation\",\"rocks\",\"props\"],\"output_contract\":\"One reusable managed iteration per worker\"}; omit owned_scope_template for managed output. For explicit workspace output, set output_mode=workspace and provide owned_scope_template with exactly one {index} placeholder.",
...
func modeCapabilityInstructions(mode string, bypassPermissions bool, agentProfile pebblestore.AgentProfile) string {
	...
	if currentMode == sessionruntime.ModePlan {
		lines = append(lines,
			...
			"Plan-mode swarm-wave assessment: while creating the structured plan, consider whether distinct alternatives would benefit from an early Iteration Swarm checkpoint. Do not launch workers during plan-only work; encode each approved wave in ordinary ordered checkpoints without adding another plan schema or executor.",
			...
			"For long work, prefer ordered checkpoints that can use Iteration Swarms for fast parallel alternatives, followed by explicit parent selection or synthesis, validation, and later dependent work only after prior outcomes are incorporated. Each wave checkpoint must be self-contained for a fresh run and name the alternatives, owned scopes, failure resolution, parent selection or synthesis, validation, and the clean resulting boundary that later work depends on.",
		)
		if exitPlanModeEnabled {
			lines = append(lines,
