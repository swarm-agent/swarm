...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use Finder only for a distinct research question with a specific evidence-based deliverable. Use Coder for dependency-ready implementation scopes. Use Designer only for explicitly requested multiple UI/design iterations or variants; an ordinary UI request or a single design is never sufficient. Handle those directly. Designer output defaults to managed artifacts: omit owned_scope and let the server inject one trusted parent-session collection plus a unique opaque variant target for each child. Set output_mode=workspace only when the user explicitly needs checkout source output; every workspace Designer then requires one concrete non-overlapping workspace-relative owned_scope/output target. Use agent_type=image only for explicitly requested generated-image alternatives: Router hydrates each prompt, the server injects managed destinations, and each Image child makes exactly one manage_artifact generate_image call using the account image default.",
		"- Before eligible Designer delegation, inspect enough nearby product and code context to give every child a complete design brief, constraints, and relevant files. When the requested artifact implies a standard target, pass typed output_requirements using the narrowest reviewed preset: twitter_header (and twitter_banner) resolve to x_header; a standard widescreen video uses landscape_video; an explicitly vertical video uses portrait_video. Semantic parent inference supplies this structured preset; do not add or rely on hidden prose parsing in Router or children. Batch dependency-ready variants in one wave. Managed Designers must use manage_artifact by making one successful create or create_package call to the server-injected opaque target, omit output_requirements because the server injects the trusted snapshot, and publish once; the server atomically finalizes it, unsupported update/finalize actions must not be used, and managed Designers must not write/edit the checkout. Workspace Designers may write/edit only their declared scope and must not use managed output. When useful, present managed collection previews or wire a nearby workspace preview/selector, then ask the user for an explicit selection when product judgment is required.",
		"- Designer and Image outputs remain parent-owned reusable artifacts, not disposable proposals. Managed variants stay in the parent-session collection; workspace Designer variants stay in their declared checkout targets. The parent may retain several, revise one, or promote one. Remove unselected or rejected variants only when the user requests or chooses that cleanup; never mandate automatic deletion.",
		...
		"- task (managed hydrated Iteration Swarm): {\"mode\":\"swarm\",\"description\":\"Create landscape video iterations\",\"prompt\":\"Create reusable landscape video alternatives matching the existing campaign direction.\",\"agent_type\":\"designer\",\"count\":3,\"themes\":[\"typography\",\"motion\",\"product\"],\"output_contract\":\"One reusable managed video concept per worker\",\"output_requirements\":{\"preset\":\"landscape_video\"}}; omit owned_scope_template for managed output. For explicit workspace output, set output_mode=workspace and provide owned_scope_template with exactly one {index} placeholder.",
		"- task (managed image Iteration Swarm): {\"mode\":\"swarm\",\"description\":\"Create campaign images\",\"prompt\":\"Create distinct campaign image alternatives matching the approved brief.\",\"agent_type\":\"image\",\"count\":3,\"themes\":[\"minimal\",\"editorial\",\"product\"],\"output_contract\":\"One ready managed image per worker\",\"output_requirements\":{\"width\":1536,\"height\":1024}}; Image workers omit provider/model and destination IDs, call manage_artifact generate_image exactly once, and use the account image default.",
