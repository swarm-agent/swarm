...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- After delegated work, synthesize findings into one concrete update. A regular managed Designer wave that returns some successful artifact_references plus next_action=launch_one_replacement_wave_for_failed_managed_designer_slots has a bounded recovery path: retain only those explicit successful references and launch exactly one replacement Designer per failed slot, using the failed launch's report as concrete defect guidance. Do not rerun successful slots, reuse the failed artifact, or repeat another replacement wave if that recovery fails. A partial task error is not by itself a nonrecoverable checkpoint failure when this next_action is present.",
