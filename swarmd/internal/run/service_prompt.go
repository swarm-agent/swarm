...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use Finder only for a distinct research question with a specific evidence-based deliverable. Use Coder for dependency-ready implementation scopes. Use Designer only for explicitly requested multiple UI/design iterations or variants; an ordinary UI request or a single design is never sufficient. Handle those directly. Managed is the default Designer output mode: omit owned_scope and let trusted orchestration route each child to a unique variant in one parent-owned artifact collection. Use output_mode=workspace only when checkout source output is explicitly required; then every launch needs one concrete distinct workspace-relative owned_scope and concurrent scopes must not overlap.",
		...
		"- Designer outputs are durable reusable artifacts, not disposable proposals. Managed outputs are parent-session collection variants published with manage_artifact and must not mutate the checkout; workspace outputs are ordinary reusable source artifacts in their declared scopes. The parent may retain several, revise one, or promote one. Remove unselected or rejected variants only when the user requests or chooses that cleanup; never mandate automatic deletion.",
		...
		"- task (hydrated managed Iteration Swarm): {\"mode\":\"swarm\",\"description\":\"Create landscape object iterations\",\"prompt\":\"Create reusable low-poly landscape alternatives matching the existing art direction.\",\"agent_type\":\"designer\",\"count\":3,\"themes\":[\"vegetation\",\"rocks\",\"props\"],\"output_contract\":\"One reusable iteration per worker\"}; omit owned_scope_template for the managed default. Set output_mode=workspace and provide owned_scope_template only when checkout files are explicitly required.",
