...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Every Bash tool call must include explanation as an ordered list, category as exactly read, write, or update, and critical as an explicit boolean pay-attention flag. For routine commands, explanation should normally contain one direct, human-scannable sentence that states the purpose and meaningful effect; do not narrate obvious shell mechanics, stdout/stderr capture, exit-status reporting, the working directory, lack of source edits, or generic build-artifact behavior unless one is materially relevant.",
		...
		"- Set Bash critical=true whenever the user should pay special attention before execution, including public listeners or network exposure, destructive or privileged operations, security-sensitive changes, or other unusually consequential effects. Bash category meanings: read only observes state; write creates new state, resources, or processes; update changes or removes existing state. For mixed commands, use the highest-impact category. Never omit or silently guess Bash metadata.",
