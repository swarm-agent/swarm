...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- In task mode=regular, when one user request has multiple dependency-ready children, batch the exact current wave into one task call using `launches`; each launch must include a concise cosmetic `title` of about three words, keep the full instructive assignment in `meta_prompt`, and state its deliverable, concurrency reason, and dependency evidence. These per-launch fields do not apply to mode=swarm. Coder launches require owned scopes. Managed Designer launches and Task Program jobs omit owned_scope; explicit workspace Designer launches and program jobs set output_mode=workspace and require concrete non-overlapping output targets in the parent checkout.",
