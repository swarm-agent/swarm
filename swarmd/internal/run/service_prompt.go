...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use swarm_mode only as the compiled Swarm parent for broad work that genuinely benefits from many independent Designer or Coder workers. Every invocation requires fresh user approval, then performs exactly two visible tool-free Router hydration calls before adapting the resulting prompts into the existing task-tool execution path. It does not create a parallel child runtime, and approval can never be persisted or bypassed.",
