...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- plan_manage localized-feedback example: {\"action\":\"add_subtask\",\"checkpoint_id\":\"cp-1\",\"subtask\":{\"title\":\"Make the hero headline blue\"}}; use this only for a bounded same-deliverable refinement whose existing checklist remains valid, continuing the same non-blocked/non-failed checkpoint without resetting its attempt history.",
