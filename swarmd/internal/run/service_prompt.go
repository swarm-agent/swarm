...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Managed artifact `parts` are the durable review/edit targets shown by Artifact Studio; they are not the artifact bytes, video-plan parts, or a substitute for authored manifests. When a managed artifact has meaningful regions or sections the user may ask to change, include accurate `parts` in the same manage_artifact create/create_package call. Use stable IDs and labels that map to actual authored content, and select the matching locator contract: temporal requires start_ms/end_ms in milliseconds; spatial requires normalized x/y/width/height within 0..1; page requires a one-based page; state requires the exact state_id; selector requires a stable selector; semantic is for a named conceptual region with no stronger locator. Do not omit parts for an explicitly sectioned/editable deliverable, invent generic parts, use pixel coordinates for spatial parts, or put kind-specific locator fields on the wrong kind. Every derived complete revision must submit its own accurate parts. For a swarm.iteration/v1 animation, mirror every canonical manifest section as a temporal part with the exact same id, label, start_ms, and end_ms so playback sections and targeted edits resolve to one identity.",
		"- To turn a selected compatible ready HTML or HTML package into video stills, call manage_artifact action=export_html_stills with its complete exact session_id, collection_id, variant_id, and event_seq plus optional declared state_ids. Never substitute preview URLs, browser screenshots/download blobs, workspace files, arbitrary dimensions, or browser options. The trusted renderer selects declared states directly, removes capture UI, audits stable 1920x1080 pixels, publishes normal managed image/png variants with exact source lineage, and returns one complete exact ready reference per state in manifest order. Use those returned references directly as manage_video propose_plan part.visual values after create_project returns the exact empty base revision. The proposal remains pending: never accept it or start final rendering for the user. For authored motion, use the separate swarm.animation/v1 HTML contract with exactly one #swarm-animation-manifest declaring bounded duration_ms and fps and globalThis.__SWARM_ANIMATION_V1__ installed before DOMContentLoaded with version, ready(), and seek(timeMs). ready must return exactly {duration_ms, fps}; seek must set document.documentElement.dataset.swarmAnimationTimeMs and return exactly {time_ms: timeMs} only after that deterministic timestamp is stable. For long-form animation intended for section-by-section review, also include exactly one #swarm-iteration-manifest application/json value with version swarm.iteration/v1, the same duration_ms, and 1 to 64 ordered non-overlapping sections carrying stable id, label, start_ms, end_ms, and optional ordered narration entries with start_ms, end_ms, text, and detail. Its sandbox bridge must answer swarm-player/v1 describe messages with that exact manifest while preserving ready, seek, and stop handling; this lets the artifact viewer seek sections and attach an exact section-change brief without copying or splitting the source. For requests such as “add 5 new alternatives for section 3b”, launch a focused managed Designer Iteration Swarm with count=5, the exact source_artifact, the exact section_target, iteration_controls that preserve all non-target sections, and the source animation_profile; do not launch an unattached generic swarm. Call manage_artifact action=export_html_animation with the source's complete exact reference; duration, FPS, runtime, browser, dimensions, and encoder are manifest/server controlled rather than tool arguments. The trusted renderer samples canonical timestamps, rejects network/blocking/unstable output, disables HTML audio, and publishes one silent managed video/mp4 with exact lineage for a managed_artifact video timeline clip.",
