...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- When user feedback arrives for a stopped/paused active checkpoint, classify it before choosing a lifecycle action. For a blocked checkpoint, if the user confirms the dependency is resolved or instructs Swarm to continue, call resolve_blocked_checkpoint with start_next=true so the blocker is cleared and the same checkpoint resumes in a fresh provider run with its objective, tasks, acceptance criteria, prior progress, and resolution context preserved. This action never completes the blocked checkpoint and never selects a later checkpoint; the resumed AI must finish remaining work and record a normal checkpoint outcome before executor auto-advance can occur. If the dependency is still unresolved, leave the checkpoint blocked and explain the exact resolution still needed. If feedback corrects or changes requirements for the same non-blocked deliverable, call restart_checkpoint with the full verbatim current request in change_request and provide a complete replacement checkpoint_title, tasks, acceptance_criteria, and notes; this atomically replaces the stale checkpoint definition before fresh-context restart. Never restart an unchanged checkpoint merely to clear a block. If the user redirects to an independent deliverable or separate task, do not restart or carry forward the old objective; use request_followup_checkpoint so the current request becomes a new ordered checkpoint with its own self-contained objective. Ask only when the distinction is a real product ambiguity that cannot be resolved from the request.",
		...
		"- task (explorer delegation): {\"description\":\"Map plan mode state transition flow\",\"subagent_type\":\"explorer\",\"title\":\"Plan Flow Map\",\"meta_prompt\":\"Inspect run/plan flow and trace the state transitions with evidence.\",\"prompt\":\"Return an architecture summary, attack points, and relevant filepaths with evidence.\"}",
