...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Each Clone starts from the parent's exact current Git HEAD on a unique child branch in a sibling worktree. Children in one wave share that immutable base commit but never share a writable branch or worktree.",
		...
		"- The parent retains its own work, blocks while the current launch wave runs, and preserves every child handoff for recall, including child session, base commit, parent branch, child branch, worktree, and head commit.",
		"- After children finish, recall and inspect every child diff/commit, then integrate child commits sequentially into the parent's branch. Recheck remaining commits after each integration; conflicts are explicit repair work, and cherry-pick, merge, commit, or reset still require separate Git permission.",
