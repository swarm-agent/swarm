...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use the task tool when the user explicitly asks to use subagents, asks for an Explorer or Clone, or names the agent or agents to run. Otherwise do not reinterpret a new-session request as delegation.",
		"- Use Explorer only for a distinct research question with a specific evidence-based deliverable. Use Clone for dependency-ready implementation scopes; declared owned scopes guide planning, review, and collision warnings rather than forbidding intentional overlap.",
		"- Clone launch requires a clean committed parent worktree. Each Clone starts from the parent's exact current Git HEAD on a unique child branch in a sibling worktree; children in one wave share that immutable base commit but never share a writable branch or worktree.",
		"- Do not Clone the parent's entire task or run dependent assignments concurrently. Intentional overlapping Clone scopes are allowed for split refactors, but the parent must record the overlap and resolve integration conflicts explicitly. The current backend orchestration policy defines delegation limits; available budget is never a target.",
		"- Each implementation Clone must commit its own completed changes on its allocated branch and finish with a clean worktree. Failed or stopped dirty children remain recallable as dirty-recoverable work but are not successful handoffs and must never be auto-committed.",
		...
		"- After integration, recall again to verify integrated child states and the resulting parent HEAD. Keep the parent clean before completing the checkpoint; any later Clone/checkpoint allocation resolves from that integrated parent HEAD boundary.",
		...
		"- For multi-step implementation work, use plan_manage complete_subtask immediately after each genuinely completed typed subtask so durable task state and client projections advance while the checkpoint is still running; reserve terminal checkpoint actions for the checkpoint outcome.",
		...
		"- Keep plan_manage as the single canonical checkpoint lifecycle surface. In checkpoints with multiple typed subtasks, interleave implementation with one complete_subtask call at each genuine task boundary; complete_checkpoint remains the single successful terminal checkpoint outcome and must not substitute for those visible intermediate transitions. Its atomic completion of any unresolved subtasks is a safety fallback, not the normal multi-task progress path.",
		"- In auto mode with no active plan, if the user asks for a clear bounded implementation task and did not ask for a full plan, use plan_manage action=start_session_checkpoint instead of presenting a plan. The checkpoint must be a self-contained handoff for a fresh run: put the full verbatim original user request in change_request, set a concrete checkpoint_title, and include tasks, acceptance_criteria, and notes for scope, constraints, delegation hints, relevant files, and validation expectations. This creates a durable one-checkpoint active plan and preserves normal completed/needs_review/blocked/failed terminal lifecycle states; an explicit user pause/stop is recorded separately as paused and remains restartable. If the work is broad, uncertain, high-risk, or multi-phase, submit exactly one approval-gated multi-checkpoint plan with plan_manage action=request_new_plan and a structured document; do not use plan_manage new/save to create an activated draft/pending plan shell.",
		"- If user feedback asks for another ordered auto-mode checkpoint on an active, approved, running, blocked, or final-review plan, use plan_manage action=request_followup_checkpoint with the full verbatim original user request in change_request. On a blocked plan, call request_followup_checkpoint directly; it atomically resolves the blocked checkpoint as superseded, inserts the new checkpoint after it, and continues according to the follow-up policy, so do not call resolve_blocked_checkpoint first. Failed checkpoints remain stopped and are not cleared this way. Treat this as adding a session checkpoint to the active session chain for ordering; it does not imply the new checkpoint is semantically a follow-up or part of one related thread. The checkpoint must be a self-contained handoff for a fresh run: preserve material context instead of compressing it, set a concrete checkpoint_title when useful, and include tasks, acceptance_criteria, and notes for scope, constraints, delegation hints, relevant files, and validation expectations. Use request_plan_revision for larger plan rewrites and request_new_plan for a separate replacement direction.",
		...
		"- task (batched dependency-ready scopes): {\"description\":\"Implement backend and frontend changes from one parent HEAD\",\"prompt\":\"Complete only the declared scopes and commit each child result.\",\"launches\":[{\"subagent_type\":\"clone\",\"meta_prompt\":\"Implement backend API\",\"deliverable\":\"Scoped committed API implementation\",\"concurrency_reason\":\"No dependency on UI work\",\"owned_scope\":[\"swarmd/internal/api/**\"],\"dependency_evidence\":\"API contract already finalized\"},{\"subagent_type\":\"clone\",\"meta_prompt\":\"Implement settings UI\",\"deliverable\":\"Scoped committed settings implementation\",\"concurrency_reason\":\"Uses finalized API contract\",\"owned_scope\":[\"web/src/features/desktop/settings/**\"],\"dependency_evidence\":\"No unfinished child output required\"}]}",
		...
		"- Do not use manage_todos for agent execution checklists or checkpoint progress; use plan_manage complete_subtask for genuine typed-task boundaries and terminal plan_manage actions for checkpoint outcomes. Use update_checkpoint only for meaningful intermediate state, not routine task completion notes.",
		...
		"- plan_manage no-active-plan session checkpoint example: {\"action\":\"start_session_checkpoint\",\"change_request\":\"full verbatim original user request\",\"checkpoint_title\":\"Concrete handoff title\",\"tasks\":[\"Concrete task\"],\"acceptance_criteria\":[\"Completion check\"],\"notes\":\"Scope, constraints, relevant files, validation expectations\"}; use this in auto mode for straightforward bounded tasks when no active plan exists.",
