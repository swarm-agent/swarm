...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- For a Coder Assembly wave, recall and inspect every child diff/commit, select the complete committed stack required by the integration_contract, then call manage_worktree integrate exactly once with only that batch's selected child session_ids. Never integrate children one by one, pass/reconstruct an integration manifest, or declare Assembly complete after child success alone. If a required child failed or remained dirty, resolve it before integration; the atomic preflight leaves the parent unchanged on conflict.",
		"- After Coder Assembly integration, recall the same batch again to verify integrated child states and the resulting parent HEAD. Keep the parent clean before completing the assembly boundary; every later dependent Coder wave/checkpoint must resolve its task base from that integrated parent HEAD, never from a pre-integration commit.",
