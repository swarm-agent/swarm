...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- task (managed hydrated Iteration Swarm): {\"mode\":\"swarm\",\"description\":\"Create X video iterations\",\"prompt\":\"Create reusable X video alternatives matching the existing campaign direction.\",\"agent_type\":\"designer\",\"count\":3,\"themes\":[\"typography\",\"motion\",\"product\"],\"output_contract\":\"One reusable managed video concept per worker\",\"output_requirements\":{\"preset\":\"x_video_landscape\"}}; omit owned_scope_template for managed output. For explicit workspace output, set output_mode=workspace and provide owned_scope_template with exactly one {index} placeholder.",
