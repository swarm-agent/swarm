...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		`- task (staged Task Program for a multi-subsystem build): {"action":"start","prompt":"Implement the approved image feature through the declared dependency stages and return committed implementations and authored tests for parent-run validation; Coders do not execute commands.","program":{"id":"image-feature-v1","stages":[{"id":"foundation","dependency_evidence":"Discovery identified the canonical contract and storage boundaries"},{"id":"audit","depends_on":["foundation"],"dependency_evidence":"Cross-system review requires all implementation jobs integrated"}],"jobs":[{"id":"core-contract","stage_id":"foundation","subagent_type":"coder","title":"Image Core Contract","meta_prompt":"Implement the canonical image-generation contract and focused local tests only; commit the completed scoped change.","deliverable":"Committed core contract with focused tests","owned_scope":["swarmd/internal/image/**"],"dependency_evidence":"Foundation work has no unfinished child dependency","acceptance_criteria":["The canonical contract works and focused tests cover it"]},{"id":"integration-audit","stage_id":"audit","depends_on":["core-contract"],"subagent_type":"coder","title":"Image Integration Audit","meta_prompt":"Audit the integrated image feature in fresh context, fix only cross-system gaps, author focused regression tests and parent-run validation commands, and commit any required corrections; do not execute tests.","deliverable":"Fresh-context source audit and parent-run validation handoff","owned_scope":["swarmd/internal/image/**"],"dependency_evidence":"All implementation jobs are integrated before cross-system review","acceptance_criteria":["Integrated source is reviewed against the feature contract; tests and exact commands are handed to the parent without claiming execution"]}]}}. Use this shape when dependent stages need integrated results and fresh context; do not collapse its jobs into one overloaded Coder.`,
