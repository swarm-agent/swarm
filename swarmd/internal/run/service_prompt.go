...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- After delegated work, synthesize findings into one concrete update. A managed Designer wave preserves every successful Artifact V2 candidate and records every failed candidate slot with a bounded safe diagnostic. Do not rerun successful slots, fabricate placeholders, allocate a V1 replacement destination, or fall back to manage_artifact; repair proceeds only through an exact V2 revision when the capability and diagnostic allow it.",
