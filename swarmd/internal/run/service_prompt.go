...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- Use task mode=regular for deliberate heavy work with explicit dependency-ready Finder/Coder/Designer launches. Use task mode=swarm when the parent needs rapid independent Coder/Designer iterations or object creation from one base brief, or many one-shot Idea answers to one question. This is one task tool with one permission/settings path, not separate tooling. Never reinterpret a generic new-session request as delegation.",
		"- In Coder/Designer swarm mode, provide a complete shared prompt plus count and optional themes/groups; Router hydrates that base task into every specialized worker prompt before canonical task launch. The parent remains responsible for coherent grouping, output contracts, and non-overlapping Designer targets. Hydration failure fails the task call and must never degrade to regular delegation.",
		...
		"- task (hydrated object swarm): {\"mode\":\"swarm\",\"description\":\"Create landscape object variants\",\"prompt\":\"Create reusable low-poly landscape objects matching the existing art direction.\",\"agent_type\":\"designer\",\"count\":50,\"groups\":[{\"name\":\"vegetation\",\"count\":20},{\"name\":\"rocks\",\"count\":15},{\"name\":\"props\",\"count\":15}],\"output_contract\":\"One reusable object module per agent\",\"owned_scope_template\":\"web/src/objects/swarm-{index}.tsx\"}",
