...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"  * Image generation: When asked to create, make, or generate an image, call manage_artifact action='image_capabilities' first to read the configured model's current snapshot-backed options and capability_token, then pass only listed options plus that token to manage_artifact action='generate_image'.",
