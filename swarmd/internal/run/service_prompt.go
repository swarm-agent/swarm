...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- For multi-step implementation work, keep agent execution progress in `plan_manage` on the active plan/checkpoint; do not create a separate agent checklist in manage_todos.",
		...
		"- Create or update agent checklist/progress items through plan_manage checkpoint fields, notes, report, or structured document patches when the task spans multiple concrete implementation steps.",
		"- Keep plan_manage as the single canonical agent progress surface: update the active checkpoint status/notes as work starts or advances, and use terminal checkpoint actions for lifecycle outcomes.",
		...
		"- If the task is short and can be completed in one concrete step, skip progress churn; otherwise keep the active plan/checkpoint updated through plan_manage.",
		...
		"- Do not use manage_todos for agent execution checklists or checkpoint progress; use plan_manage update_checkpoint, patch, or terminal checkpoint actions instead.",
		"- plan_manage agent progress example: {\"action\":\"update_checkpoint\",\"checkpoint_id\":\"cp-1\",\"checkpoint\":{\"status\":\"in_progress\",\"notes\":\"Started implementation\"},\"update_summary\":\"started checkpoint work\",\"update_scope\":\"cp-1\"}",
		"- plan_manage checkpoint checklist example: {\"action\":\"update_checkpoint\",\"checkpoint_id\":\"cp-1\",\"checkpoint\":{\"tasks\":[\"[x] Update prompt\",\"[ ] Add targeted test\"]},\"update_summary\":\"updated checkpoint checklist\",\"update_scope\":\"cp-1\"}",
...
func modeCapabilityInstructions(mode string, bypassPermissions bool, agentProfile pebblestore.AgentProfile) string {
	...
	if currentMode == sessionruntime.ModePlan {
		...
		lines = append(lines,
			...
			"When an active plan exists and the work is multi-step, keep agent execution progress in plan_manage on the active plan/checkpoint; do not use manage_todos for agent self-tracking.",
		)
