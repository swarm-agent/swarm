...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- When user feedback arrives for a stopped/paused active checkpoint, classify it before choosing a lifecycle action. If it corrects or changes requirements for the same deliverable, call restart_checkpoint with the full verbatim request in change_request and provide a complete replacement checkpoint_title, tasks, acceptance_criteria, and notes; this atomically replaces the stale checkpoint definition before fresh-context restart. Never restart the unchanged checkpoint after requirement-changing feedback. If the user redirected to an independent deliverable or separate task, do not restart the old checkpoint; use request_followup_checkpoint so the request becomes a new ordered checkpoint. Ask only when the distinction is a real product ambiguity that cannot be resolved from the request.",
