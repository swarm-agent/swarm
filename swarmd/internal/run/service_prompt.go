...
func masterHarnessPromptWithScope(scope tool.WorkspaceScope) string {
	...
	return strings.TrimSpace(strings.Join([]string{
		...
		"- plan_manage active-plan session checkpoint example: {\"action\":\"request_followup_checkpoint\",\"change_request\":\"full verbatim original user request\",\"checkpoint_title\":\"Concrete handoff title\",\"tasks\":[\"Concrete task\"],\"acceptance_criteria\":[\"Completion check\"],\"notes\":\"Scope, constraints, relevant files, validation expectations\"}; despite the legacy action name, this appends one ordered checkpoint to the active session chain. Use request_plan_revision for broader approved-plan rewrites and request_new_plan for a separate new plan proposal.",
