...
func NewAutomationExecutionHost(runs *Service, repository *store.SessionStore, apply func(sessions.SessionMutationInput) (sessions.SessionMutationResult, error)) (*AutomationExecutionHost, error) {
	if runs == nil || runs.sessions == nil || repository == nil || apply == nil { return nil, automation.ErrInvalid }
...
func (h *AutomationExecutionHost) Prepare(ctx context.Context, p automation.Principal, def store.AutomationRecord) (store.SessionSnapshot, error) {
	...
	if err != nil { return empty, err }; if !found { return empty, automation.ErrDenied }
	...
	if err != nil { return empty, err }
	selection := func(a store.AgentModelAssignment) store.ModelProfileSelection { return store.ModelProfileSelection{Provider: a.Provider, Model: a.Model, Thinking: a.Thinking, ServiceTier: a.ServiceTier, ContextMode: a.ContextMode} }
...
func (h *AutomationExecutionHost) current(snapshot store.SessionSnapshot, key string) (store.SessionSnapshot, bool, error) {
	if snapshot.ID != "automation-"+key || strings.TrimSpace(snapshot.AccountScopeID) == "" { return store.SessionSnapshot{}, false, automation.ErrDenied }
	current, found, err := h.runs.sessions.GetSession(snapshot.ID)
	if err != nil { return current, found, err }
	if found && (current.AccountScopeID != snapshot.AccountScopeID || current.Metadata["automation_execution_key"] != key || !current.WorktreeEnabled) { return current, false, automation.ErrDenied }
...
func (h *AutomationExecutionHost) Start(ctx context.Context, snapshot store.SessionSnapshot, key string) error {
	return h.store.WithAutomationExecutionFence(snapshot.AccountScopeID, key, false, func() error {
		...
		if err != nil { return err }; if !ok || plan.Document == nil || len(plan.Document.Checkpoints) == 0 { return automation.ErrInvalid }
		...
		if cp.RunID != runID {
			...
			if err != nil { return err }; attempt = result.AttemptID
		} else { attempt = cp.AttemptID }
...
func (h *AutomationExecutionHost) Cancel(ctx context.Context, snapshot store.SessionSnapshot, key string) error {
	...
	if _, _, err := h.current(snapshot, key); err != nil { return err }
	return h.store.WithAutomationExecutionFence(snapshot.AccountScopeID, key, true, func() error {
		...
		if !found {
			...
			runID := "automation-run:"+key
			if ok && plan.Document != nil && plan.Document.ExecutionState != nil && plan.Document.ExecutionState.CurrentRunID != "" { runID = plan.Document.ExecutionState.CurrentRunID }
			intent, found, err = h.runs.sessions.GetSessionRunIntent(current.ID, runID)
		}
		if err != nil || !found { return err }
		if intent.Status != sessions.RunIntentPendingExecutor && intent.Status != sessions.RunIntentRunning && intent.Status != sessions.RunIntentCancelled { return nil }
		...
		if errors.Is(err, ErrSessionRunNotActive) { return nil }; return err
	})
}
