...
func TestRecoveryCopyActionIntegration(t *testing.T) {
 for _, fail := range []bool{false, true} {
  t.Run(map[bool]string{false:"publish", true:"store-failure"}[fail], func(t *testing.T) {
   ...
   put := func(name string, data []byte, mode os.FileMode) { t.Helper(); if err := os.WriteFile(filepath.Join(lane,name),data,mode); err != nil { t.Fatal(err) } }
   ...
   if err := store.CompleteRepositoryHistoryMaintenance(context.Background()); err != nil { t.Fatal(err) }
   ...
   svc.SetSessionWorkspaceCanonicalizer(func(SessionWorkspaceCanonicalizeInput)(SessionWorkspaceCanonicalization,error){ return SessionWorkspaceCanonicalization{WorkspaceID:entry.WorkspaceID,WorkspaceGeneration:entry.WorkspaceGeneration,SourceWorkspacePath:repo,WorkspaceName:"repo",WorkspaceState:"active"},nil })
   before, err := worktreeruntime.InspectRecoveryWorktree(repo,lane)
   if err != nil { t.Fatal(err) }
   index, err := os.ReadFile(filepath.Join(before.GitDir,"index")); if err != nil { t.Fatal(err) }
   ...
   apply := func(input sessionruntime.SessionMutationInput)(sessionruntime.SessionMutationResult,error){
    if input.WorktreeRecovery.Action == "publish_copy" { destination = input.Session.WorktreeRootPath; if fail { return sessionruntime.SessionMutationResult{},errors.New("injected store failure") } }
    return sessions.ApplySessionMutation(input)
   }
   _, err = svc.recoverSessionWorktree(owner.ID,principal,args,apply)
   if (err != nil) != fail { t.Fatalf("copy result: %v",err) }
   if destination == "" || destination == lane { t.Fatalf("missing isolated copy: %q",destination) }
   if got := runTestGit(t,destination,"show",":layers"); got != "A" { t.Fatalf("index flattened: %q",got) }
   data, err := os.ReadFile(filepath.Join(destination,"layers")); if err != nil || string(data) != "B" { t.Fatalf("worktree: %q %v",data,err) }
   data, err = os.ReadFile(filepath.Join(destination,"binary")); if err != nil || !bytes.Equal(data,[]byte{0,1,255}) { t.Fatalf("binary: %v %v",data,err) }
   info, err := os.Stat(filepath.Join(destination,"binary")); if err != nil || info.Mode().Perm() != 0755 { t.Fatalf("mode: %v %v",info,err) }
   after, err := worktreeruntime.InspectRecoveryWorktree(repo,lane); if err != nil || after != before { t.Fatalf("source changed: %+v %v",after,err) }
   data, err = os.ReadFile(filepath.Join(before.GitDir,"index")); if err != nil || !bytes.Equal(data,index) { t.Fatal("source index changed") }
   current, _, err := sessions.GetSession(owner.ID); if err != nil { t.Fatal(err) }
   want := destination; if fail { want = lane }
   if current.WorktreeRootPath != want || current.WorkspacePath != repo { t.Fatalf("partial or wrong switch: %+v",current) }
   inventory := runTestGit(t,repo,"worktree","list","--porcelain")
   if _, err := svc.recoverSessionWorktree(owner.ID,principal,args,apply); err == nil { t.Fatal("replayed stale operation accepted") }
   if got := runTestGit(t,repo,"worktree","list","--porcelain"); got != inventory { t.Fatal("retry duplicated allocation") }
   claims, err := store.InspectWorktreeOwnership(principal.AccountScopeID,principal.UserID,[]string{lane}); if err != nil { t.Fatal(err) }
   state := "copied"; if fail { state = "reserved_copy" }
   if claims[0].OperationState != state { t.Fatalf("lost recovery state: %+v",claims) }
  })
 }
}
