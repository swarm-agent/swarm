...
func (s *Service) compileResolvedAgentToolContract(accountScopeID string, profile pebblestore.AgentProfile) (ResolvedAgentToolContract, *permission.Policy, map[string]bool, error) {
	...
	if isSwarmPrimary {
		// Swarm is a compiled system agent. Its code-owned control-plane
		// capabilities cannot be narrowed by stale account profile state.
		...
		// swarm_mode is a compiled Swarm-parent capability, never an inheritable
		// delegation primitive for other primaries, sidechats, or child sessions.
