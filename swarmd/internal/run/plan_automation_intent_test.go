...
func TestPlanAutomationIntentFidelity(t *testing.T) {
	...
	for _, name := range []string{"exit_plan_mode", "plan_manage"} {
		...
		if err != nil { t.Fatal(err) }
		if !reflect.DeepEqual(doc, clone) || clone.Automation.DefinitionRevision != 7 || clone.Automation.Definition.Schedule.Expression != "0 18 * * *" { t.Fatal("intent lost") }
		clone.Automation.Definition.Schedule.Expression = "changed"
		if doc.Automation.Definition.Schedule.Expression != "0 18 * * *" { t.Fatal("clone aliases source") }
	}
	doc, err := planDocumentFromArgs(map[string]any{"document": map[string]any{"title": "Hourly review"}})
	if err != nil || doc.Automation != nil { t.Fatal("prose became automation intent", err) }
}
