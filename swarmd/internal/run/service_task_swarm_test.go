...
func TestDesignerSectionIterationCarriesDurableTarget(t *testing.T) {
	...
	if err != nil || !strings.Contains(prompt, "selected source-bound review/edit target") || !strings.Contains(prompt, `"id":"3b"`) || !strings.Contains(prompt, "monolithic revision contract") || !strings.Contains(prompt, "exactly one manage_artifact create or create_package call") || strings.Contains(prompt, "manage_artifact action=read_part") || strings.Contains(prompt, "manage_artifact action=publish_part") {
