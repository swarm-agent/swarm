...
func parseRecoveryArguments(raw map[string]any) (r recoveryArguments, err error) {
 ...
 for _, key := range []string{"owner_session_id", "ownership_revision", "head", "fingerprint", "operation_id", "files"} {
  if _, ok := raw[key]; ok && action != "reclaim_worktree" && action != "copy_worktree" { return r, errors.New("recovery evidence is only accepted by recovery actions") }
 }
 if action != "reclaim_worktree" && action != "copy_worktree" { return r, nil }
 for key, dest := range map[string]*string{"owner_session_id": &r.Owner, "head": &r.HEAD, "fingerprint": &r.Fingerprint, "operation_id": &r.Operation} {
  ...
  if !ok || value == "" || strings.TrimSpace(value) != value { return r, fmt.Errorf("recovery requires exact %s", key) }
  *dest = value
 }
 number, ok := raw["ownership_revision"].(json.Number)
 if !ok { return r, errors.New("recovery requires ownership_revision") }
 r.Revision, err = strconv.ParseUint(string(number), 10, 64)
 if err != nil || r.Revision == 0 { return r, errors.New("invalid ownership_revision") }
 if len(r.Operation) > 128 || len(r.Fingerprint) != 64 { return r, errors.New("invalid recovery operation or fingerprint") }
 if _, err = hex.DecodeString(r.Fingerprint); err != nil { return r, err }
 if action == "copy_worktree" {
  files, ok := raw["files"].([]any)
  if !ok || len(files) == 0 || len(files) > 2048 { return r, errors.New("copy_worktree requires explicit bounded files") }
  for _, value := range files {
   file, ok := value.(string)
   if !ok || file == "" { return r, errors.New("files must contain exact nonempty strings") }
   r.Files = append(r.Files, file)
  }
 } else if _, ok := raw["files"]; ok { return r, errors.New("reclaim_worktree does not import files") }
 return r, nil
}
...
func (s *Service) recoverSessionWorktree(sessionID string, principal identity.Principal, args manageWorkspaceArguments, apply func(sessionruntime.SessionMutationInput) (sessionruntime.SessionMutationResult, error)) (string, error) {
 if apply == nil || s.worktrees == nil || s.sessionWorkspaceCanonicalize == nil { return "", errors.New("recovery requires canonical publisher and worktree service") }
 if args.WorkspaceID == "" || args.WorkspaceGeneration <= 0 || args.WorktreePath == "" || args.WorkspaceIDs != nil || args.PrimaryWorkspaceID != "" || args.WorkspacePathSet || args.WorkspaceNameSet || args.ThemeIDSet || args.ContentSet || args.ExpectedRevision != 0 || args.Intent != "" || args.PermissionScope != "" { return "", errors.New("recovery requires exact saved workspace identity and worktree_path only") }
 session, ok, err := s.sessions.GetSession(sessionID)
 if err != nil { return "", err }
 if !ok || session.AccountScopeID != principal.AccountScopeID || session.UserID != principal.UserID || mapString(session.Metadata, "lineage_kind") == "delegated_subagent" { return "", errors.New("recovery requires the owning primary session") }
 if err := s.sessions.EnsureWorkspaceTransitionIdle(sessionID); err != nil { return "", err }
 canonical, err := s.canonicalSessionWorkspace(principal, args.WorkspaceID, args.WorkspaceGeneration)
 if err != nil { return "", err }
 if session.WorkspacePath != canonical.SourceWorkspacePath || mapString(session.Metadata, "swarm_v3_source_workspace_id") != canonical.WorkspaceID || manageWorkspaceInt64(session.Metadata["swarm_v3_source_workspace_generation"]) != canonical.WorkspaceGeneration { return "", errors.New("recovery cannot change saved source identity") }
 ...
 if err != nil { return "", err }
 if source.HEAD != r.HEAD || source.Fingerprint != r.Fingerprint || source.Path == canonical.SourceWorkspacePath { return "", errors.New("stale or nonisolated recovery source") }
 ...
 publish := func(action string, next *pebblestore.SessionSnapshot, admission *pebblestore.WorktreeAdmissionEvidence) (sessionruntime.SessionMutationResult, error) {
  ...
  if err != nil { return sessionruntime.SessionMutationResult{}, err }
  ...
  if err == nil && result.Conflict != nil { err = errors.New(result.Conflict.Message) }
  if err == nil && result.Error != nil { err = errors.New(result.Error.Message) }
  if err == nil { seq = result.LastSeq; mutation.ExpectedRevision++ }
  return result, err
 }
 reserve, finish := "reserve", "publish"
 if args.Action == "copy_worktree" { reserve, finish = "reserve_copy", "publish_copy" }
 ...
 if args.Action == "copy_worktree" {
  ...
  // Do not run git status: its optional index refresh would alter source evidence.
  head, err := os.ReadFile(filepath.Join(source.GitDir, "HEAD"))
  if err != nil { return retainedError(err) }
  ref := strings.TrimSpace(string(head))
  if !strings.HasPrefix(ref, "ref: refs/heads/") { return retainedError(errors.New("reclaim requires an attached branch")) }
  allocation.BranchName = strings.TrimPrefix(ref, "ref: refs/heads/")
 }
 if err := worktreeruntime.ValidateOwnedIdentity(canonical.SourceWorkspacePath, allocation.WorkspacePath, allocation.BranchName, allocation.BaseCommit); err != nil { return retainedError(err) }
 next := session
 ...
 for _, grant := range pebblestore.NormalizeSessionWorkspaceGrants(session) { if grant.Kind != pebblestore.WorkspaceGrantWorktree { grants = append(grants, grant) } }
 available := true
 ...
 if err := worktreeruntime.ValidateRecoveryIdentity(canonical.SourceWorkspacePath, source); err != nil { return retainedError(err) }
 var admission *pebblestore.WorktreeAdmissionEvidence
 if args.Action == "copy_worktree" {
  if err := worktreeruntime.ValidateRecoveryIdentity(canonical.SourceWorkspacePath, destination); err != nil { return retainedError(err) }
 ...
 if err != nil { return retainedError(err) }
 if fresh.SourceWorkspacePath != canonical.SourceWorkspacePath || fresh.WorkspaceGeneration != canonical.WorkspaceGeneration { return retainedError(errors.New("saved workspace changed before publication")) }
