...
func compactRunStateLifecycleInstructions(state compactRunState) string {
	...
	return planPresenceInstruction + " An active plan exists; the injected plan and checkpoint fields are authoritative. Continue the current work when it is runnable without injecting or following any no-plan bootstrap path. The selected checkpoint's current objective governs its run; earlier plan goals and checkpoint objectives are historical context, not competing instructions. Maintain the active checkpoint's typed subtasks inline: small fixes, validation, documentation, review, and commit preparation for the same deliverable are subtasks; an independent deliverable or separate failure/review boundary is a new checkpoint. If new user feedback changes requirements for a stopped or paused checkpoint, do not blindly restart the stale definition: classify whether it is the same deliverable or an independent task. For the same deliverable, use restart_checkpoint with change_request and complete replacement title/tasks/acceptance_criteria/notes so replacement and restart are atomic; for an independent redirected deliverable, create a new ordered checkpoint with request_followup_checkpoint whose objective is derived only from the current request. Routine subtask mutations do not require plan approval. Ask the user only for a real product decision or a risky operation."
}
