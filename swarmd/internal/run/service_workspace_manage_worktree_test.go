...
func TestManageWorkspaceAdoptWorktreeKeepsSameSessionAndRefreshesScope(t *testing.T) {
	...
	if err := invoke(sessionSvc.ApplySessionMutation); err == nil {
	...
	if data, err := os.ReadFile(resumePath); err != nil || string(data) != "unfinished work" {
		t.Fatalf("reclaim changed source: %q %v", data, err)
	}
	claims, err = sessionStore.InspectWorktreeOwnership(principal.AccountScopeID, principal.UserID, []string{worktreePath})
	if err != nil || claims[0].OperationState != "published" {
	...
	if err := invoke(func(input sessionruntime.SessionMutationInput) (sessionruntime.SessionMutationResult, error) {
		if input.WorktreeRecovery.Action == "publish" {
			return sessionruntime.SessionMutationResult{}, errors.New("injected publication failure")
		}
	...
	claims, err = sessionStore.InspectWorktreeOwnership(principal.AccountScopeID, principal.UserID, []string{worktreePath})
	if err != nil || claims[0].OperationState != "reserved" || claims[0].OwnerSessionID != sessionID {
		t.Fatalf("failed publication lost reservation: %+v %v", claims, err)
	}
}
