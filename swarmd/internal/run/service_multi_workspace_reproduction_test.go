...
func TestMultiWorkspaceIdentityTransitions(t *testing.T) {
	for _, shape := range []string{"nested-managed", "sibling-managed", "nested-unmanaged"} {
		t.Run(shape, func(t *testing.T) {
			...
			retained := load()
			if err := store.CreateSessionForAccount(pebblestore.SessionSnapshot{ID: "held-child", WorkspacePath: parentPath, Metadata: map[string]any{"parent_session_id": id, "lineage_kind": "delegated_subagent"}}, principal.UserID, principal.AccountScopeID); err != nil {
				t.Fatal(err)
			}
