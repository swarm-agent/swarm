...
func TestMasterHarnessRoutesAgentProgressToPlanManageAndKeepsTodosUserOwned(t *testing.T) {
	...
	for _, want := range []string{
		...
		"On a blocked plan, call transition_checkpoint_boundary directly",
		"Session mode=auto is not evidence that a plan exists",
		...
		"independently shippable work or a separate review/failure boundary from a parent provider turn means transition_checkpoint_boundary",
		...
		"neither the model nor approval UI can disable or opt out of isolation",
		"without explicitly asking for subagents",
		"naming an agent or agents",
	} {
		if !strings.Contains(prompt, want) {
