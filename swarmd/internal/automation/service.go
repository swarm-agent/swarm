...
type Executor interface {
	Dispatch(context.Context, Principal, store.AutomationRecord) (string, error)
}
...
type Publisher interface {
	Publish(context.Context, store.AutomationRecord) error
}
...
func New(repo Repository, plans CanonicalPlans, access Access, now func() time.Time) (*Service, error) {
	if repo == nil || plans == nil || access == nil || now == nil {
...
func (s *Service) authorize(ctx context.Context, p Principal, scope store.AutomationScope, action string) error {
	if p.SubjectID == "" || p.AccountID == "" || p.AccountID != scope.AccountID || scope.WorkspaceID == "" {
		return ErrDenied
	}
...
func (s *Service) plan(ctx context.Context, p Principal, scope store.AutomationScope, ref *store.AutomationPlanReference) error {
	if ref.SessionID == "" || ref.PlanID == "" || ref.Revision == 0 || ref.Revision > uint64(^uint(0)>>1) {
		return ErrInvalid
	}
	if err := s.access.PlanSession(ctx, p, scope, ref.SessionID); err != nil {
		return err
	}
	...
	if !found || plan.AccountScopeID != scope.AccountID || plan.SessionID != ref.SessionID || plan.ID != ref.PlanID || uint64(plan.Version) != ref.Revision || plan.ApprovalState != "approved" || plan.Document == nil {
		return ErrDenied
	}
	...
	if ref.DocumentSHA256 != "" && ref.DocumentSHA256 != digest {
		return ErrDenied
	}
...
func (s *Service) execution(ctx context.Context, p Principal, scope store.AutomationScope, d store.AutomationDefinition, action string) error {
	if d.Authorization.Mode != "approved_policy" || d.Authorization.ApprovalReference == "" || d.Authorization.ExpiresAt <= s.now().UnixMilli() {
		return ErrDenied
	}
...
func (s *Service) SaveDefinition(ctx context.Context, p Principal, scope store.AutomationScope, id, mutation string, expected uint64, d store.AutomationDefinition) (store.AutomationRecord, bool, error) {
	...
	if d.Authorization.Mode != "approval_required" && d.Authorization.Mode != "approved_policy" {
	...
	for i := range d.Plans {
		...
		if found && old.Definition != nil && ref.DocumentSHA256 == "" {
			for _, binding := range old.Definition.Plans {
				prior := binding.Plan
				if prior.SessionID == ref.SessionID && prior.PlanID == ref.PlanID && prior.Revision == ref.Revision {
					ref.DocumentSHA256 = prior.DocumentSHA256
				}
			}
		}
	}
...
func (s *Service) CheckRun(ctx context.Context, p Principal, scope store.AutomationScope, id string, revision uint64) (store.AutomationRecord, error) {
	if err := s.authorize(ctx, p, scope, "run"); err != nil {
	...
	if r.Definition == nil || !r.Definition.Enabled {
	...
	for _, binding := range r.Definition.Plans {
		if binding.Plan.DocumentSHA256 == "" {
			return store.AutomationRecord{}, ErrDenied
		}
		if err := s.plan(ctx, p, scope, &binding.Plan); err != nil {
			return store.AutomationRecord{}, err
		}
	}
	if err := s.execution(ctx, p, scope, *r.Definition, "run"); err != nil {
		return store.AutomationRecord{}, err
	}
	return r, nil
}
func (s *Service) Search(ctx context.Context, p Principal, q store.AutomationSearch) ([]store.AutomationRecord, string, error) {
	if err := s.authorize(ctx, p, q.Scope, "read"); err != nil {
		return nil, "", err
	}
	if q.Limit == 0 {
		q.Limit = 20
	}
	if q.Limit < 1 || q.Limit > 50 || len(q.Query) > 256 || len(q.Cursor) > 4096 {
		return nil, "", ErrInvalid
	}
...
type Summary struct {
	Text, SubjectID, SessionID, OccurrenceID string
	OccurrenceRevision                       uint64
}
...
func (s *Service) Context(ctx context.Context, p Principal, scope store.AutomationScope, id string) (ContextBundle, error) {
	...
	if found && r.Context != nil {
		b.Revision, b.UserInstructions, b.Summaries = r.Revision, cloneMap(r.Context.UserLocked), cloneMap(r.Context.AgentOwned)
	}
	return b, nil
}
func (s *Service) UpdateContext(ctx context.Context, p Principal, scope store.AutomationScope, id, mutation string, expected uint64, locked map[string]string, summary *Summary) (store.AutomationRecord, bool, error) {
	if err := s.authorize(ctx, p, scope, "context"); err != nil {
		return store.AutomationRecord{}, false, err
	}
	...
	if summary != nil {
		if p.Role != "agent" || summary.OccurrenceRevision == 0 || len(summary.Text) == 0 || len(summary.Text) > 4096 {
			return store.AutomationRecord{}, false, ErrDenied
		}
		...
		if !found || r.Occurrence == nil || r.Occurrence.SessionID == "" {
			return store.AutomationRecord{}, false, ErrDenied
		}
		if err := s.access.OccurrenceSession(ctx, p, scope, r.Occurrence.SessionID); err != nil {
			return store.AutomationRecord{}, false, err
		}
		attributed := *summary
		...
		if b.Summaries == nil {
			b.Summaries = map[string]string{}
		}
		b.Summaries[summary.OccurrenceID] = string(data)
	}
	data, err := json.Marshal(b)
	if err != nil || len(data) > 24000 || len(b.UserInstructions) > 64 || len(b.Summaries) > 64 {
		return store.AutomationRecord{}, false, ErrInvalid
	}
...
func RetryDelay(attempt int, transient, sideEffectsPossible bool) (time.Duration, bool) {
	if !transient || sideEffectsPossible || attempt < 1 || attempt > 3 {
		return 0, false
	}
