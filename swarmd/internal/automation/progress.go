...
func scheduleSlots(ctx context.Context, r store.AutomationRecord, start, end int64) ([]ProgressSlot, error) {
	...
	out := []ProgressSlot{}
	if s.Kind != "cron" && s.Kind != "interval" {
		return out, nil
	}
	if start < r.WrittenAt {
		start = r.WrittenAt
	}
	step := int64(60000)
	...
	for ; at < end; at += step {
		...
		if due {
			out = append(out, ProgressSlot{ScheduledAt: at, DefinitionRevision: r.Revision, Forecast: true})
		}
	}
	return out, nil
}
...
func (s *Service) ScheduleProgress(ctx context.Context, p Principal, scope store.AutomationScope, id, timezone string) (Progress, error) {
	...
	if out.Schedule.Kind == "interval" {
		anchor := head.WrittenAt
		out.IntervalAnchor = &anchor
	}
	...
	for n := 0; n < 50; n++ {
		...
		if r.WrittenAt <= out.DayStart || r.Revision == 1 {
			break
		}
		end = r.WrittenAt
		if n == 49 {
			out.ForecastComplete = false
			break
		}
	...
	for page := 0; page < 8; page++ {
		...
		for _, row := range rows {
			o := row.Occurrence
			if o == nil || seen[row.ID] {
				continue
			}
			seen[row.ID] = true
			if o.State == "pending" || o.State == "running" || o.State == "cancelling" {
				active = true
			}
			kind := o.TriggerKind
			...
			if kind == "" && o.TriggerIdentity == executionKey("schedule", "", fmt.Sprint(o.ScheduledAt)) {
				kind = "schedule"
			}
			if kind == "" {
				kind = "unknown"
			...
			if out.LatestRecorded == nil || row.WrittenAt > out.LatestRecorded.RecordedAt {
				copy := item
				out.LatestRecorded = &copy
			}
			if o.ScheduledAt < out.DayStart || o.ScheduledAt >= out.DayEnd {
				continue
			}
			...
			case "schedule":
				out.Counts[o.State]++
			case "manual":
				out.ManualCounts[o.State]++
			case "unknown":
				out.UnknownTriggerCount++
			}
		}
		cursor = next
		if cursor == "" {
	...
	if len(slots) > 100 {
		slots = slots[:100]
		out.UpcomingComplete = false
	}
	out.UpcomingSlots = slots
	switch {
	case out.Schedule.Kind == "manual" || out.Schedule.Kind == "event":
		out.NoNextReason = "not_time_scheduled"
	case !head.Definition.Enabled:
		out.NoNextReason = "paused"
	case head.Definition.Authorization.Mode != "approved_policy":
		out.NoNextReason = "approval_required"
	case head.Definition.Authorization.ExpiresAt <= out.AsOf:
		out.NoNextReason = "expired"
	case out.Schedule.OverlapPolicy == "serialize" && (!out.HistoryComplete || active):
		out.NoNextReason = "overlap_or_history_unknown"
	default:
		_, checkErr := s.CheckRun(ctx, p, scope, id, head.Revision)
		if checkErr != nil {
			out.NoNextReason = "authorization_or_plan_unavailable"
		} else if len(slots) == 0 {
			out.NoNextReason = "outside_48h_horizon"
		} else if slots[0].ScheduledAt >= head.Definition.Authorization.ExpiresAt {
			out.NoNextReason = "expires_before_next_slot"
		} else {
			slot := slots[0]
			out.NextEligible = &slot
		}
	}
	return out, nil
}
