...
func TestApprovedEventAdmission(t *testing.T) {
	...
	for _, tc := range []struct { p Principal; revision uint64; source string }{
		{foreign, 2, "ci"}, {user, 1, "ci"}, {user, 2, "other"},
	} {
		bad := trigger
		bad.Source = tc.source
		if _, err := e.Admit(bind(tc.p), tc.p, scope, "check", tc.revision, bad); err == nil { t.Fatal("unauthorized event accepted") }
	}
	rows, _, err := db.SearchAutomationRecords(store.AutomationSearch{Scope: scope, Kind: "occurrence", Limit: 10})
	if err != nil || len(rows) != 0 { t.Fatal("denied admission wrote occurrence") }
	first, err := e.Admit(ctx, user, scope, "check", 2, trigger)
	if err != nil { t.Fatal(err) }
	replay, err := e.Admit(ctx, user, scope, "check", 2, trigger)
	if err != nil || replay.ID != first.ID || replay.Revision != first.Revision { t.Fatal("event replay not idempotent", err) }
	if _, err := a.RevokeUser(ctx, scope, grant.ID, grant.Revision); err != nil { t.Fatal(err) }
	trigger.Identity = "after-revoke"
	if _, err := e.Admit(ctx, user, scope, "check", 2, trigger); err == nil { t.Fatal("revoked event accepted") }
	rows, _, err = db.SearchAutomationRecords(store.AutomationSearch{Scope: scope, Kind: "occurrence", Limit: 10})
	if err != nil || len(rows) != 1 || rows[0].ID != first.ID { t.Fatal("revoked event mutated occurrences") }
}
