...
func NewPolicyApproval(repo ApprovalRepository, plans CanonicalPlans, ownership ApprovalOwnership, identity ApprovalIdentity, now func() time.Time) (*PolicyApproval, error) {
	if repo == nil || plans == nil || ownership == nil || identity.Current == nil || identity.ExplicitUser == nil || now == nil { return nil, ErrInvalid }
...
func (a *PolicyApproval) Workspace(ctx context.Context, p Principal, scope store.AutomationScope, action string) error {
	...
	if actual != p || p.SubjectID == "" || p.AccountID == "" || p.AccountID != scope.AccountID || scope.WorkspaceID == "" || (p.Role != "user" && p.Role != "agent" && p.Role != "system") { return ErrDenied }
...
func (a *PolicyApproval) OccurrenceSession(ctx context.Context, p Principal, scope store.AutomationScope, id string) error {
	if err := a.Workspace(ctx, p, scope, "context"); err != nil { return err }
...
func (a *PolicyApproval) ApproveUser(ctx context.Context, input ApprovalRequest) (store.AutomationApproval, error) {
	...
	if p.Role != "user" { return store.AutomationApproval{}, ErrDenied }
	if err := a.Workspace(ctx, p, input.Scope, "approve"); err != nil { return store.AutomationApproval{}, err }
	r, found, err := a.repo.GetAutomationRecord(input.Scope, input.AutomationID, "definition", input.AutomationID, 0)
	if err != nil { return store.AutomationApproval{}, err }
	if !found || r.Definition == nil || input.DefinitionRevision == 0 || r.Revision != input.DefinitionRevision { return store.AutomationApproval{}, ErrDenied }
	digest, err := ApprovalPolicyDigest(*r.Definition)
	if err != nil { return store.AutomationApproval{}, err }
	if input.PolicySHA256 != digest || r.Definition.Authorization.ExpiresAt <= a.now().UnixMilli() { return store.AutomationApproval{}, ErrDenied }
	if err := a.checkPlans(ctx, p, r.Scope, *r.Definition); err != nil { return store.AutomationApproval{}, err }
	var nonce [32]byte
	if _, err := rand.Read(nonce[:]); err != nil { return store.AutomationApproval{}, err }
...
func (a *PolicyApproval) Request(ctx context.Context, p Principal, r store.AutomationRecord) (string, error) {
	if r.Definition == nil { return "", ErrInvalid }
	if err := a.Workspace(ctx, p, r.Scope, "approve"); err != nil { return "", err }
	digest, err := ApprovalPolicyDigest(*r.Definition)
	if err != nil { return "", err }
...
func (a *PolicyApproval) RevokeUser(ctx context.Context, scope store.AutomationScope, reference string, expected uint64) (store.AutomationApproval, error) {
	...
	if err != nil { return store.AutomationApproval{}, err }
	if p.Role != "user" { return store.AutomationApproval{}, ErrDenied }
	if err := a.Workspace(ctx, p, scope, "revoke"); err != nil { return store.AutomationApproval{}, err }
...
func (a *PolicyApproval) checkPlans(ctx context.Context, p Principal, scope store.AutomationScope, d store.AutomationDefinition) error {
	if err := store.ValidateAutomationBindings(d.Plans); err != nil { return err }
	s := Service{plans: a.plans, access: a}
	for _, binding := range d.Plans {
		ref := binding.Plan
		if ref.DocumentSHA256 == "" { return ErrDenied }
		if err := s.plan(ctx, p, scope, &ref); err != nil { return err }
	}
	return nil
}
...
func (a *PolicyApproval) Execution(ctx context.Context, p Principal, scope store.AutomationScope, d store.AutomationDefinition, action string) error {
	if err := a.Workspace(ctx, p, scope, action); err != nil { return err }
	g, found, err := a.repo.GetAutomationApproval(scope, d.Authorization.ApprovalReference)
	if err != nil { return err }
	if !found || g.Scope != scope || g.RevokedAt != 0 || g.ExpiresAt <= a.now().UnixMilli() || d.Authorization.Mode != "approved_policy" || d.Authorization.ExpiresAt != g.ExpiresAt { return ErrDenied }
	...
	if err := a.ownership.Workspace(ctx, owner, scope, "approve"); err != nil { return err }
	digest, err := ApprovalPolicyDigest(d)
	if err != nil { return err }
	if digest != g.PolicySHA256 { return ErrDenied }
...
func (a *PolicyApproval) Verify(ctx context.Context, p Principal, r store.AutomationRecord) error {
	if r.Definition == nil || r.Revision == 0 { return ErrDenied }
	if err := a.Execution(ctx, p, r.Scope, *r.Definition, "run"); err != nil { return err }
	g, found, err := a.repo.GetAutomationApproval(r.Scope, r.Definition.Authorization.ApprovalReference)
	if err != nil { return err }
	if !found || g.AutomationID != r.AutomationID || g.RevokedAt != 0 { return ErrDenied }
	current, found, err := a.repo.GetAutomationRecord(r.Scope, r.AutomationID, "definition", r.AutomationID, 0)
	if err != nil { return err }
	if !found || current.Revision != r.Revision || !reflect.DeepEqual(current.Definition, r.Definition) || !r.Definition.Enabled { return ErrDenied }
	return nil
}
