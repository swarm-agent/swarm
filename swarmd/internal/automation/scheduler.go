...
func (e *ExecutionService) TickAt(ctx context.Context, p Principal, scope store.AutomationScope, id string, revision uint64, at time.Time) error {
	...
	now := at.UnixMilli()
	if now <= previous || now < def.WrittenAt {
		return nil
	}
	var candidate int64
	if s.Kind == "interval" {
...
func (e *ExecutionService) RecoverPage(ctx context.Context, p Principal, scope store.AutomationScope, id, cursor string) (string, error) {
	if err := e.domain.authorize(ctx, p, scope, "run"); err != nil {
		return cursor, err
	}
	...
	for _, r := range records {
		...
		case "cancelling":
			if r.SubjectID != p.SubjectID {
				failures = append(failures, ErrDenied)
				continue
			}
			...
			repo, stored := e.domain.repo.(interface {
