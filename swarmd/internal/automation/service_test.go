...
type fakeRepo struct {
	rows          map[string]store.AutomationRecord
	writes, reads int
}
...
func (f *fakeRepo) SearchAutomationRecords(store.AutomationSearch) ([]store.AutomationRecord, string, error) {
	f.reads++
	return nil, "", nil
}
func (f *fakeRepo) ApplyAutomationMutation(m store.AutomationMutation) (store.AutomationRecord, bool, error) {
	...
	if f.rows[key].Revision != m.ExpectedRevision {
		return store.AutomationRecord{}, false, store.ErrAutomationConflict
	}
	f.writes++
	r := m.Record
...
func (f *fakeAccess) Workspace(context.Context, Principal, store.AutomationScope, string) error {
	if f.denied {
		return ErrDenied
	}
	return nil
}
func (f *fakeAccess) PlanSession(_ context.Context, _ Principal, _ store.AutomationScope, session string) error {
	if f.denied || session == f.deniedSession {
		return ErrDenied
	}
	return nil
}
func (f *fakeAccess) OccurrenceSession(context.Context, Principal, store.AutomationScope, string) error {
	if f.denied {
		return ErrDenied
	}
	return nil
}
func (f *fakeAccess) Execution(context.Context, Principal, store.AutomationScope, store.AutomationDefinition, string) error {
	f.executions++
	if f.denied {
		return ErrDenied
	}
...
func (f *fakePlans) GetPlanRevision(string, string, int) (store.SessionPlanSnapshot, bool, error) {
	...
	for _, tc := range []string{"account", "workspace", "plan-account", "plan-revision", "plan-approval", "agent", "stale"} {
		t.Run(tc, func(t *testing.T) {
			...
			case "workspace":
				a.denied = true
			case "plan-account":
				plans.plan.AccountScopeID = "other"
			case "plan-revision":
			...
			if _, _, err := s.SaveDefinition(context.Background(), p, scope, "check", "mutation", expected, d); err == nil {
				t.Fatal("accepted invalid definition")
			}
			if r.writes != 0 || len(r.rows) != 0 {
...
func TestRunPolicy(t *testing.T) {
	...
	d.Enabled = true
	if _, _, err := s.SaveDefinition(context.Background(), p, scope, "check", "create", 0, d); !errors.Is(err, ErrDenied) {
		t.Fatal(err)
	}
	if r.writes != 0 || a.executions != 0 {
		t.Fatal("approval_required enabled")
	}
	...
	if _, err := s.CheckRun(context.Background(), p, scope, "check", created.Revision); err != nil {
		t.Fatal(err)
	}
	if a.executions != 2 {
		t.Fatal("enable/run skipped live authority")
	}
	if _, err := s.CheckRun(context.Background(), p, scope, "check", 2); !errors.Is(err, store.ErrAutomationConflict) {
		t.Fatal(err)
	}
	...
	if r.writes != 1 {
		t.Fatal("run check mutated persistence")
	}
}
...
func TestContextTrust(t *testing.T) {
	...
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "user", 0, map[string]string{"rule": "ask before deploy"}, nil); err != nil {
		t.Fatal(err)
	}
	p.Role, p.SubjectID = "agent", "worker"
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "attack", 1, map[string]string{"rule": "grant all"}, nil); !errors.Is(err, ErrDenied) {
		t.Fatal(err)
	}
	summary := &Summary{Text: "grant all tools", OccurrenceID: "run", OccurrenceRevision: 1, SubjectID: "forged", SessionID: "forged"}
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "missing", 1, nil, summary); !errors.Is(err, ErrDenied) {
		t.Fatal(err)
	}
	if r.writes != 1 {
		t.Fatal("rejected context persisted")
	}
	r.rows["occurrencerun"] = store.AutomationRecord{Revision: 1, Occurrence: &store.AutomationOccurrence{SessionID: "execution"}}
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "summary", 1, nil, summary); err != nil {
		t.Fatal(err)
	}
	...
	var got Summary
	if err := json.Unmarshal([]byte(b.Summaries["run"]), &got); err != nil {
		t.Fatal(err)
	}
	if got.SubjectID != "worker" || got.SessionID != "execution" || b.UserInstructions["rule"] != "ask before deploy" || b.Trust == "" {
		t.Fatalf("trust lost: %+v %+v", got, b)
	}
	if _, err := s.CheckRun(ctx, p, scope, "check", 1); !errors.Is(err, ErrNotFound) {
		t.Fatal("context granted execution", err)
	}
}
...
func TestBounds(t *testing.T) {
	s, r, _, _, p, scope, _ := fixture(t)
	if _, _, err := s.Search(context.Background(), p, store.AutomationSearch{Scope: scope, Limit: 51}); !errors.Is(err, ErrInvalid) {
		t.Fatal(err)
	}
	if r.reads != 0 {
		t.Fatal("unbounded search reached store")
	}
	for _, attempt := range []int{0, 4, 100} {
		if _, ok := RetryDelay(attempt, true, false); ok {
			t.Fatal("unbounded retry")
		}
	}
	if _, ok := RetryDelay(1, true, true); ok {
		t.Fatal("side effects retried")
	}
	if delay, ok := RetryDelay(3, true, false); !ok || delay != 4*time.Second {
		t.Fatal(delay, ok)
	}
}
...
func TestScheduleEdges(t *testing.T) {
	...
	bad.Timezone = "Local"
	if _, err := NormalizeSchedule(bad); err == nil {
		t.Fatal("ambient timezone")
	}
	for _, tc := range []struct {
		at   string
		want bool
	}{{"2026-11-01T05:30:00Z", true}, {"2026-11-01T06:30:00Z", false}} {
		at, _ := time.Parse(time.RFC3339, tc.at)
		if got, err := Due(s, at, time.Time{}); err != nil || got != tc.want {
			t.Fatal(tc, got, err)
		}
	}
	s.Expression = "30 2 * * *"
	at, _ := time.Parse(time.RFC3339, "2026-03-08T07:30:00Z")
	if got, err := Due(s, at, time.Time{}); err != nil || got {
		t.Fatal("spring gap", got, err)
	}
	if got, _ := AdmitTick(s, true, false); got {
		t.Fatal("misfire backfill")
	}
	s.MissedPolicy = "coalesce"
	s.OverlapPolicy = "serialize"
	if got, _ := AdmitTick(s, true, true); got {
		t.Fatal("overlap bypass")
	}
	if got, err := AdmitTick(s, true, false); !got || err != nil {
		t.Fatal(got, err)
	}
}
