...
func TestPinnedExecutionDocumentResetsOnlyProgress(t *testing.T) {
	...
	if cp.Status != "pending" || cp.Report != "" || cp.RunID != "" || cp.Subtasks[0].Result != "" || cp.Subtasks[0].Status != "pending" || cp.Objective != "Inspect" || cp.AcceptanceCriteria[0] != "Evidence" { t.Fatalf("incorrect reset: %+v", cp) }
	if plans.plan.Document.Checkpoints[0].Status != "completed" { t.Fatal("source was mutated") }
	plans.plan.Document.Checkpoints[0].Objective = "changed"
	if _, err := v.pinnedDocument(context.Background(), p, def, "new-plan"); !errors.Is(err, ErrDenied) { t.Fatalf("changed source accepted: %v", err) }
}
...
func TestV3RuntimeRejectsMissingAuthorities(t *testing.T) {
	if _, err := NewV3Runtime(nil, nil, nil, nil, nil, nil); !errors.Is(err, ErrInvalid) { t.Fatalf("constructor: %v", err) }
}
