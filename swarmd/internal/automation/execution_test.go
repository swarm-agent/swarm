...
func (f triggerAuthorityFake) Verify(context.Context, Principal, store.AutomationScope, Trigger) error {
	return f.err
}
...
func TestExecutionAdmissionAndRevocation(t *testing.T) {
	...
	writes := repo.writes
	if _, err := e.Admit(context.Background(), p, scope, "automation", 1, trigger); !errors.Is(err, ErrDenied) {
		t.Fatalf("admission: %v", err)
	}
	if repo.writes != writes || runtime.calls != 0 {
		t.Fatal("denied trigger caused side effects")
	}
	...
	access.denied = true
	writes = repo.writes
	if _, err := e.Dispatch(context.Background(), p, scope, "automation", r.ID); !errors.Is(err, ErrDenied) {
		t.Fatalf("revoked dispatch: %v", err)
	}
	if runtime.calls != 0 || repo.writes != writes {
		t.Fatal("revoked dispatch caused side effects")
	}
}
...
func TestExecutionPartialFailurePreservesPending(t *testing.T) {
	...
	writes := repo.writes
	if _, err := e.Dispatch(context.Background(), p, scope, "automation", r.ID); err == nil {
		t.Fatal("failure reported success")
	}
	if repo.writes != writes {
		t.Fatal("partial failure changed durable state")
	}
	persisted := repo.rows["occurrence"+r.ID]
	if persisted.Occurrence.State != "pending" || persisted.Occurrence.SessionID != "" {
		t.Fatal("pending recovery record lost")
	}
	...
	if out.ID != r.ID || out.Occurrence.State != "running" || out.Occurrence.SessionID != "execution-session" {
...
func TestExecutionCancellationFenceRecovery(t *testing.T) {
	...
	if err := db.ClaimAutomationDispatch(scope, "automation", r.ID); err != nil {
		t.Fatal(err)
	}
	// An exact but nonexistent/stale revision cannot reach the runtime.
	if _, err := e.Cancel(ctx, p, scope, "automation", r.ID, r.Revision+1, "stale"); !errors.Is(err, store.ErrAutomationConflict) {
		t.Fatalf("stale: %v", err)
	}
	if runtime.stops != 0 {
		t.Fatal("stale stop effect")
	}
	head, _, err := db.GetAutomationRecord(scope, "automation", "occurrence", r.ID, 0)
	if err != nil || head.Revision != r.Revision {
		t.Fatal("stale request mutated head")
	}
	interrupted := errors.New("stop interrupted")
	runtime.stop = func(admitted store.AutomationRecord) error {
		if admitted.Occurrence.State != "cancelling" {
			t.Fatal("stop before durable admission")
		}
		...
		if _, err := e.Cancel(ctx, p, scope, "automation", r.ID, r.Revision, "competitor"); !errors.Is(err, store.ErrAutomationConflict) {
			t.Fatalf("competitor: %v", err)
		}
		if runtime.stops != 1 {
			t.Fatal("competing stop effect")
		}
		if err := db.ClaimAutomationDispatch(scope, "automation", r.ID); !errors.Is(err, store.ErrAutomationConflict) {
			t.Fatalf("dispatch fence: %v", err)
		}
		if err := db.ClaimAutomationDispatch(scope, "automation", next.ID); !errors.Is(err, store.ErrAutomationConflict) {
			t.Fatalf("reservation released: %v", err)
		}
		if _, err := e.transition(p, admitted, "completed", ""); !errors.Is(err, store.ErrAutomationConflict) {
			t.Fatalf("writer bypass: %v", err)
		}
		return interrupted
	}
	out, err := e.Cancel(ctx, p, scope, "automation", r.ID, r.Revision, "cancel")
	if !errors.Is(err, interrupted) || out.Occurrence.State != "cancelling" {
	...
	out, err = e.Cancel(ctx, p, scope, "automation", r.ID, r.Revision, "cancel")
	if err != nil || out.Occurrence.State != "cancelled" || out.Revision != r.Revision+2 {
		t.Fatalf("restart: %+v %v", out, err)
	}
	if runtime.stops != 2 {
		t.Fatal("retry did not resume stop")
	}
	if _, err := e.Cancel(ctx, p, scope, "automation", r.ID, r.Revision, "cancel"); err != nil {
		t.Fatal(err)
	}
	if runtime.stops != 2 {
		t.Fatal("terminal replay stopped again")
	}
	if err := db.ClaimAutomationDispatch(scope, "automation", next.ID); err != nil {
		t.Fatalf("confirmed stop retained reservation: %v", err)
	}
}
...
func (c *outcomeCatalog) GetSession(string) (store.SessionSnapshot, bool, error) {
	return c.session, true, nil
}
func (c *outcomeCatalog) GetPlan(string, string) (store.SessionPlanSnapshot, bool, error) {
...
func TestExecutionCanonicalOutcomeRecovery(t *testing.T) {
	...
	if _, err := e.ReconcileOutcome(ctx, p, scope, "automation", r.ID); !errors.Is(err, ErrDenied) {
		t.Fatal(err)
	}
	head, _, _ := db.GetAutomationRecord(scope, "automation", "occurrence", r.ID, 0)
	if head.Revision != r.Revision {
		t.Fatal("foreign evidence mutated occurrence")
	}
	...
	for _, state := range []string{"blocked", "completed", "completed"} {
		...
		if err != nil || out.Occurrence.State != state {
			t.Fatalf("%s: %+v %v", state, out, err)
		}
	}
	b, err := s.Context(ctx, p, scope, "automation")
	if err != nil || b.UserInstructions["instruction"] != "preserve" || len(b.Summaries) != 64 || b.Summaries["0"] != "old" || b.Summaries["63"] != "old" {
		t.Fatalf("context: %+v %v", b, err)
	}
	...
	if err != nil || len(audits) != 2 {
		t.Fatalf("audit replay: %d %v", len(audits), err)
	}
}
