...
func TestSchedulerDurablePageWindow(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		if err := e.TickAt(context.Background(), p, scope, "automation", 1, time.UnixMilli(240000)); err != nil { t.Fatal(err) }
	}
	rows, _, err := db.SearchAutomationRecords(store.AutomationSearch{Scope: scope, AutomationID: "automation", Kind: "occurrence", Limit: 50})
	if err != nil || len(rows) != 1 || rows[0].Occurrence.ScheduledAt != 220000 { t.Fatalf("delayed page lost or duplicated due anchor: %+v %v", rows, err) }
	// No retained sweep covers this later downtime: skip still means skip.
	if err := e.Tick(context.Background(), p, scope, "automation", 1); err != nil { t.Fatal(err) }
	rows, _, err = db.SearchAutomationRecords(store.AutomationSearch{Scope: scope, AutomationID: "automation", Kind: "occurrence", Limit: 50})
	if err != nil || len(rows) != 1 { t.Fatalf("downtime was backfilled: %+v %v", rows, err) }
	if err := e.TickAt(context.Background(), p, scope, "automation", 1, time.UnixMilli(1000000)); !errors.Is(err, ErrInvalid) { t.Fatal("future sweep accepted", err) }
}
