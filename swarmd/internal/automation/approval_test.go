...
func TestPolicyApprovalRejectsUntrustedAndStaleGrants(t *testing.T) {
	...
	a, err := NewPolicyApproval(db, plans, ownership, ApprovalIdentity{
		...
		ExplicitUser: func(context.Context) (Principal, error) { if !explicit { return Principal{}, ErrDenied }; return actual, nil },
	...
	for _, scenario := range []string{"agent", "forged-user", "wrong-account", "stale-digest", "stale-revision"} {
		t.Run(scenario, func(t *testing.T) {
			...
			bad := input
			switch scenario {
			case "agent": actual.Role = "agent"
			case "forged-user": explicit = false
			case "wrong-account": bad.Scope.AccountID = "other"
			case "stale-digest": bad.PolicySHA256 = "stale"
			case "stale-revision": bad.DefinitionRevision++
			}
			if _, err := a.ApproveUser(ctx, bad); err == nil { t.Fatal("approval accepted") }
			if scenario == "forged-user" { if _, err := a.Request(ctx, user, r); err == nil { t.Fatal("request bypassed explicit origin") } }
			current, _, err := db.GetAutomationRecord(scope, "check", "definition", "check", 0)
			if err != nil || current.Revision != 1 { t.Fatal("rejected request mutated definition") }
		})
	}
	actual, explicit = user, true
	...
	if err != nil { t.Fatal(err) }
	if err := a.Verify(ctx, user, r); err != nil { t.Fatal(err) }
	changed := d
	changed.Authorization.TargetIDs = []string{"different"}
	if err := a.Execution(ctx, user, scope, changed, "run"); err == nil { t.Fatal("changed target accepted") }
	plans.plan.Document.Title = "rewritten"
	if err := a.Verify(ctx, user, r); err == nil { t.Fatal("stale plan accepted") }
	plans.plan.Document.Title = ""
	now = g.ExpiresAt
	if err := a.Verify(ctx, user, r); err == nil { t.Fatal("expired grant accepted") }
	now = 100001
	actual.Role = "agent"
	explicit = false
	if _, err := a.RevokeUser(ctx, scope, g.ID, 1); err == nil { t.Fatal("agent revoked grant") }
	persisted, _, _ := db.GetAutomationApproval(scope, g.ID)
	if persisted.Revision != 1 || persisted.RevokedAt != 0 { t.Fatal("denied revoke mutated grant") }
	actual, explicit = user, true
	if _, err := a.RevokeUser(ctx, scope, g.ID, 1); err != nil { t.Fatal(err) }
	if err := a.Verify(ctx, user, r); err == nil { t.Fatal("revoked grant accepted") }
}
