...
func eventID(v string) bool { return v != "" && len(v) <= 256 && utf8.ValidString(v) && strings.TrimSpace(v) == v }

func NewEventAuthority(registrations []EventRegistration) (*EventAuthority, error) {
	if len(registrations) > 256 { return nil, ErrInvalid }
	for _, r := range registrations {
		if r.Principal.Role != "user" || r.Principal.AccountID != r.Scope.AccountID || !eventID(r.Principal.SubjectID) || !eventID(r.Scope.AccountID) || !eventID(r.Scope.WorkspaceID) || !eventID(r.AutomationID) || r.Revision == 0 || !eventID(r.Source) { return nil, ErrInvalid }
	}
...
func (a *EventAuthority) VerifyAdmission(ctx context.Context, p Principal, scope store.AutomationScope, id string, revision uint64, t Trigger) error {
	...
	if err != nil || actual != p || t.Kind != "event" || !eventID(t.Identity) || !eventID(t.Source) || t.ScheduledAt <= 0 { return ErrDenied }
	if a != nil { for _, r := range a.registrations {
		if r.Principal == p && r.Scope == scope && r.AutomationID == id && r.Revision == revision && r.Source == t.Source { return nil }
	} }
	return ErrDenied
}
