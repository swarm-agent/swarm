...
func TestRuntimeIdentityOrigins(t *testing.T) {
	...
	for _, origin := range []string{"user", "agent", "system"} {
		...
		if err != nil || p.Role != origin || p.AccountID != "account" {
			t.Fatal(p, err)
		}
		if origin == "agent" && p.SubjectID != session {
			t.Fatal("agent became owner")
		}
		_, err = resolver.ExplicitUser(ctx)
		if (err == nil) != (origin == "user") {
			t.Fatal("incorrect approval origin", origin)
		}
		...
		for _, kind := range []string{"manual", "schedule", "event"} {
			...
			if (err == nil) != allowed {
				t.Fatal("trigger origin mismatch", origin, kind)
			}
		}
		p.SubjectID = "substituted"
		if err := (RuntimeTriggers{}).Verify(ctx, p, scope, Trigger{Kind: "manual"}); err == nil {
			t.Fatal("substituted principal accepted")
		}
	}
	forged := context.WithValue(context.Background(), "principal", Principal{AccountID: "account", SubjectID: "owner", Role: "user"})
	if _, err := resolver.Current(forged); err == nil {
		t.Fatal("string context key granted identity")
	}
	if _, err := BindRuntimeIdentity(context.Background(), identity.Principal{}, "user", ""); err == nil {
		t.Fatal("invalid identity accepted")
	}
	if _, err := BindRuntimeIdentity(context.Background(), verified, "agent", ""); err == nil {
		t.Fatal("agent without session accepted")
	}
}
