...
func TestProgressScheduleSlots(t *testing.T) {
	for _, tc := range []struct {
		...
		if err != nil || len(slots) != tc.want {
			t.Fatalf("%s %s: %d %v", tc.date, tc.expression, len(slots), err)
		}
	}
	...
	if err != nil || len(slots) != 2 || slots[0].ScheduledAt != anchor+3601000 || slots[1].ScheduledAt != anchor+7202000 {
		t.Fatalf("anchor parity: %+v %v", slots, err)
	}
}
...
func (r *progressRepo) SearchAutomationRecords(store.AutomationSearch) ([]store.AutomationRecord, string, error) {
	r.pages++
	if r.incomplete {
		return r.occurrences, "continuation", nil
	}
...
func (r *progressRepo) GetAutomationRecord(scope store.AutomationScope, id, kind, record string, rev uint64) (store.AutomationRecord, bool, error) {
	if rev != 0 && r.revisions != nil {
		v, ok := r.revisions[rev]
		return v, ok, nil
	}
...
func TestProgressCohortAndBounds(t *testing.T) {
	...
	if r.pages != 8 || out.HistoryComplete || out.Counts["completed"] != 1 || out.ManualCounts["completed"] != 1 || out.UnknownTriggerCount != 1 {
		t.Fatalf("bounded cohort: %+v pages=%d", out, r.pages)
	}
	if len(out.PlannedSlots) != 18 || out.PlannedSlots[0].DefinitionRevision != 1 || out.PlannedSlots[12].DefinitionRevision != 2 {
		t.Fatalf("revision windows: %+v", out.PlannedSlots)
	}
	if out.NextEligible != nil || out.NoNextReason != "approval_required" {
		t.Fatalf("approval: %+v", out)
	}
	r.incomplete = false
	out, err = s.ScheduleProgress(context.Background(), p, scope, "auto", "UTC")
	if err != nil || !out.HistoryComplete {
	...
	if !errors.Is(err, ErrDenied) || base.reads != reads || r.pages != pages || base.writes != 0 {
		t.Fatal("cross-account read/write")
	}
}
...
func TestProgressUnavailableStates(t *testing.T) {
	for _, reason := range []string{"paused", "expired", "not_time_scheduled"} {
		...
		if reason == "not_time_scheduled" {
			d.Schedule.Kind = "manual"
			d.Schedule.IntervalSeconds = 0
		}
		repo.rows["definitionauto"] = store.AutomationRecord{Revision: 1, WrittenAt: 1000, Definition: &d}
		out, err := s.ScheduleProgress(context.Background(), p, scope, "auto", "UTC")
		if err != nil || out.NoNextReason != reason || out.NextEligible != nil {
			t.Fatalf("%s: %+v %v", reason, out, err)
		}
		if reason == "not_time_scheduled" && len(out.PlannedSlots) != 0 {
			t.Fatal("manual quota")
		}
		if _, err := s.ScheduleProgress(context.Background(), p, scope, "auto", "Local"); !errors.Is(err, ErrInvalid) {
