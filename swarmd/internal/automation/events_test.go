...
func TestRegisteredEventAdmission(t *testing.T) {
	...
	for _, target := range []struct {
		...
		if _, err := e.Admit(ctx, p, target.scope, target.id, target.revision, trigger); err == nil {
			t.Fatal("unregistered target accepted")
		}
	}
	if _, err := e.Admit(context.Background(), p, scope, "automation", 1, trigger); err == nil {
	...
	if err != nil || len(rows) != 0 {
		t.Fatal("denied intake mutated storage")
	}
