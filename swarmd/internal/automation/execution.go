...
func (e *ExecutionService) Admit(ctx context.Context, p Principal, scope store.AutomationScope, id string, revision uint64, trigger Trigger) (store.AutomationRecord, error) {
	...
	if trigger.Kind == "event" {
		exact, ok := e.triggers.(interface {
			VerifyAdmission(context.Context, Principal, store.AutomationScope, string, uint64, Trigger) error
		})
		if !ok {
			return store.AutomationRecord{}, ErrDenied
		}
		if err := exact.VerifyAdmission(ctx, p, scope, id, revision, trigger); err != nil {
			return store.AutomationRecord{}, err
		}
	} else if err := e.triggers.Verify(ctx, p, scope, trigger); err != nil {
		return store.AutomationRecord{}, err
	}
	...
	switch trigger.Kind {
	case "manual":
		if p.Role != "user" || trigger.Source != "" {
			return store.AutomationRecord{}, ErrDenied
		}
	case "event":
		if schedule.Kind != "event" || trigger.Source != schedule.TriggerSource {
			return store.AutomationRecord{}, ErrDenied
		}
	case "schedule":
		if p.Role != "system" || trigger.Source != "" {
			return store.AutomationRecord{}, ErrDenied
		}
...
func (e *ExecutionService) Dispatch(ctx context.Context, p Principal, scope store.AutomationScope, id, occurrenceID string) (store.AutomationRecord, error) {
	...
	if !found || r.Occurrence == nil {
		return r, ErrNotFound
	}
	if r.Occurrence.State != "pending" {
		return r, nil
	}
	...
	claims, ok := s.repo.(interface {
		ClaimAutomationDispatch(store.AutomationScope, string, string) error
	})
	if !ok {
		return r, ErrInvalid
	}
	if err := claims.ClaimAutomationDispatch(scope, id, occurrenceID); err != nil {
		return r, err
	}
...
func (e *ExecutionService) RecordOutcome(ctx context.Context, p Principal, scope store.AutomationScope, id, occurrenceID, state, summary string, facts map[string]string) (store.AutomationRecord, error) {
	s := e.domain
	if err := s.authorize(ctx, p, scope, "outcome"); err != nil {
		return store.AutomationRecord{}, err
	}
	if p.Role != "agent" || summary == "" || len(summary) > 4096 || len(facts) > 32 {
		return store.AutomationRecord{}, ErrDenied
	}
	switch state {
...
func (e *ExecutionService) recordOutcome(p Principal, r store.AutomationRecord, state, summary string, facts map[string]string) (store.AutomationRecord, error) {
	...
	if r.Occurrence.State != "running" && r.Occurrence.State != "blocked" && r.Occurrence.State != state {
		return r, store.ErrAutomationConflict
	}
...
func (e *ExecutionService) Cancel(ctx context.Context, p Principal, scope store.AutomationScope, id, occurrenceID string, expectedRevision uint64, mutationID string) (store.AutomationRecord, error) {
	if err := e.domain.authorize(ctx, p, scope, "cancel"); err != nil {
		return store.AutomationRecord{}, err
	}
	if p.Role != "user" {
		return store.AutomationRecord{}, ErrDenied
	}
	if expectedRevision == 0 || mutationID == "" {
		return store.AutomationRecord{}, ErrInvalid
	}
	canceller, ok := e.runtime.(interface {
		Cancel(context.Context, Principal, store.AutomationRecord) error
	})
	if !ok {
		return store.AutomationRecord{}, ErrInvalid
	}
	...
	if r.Occurrence.State == "cancelled" {
		return r, nil
	}
	if err := canceller.Cancel(ctx, p, r); err != nil {
		return r, err
	}
...
func canonicalContext(locked, summaries map[string]string, r store.AutomationRecord, summary string) (map[string]string, string, bool) {
	...
	if raw, exists := summaries[key]; exists {
		...
		if previous.ScheduledAt > current.ScheduledAt || (previous.ScheduledAt == current.ScheduledAt && (previous.OccurrenceID > current.OccurrenceID || (previous.OccurrenceID == current.OccurrenceID && previous.Revision >= current.Revision))) {
			return summaries, "", false
		}
	}
	...
	if err != nil || len(encoded) > 20000 || len(maintained) > 64 {
		return summaries, "", false
	}
