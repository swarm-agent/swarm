...
func TestAutomationReviewContext(t *testing.T) {
	for _, scenario := range []string{"valid", "stale", "foreign", "parent", "digest", "identity"} {
		t.Run(scenario, func(t *testing.T) {
			...
			case "foreign": plans.plan.AccountScopeID = "foreign"
			case "parent": parent = "other"
			case "digest": d.Plans[0].Plan.DocumentSHA256 = "forged"
			case "identity": p.AccountID = "foreign"
			}
			...
			if scenario == "valid" {
				if err != nil || got.Revision != 3 || got.Definition.SessionID != parent { t.Fatalf("review: %+v %v", got, err) }
			} else if err == nil { t.Fatal("forged review admitted") }
			if repo.writes != 0 { t.Fatal("review mutated definition") }
		})
	}
}
...
func TestAutomationEditProposal(t *testing.T) {
	for _, scenario := range []string{"replace", "omit", "digest", "foreign", "unapproved", "stale", "forged-context"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "omit" { next.Plans = nil } else { plans.plan.Version = 2; next.Plans[0].Plan.Revision = 2 }
			switch scenario {
			case "digest": next.Plans[0].Plan.DocumentSHA256 = "forged"
			case "foreign": next.Plans[0].Plan.SessionID = "foreign"
			case "unapproved": plans.plan.ApprovalState = "pending"
			case "stale": metadata["automation_review_revision"] = "2"
			case "forged-context": metadata["automation_review_id"] = "other"
			}
			...
			if scenario == "replace" || scenario == "omit" {
				...
				want := uint64(2); if scenario == "omit" { want = 1 }
				if got.Revision != 3 || got.Definition.Enabled || got.Definition.Authorization.Mode != "approval_required" || got.Definition.Plans[0].Plan.Revision != want { t.Fatalf("invalid proposal: %+v", got) }
			} else if err == nil { t.Fatal("invalid proposal accepted") }
			if repo.writes != 0 || !repo.rows["definitionauto"].Definition.Enabled || d.Plans[0].Plan.Revision != 1 { t.Fatal("proposal changed live definition") }
		})
	}
}
