...
func TestAutomationBindingGraph(t *testing.T) {
	for _, kind := range []string{"empty", "duplicate", "missing", "self", "cycle", "duplicate-edge", "bound"} {
		t.Run(kind, func(t *testing.T) {
			...
			second := d.Plans[0]
			second.ID = "second"
			second.DependsOn = []string{"primary"}
			d.Plans = append(d.Plans, second)
			switch kind {
			case "empty":
				d.Plans = nil
			case "duplicate":
				d.Plans[1].ID = "primary"
			case "missing":
				d.Plans[1].DependsOn = []string{"missing"}
			case "self":
				d.Plans[1].DependsOn = []string{"second"}
			case "cycle":
				d.Plans[0].DependsOn = []string{"second"}
			case "duplicate-edge":
				d.Plans[1].DependsOn = []string{"primary", "primary"}
			case "bound":
				d.Plans = make([]store.AutomationPlanBinding, 17)
			}
			if _, _, err := s.SaveDefinition(context.Background(), p, scope, "check", "create", 0, d); err == nil {
				t.Fatal("invalid graph accepted")
			}
			if r.writes != 0 || a.executions != 0 {
				t.Fatal("invalid graph reached write/grant")
			}
		})
	}
}
...
func TestAutomationSecondPlanAuthority(t *testing.T) {
	for _, failure := range []string{"account", "ownership", "pin", "missing"} {
		t.Run(failure, func(t *testing.T) {
			...
			second.Document = &store.SessionPlanDocument{}
			canonical := bindingPlans{"session": plans.plan, "second-session": second}
			s.plans = canonical
			...
			if created.SubjectID != p.SubjectID || created.Actor != "user" || created.WrittenAt != 100000 || len(created.Definition.Plans[1].Plan.DocumentSHA256) != 64 {
				t.Fatal("missing attribution/pin", created)
			}
			switch failure {
			case "ownership":
				a.deniedSession = second.SessionID
			case "account":
				second.AccountScopeID = "foreign"
				canonical[second.SessionID] = second
			case "pin":
				second.Document.Title = "replacement"
			case "missing":
				delete(canonical, second.SessionID)
			}
			if _, err := s.CheckRun(context.Background(), p, scope, "check", 1); err == nil {
				t.Fatal("second plan accepted on run")
			}
			if _, _, err := s.SaveDefinition(context.Background(), p, scope, "check", "update", 1, d); err == nil {
				t.Fatal("second plan accepted on save")
			}
			if r.writes != 1 || a.executions != 1 {
				t.Fatal("second plan rejection reached write/grant")
			}
		})
	}
}
...
func TestAutomationHistoryAuthority(t *testing.T) {
	...
	if _, _, err := s.History(context.Background(), p, scope, "check", "context", "check", 0, 51); err == nil {
		t.Fatal("unbounded history")
	}
	scope.AccountID = "foreign"
	if _, _, err := s.History(context.Background(), p, scope, "check", "context", "check", 0, 1); err == nil {
		t.Fatal("foreign history")
	}
	if r.reads != 0 {
		t.Fatal("rejected history reached store")
	}
}
...
func TestAutomationContextHistory(t *testing.T) {
	...
	if _, _, err := s.SaveDefinition(ctx, p, scope, "check", "create", 0, d); err != nil {
		t.Fatal(err)
	}
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "first", 0, map[string]string{"rule": "original"}, nil); err != nil {
		t.Fatal(err)
	}
	p.SubjectID = "second-human"
	if _, _, err := s.UpdateContext(ctx, p, scope, "check", "second", 1, map[string]string{"rule": "updated"}, nil); err != nil {
		t.Fatal(err)
	}
	rows, next, err := s.History(ctx, p, scope, "check", "context", "check", 0, 1)
	if err != nil || len(rows) != 1 || next != 2 || rows[0].SubjectID != "second-human" || rows[0].Revision != 2 {
		t.Fatal("first page", rows, next, err)
	}
	rows, next, err = s.History(ctx, p, scope, "check", "context", "check", next, 1)
	if err != nil || len(rows) != 1 || next != 0 || rows[0].SubjectID != "human" || rows[0].WrittenAt != 100000 || rows[0].Context.UserLocked["rule"] != "original" {
		t.Fatal("immutable history", rows, next, err)
	}
	scope.AccountID = "foreign"
	p.AccountID = "foreign"
	rows, next, err = s.History(ctx, p, scope, "check", "context", "check", 0, 1)
	if err != nil || len(rows) != 0 || next != 0 {
		t.Fatal("history scope leak", rows, err)
	}
}
