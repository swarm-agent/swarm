...
func TestAutomationV2ExpiredReceiptReplay(t *testing.T) {
	...
	if _,err := ids.PutUser(pebblestore.UserRecord{ID:"owner",Username:"owner"}); err != nil { t.Fatal(err) }
	if _,err := ids.PutAccountScope(pebblestore.AccountScopeRecord{ID:"account",Type:pebblestore.AccountScopeTypePersonal,CreatedByUserID:"owner"}); err != nil { t.Fatal(err) }
	if _,err := ids.PutAccountUser(pebblestore.AccountUserRecord{ID:"member",AccountScopeID:"account",UserID:"owner",Status:"active"}); err != nil { t.Fatal(err) }
	workspace,err := pebblestore.NewWorkspaceStore(db).AddForAccount("account",t.TempDir(),"Workspace"); if err != nil { t.Fatal(err) }
	available := true
	if err := ss.CreateSession(pebblestore.SessionSnapshot{ID:"conversation",AccountScopeID:"account",UserID:"owner",WorkspacePath:workspace.Path,WorkspaceGrants:[]pebblestore.WorkspaceGrant{{Kind:pebblestore.WorkspaceGrantPrimary,WorkspaceID:workspace.WorkspaceID,Path:workspace.Path,Available:&available}}}); err != nil { t.Fatal(err) }
	...
	digest,err := pebblestore.AutomationV2DocumentDigest(doc); if err != nil { t.Fatal(err) }
	...
	key := func(kind string) string { return fmt.Sprintf("automation/v2/%s/%x/%x",kind,"account","conversation") }
	if err := db.PutJSON(key("proposal"),p); err != nil { t.Fatal(err) }
	svc := NewService(ss,nil)
	if _,err := svc.AcceptAutomationV2("account","owner",workspace.WorkspaceID,"conversation",p.AutomationV2Review); err == nil { t.Fatal("fresh expired grant accepted") }
	...
	if err := db.PutJSON(key("accepted"),want); err != nil { t.Fatal(err) }
	got,err := svc.AcceptAutomationV2("account","owner",workspace.WorkspaceID,"conversation",p.AutomationV2Review)
	if err != nil || !reflect.DeepEqual(want,got) { t.Fatal("historical receipt changed",err) }
	if _,ok,err := ss.GetV3SessionActiveRunIntent("conversation"); err != nil || ok { t.Fatal("replay started run",err) }
}
