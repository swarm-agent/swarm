...
// canonical pending review. Revisions never edit an accepted automation.
func (s *Service) ProposeAutomationV2(account, user, workspace, sessionID string, document *pebblestore.SessionPlanDocument, expected pebblestore.AutomationV2Review) (pebblestore.AutomationV2Proposal, error) {
	if s == nil || s.store == nil { return pebblestore.AutomationV2Proposal{},errors.New("session store required") }
	if document == nil { return pebblestore.AutomationV2Proposal{},errors.New("complete document required") }
	b,err := json.Marshal(document)
	if err != nil || len(b) > 256*1024 { return pebblestore.AutomationV2Proposal{},errors.New("invalid or oversized document") }
	var doc pebblestore.SessionPlanDocument
	if err := json.Unmarshal(b,&doc); err != nil { return pebblestore.AutomationV2Proposal{},err }
	if doc.AutomationV2 == nil || doc.Automation != nil { return pebblestore.AutomationV2Proposal{},errors.New("exclusive automation_v2 required") }
	if doc.AutomationV2.Expiration == (pebblestore.AutomationV2Expiration{}) { doc.AutomationV2.Expiration.Kind = "indefinite" }
	if err := validateAutomationV2Proposal(&doc); err != nil { return pebblestore.AutomationV2Proposal{},err }
	if err := s.authenticatePlanDocumentArtifacts(account,sessionID,&doc); err != nil { return pebblestore.AutomationV2Proposal{},err }
	return s.store.ProposeAutomationV2(account,user,workspace,sessionID,doc,expected)
}
func validateAutomationV2Proposal(doc *pebblestore.SessionPlanDocument) error {
	if err := ValidateExecutablePlanDocument(doc); err != nil { return err }
	if err := pebblestore.ValidateAutomationV2Settings(doc.AutomationV2,time.Now().UnixMilli()); err != nil { return err }
	if doc.ExecutionState != nil || len(doc.OriginalCheckpoints) != 0 || (doc.Status != "" && doc.Status != "pending") { return errors.New("automation proposal cannot contain execution state") }
	for _, c := range doc.Checkpoints {
		if c.Status != "pending" || c.AttemptID != "" || c.RunID != "" || c.SessionID != "" || c.StartedAt != 0 || c.CompletedAt != 0 || len(c.Attempts) != 0 || c.Review != nil || c.Handoff != nil || c.Recommendation != nil || c.Report != "" || c.Result != "" || c.ActiveSubtaskID != "" { return errors.New("automation checkpoints must be unexecuted") }
		for _, t := range c.Subtasks { if (t.Status != "" && t.Status != "pending") || t.StartedAt != 0 || t.CompletedAt != 0 || t.Result != "" { return errors.New("automation subtasks must be unexecuted") } }
	}
	return nil
}
