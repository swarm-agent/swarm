...
func TestSwarmModeAuthorizationAlwaysRequiresFreshOneTimeApproval(t *testing.T) {
	service, _ := openSubagentReservationTestServices(t)
	service.SetBypassPermissions(true)

	authorize := func(callID string) AuthorizationResult {
		t.Helper()
		result, err := service.AuthorizeToolCall(AuthorizationInput{
			SessionID: "session-swarm-auth", AccountScopeID: "account-swarm-auth", RunID: "run-swarm-auth", CallID: callID,
			ToolName: "swarm_mode", ToolArguments: `{"swarm_mode":true,"hydration_rounds":2}`,
			Mode: "auto", SubagentReservation: &SubagentReservationResult{
				Decision: SubagentReservationAsk,
				Reason:   "this delegation surface requires fresh approval for every exact wave",
			},
		})
		if err != nil {
			t.Fatalf("authorize %s: %v", callID, err)
		}
		return result
	}

	first := authorize("call-swarm-auth-1")
	if first.Decision != AuthorizationPending || first.Source != "swarm_mode_approval" || first.Record == nil {
		t.Fatalf("first authorization = %#v, want pending Swarm approval", first)
	}
	if _, _, err := service.ResolveWithPolicyAndArguments("session-swarm-auth", first.Record.ID, ActionAllowAlways, "ok", ""); err == nil || !strings.Contains(err.Error(), "one-time only") {
		t.Fatalf("persistent Swarm approval error = %v", err)
	}
	if _, err := service.ResolveWithArguments("session-swarm-auth", first.Record.ID, ActionAllowOnce, "ok", `{"manifest_hash":"approved"}`); err != nil {
		t.Fatalf("resolve one-time Swarm approval: %v", err)
	}

	second := authorize("call-swarm-auth-2")
	if second.Decision != AuthorizationPending || second.Record == nil || second.Record.ID == first.Record.ID {
		t.Fatalf("second authorization = %#v, want a distinct fresh approval", second)
	}
}

func TestReservationAlwaysAskOverridesAutomaticBudgetButNotHardDenials(t *testing.T) {
	service, writer := openSubagentReservationTestServices(t)
	const accountScopeID = "account-swarm-always-ask"
	policy := DefaultSubagentPolicy()
	policy.AutomaticLaunchesPerParentRun = 100
	policy.ActiveChildLimit = 3
	if _, err := writer.UpdateSubagentPolicyForAccount(accountScopeID, policy); err != nil {
		t.Fatal(err)
	}
	result, err := service.ReserveSubagentWave(SubagentReservationRequest{
		SessionID: "session-swarm", AccountScopeID: accountScopeID, RunID: "run-swarm",
		CallID: "call-swarm", ManifestHash: "manifest-swarm", LaunchCount: 3, AlwaysAsk: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if result.Decision != SubagentReservationAsk {
		t.Fatalf("always-ask decision = %#v", result)
	}
	over, err := service.ReserveSubagentWave(SubagentReservationRequest{
		SessionID: "session-swarm", AccountScopeID: accountScopeID, RunID: "run-other",
		CallID: "call-over", ManifestHash: "manifest-over", LaunchCount: 4, AlwaysAsk: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if over.Decision != SubagentReservationDeny {
		t.Fatalf("hard limit was bypassed: %#v", over)
	}
}
