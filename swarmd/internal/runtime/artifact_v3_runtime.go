...
func injectArtifactV3AnimationAdapter(body []byte, durationMs int64, fps int) []byte {
	script := fmt.Sprintf(`<script data-swarm-artifact-v3-animation>(function(){"use strict";const duration=%d,fps=%d;const domReady=new Promise(resolve=>{if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",resolve,{once:true});else resolve()});const animations=()=>Array.from(document.getAnimations({subtree:true}));const paint=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));globalThis.__SWARM_ANIMATION_BIND__({version:"swarm.animation/v1",ready:async()=>{await domReady;for(const animation of animations())animation.pause();return {duration_ms:duration,fps}},seek:async timeMs=>{await domReady;for(const animation of animations()){animation.pause();animation.currentTime=timeMs}await paint();for(const animation of animations()){animation.currentTime=timeMs;animation.pause()}document.documentElement.dataset.swarmAnimationTimeMs=String(timeMs);return {time_ms:timeMs}}})})();</script>`, durationMs, fps)
	lower := strings.ToLower(string(body))
