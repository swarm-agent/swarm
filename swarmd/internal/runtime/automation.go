...
type automationSessionCatalog interface {
	GetSession(string) (store.SessionSnapshot, bool, error)
}
type automationMembership interface {
	GetAccountUser(string, string) (store.AccountUserRecord, bool, error)
}
...
func (a automationAccess) owner(p automation.Principal) (identity.Principal, error) {
	...
	if p.Role == "agent" {
		if a.sessions == nil {
			return identity.Principal{}, automation.ErrDenied
		}
		s, found, err := a.sessions.GetSession(p.SubjectID)
		if err != nil {
			return identity.Principal{}, err
		}
		if !found || s.AccountScopeID != p.AccountID {
			return identity.Principal{}, automation.ErrDenied
		}
		user = s.UserID
	} else if p.Role != "user" && p.Role != "system" {
		return identity.Principal{}, automation.ErrDenied
	}
	if a.members == nil || p.AccountID == "" || user == "" {
		return identity.Principal{}, automation.ErrDenied
	}
	...
	if !found || m.Status != store.AccountUserStatusActive || m.UserID != user || m.AccountScopeID != p.AccountID {
		return identity.Principal{}, automation.ErrDenied
	}
...
func (a automationAccess) Workspace(ctx context.Context, p automation.Principal, scope store.AutomationScope, action string) error {
	...
	if a.workspaces == nil || p.AccountID != scope.AccountID || scope.WorkspaceID == "" {
	...
	if !found || entry.AccountScopeID != scope.AccountID || entry.WorkspaceID != scope.WorkspaceID {
		return automation.ErrDenied
	}
	return nil
}
func (a automationAccess) PlanSession(ctx context.Context, p automation.Principal, scope store.AutomationScope, id string) error {
	...
	if !found || s.AccountScopeID != scope.AccountID || s.UserID != owner.UserID {
		return automation.ErrDenied
	}
	for _, grant := range s.WorkspaceGrants {
		if grant.Kind == store.WorkspaceGrantPrimary && grant.WorkspaceID == scope.WorkspaceID && (grant.Available == nil || *grant.Available) {
			return nil
		}
	}
	return automation.ErrDenied
...
func (a automationAccess) OccurrenceSession(ctx context.Context, p automation.Principal, scope store.AutomationScope, id string) error {
	if p.Role == "agent" && p.SubjectID != id {
		return automation.ErrDenied
	}
...
func automationSweepAt(ctx context.Context, db *store.Store, execution automationScheduleExecution, now time.Time, outcomes ...func(context.Context, automation.Principal, store.AutomationRecord) error) error {
	...
	if err := db.GetAutomationSchedulerPosition("catalog", &pos); err != nil {
		return err
	}
	old := pos
	...
	if pos.At == 0 {
		...
		if err := db.SaveAutomationSchedulerPosition("catalog", old, pos); err != nil {
			return err
		}
		old = pos
	}
	...
	save := func() error {
		...
		if pos.At != 0 {
			return errAutomationContinue
		}
		return nil
	}
	if pos.AccountID == "" {
		key, account, _, err := db.SchedulerCatalogNext("", pos.AccountKey)
		if errors.Is(err, store.ErrAutomationInvalid) {
			pos = automationSweepPosition{}
	...
	for _, row := range rows {
		...
		for _, reconcile := range outcomes {
			failures = append(failures, reconcile(trusted, p, row))
		}
	}
...
func automationRunDefinition(ctx context.Context, db automationPositionStore, execution automationScheduleExecution, p automation.Principal, row store.AutomationRecord) error {
	...
	var cursor string
	if err := db.GetAutomationSchedulerPosition(key, &cursor); err != nil {
		return err
	}
	// A disabled/revoked definition must not prevent completion of a durable stop.
	var tickErr error
	if row.Definition != nil && row.Definition.Enabled {
		tickErr = execution.Tick(ctx, p, row.Scope, row.AutomationID, row.Revision)
	}
	next, err := execution.RecoverPage(ctx, p, row.Scope, row.AutomationID, cursor)
	if errors.Is(err, automation.ErrRecoveryCursor) {
		next = ""
	}
	if next == cursor && next != "" && err == nil {
		err = errors.New("automation recovery cursor did not advance")
	}
...
func automationSweepFailure(err error) error {
	if err == errAutomationContinue {
		return nil
	}
	if joined, ok := err.(interface{ Unwrap() []error }); ok {
		var failures []error
		for _, child := range joined.Unwrap() {
			failures = append(failures, automationSweepFailure(child))
		}
...
func startAutomationLoop(parent context.Context, interval time.Duration, sweep func(context.Context) error, report func(error)) *automationLoop {
	...
	go func() {
		...
		for {
			...
			if failure := automationSweepFailure(err); failure != nil && ctx.Err() == nil {
				report(failure)
			}
			timer.Reset(delay)
			select {
...
func (d *Daemon) StartAutomationScheduling(ctx context.Context) error {
	...
	if d.automationClosed || d.automationLoop != nil || d.automationExecution == nil {
		return automation.ErrInvalid
	}
...
func (d *Daemon) automationOutcomes(ctx context.Context, p automation.Principal, definition store.AutomationRecord) error {
	...
	var cursor string
	if err := d.store.GetAutomationSchedulerPosition(key, &cursor); err != nil {
		return err
	}
	...
	if err != nil {
		if errors.Is(err, store.ErrAutomationInvalid) && cursor != "" {
			return errors.Join(err, d.store.SaveAutomationSchedulerPosition(key, cursor, ""))
		}
		return err
	}
	var failures []error
	for _, row := range rows {
		...
		case "completed", "failed", "cancelled", "skipped":
			...
			if !found {
				failures = append(failures, automation.ErrInvalid)
				continue
			}
			delivery, err := notification.NewAutomationDeliveryService(d.store, d.notificationService, node.SwarmID)
			if err == nil {
				err = delivery.Deliver(ctx, store.AutomationDeliveryReference{Scope: row.Scope, AutomationID: row.AutomationID, OccurrenceID: row.ID, Revision: row.Revision})
			}
			failures = append(failures, err)
		}
	}
