...
func (d *Daemon) StartAutomationV2Scheduling(ctx context.Context) error {
	...
	d.automationLoop = startAutomationLoop(ctx, time.Second, func(ctx context.Context) error { return d.automationV2Scheduler.Sweep(ctx, time.Now()) }, func(error) { log.Print("automation v2 sweep failed; durable pending work retained") })
	return nil
}
