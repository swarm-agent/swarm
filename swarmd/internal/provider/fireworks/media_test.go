...
func TestBuildFireworksChatCompletionRequestRejectsInvalidMedia(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unsupported modality", contract: validContract, content: []map[string]any{{"type": "session_media", "media": fireworksTestPayload("video", "video/mp4", "mp4", []byte("video"))}}},
		{name: "unsupported mime", contract: validContract, content: []map[string]any{{"type": "session_media", "media": fireworksTestPayload("image", "image/svg+xml", "svg", []byte("svg"))}}},
