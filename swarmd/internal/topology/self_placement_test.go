...
func TestEnsureLocalSelfPlacementForAccountCreatesHostRuntimeAndPlacement(t *testing.T) {
	...
	service := NewService(topologyStore, swarmStore, nil, nil, nil, nil, nil, nil)
...
func TestEnsureLocalSelfPlacementForAccountFailsWhenLocalNodeMissing(t *testing.T) {
	...
	service := NewService(pebblestore.NewTopologyStore(store), pebblestore.NewSwarmStore(store), nil, nil, nil, nil, nil, nil)
