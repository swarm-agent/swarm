...
func TestEnsureLocalSelfPlacementForAccountCreatesHostRuntimeAndPlacement(t *testing.T) {
	...
	if runtime.Relationship != "self" || runtime.Name != "Local" || runtime.UserID != "user-a" {
...
func TestEnsureLocalSelfPlacementForAccountFailsWhenLocalNodeMissing(t *testing.T) {
	...
	service := NewService(pebblestore.NewTopologyStore(store), pebblestore.NewSwarmStore(store), nil, nil, nil)
