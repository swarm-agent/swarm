...
func NewInitialExecutionEpoch(sessionID, userID, accountScopeID string, firstSeq uint64, now int64) ExecutionEpoch {
	if now == 0 { now = time.Now().UnixMilli() }
...
func setExecutionEpochInBatch(batch *pebble.Batch, epoch ExecutionEpoch, active bool) error {
	...
	if err != nil { return fmt.Errorf("marshal execution epoch: %w", err) }
	if err := batch.Set([]byte(KeyExecutionEpoch(epoch.SessionID, epoch.EpochID)), payload, nil); err != nil { return err }
	if err := batch.Set([]byte(KeyExecutionEpochOrdinal(epoch.SessionID, epoch.Ordinal)), []byte(epoch.EpochID), nil); err != nil { return err }
	if active { return batch.Set([]byte(KeyExecutionEpochActive(epoch.SessionID)), payload, nil) }
	return nil
}
...
func (s *SessionStore) BeginExecutionEpoch(input BeginExecutionEpochInput) (BeginExecutionEpochResult, error) {
	...
	if s == nil || s.store == nil { return BeginExecutionEpochResult{}, errors.New("session store is not configured") }
	if input.SessionID == "" || input.ClientRequestID == "" || input.PayloadHash == "" { return BeginExecutionEpochResult{}, errors.New("session id, client request id, and payload hash are required") }
	...
	if record, ok, err := s.getV3SessionIdempotencyRecordByKey(idemKey); err != nil { return BeginExecutionEpochResult{}, err
	} else if ok {
		if record.PayloadHash != input.PayloadHash { return BeginExecutionEpochResult{}, ErrV3IdempotencyConflict }
		epoch, exists, err := s.GetExecutionEpoch(input.SessionID, record.Result.RunID)
		if err != nil || !exists { return BeginExecutionEpochResult{}, fmt.Errorf("replayed execution epoch is unavailable: %w", err) }
		event, _, err := s.GetV3SessionEvent(input.SessionID, record.Result.LastSeq)
		if err != nil { return BeginExecutionEpochResult{}, err }
...
func (s *SessionStore) beginFreshExecutionEpoch(input BeginExecutionEpochInput, idemKey string) (BeginExecutionEpochResult, error) {
	...
	seq, err := s.readV3SessionSequence(input.SessionID); if err != nil { return BeginExecutionEpochResult{}, err }
	predecessor, hasPredecessor, err := s.GetActiveExecutionEpoch(input.SessionID); if err != nil { return BeginExecutionEpochResult{}, err }
	legacy := (*ExecutionEpochLegacyPrefix)(nil)
	if !hasPredecessor {
		legacy, err = s.readLegacyEpochPrefix(input.SessionID, seq); if err != nil { return BeginExecutionEpochResult{}, err }
	...
	boundarySeq := seq + 1
	predecessor.Status = ExecutionEpochStatusSealed; predecessor.LastRootSeq = seq; predecessor.SealedAt = now; predecessor.UpdatedAt = now
	epochID := strings.TrimSpace(input.EpochID); if epochID == "" { epochID = fmt.Sprintf("epoch-%020d", predecessor.Ordinal+1) }
	epoch := ExecutionEpoch{EpochID: epochID, SessionID: input.SessionID, UserID: strings.TrimSpace(input.UserID), AccountScopeID: input.AccountScopeID, ParentEpochID: predecessor.EpochID, Ordinal: predecessor.Ordinal+1, Status: ExecutionEpochStatusActive, FirstRootSeq: boundarySeq, Boundary: ExecutionEpochBoundary{Reason: strings.TrimSpace(input.Reason), PlanID: strings.TrimSpace(input.PlanID), CheckpointID: strings.TrimSpace(input.CheckpointID), SourceMessageID: strings.TrimSpace(input.SourceMessageID), PredecessorLastRootSeq: seq}, ProviderPolicy: input.ProviderPolicy, CreatedAt: now, UpdatedAt: now}
	payload, _ := json.Marshal(map[string]any{"epoch_id": epoch.EpochID, "parent_epoch_id": epoch.ParentEpochID, "ordinal": epoch.Ordinal, "reason": epoch.Boundary.Reason, "plan_id": epoch.Boundary.PlanID, "checkpoint_id": epoch.Boundary.CheckpointID})
	...
	reserved, err := s.store.sessionMutations.reserveOutbox(s.store, 1); if err != nil { return BeginExecutionEpochResult{}, err }
	committed := false; defer func(){ if !committed { s.store.sessionMutations.abandonOutbox(reserved) } }()
	...
	if session, ok, getErr := s.GetSession(input.SessionID); getErr != nil { return BeginExecutionEpochResult{}, getErr } else if ok { outbox.Membership = newV3RealtimeOutboxMembershipFromSession(session, now) }
	...
	batch := s.store.NewBatch(); defer batch.Close()
	if err := setExecutionEpochInBatch(batch, predecessor, false); err != nil { return BeginExecutionEpochResult{}, err }
	if err := setExecutionEpochInBatch(batch, epoch, true); err != nil { return BeginExecutionEpochResult{}, err }
	if epoch.Boundary.PlanID != "" || epoch.Boundary.CheckpointID != "" { if err := batch.Set([]byte(KeyExecutionEpochBoundary(input.SessionID, epoch.Boundary.PlanID, epoch.Boundary.CheckpointID)), []byte(epoch.EpochID), nil); err != nil { return BeginExecutionEpochResult{}, err } }
	for key, value := range map[string]any{KeyV3SessionEvent(input.SessionID, boundarySeq): event, KeyV3SessionProjection(input.SessionID): projection, KeyV3RealtimeOutbox(reserved[0]): outbox, idemKey: idem} { raw, marshalErr := json.Marshal(value); if marshalErr != nil { return BeginExecutionEpochResult{}, marshalErr }; if err := batch.Set([]byte(key), raw, nil); err != nil { return BeginExecutionEpochResult{}, err } }
	ref, err := marshalV3RealtimeOutboxReference(outbox); if err != nil { return BeginExecutionEpochResult{}, err }
	for _, key := range []string{KeyV3RealtimeOutboxBySessionEndpoint(input.SessionID, reserved[0]), KeyV3RealtimeOutboxBySessionSeq(input.SessionID, boundarySeq), KeyV3RealtimeOutboxByAuthScope(input.AccountScopeID, input.UserID, reserved[0])} { if err := batch.Set([]byte(key), ref, nil); err != nil { return BeginExecutionEpochResult{}, err } }
	if err := batch.Set([]byte(KeyV3SessionSequence(input.SessionID)), uint64ToBytes(boundarySeq), nil); err != nil { return BeginExecutionEpochResult{}, err }
	if input.RunID != "" { run, ok, getErr := s.GetV3SessionRunIntent(input.SessionID, input.RunID); if getErr != nil { return BeginExecutionEpochResult{}, getErr }; if ok { run.EpochID = epoch.EpochID; raw, _ := json.Marshal(run); if err := batch.Set([]byte(KeyV3SessionRunIntent(input.SessionID, input.RunID)), raw, nil); err != nil { return BeginExecutionEpochResult{}, err } } }
	if err := batch.Commit(pebble.Sync); err != nil { return BeginExecutionEpochResult{}, err }
	committed = true; if err := s.store.sessionMutations.commitOutbox(s.store, reserved); err != nil { return BeginExecutionEpochResult{}, err }
...
func (s *SessionStore) readLegacyEpochPrefix(sessionID string, seq uint64) (*ExecutionEpochLegacyPrefix, error) {
	...
	if projection, ok, err := s.GetV3SessionProjection(sessionID); err != nil { return nil, err } else if ok { prefix.ProjectionHighWatermark = projection.ProjectionHighWatermarkSeq }
	var active SessionPlanActive
	if ok, err := s.store.GetJSON(KeySessionPlanActive(sessionID), &active); err != nil { return nil, err } else if ok { prefix.ActivePlanID = active.PlanID }
	var lifecycle SessionLifecycleSnapshot
	if ok, err := s.store.GetJSON(KeySessionLifecycle(sessionID), &lifecycle); err != nil { return nil, err } else if ok { prefix.LifecycleGeneration = lifecycle.Generation }
	return prefix, nil
}
