...
func TestAutomationSchedulerPositionRestart(t *testing.T) {
	...
	if err := s.SaveAutomationSchedulerPosition(key, "", "opaque"); err != nil {
		t.Fatal(err)
	}
	...
	defer s.Close()
	var got string
	if err := s.GetAutomationSchedulerPosition(key, &got); err != nil || got != "opaque" {
		t.Fatal(got, err)
	}
	if err := s.SaveAutomationSchedulerPosition(key, "", "stale"); !errors.Is(err, ErrAutomationConflict) {
		t.Fatal(err)
	}
	if err := s.GetAutomationSchedulerPosition(key, &got); err != nil || got != "opaque" {
		t.Fatal(got, err)
	...
	if err := s.GetAutomationSchedulerPosition(foreign, &got); err != nil || got != "" {
		t.Fatal("scope leak", got, err)
	}
}
...
func TestAutomationSchedulerHistoryPagination(t *testing.T) {
	...
	for i := 0; i < 1050; i++ {
		...
		if _, _, err := s.ApplyAutomationMutation(AutomationMutation{Record: r, MutationID: id, Actor: "system", SubjectID: "writer", WrittenAt: 100000}); err != nil {
			t.Fatal(err)
		}
	}
	...
	for page := 0; page < 100; page++ {
		var cursor string
		if err := s.GetAutomationSchedulerPosition(key, &cursor); err != nil {
			t.Fatal(err)
		}
		...
		if err := s.SaveAutomationSchedulerPosition(key, cursor, next); err != nil {
			t.Fatal(err)
		}
		if next == "" {
			break
	...
	if len(seen) != 1050 {
		t.Fatalf("history starved: %d", len(seen))
	}
	if _, _, _, err := s.SchedulerCatalogNext("account", "foreign-prefix"); !errors.Is(err, ErrAutomationInvalid) {
		t.Fatal("foreign catalog cursor accepted", err)
	}
}
...
func TestAutomationSchedulerCatalogPagination(t *testing.T) {
	...
	for i := 0; i < 140; i++ {
		...
		if err := s.db.Set([]byte(AccountScopePrefix()+id), data, pebble.Sync); err != nil {
			t.Fatal(err)
		}
	}
	var cursor string
	for i := 0; i < 140; i++ {
		...
		if err != nil || account.ID != fmt.Sprintf("a-%03d", i) {
			t.Fatal(i, account, err)
		}
		cursor = key
