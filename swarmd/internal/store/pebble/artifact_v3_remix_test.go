...
func TestArtifactV3RemixThreeCandidatesRepeat(t *testing.T) {
	...
	if err := json.Unmarshal(project.Files[ArtifactV3ManifestFilename], &manifest); err != nil { t.Fatal(err) }
	...
	for i := range manifest.Parts {
		if manifest.Parts[i].Locator.Path == "index.html" { manifest.Parts[i].Locator.Path = manifest.Entrypoint }
		for j, name := range manifest.Parts[i].Locator.Paths { if name == "index.html" { manifest.Parts[i].Locator.Paths[j] = manifest.Entrypoint } }
	}
	...
	for round, intent := range []string{ArtifactV3RevisionFocusedParts, ArtifactV3RevisionWholeProject, ArtifactV3RevisionFocusedParts} {
		...
		for candidate := 0; candidate < 3; candidate++ {
			...
			if round == 1 {
				...
				if err := json.Unmarshal(next.Files[ArtifactV3ManifestFilename], &revised); err != nil { t.Fatal(err) }
				var parts []ArtifactV3Part
				for _, part := range revised.Parts {
					if part.ID == "retired" { continue }
					if part.ID == "pricing" { part.Locator = ArtifactV3Locator{Kind:"file",Path:"pages/main.html"} }
			...
			if round == 1 && candidate == 0 {
				...
				for name, body := range next.Files { bad.Files[name] = body }
				var policy ArtifactV3Manifest
				if err := json.Unmarshal(bad.Files[ArtifactV3ManifestFilename],&policy); err != nil { t.Fatal(err) }
				policy.Entrypoint = "assets/brand.txt"
				bad.Files[ArtifactV3ManifestFilename], err = json.Marshal(policy)
				if err != nil { t.Fatal(err) }
				if _, err := service.SubmitCandidate(ctx,ArtifactV3SubmitCandidateInput{Owner:owner,ArtifactID:"rounds",TurnID:turnID,CandidateID:"policy-override",TransactionID:"policy-override",ExpectedHead:head,Project:bad}); !errors.Is(err,ErrArtifactV3Invalid) { t.Fatalf("non-Part policy override: %v",err) }
				after, _, err := sessions.GetArtifactV3Repository(owner.AccountScopeID,owner.UserID,"rounds")
				if err != nil || !reflect.DeepEqual(before,after) { t.Fatalf("policy rejection mutated projection: %v",err) }
				ref, err := repository.CandidateRef(turnID,"policy-override")
				if err != nil { t.Fatal(err) }
				if _, err := repository.ref(ctx,ref); !errors.Is(err,ErrArtifactV3NotFound) { t.Fatalf("policy rejection created ref: %v",err) }
			}
		...
		if err != nil || string(oldManifest) != string(originalManifest) { t.Fatalf("old manifest mutated: %v",err) }
		asset, err := repository.ReadFile(ctx, selectedOID, "assets/brand.txt")
		if err != nil || string(asset) != string(project.Files["assets/brand.txt"]) { t.Fatalf("asset changed: %v",err) }
		selectedManifest, err := repository.ReadFile(ctx, selectedOID, ArtifactV3ManifestFilename)
		if err != nil { t.Fatal(err) }
		var selected ArtifactV3Manifest
		if err := json.Unmarshal(selectedManifest,&selected); err != nil { t.Fatal(err) }
		if round > 0 {
			seen := map[string]ArtifactV3Part{}
			for _, part := range selected.Parts { seen[part.ID] = part }
			if _, ok := seen["retired"]; ok { t.Fatal("retired Part survived") }
			if seen["brand"].Locator.Kind != "semantic" || seen["pricing"].Locator.Kind != "file" { t.Fatalf("structural reconciliation lost: %+v",selected) }
		}
		head = selectedOID
