...
// ArtifactV3VideoReference is Video Studio's complete exact reference to one
...
type VideoTimelineClip struct {
	...
	ArtifactV3Ref   *ArtifactV3VideoReference          `json:"artifact_v3_ref,omitempty"`
...
type VideoAnimationCandidate struct {
	...
	V3Source *ArtifactV3VideoReference          `json:"artifact_v3_source,omitempty"`
...
type VideoAnimationCandidateSet struct {
	...
	V3SelectedSource    *ArtifactV3VideoReference          `json:"artifact_v3_selected_source,omitempty"`
	V3Derivative        *ArtifactV3VideoReference          `json:"artifact_v3_derivative,omitempty"`
...
type VideoPlanPart struct {
	...
	ArtifactV3Source    *ArtifactV3VideoReference          `json:"artifact_v3_source,omitempty"`
	ArtifactV3Still     *ArtifactV3VideoReference          `json:"artifact_v3_still,omitempty"`
	...
	ArtifactV3Visual    *ArtifactV3VideoReference          `json:"artifact_v3_visual,omitempty"`
...
const (
	...
	VideoEditProposalIntentArtifactV3Convert = "artifact_v3_conversion"
...
func validateV3VideoProjectMutationInput(input V3SessionMutationInput) error {
	...
	case V3SessionMutationCreateVideoProject:
		...
		if input.VideoProject.EditProposal != nil {
			...
			if err := validateVideoPlan(*ep.Plan); err != nil {
	...
	case V3SessionMutationCreateVideoEditProposal, V3SessionMutationSelectVideoAnimationCandidate, V3SessionMutationPromoteVideoAnimationDerivative, V3SessionMutationUpdateVideoComposition, V3SessionMutationAcceptVideoEditProposal, V3SessionMutationRejectVideoEditProposal:
		...
		if input.Kind == V3SessionMutationUpdateVideoComposition {
			...
			if proposal.Intent != VideoEditProposalIntentGeneral && proposal.Intent != VideoEditProposalIntentHTMLIteration && proposal.Intent != VideoEditProposalIntentStoryboardImport && proposal.Intent != VideoEditProposalIntentArtifactV2Convert && proposal.Intent != VideoEditProposalIntentArtifactV3Convert {
...
func validateVideoTimeline(timeline VideoProjectTimeline) error {
	...
	for i, clip := range timeline.Clips {
		...
		case VideoClipSourceKindManagedArtifact:
			authorities := 0
			for _, present := range []bool{clip.ArtifactRef != nil, clip.DesignInput != nil, clip.ArtifactV2Ref != nil, clip.ArtifactV3Ref != nil} {
				if present {
					authorities++
				}
			}
			if authorities != 1 {
				return fmt.Errorf("managed_artifact clip %q requires exactly one artifact_ref, artifact_v2_ref, artifact_v3_ref, or design_input authority", clip.ID)
			}
...
func sameArtifactV3Source(a, b ArtifactV3VideoReference) bool {
	return a.SessionID == b.SessionID && a.ArtifactID == b.ArtifactID && a.RevisionID == b.RevisionID &&
		a.CommitOID == b.CommitOID && a.TreeOID == b.TreeOID && a.ManifestDigestSHA256 == b.ManifestDigestSHA256 &&
		a.BuildID == b.BuildID && a.ValidationID == b.ValidationID && a.EventSeq == b.EventSeq &&
		a.PartID == b.PartID && a.CaptureStateID == b.CaptureStateID
}

func validateArtifactV3VideoReference(ref ArtifactV3VideoReference, wantMediaType string, derivative bool) error {
	for field, value := range map[string]string{
		"session_id": ref.SessionID, "artifact_id": ref.ArtifactID, "revision_id": ref.RevisionID,
		"commit_oid": ref.CommitOID, "tree_oid": ref.TreeOID, "manifest_digest_sha256": ref.ManifestDigestSHA256,
		"build_id": ref.BuildID, "validation_id": ref.ValidationID, "digest_sha256": ref.DigestSHA256,
		"media_type": ref.MediaType, "animation_profile": ref.AnimationProfile,
	} {
	...
	if ref.EventSeq == 0 || ref.DurationMs <= 0 || ref.DurationMs > MaxVideoTimelineDurationMs || ref.FPS <= 0 || ref.FPS > 60 {
		return errors.New("event_seq, duration_ms, and fps must be positive and bounded")
	}
	if ref.MediaType != wantMediaType {
		return fmt.Errorf("media_type must be %q", wantMediaType)
	}
	if derivative != (ref.DerivativeID != "") {
		return errors.New("derivative identity does not match reference kind")
	}
	return nil
}
...
func validateVideoPlanProposal(plan VideoPlanProposal) error {
	...
	for index, part := range plan.Parts {
		...
		authorities := 0
		for _, present := range []bool{part.Visual != nil, part.ArtifactV2Visual != nil, part.ArtifactV3Visual != nil} {
			if present {
				authorities++
			}
		}
		if authorities != 1 {
			return fmt.Errorf("video plan part %q requires exactly one complete render-ready visual authority", part.ID)
		}
		...
		if candidates := part.AnimationCandidates; candidates != nil {
			...
			case VideoAnimationCandidateStatusAwaitingSelection:
				if candidates.SelectedCandidateID != "" || candidates.SelectedSource != nil || candidates.Derivative != nil || candidates.V2SelectedSource != nil || candidates.V2Derivative != nil || candidates.V3SelectedSource != nil || candidates.V3Derivative != nil {
			...
			case VideoAnimationCandidateStatusAwaitingExport:
				_, selected := seenCandidates[candidates.SelectedCandidateID]
				selectedSources := boolCount(candidates.SelectedSource != nil, candidates.V2SelectedSource != nil, candidates.V3SelectedSource != nil)
				if !selected || selectedSources != 1 || candidates.Derivative != nil || candidates.V2Derivative != nil || candidates.V3Derivative != nil {
			...
			case VideoAnimationCandidateStatusReady:
				if _, ok := seenCandidates[candidates.SelectedCandidateID]; !ok {
					return fmt.Errorf("video plan part %q ready animation requires a selected candidate", part.ID)
				}
				legacyReady := candidates.SelectedSource != nil && candidates.Derivative != nil
				v2Ready := candidates.V2SelectedSource != nil && candidates.V2Derivative != nil
				v3Ready := candidates.V3SelectedSource != nil && candidates.V3Derivative != nil
				if boolCount(legacyReady, v2Ready, v3Ready) != 1 {
					return fmt.Errorf("video plan part %q ready animation requires exactly one native selected source and MP4 derivative authority", part.ID)
				}
...
func ResolveVideoPlanRenderAuthority(revision VideoProjectRevisionSnapshot, sourceProposal *VideoEditProposalSnapshot) (*VideoPlanProposal, error) {
	...
	if err != nil || plan == nil || sourceProposal == nil {
		if plan == nil && sourceProposal != nil && sourceProposal.Plan != nil {
			cloned := *sourceProposal.Plan
			return &cloned, nil
		}
		return plan, err
	}
...
func ResolveAuthoritativeVideoPlan(
	...
	if reader == nil {
		return nil, errors.New("video plan render authority reader is required")
	}
	if revision.AccountScopeID != "" && accountScopeID != "" && revision.AccountScopeID != accountScopeID {
		return nil, errors.New("video revision ownership does not match authenticated principal")
	}
	if revision.UserID != "" && userID != "" && revision.UserID != userID {
		return nil, errors.New("video revision ownership does not match authenticated principal")
	}

	proposalID := VideoPlanRenderAuthorityProposalID(revision.Timeline)
	if proposalID == "" {
		return ResolveVideoPlanRenderAuthority(revision, nil)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("inspect pending video proposals before render: %w", err)
	}
	if pending := PendingVideoEditProposalForRevision(revision, proposals); pending != nil {
		return nil, fmt.Errorf("final render blocked: revision %q is the pending working cut for proposal %q; confirm or reject the pending changes before rendering", revision.ID, pending.ID)
	}
	...
	if found {
		if (proposal.AccountScopeID != "" && accountScopeID != "" && proposal.AccountScopeID != accountScopeID) ||
			(proposal.UserID != "" && userID != "" && proposal.UserID != userID) {
			return nil, errors.New("video plan render authority ownership does not match authenticated principal")
		}
		if proposal.Status == VideoEditProposalStatusRejected {
			return nil, fmt.Errorf("video plan render authority proposal %q is rejected", proposalID)
		}
		if proposal.Status != VideoEditProposalStatusAccepted {
			return nil, fmt.Errorf("video plan render authority proposal %q is not accepted (status %s)", proposalID, proposal.Status)
		}
		if proposal.Plan == nil {
			return nil, fmt.Errorf("video plan render authority proposal %q has no plan", proposalID)
		}
		return ResolveVideoPlanRenderAuthority(revision, &proposal)
	}

	// 3. Authority proposal not found in the current project: check authenticated bounded exact-source lineage.
	project, ok, err := reader.GetVideoProject(accountScopeID, revision.SessionID, revision.ProjectID)
	if err != nil || !ok {
		return nil, fmt.Errorf("video plan render authority proposal %q not found", proposalID)
	}
	if (project.AccountScopeID != "" && accountScopeID != "" && project.AccountScopeID != accountScopeID) ||
		(project.UserID != "" && userID != "" && project.UserID != userID) {
		return nil, errors.New("video project ownership does not match authenticated principal")
	}

	sourceSessionID := strings.TrimSpace(firstNonEmptyMetadataString(project.Metadata, "source_session_id", ""))
	sourceProjectID := strings.TrimSpace(firstNonEmptyMetadataString(project.Metadata, "source_project_id", ""))
	sourceRevisionID := strings.TrimSpace(firstNonEmptyMetadataString(project.Metadata, "source_revision_id", ""))
	...
	if err != nil || !ok {
		return nil, fmt.Errorf("source video project %q not found for render authority lineage: %w", sourceProjectID, err)
	}
	if (sourceProject.AccountScopeID != "" && accountScopeID != "" && sourceProject.AccountScopeID != accountScopeID) ||
		(sourceProject.UserID != "" && userID != "" && sourceProject.UserID != userID) {
		return nil, errors.New("source video project ownership does not match authenticated principal")
	}

	sourceRevision, ok, err := reader.GetVideoProjectRevision(accountScopeID, sourceSessionID, sourceProjectID, sourceRevisionID)
	if err != nil || !ok {
		return nil, fmt.Errorf("source video revision %q not found for render authority lineage", sourceRevisionID)
	}
	if (sourceRevision.AccountScopeID != "" && accountScopeID != "" && sourceRevision.AccountScopeID != accountScopeID) ||
		(sourceRevision.UserID != "" && userID != "" && sourceRevision.UserID != userID) {
		return nil, errors.New("source video revision ownership does not match authenticated principal")
	}
	...
	if !sameExactVideoTimeline(revision.Timeline, sourceRevision.Timeline) {
		return nil, fmt.Errorf("render authority lineage rejected: revision %q timeline does not match exact source revision %q cut", revision.ID, sourceRevision.ID)
	}
	...
	if err == nil {
		if pending := PendingVideoEditProposalForRevision(sourceRevision, sourceProposals); pending != nil {
			return nil, fmt.Errorf("render authority lineage blocked: source revision %q is pending working cut for proposal %q", sourceRevision.ID, pending.ID)
		}
	}

	sourceProposal, ok, err := reader.GetVideoEditProposal(accountScopeID, sourceSessionID, sourceProjectID, proposalID)
	if err != nil || !ok {
		return nil, fmt.Errorf("video plan render authority proposal %q not found in source lineage", proposalID)
	}
	if (sourceProposal.AccountScopeID != "" && accountScopeID != "" && sourceProposal.AccountScopeID != accountScopeID) ||
		(sourceProposal.UserID != "" && userID != "" && sourceProposal.UserID != userID) {
		return nil, errors.New("source video edit proposal ownership does not match authenticated principal")
	}
	if sourceProposal.Status == VideoEditProposalStatusRejected {
		return nil, fmt.Errorf("source video edit proposal %q is rejected", proposalID)
	}
	if sourceProposal.Status != VideoEditProposalStatusAccepted {
		return nil, fmt.Errorf("source video edit proposal %q is not accepted (status %s)", proposalID, sourceProposal.Status)
	}
	if sourceProposal.Plan == nil {
		return nil, fmt.Errorf("source video edit proposal %q has no plan", proposalID)
	}

	boundProposal := sourceProposal
	boundProposal.ProjectID = revision.ProjectID
	boundProposal.SessionID = revision.SessionID
	boundProposal.WorkingRevisionID = revision.ID

	return ResolveVideoPlanRenderAuthority(revision, &boundProposal)
}
...
func unresolvedSelectedVideoAnimationPart(plan *VideoPlanProposal, selected []string) string {
	...
	for _, part := range plan.Parts {
		...
		if candidates.Status == VideoAnimationCandidateStatusReady ||
			(candidates.Status == VideoAnimationCandidateStatusAwaitingExport && candidates.SelectedCandidateID != "" && boolCount(candidates.SelectedSource != nil, candidates.V2SelectedSource != nil, candidates.V3SelectedSource != nil) == 1 && candidates.Derivative == nil && candidates.V2Derivative == nil && candidates.V3Derivative == nil) {
			continue
		}
		return part.ID
	}
...
func (s *SessionStore) CreateVideoProject(input CreateVideoProjectInput) (VideoProjectSnapshot, *VideoProjectRevisionSnapshot, error) {
	...
	if initialProposal != nil {
		mutPayloadMap["initial_proposal_id"] = initialProposal.ID
	}
	mutPayload, _ := json.Marshal(mutPayloadMap)
