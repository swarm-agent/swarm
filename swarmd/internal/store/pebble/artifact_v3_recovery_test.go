...
func TestArtifactV3RecoveryProducerLineage(t *testing.T) {
	...
	for _, metadata := range []map[string]any{
		...
		if !reflect.DeepEqual(before, after) {
			t.Fatal("rejected lineage mutated session")
		}
	}
	if err := s.ValidateArtifactV3DraftProducer("foreign", "user-1", "parent", "child"); err == nil {
...
func TestArtifactV3RecoveryChildResumeAdmission(t *testing.T) {
	...
	if err := s.validateArtifactV3DraftResume(input, repository, next, 1000); err == nil {
		t.Fatal("active producer admitted")
	}
	...
	for _, mutate := range []func(*ArtifactV3DraftProjection){
		...
		func(d *ArtifactV3DraftProjection) {
			d.State = []byte(`{"ProducerSessionID":"parent","ProducerRunID":"new-run","Publishing":false,"Finished":null,"Gate":{},"Project":{"asset":"exact"}}`)
		},
	} {
		bad := next
		mutate(&bad)
		if err := s.validateArtifactV3DraftResume(input, repository, bad, 1000); err == nil {
			t.Fatal("changed bytes/readiness admitted")
		}
	}
	input.ArtifactV3.Resume.ProjectionSeq--
	if err := s.validateArtifactV3DraftResume(input, repository, next, 1000); err == nil {
		t.Fatal("stale projection admitted")
	}
	if !reflect.DeepEqual(repository.Drafts["old"], old) || len(repository.Drafts) != 1 {
		t.Fatal("admission mutated retained draft")
	}
}
