...
func TestAutomationProgressTriggerImmutable(t *testing.T) {
	...
	if _, _, err := s.ApplyAutomationMutation(m); !errors.Is(err, ErrAutomationConflict) { t.Fatalf("kind mutation: %v", err) }
	for _, revision := range []uint64{0, 1} {
		got, found, err := s.GetAutomationRecord(def.Record.Scope, "check", "occurrence", "run", revision)
		if err != nil || !found || got.Revision != 1 || got.Occurrence.TriggerKind != "manual" || got.Occurrence.State != "pending" { t.Fatalf("partial mutation: %+v %v", got, err) }
	}
}
