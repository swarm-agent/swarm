...
func TestAutomationApprovalCASRestart(t *testing.T) {
	...
	if _, err := s.CreateAutomationApproval(g); !errors.Is(err, ErrAutomationConflict) {
	...
	for _, scope := range []AutomationScope{{AccountID: "other", WorkspaceID: g.Scope.WorkspaceID}, {AccountID: g.Scope.AccountID, WorkspaceID: "other"}} {
		if _, found, err := s.GetAutomationApproval(scope, g.ID); err != nil || found {
			t.Fatal("cross-scope read")
		}
		if _, err := s.RevokeAutomationApproval(scope, g.ID, g.SubjectID, 1, 101); err == nil {
			t.Fatal("cross-scope revoke")
		}
	}
	if _, err := s.RevokeAutomationApproval(g.Scope, g.ID, "other", 1, 101); err == nil {
		t.Fatal("wrong owner revoke")
	}
	if _, err := s.RevokeAutomationApproval(g.Scope, g.ID, g.SubjectID, 2, 101); err == nil {
		t.Fatal("stale revoke")
	}
	unchanged, _, _ := s.GetAutomationApproval(g.Scope, g.ID)
	if unchanged != created {
		t.Fatal("rejected write changed grant")
	}
	if _, err := s.RevokeAutomationApproval(g.Scope, g.ID, g.SubjectID, 1, 101); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || !found || got.Revision != 2 || got.RevokedAt != 101 {
	...
	if _, err := s.CreateAutomationApproval(g); err == nil {
		t.Fatal("stale definition approved")
	}
	if _, found, err := s.GetAutomationApproval(g.Scope, g.ID); err != nil || found {
