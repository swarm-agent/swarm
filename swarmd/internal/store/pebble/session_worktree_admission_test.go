...
func TestWorktreeAdmissionSuccessor(t *testing.T) {
	...
	if _, err := s.ApplyV3SessionMutation(input); !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("event-name admission: %v", err) }
	...
	if err != nil || got.WorkspacePath != source || got.WorktreeRootPath != path { t.Fatalf("source/runtime: %+v %v", got, err) }
	input.WorktreeAdmission = nil
	input.IdempotencyKey, input.PayloadHash = "same-owner-update", "same-owner-update"
	if _, err := s.ApplyV3SessionMutation(input); err != nil { t.Fatalf("same-owner update: %v", err) }
}
...
func TestWorktreeAdmissionNotJSONAuthority(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"WorktreeAdmission":{"Kind":"allocated"},"worktree_admission":{"Kind":"allocated"}}`), &input); err != nil { t.Fatal(err) }
	if input.WorktreeAdmission != nil { t.Fatal("decoded trusted evidence") }
}
	...
	for _, foreign := range []bool{false, true} {
		t.Run(map[bool]string{false:"same-owner", true:"foreign-history"}[foreign], func(t *testing.T) {
			...
			if foreign {
				...
				defer batch.Close()
				if err := s.retainRepositoryHistoryInBatch(batch, other, false, false); err != nil { t.Fatal(err) }
			...
			if foreign {
				if !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("foreign migration: %v", err) }
				if _, err := s.InspectWorktreeOwnership("account", "user", []string{path}); err == nil { t.Fatal("partial claim") }
...
func TestWorktreeAdmissionRestartReservation(t *testing.T) {
	...
	if _, err := s.ApplyV3SessionMutation(recoveryInput("receiver", "reserve", path, 1, 1)); err != nil { t.Fatal(err) }
	...
	if _, err := s.ApplyV3SessionMutation(input); !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("stale publish: %v", err) }
	records, err := s.InspectWorktreeOwnership("account", "user", []string{path})
	if err != nil || len(records) != 1 || records[0].Revision != 2 || records[0].ClaimantSessionID != "receiver" { t.Fatalf("reservation lost: %+v %v", records, err) }
	current, _, err := s.GetSession("receiver")
	if err != nil || current.WorktreeRootPath != "" { t.Fatalf("partial switch: %+v %v", current, err) }
}
