...
func (s *SessionStore) setAutomationPermissionInBatch(batch *pebble.Batch, in V3SessionMutationInput) error {
	...
	if !found || !reflect.DeepEqual(current, p.Previous) { return ErrAutomationConflict }
	raw, err := json.Marshal(sanitizePermissionRecord(p.Record))
	if err != nil { return err }
	if err := putPermissionRecordInBatch(batch, p.Record, &p.Previous, raw); err != nil { return err }
	...
	if err != nil { return err }
	if err := putPermissionSummaryInBatch(batch, p.Summary, raw, previous, found); err != nil { return err }
	if p.Record.RunID == "" { return nil }
	...
	for _, id := range wait.PendingPermissionIDs { if id != p.Record.ID { next = append(next, id) } }
	key := []byte(KeyRunWait(in.SessionID, p.Record.RunID))
	if len(next) == 0 { return batch.Delete(key, nil) }
