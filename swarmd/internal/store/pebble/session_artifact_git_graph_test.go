...
func TestArtifactGitGraphTenCandidatesFinalizeThenAcceptOnce(t *testing.T) {
	...
	for index := 1; index <= 10; index++ {
		...
		if created.Artifact == nil || created.Artifact.Chain == nil || created.Artifact.Step == nil { t.Fatalf("create projection %s = %#v", id, created.Artifact) }
		...
		if err != nil { t.Fatalf("finalize %s: %v", id, err) }
	...
	if err != nil || !ok { t.Fatalf("chain: ok=%t err=%v", ok, err) }
	if chain.GraphState != SessionArtifactGraphAuthoritative || chain.Head.VariantID != "" { t.Fatalf("finalization advanced head: %+v", chain) }
	...
	if persistedErr != nil || !persistedOK || len(persistedStep.Candidates) != 10 { t.Fatalf("restarted step = %+v ok=%t err=%v", persistedStep, persistedOK, persistedErr) }
	step, ok, err := sessions.GetSessionArtifactStep("account-1", "user-1", chain.ID, "step-one")
	if err != nil || !ok || len(step.Candidates) != 10 || step.Accepted != nil { t.Fatalf("step = %+v ok=%t err=%v", step, ok, err) }
	for index, candidate := range step.Candidates {
		if candidate.VariantID != fmt.Sprintf("candidate-%02d", index+1) || candidate.EventSeq == 0 { t.Fatalf("candidate %d = %+v", index, candidate) }
	}
	chosen := ready[4]
	...
	for index, stepID := range []string{"parallel-a", "parallel-b"} {
		...
		if _, err := apply("create-"+id, V3SessionMutationCreateArtifact, V3ArtifactMutation{Collection: SessionArtifactCollection{ID: id, Name: id}, Variant: &SessionArtifactVariant{ID: id, Filename: id + ".html", MediaType: "text/html", RevisionRoundID: stepID, Lineage: lineage}}); err != nil { t.Fatal(err) }
...
func TestArtifactGitGraphTypedPartAndLegacyCompatibility(t *testing.T) {
	...
	if err != nil || len(validated) != 1 || validated[0].Part == nil || *validated[0].Part != part { t.Fatalf("typed part = %#v err=%v", validated, err) }
	...
	if err != nil { t.Fatal(err) }
	if projected.GraphState != SessionArtifactGraphLegacyUnproven || chain.GraphState != SessionArtifactGraphLegacyUnproven || chain.Head.VariantID != "" || chain.Root.VariantID != "" { t.Fatalf("legacy projection fabricated graph facts: variant=%+v chain=%+v", projected, chain) }
}
