...
func TestAutomationExecutionFenceSurvivesFailedCancelAndRestart(t *testing.T) {
	...
	if err := NewSessionStore(db).WithAutomationExecutionFence("account", key, true, func() error { return failure }); !errors.Is(err, failure) {
		t.Fatal(err)
	}
	...
	if !errors.Is(err, ErrAutomationExecutionCancelled) || called {
		t.Fatalf("cancel lost: err=%v called=%v", err, called)
	}
	if err := NewSessionStore(db).WithAutomationExecutionFence("other", key, false, func() error { called = true; return nil }); err != nil || !called {
...
func TestAutomationExecutionFenceCancelRace(t *testing.T) {
	...
	go func() {
		done <- NewSessionStore(db).WithAutomationExecutionFence("account", key, true, func() error { close(entered); <-release; return nil })
	}()
	select {
	case <-entered:
	...
	go func() {
		start <- NewSessionStore(db).WithAutomationExecutionFence("account", key, false, func() error { return errors.New("unauthorized callback") })
	}()
	close(release)
	select {
	case err := <-done:
	...
	select {
	case err := <-start:
		if !errors.Is(err, ErrAutomationExecutionCancelled) {
			t.Fatalf("race admitted: %v", err)
		}
	case <-time.After(time.Second):
