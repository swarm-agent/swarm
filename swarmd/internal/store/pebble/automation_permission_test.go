...
func TestAutomationPermissionBatchExactCancellation(t *testing.T) {
	...
	for _, record := range []PermissionRecord{previous, unrelated} {
		if err := permissions.PutPermission(record, nil); err != nil { t.Fatal(err) }
	}
	previous, _, err = permissions.GetPermission("session", "review")
	if err != nil { t.Fatal(err) }
	if err := permissions.UpsertRunWait(RunWaitState{SessionID: "session", RunID: "run", PendingPermissionIDs: []string{"review", "other"}}); err != nil { t.Fatal(err) }
	updated := previous
	...
	batch := db.NewBatch()
	if err := sessions.setAutomationPermissionInBatch(batch, in); err == nil { t.Fatal("stale permission accepted") }
	batch.Close()
	pending, err := permissions.ListPendingPermissions("session", 10)
	if err != nil || len(pending) != 2 { t.Fatalf("rejection mutated pending: %v %v", pending, err) }
	in.AutomationPermission.Previous = previous
	batch = db.NewBatch()
	defer batch.Close()
	if err := sessions.setAutomationPermissionInBatch(batch, in); err != nil { t.Fatal(err) }
	if err := batch.Commit(pebble.Sync); err != nil { t.Fatal(err) }
	pending, err = permissions.ListPendingPermissions("session", 10)
	if err != nil || len(pending) != 1 || pending[0].ID != "other" { t.Fatalf("wrong permission consumed: %v %v", pending, err) }
	got, _, err := permissions.GetPermission("session", "review")
	if err != nil || got.Status != PermissionStatusCancelled || got.ExecutionStatus != PermissionExecCancelled { t.Fatalf("review could execute: %+v %v", got, err) }
	wait, found, err := permissions.GetRunWait("session", "run")
	if err != nil || !found || len(wait.PendingPermissionIDs) != 1 || wait.PendingPermissionIDs[0] != "other" { t.Fatalf("wrong wait detached: %+v %v", wait, err) }
}
