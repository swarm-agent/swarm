...
func TestWorktreeRecoveryCopyPublication(t *testing.T) {
	...
	if _, err := s.ApplyV3SessionMutation(reserve); err != nil { t.Fatal(err) }
	owner, _, _ := s.GetSession("owner")
	if _, err := s.ApplyV3SessionMutation(V3SessionMutationInput{SessionID: "owner", UserID: "user", AccountScopeID: "account", Kind: V3SessionMutationUpdateSettings, IdempotencyKey: "write", PayloadHash: "write", Session: &owner}); !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("reserved writer: %v", err) }
	...
	if _, err := s.ApplyV3SessionMutation(input); !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("changed source accepted: %v", err) }
	...
	if _, err := s.ApplyV3SessionMutation(recoveryInput("receiver", "release", path, 2, 2)); err != nil { t.Fatal(err) }
	released, err := s.InspectWorktreeOwnership("account", "user", []string{path})
	if err != nil || released[0].OwnerSessionID != "owner" || released[0].ClaimantSessionID != "" { t.Fatalf("release: %+v %v", released, err) }
	current, _, _ = s.GetSession("receiver")
	current.WorkspacePath = filepath.Join(root, "source")
	if _, err := s.ApplyV3SessionMutation(V3SessionMutationInput{SessionID: "receiver", UserID: "user", AccountScopeID: "account", Kind: V3SessionMutationUpdateSettings, IdempotencyKey: "source", PayloadHash: "source", Session: &current}); err != nil { t.Fatal(err) }
	...
	if _, err := s.ApplyV3SessionMutation(input); !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("wrong evidence: %v", err) }
	if _, err := s.InspectWorktreeOwnership("account", "user", []string{destination}); err == nil { t.Fatal("partial destination") }
	input.WorktreeRecovery.Evidence = strings.Repeat("a", 64)
	if _, err := s.ApplyV3SessionMutation(input); err != nil { t.Fatal(err) }
	records, err := s.InspectWorktreeOwnership("account", "user", []string{path, destination})
	if err != nil { t.Fatal(err) }
	if records[0].OwnerSessionID != "owner" || records[0].ClaimantSessionID != "" || records[0].Revision != 5 || records[1].OwnerSessionID != "receiver" || records[1].CopySource == nil || records[1].CopySource.Path != path || records[1].CopySource.Revision != 4 { t.Fatalf("copy claims: %+v", records) }
	if result, err := s.ApplyV3SessionMutation(input); err != nil || !result.Replayed { t.Fatalf("copy replay: %+v %v", result, err) }
}
...
func TestWorktreeRecoveryCopyRestartWriterGuard(t *testing.T) {
	...
	createRecoverySession(t, s, "owner", path)
	createRecoverySession(t, s, "receiver", "")
	if _, err := s.ApplyV3SessionMutation(recoveryInput("receiver", "reserve_copy", path, 1, 1)); err != nil { t.Fatal(err) }
	...
	for _, status := range []string{V3RunIntentPendingExecutor, V3RunIntentRunning, V3RunIntentCompleted} {
		...
		if status == V3RunIntentCompleted {
			if err != nil { t.Fatal(err) }
		} else if !errors.Is(err, ErrWorktreeRecoveryConflict) { t.Fatalf("writer admitted: %s %v", status, err) }
	}
	records, err := s.InspectWorktreeOwnership("account", "user", []string{path})
	if err != nil || records[0].Revision != 2 || records[0].OperationState != "reserved_copy" || records[0].OwnerSessionID != "owner" { t.Fatalf("reservation changed: %+v %v", records, err) }
}
