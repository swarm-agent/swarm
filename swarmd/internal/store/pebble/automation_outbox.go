...
func (s *Store) ClaimAutomationDelivery(ref AutomationDeliveryReference, now int64) (AutomationRecord, AutomationDelivery, bool, error) {
	...
	if ref.Revision == 0 || now <= 0 {
		return AutomationRecord{}, d, false, ErrAutomationInvalid
	}
	r, found, err := s.GetAutomationRecord(ref.Scope, ref.AutomationID, "occurrence", ref.OccurrenceID, ref.Revision)
	if err != nil {
		return r, d, false, err
	}
	if !found || r.Occurrence == nil {
		return r, d, false, ErrAutomationInvalid
	}
	switch r.Occurrence.State {
	case "completed", "failed", "blocked", "cancelled", "skipped":
	default:
		return r, d, false, ErrAutomationInvalid
	}
	...
	if found && d.Reference != ref {
		return r, d, false, ErrAutomationConflict
	}
	if d.Acked || d.Attempts >= 5 || now < d.NextAt {
	...
	batch := s.NewBatch()
	defer batch.Close()
	if err := batch.Set([]byte(key), b, nil); err != nil {
		return r, d, false, err
	}
...
func (s *Store) AckAutomationDelivery(d AutomationDelivery) error {
	...
	if !found || current.Reference != d.Reference || current.Attempts != d.Attempts {
