...
func automationKey(r AutomationRecord) (string, error) {
	...
	if err != nil || !automationValidID(r.AutomationID) || !automationValidID(r.ID) { return "", ErrAutomationInvalid }
	switch r.Kind { case "definition", "occurrence", "context", "audit": default: return "", ErrAutomationInvalid }
	return prefix + automationPart(r.AutomationID) + ":" + r.Kind + ":" + automationPart(r.ID), nil
}
func automationRevisionKey(key string, revision uint64) string { return fmt.Sprintf("%s:revision:%020d", key, revision) }

func validateAutomationRecord(r AutomationRecord) error {
	count := 0
	for _, present := range []bool{r.Definition != nil, r.Occurrence != nil, r.Context != nil, r.Outcome != nil} { if present { count++ } }
	if count != 1 { return ErrAutomationInvalid }
	switch r.Kind {
	case "definition":
		d := r.Definition
		...
		switch s.Kind {
		case "manual":
		case "interval": if s.IntervalSeconds < 1 { return ErrAutomationInvalid }
		case "cron": if s.Expression == "" || s.Timezone == "" { return ErrAutomationInvalid }
		case "event": if s.TriggerSource == "" { return ErrAutomationInvalid }
		default: return ErrAutomationInvalid
		}
		a := d.Authorization
		if a.Mode != "approval_required" && a.Mode != "approved_policy" { return ErrAutomationInvalid }
		if a.Mode == "approved_policy" && (!automationValidID(a.ApprovalReference) || a.ExpiresAt <= 0) { return ErrAutomationInvalid }
	case "occurrence":
		if r.Occurrence == nil || r.Occurrence.DefinitionRevision == 0 || !automationValidID(r.Occurrence.TriggerIdentity) { return ErrAutomationInvalid }
		switch r.Occurrence.State { case "pending", "running", "blocked", "completed", "failed", "cancelled", "skipped": default: return ErrAutomationInvalid }
	case "context":
		if r.Context == nil || r.ID != r.AutomationID || len(r.Context.UserLocked) > 64 || len(r.Context.AgentOwned) > 64 { return ErrAutomationInvalid }
	case "audit":
		if r.Outcome == nil || !automationValidID(r.Outcome.Kind) || r.Outcome.Summary == "" { return ErrAutomationInvalid }
	default: return ErrAutomationInvalid
	}
	payload, err := json.Marshal(r)
	if err != nil || len(payload) > 32768 { return ErrAutomationInvalid }
	return nil
}
...
func (s *Store) ApplyAutomationMutation(m AutomationMutation) (AutomationRecord, bool, error) {
	...
	if err != nil { return zero, false, err }
	if found {
		if receipt.Hash != hash { return zero, false, ErrAutomationConflict }
		return receipt.Record, false, nil
	}
	var old AutomationRecord
	found, err = s.GetJSON(key + ":head", &old)
	if err != nil { return zero, false, err }
	if old.Revision != m.ExpectedRevision || m.ExpectedRevision == ^uint64(0) { return zero, false, ErrAutomationConflict }
	r := m.Record
	if r.Kind != "definition" {
		...
		lookup := dk + ":head"
		if r.Occurrence != nil { lookup = automationRevisionKey(dk, r.Occurrence.DefinitionRevision) }
		var def AutomationRecord
		ok, err := s.GetJSON(lookup, &def)
		if err != nil { return zero, false, err }
		if !ok { return zero, false, ErrAutomationInvalid }
	}
	if r.Kind == "audit" && found { return zero, false, ErrAutomationConflict }
	...
	if r.Context != nil && m.Actor != "user" {
		...
		if found { locked = old.Context.UserLocked }
		if len(locked) != len(r.Context.UserLocked) || (len(locked) != 0 && !reflect.DeepEqual(locked, r.Context.UserLocked)) { return zero, false, ErrAutomationConflict }
	}
	if r.Occurrence != nil {
		o := r.Occurrence
		if !found && o.State != "pending" { return zero, false, ErrAutomationInvalid }
		if found {
			p := old.Occurrence
			if p.DefinitionRevision != o.DefinitionRevision || p.TriggerIdentity != o.TriggerIdentity || p.ScheduledAt != o.ScheduledAt || (p.SessionID != "" && p.SessionID != o.SessionID) { return zero, false, ErrAutomationConflict }
			if !automationStateTransition(p.State, o.State) { return zero, false, ErrAutomationConflict }
		}
	...
	for _, k := range []string{key + ":head", automationRevisionKey(key, r.Revision)} { if err := put(k, r); err != nil { return zero, false, err } }
	if r.Occurrence != nil {
		prefix, _ := automationPrefix(r.Scope)
		if err := put(prefix + automationPart(r.AutomationID) + ":trigger:" + automationPart(r.Occurrence.TriggerIdentity), r.ID); err != nil { return zero, false, err }
	}
	if err := put(receiptKey, automationReceipt{Hash: hash, Record: r}); err != nil { return zero, false, err }
	if err := batch.Commit(pebble.Sync); err != nil { return zero, false, err }
	return r, true, nil
}

func automationStateTransition(from, to string) bool {
	if from == "completed" || from == "failed" || from == "cancelled" || from == "skipped" { return false }
	if from == to { return true }
	switch from {
	case "pending": return to == "running" || to == "cancelled" || to == "skipped"
	case "running": return to == "blocked" || to == "completed" || to == "failed" || to == "cancelled"
	case "blocked": return to == "running" || to == "cancelled" || to == "failed"
	}
	return false
}
...
func (s *Store) GetAutomationRecord(scope AutomationScope, automationID, kind, id string, revision uint64) (AutomationRecord, bool, error) {
	...
	if err != nil { return r, false, err }
	if revision == 0 { key += ":head" } else { key = automationRevisionKey(key, revision) }
...
func (s *Store) SearchAutomationRecords(q AutomationSearch) ([]AutomationRecord, string, error) {
	...
	if err != nil || q.Limit < 1 || q.Limit > 100 || len(q.Query) > 256 { return nil, "", ErrAutomationInvalid }
	if q.AutomationID != "" {
		if !automationValidID(q.AutomationID) { return nil, "", ErrAutomationInvalid }
		prefix += automationPart(q.AutomationID) + ":"
	}
	if q.Kind != "" { switch q.Kind { case "definition", "occurrence", "context", "audit": default: return nil, "", ErrAutomationInvalid } }
	cursor := automationCursor{Scope: q.Scope, AutomationID: q.AutomationID, Kind: q.Kind, Query: q.Query}
	start := prefix
	if q.Cursor != "" {
		if len(q.Cursor) > 4096 { return nil, "", ErrAutomationInvalid }
		data, e := base64.RawURLEncoding.DecodeString(q.Cursor)
		var c automationCursor
		if e != nil || json.Unmarshal(data, &c) != nil || c.Scope != q.Scope || c.AutomationID != q.AutomationID || c.Kind != q.Kind || c.Query != q.Query || !strings.HasPrefix(c.Key, prefix) { return nil, "", ErrAutomationInvalid }
		start = c.Key + "\x00"
	}
	...
	for valid && scanned < 512 && len(rows) < q.Limit {
		...
		if strings.HasSuffix(key, ":head") {
			...
			if (q.Kind == "" || q.Kind == r.Kind) && (q.Query == "" || strings.Contains(strings.ToLower(string(iter.Value())), strings.ToLower(q.Query))) { rows = append(rows, r) }
		}
		valid = iter.Next()
	}
	if err := iter.Error(); err != nil { return nil, "", err }
	if !valid { return rows, "", nil }
