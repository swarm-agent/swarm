...
func TestVideoEditProposalAcceptsSelectedOperationsWithoutRendering(t *testing.T) {
	...
	if unchanged.CurrentRevisionID != base.ID || unchanged.RevisionCount != 1 || unchanged.ActiveRenderJobID != "" { t.Fatalf("proposal advanced project or rendered: %+v", unchanged) }
	...
	if err != nil { t.Fatal(err) }
	if accepted.Status != VideoEditProposalStatusAccepted || accepted.AcceptedRevisionID != revision.ID || revision.AcceptedProposalID != proposal.ID { t.Fatalf("proposal lineage missing: proposal=%+v revision=%+v", accepted, revision) }
	if updated.RevisionCount != 2 || updated.CurrentRevisionID != revision.ID || updated.ActiveRenderJobID != "" { t.Fatalf("accept state unexpected: %+v", updated) }
	if len(revision.Timeline.Transitions) != 1 || revision.Timeline.Clips[0].Volume == .5 { t.Fatalf("selected operations not applied exactly: %+v", revision.Timeline) }
}
...
func TestVideoEditProposalStaleAcceptanceFailsClosed(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if _, _, err := store.CreateVideoProjectRevision(CreateVideoProjectRevisionInput{AccountScopeID: "acc", UserID: "usr", SessionID: "sess", ProjectID: project.ID, Timeline: *proposalTestTimeline(), NowUnixMs: 250}); err != nil { t.Fatal(err) }
	...
	if err == nil || !strings.Contains(err.Error(), "stale video edit proposal") { t.Fatalf("expected stale conflict, got %v", err) }
	stored, _, _ := store.GetVideoEditProposal("acc", "sess", project.ID, proposal.ID)
	if stored.Status != VideoEditProposalStatusPending || stored.AcceptedRevisionID != "" { t.Fatalf("stale proposal mutated: %+v", stored) }
}
...
func TestVideoTimelineTransitionsAreFirstClassAndValidated(t *testing.T) {
	...
	if err := validateVideoTimeline(timeline); err != nil { t.Fatalf("valid cut rejected: %v", err) }
	...
	if err := validateVideoTimeline(timeline); err == nil { t.Fatal("expected missing transition endpoint rejection") }
}
