...
func TestArtifactV3ResumeExpiredDraftPreservesEnvelope(t *testing.T) {
	...
	for _, run := range []V3SessionRunIntent{{RunID: "first", Status: V3RunIntentPendingExecutor}, {RunID: "first", Status: V3RunIntentRunning}, {RunID: "first", Status: V3RunIntentCompleted}, {RunID: "second", Status: V3RunIntentPendingExecutor}, {RunID: "second", Status: V3RunIntentRunning}} {
		_, err := sessions.ApplyV3SessionMutation(V3SessionMutationInput{SessionID: owner.SessionID, AccountScopeID: owner.AccountScopeID, UserID: owner.UserID, Kind: V3SessionMutationRecordRunIntent, IdempotencyKey: run.RunID + run.Status, PayloadHash: run.RunID + run.Status, RunIntent: &run})
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	if _, err := service.SaveDraft(owner, "artifact", "expired-save", "", old, old.Sequence); err == nil {
		t.Fatal("ordinary save renewed expired grant")
	}
	next := old
	...
	if err := service.ResumeDraft(owner, "artifact", "resume", next, old.Sequence, resume); err != nil {
		t.Fatal(err)
	}
	after, _, _ := sessions.GetArtifactV3Repository(owner.AccountScopeID, owner.UserID, "artifact")
	if _, ok := after.Drafts["old"]; ok {
		t.Fatal("old grant retained")
	}
	if len(after.Drafts) != 1 || after.HeadCommitOID != "" || after.Drafts["new"].Sequence != old.Sequence+1 || string(after.Drafts["new"].State) != string(next.State) {
		t.Fatalf("invalid handoff: %+v", after)
	}
	if err := service.ResumeDraft(owner, "artifact", "stale", next, old.Sequence, resume); err == nil {
