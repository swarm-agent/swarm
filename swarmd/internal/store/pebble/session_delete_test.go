...
func TestDeleteSessionsPurgesContentAndRetainsDurableRemoval(t *testing.T) {
	...
	if tombstone.Session.ID != "" || tombstone.WorkspacePath != session.WorkspacePath || tombstone.UserID != session.UserID || tombstone.AccountScopeID != session.AccountScopeID || tombstone.ArtifactCleanupPending || len(tombstone.ArtifactGitCleanup) != 0 {
		t.Fatalf("deletion tombstone retained private session data or invented artifact cleanup scope: %+v", tombstone)
	}
	pending, err := sessions.ListPendingV3SessionArtifactCleanups(10)
	if err != nil || len(pending) != 0 {
		t.Fatalf("empty session unexpectedly retained artifact cleanup work: pending=%+v err=%v", pending, err)
	}
