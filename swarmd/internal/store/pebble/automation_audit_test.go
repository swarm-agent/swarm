...
func TestAutomationIdentifierIntegrity(t *testing.T) {
	...
	for _, field := range []string{"account", "workspace", "automation", "mutation"} {
		...
		switch field {
		case "account":
			m.Record.Scope.AccountID = bad
		case "workspace":
			m.Record.Scope.WorkspaceID = bad
		case "automation":
			m.Record.AutomationID, m.Record.ID = bad, bad
		case "mutation":
			m.MutationID = bad
		}
		if _, fresh, err := s.ApplyAutomationMutation(m); !errors.Is(err, ErrAutomationInvalid) || fresh {
			t.Fatalf("%s: %v %v", field, fresh, err)
		}
	}
	for _, scope := range []AutomationScope{{AccountID: "a:b", WorkspaceID: "c"}, {AccountID: "a", WorkspaceID: "b:c"}} {
		...
		if _, fresh, err := s.ApplyAutomationMutation(m); err != nil || !fresh {
			t.Fatal("namespace collision", fresh, err)
		}
		rows, _, err := s.SearchAutomationRecords(AutomationSearch{Scope: scope, Limit: 10})
		if err != nil || len(rows) != 1 || rows[0].Scope != scope {
			t.Fatal("namespace leak", rows, err)
		}
	}
	m := automationFixture()
	if _, fresh, err := s.ApplyAutomationMutation(m); err != nil || !fresh {
		t.Fatal("rejected mutation left state", fresh, err)
	}
}
...
func TestAutomationAttributionReplay(t *testing.T) {
	...
	for _, bad := range []string{"subject", "time", "record"} {
		...
		case "time":
			invalid.WrittenAt = 0
		case "record":
			invalid.Record.SubjectID = "forged"
		}
		if _, fresh, err := s.ApplyAutomationMutation(invalid); err == nil || fresh {
			t.Fatal("invalid attribution accepted", bad)
		}
	}
	first, fresh, err := s.ApplyAutomationMutation(m)
	if err != nil || !fresh {
		t.Fatal("invalid writes left state", err)
	}
	update := automationFixture()
	update.ExpectedRevision = 1
	update.MutationID = "update"
	update.SubjectID = "second-writer"
	update.WrittenAt = 200000
	if _, _, err := s.ApplyAutomationMutation(update); err != nil {
		t.Fatal(err)
	}
	m.WrittenAt = 300000
	replay, fresh, err := s.ApplyAutomationMutation(m)
	if err != nil || fresh || replay.SubjectID != first.SubjectID || replay.Actor != first.Actor || replay.WrittenAt != 100000 {
		t.Fatal("replay attribution changed", replay, err)
	}
	m.SubjectID = "impersonator"
	if _, _, err := s.ApplyAutomationMutation(m); !errors.Is(err, ErrAutomationConflict) {
		t.Fatal("writer collision accepted", err)
	}
	for rev, subject := range map[uint64]string{1: "writer", 2: "second-writer"} {
		...
		if err != nil || !found || r.SubjectID != subject || r.WrittenAt != int64(rev)*100000 || r.Actor != "user" {
			t.Fatal("historical attribution lost", r, err)
		}
	}
}
