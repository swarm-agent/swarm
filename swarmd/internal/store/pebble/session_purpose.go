...
const (
	...
	SessionPurposeAutomationExecution  = "automation_execution"
)
...
// SessionAutomationExecutionWorkspace is presentation/context identity, never
// an automation binding, execution policy, or workspace permission grant.
func SessionAutomationExecutionWorkspace(session SessionSnapshot) string {
	purpose, _ := session.Metadata[SessionPurposeMetadataKey].(string)
	workspaceID, _ := session.Metadata[SessionPurposeWorkspaceMetadataKey].(string)
	if purpose != SessionPurposeAutomationExecution {
		return ""
	}
	return workspaceID
}
