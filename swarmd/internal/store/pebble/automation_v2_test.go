...
func testAutomationV2AtomicAcceptance(t *testing.T, expiration AutomationV2Expiration) {
	...
	if _,err := identity.PutUser(UserRecord{ID:"owner",Username:"owner"}); err != nil { t.Fatal(err) }
	if _,err := identity.PutAccountScope(AccountScopeRecord{ID:"account",Type:AccountScopeTypePersonal,CreatedByUserID:"owner"}); err != nil { t.Fatal(err) }
	if _,err := identity.PutAccountUser(AccountUserRecord{ID:"membership",AccountScopeID:"account",UserID:"owner",Status:"active"}); err != nil { t.Fatal(err) }
	workspace,err := NewWorkspaceStore(db).AddForAccount("account",t.TempDir(),"Workspace"); if err != nil { t.Fatal(err) }
	...
	p,err := s.ProposeAutomationV2("account","owner",workspaceID,original.ID,doc,AutomationV2Review{}); if err != nil { t.Fatal(err) }
	if _,ok,err := s.GetAutomationV2Record("account","owner",workspaceID,original.ID); err != nil || ok { t.Fatal("pending authorization",ok,err) }
	...
	if _,err := s.AcceptAutomationV2("account","owner",workspaceID,original.ID,bad); err == nil { t.Fatal("stale accepted") }
	if _,err := s.AcceptAutomationV2("account","foreign",workspaceID,original.ID,p.AutomationV2Review); err == nil { t.Fatal("foreign accepted") }
	restore := s.SetAutomationV2CommitHookForTest(func(string) error { return errors.New("injected") })
	if _,err := s.AcceptAutomationV2("account","owner",workspaceID,original.ID,p.AutomationV2Review); err == nil { t.Fatal("failure accepted") }
	restore()
	current,_,err := s.GetSession(original.ID); if err != nil || current.AutomationV2 != nil { t.Fatal("partial binding",err) }
	if _,ok,err := s.GetAutomationV2Record("account","owner",workspaceID,original.ID); err != nil || ok { t.Fatal("partial record",err) }
	var wg sync.WaitGroup
	results := make(chan AutomationV2Record,2)
	failures := make(chan error,2)
	for i:=0;i<2;i++ { wg.Add(1); go func(){ defer wg.Done(); r,e := s.AcceptAutomationV2("account","owner",workspaceID,original.ID,p.AutomationV2Review); results<-r; failures<-e }() }
	...
	s = NewSessionStore(db)
	replayed,err := s.AcceptAutomationV2("account","owner",workspaceID,original.ID,p.AutomationV2Review)
...
func TestAutomationV2AuthorityIntegrityReplay(t *testing.T) {
	...
	workspace,err := NewWorkspaceStore(db).AddForAccount("account",t.TempDir(),"Workspace"); if err != nil { t.Fatal(err) }
	available := true
	if err := s.CreateSession(SessionSnapshot{ID:"conversation",AccountScopeID:"account",UserID:"owner",WorkspacePath:workspace.Path,WorkspaceGrants:[]WorkspaceGrant{{Kind:WorkspaceGrantPrimary,WorkspaceID:workspace.WorkspaceID,Path:workspace.Path,Available:&available}}}); err != nil { t.Fatal(err) }
	...
	snapshot := func() map[string]string {
		t.Helper(); it,err := db.db.NewIter(nil); if err != nil { t.Fatal(err) }; defer it.Close()
		out := map[string]string{}; for ok := it.First(); ok; ok=it.Next() { out[string(it.Key())]=string(it.Value()) }; if err := it.Error(); err != nil { t.Fatal(err) }; return out
	}
	reject := func(call func() error) { t.Helper(); before := snapshot(); if err := call(); err == nil { t.Fatal("operation unexpectedly succeeded") }; if !reflect.DeepEqual(before,snapshot()) { t.Fatal("rejection wrote durable state") } }
	propose := func(expected AutomationV2Review) (AutomationV2Proposal,error) { return s.ProposeAutomationV2("account","owner",workspace.WorkspaceID,"conversation",doc,expected) }
	reject(func() error { _,err := propose(AutomationV2Review{}); return err }) // absent membership
	if _,err := identity.PutAccountUser(member); err != nil { t.Fatal(err) }
	for _,review := range []AutomationV2Review{{ProposalID:"partial"},{Revision:1},{Digest:"partial"},{ProposalID:"overflow",Revision:^uint64(0),Digest:"digest"}} { reject(func() error { _,err := propose(review); return err }) }
	p,err := propose(AutomationV2Review{}); if err != nil { t.Fatal(err) }
	doc.Title="Second"
	second,err := propose(p.AutomationV2Review); if err != nil { t.Fatal(err) }
	replayed,err := propose(p.AutomationV2Review); if err != nil || !reflect.DeepEqual(second,replayed) { t.Fatal("replay regenerated proposal",err) }
	doc.Title="Changed duplicate"
	reject(func() error { _,err := propose(p.AutomationV2Review); return err })
	third,err := propose(second.AutomationV2Review); if err != nil { t.Fatal(err) }
	for _,want := range []AutomationV2Proposal{p,second,third} {
		...
		if err != nil || !ok || !reflect.DeepEqual(want,historical) { t.Fatal("history lost",err) }
	}
	doc.Title="Second"
	reject(func() error { _,err := propose(p.AutomationV2Review); return err })
	accept := func() error { _,err := s.AcceptAutomationV2("account","owner",workspace.WorkspaceID,"conversation",third.AutomationV2Review); return err }
	for _,mutate := range []func(*AutomationV2Proposal){
		...
		bad := third; mutate(&bad)
		if err := db.PutJSON(automationV2Key("proposal","account","conversation"),bad); err != nil { t.Fatal(err) }
		reject(accept)
	}
	if err := db.PutJSON(automationV2Key("proposal","account","conversation"),third); err != nil { t.Fatal(err) }
	restore := s.SetAutomationV2CommitHookForTest(func(string) error { return errors.New("injected") }); reject(accept); restore()
	if err := accept(); err != nil { t.Fatal(err) }
	member.Status="revoked"; if _,err := identity.PutAccountUser(member); err != nil { t.Fatal(err) }
	reject(accept)
	reject(func() error { _,_,err := s.GetAutomationV2Proposal("account","owner",workspace.WorkspaceID,"conversation"); return err })
	reject(func() error { _,_,err := s.ListAutomationV2Records("account","owner",workspace.WorkspaceID,"",10); return err })
	member.Status="active"; if _,err := identity.PutAccountUser(member); err != nil { t.Fatal(err) }
	reject(func() error { _,err := s.AcceptAutomationV2("foreign","owner",workspace.WorkspaceID,"conversation",third.AutomationV2Review); return err })
	if err := NewWorkspaceStore(db).DeleteForAccount("account","owner",workspace.Path); err != nil { t.Fatal(err) }
	reject(accept)
	reject(func() error { _,_,err := s.ListAutomationV2Records("account","owner",workspace.WorkspaceID,"",10); return err })
}
