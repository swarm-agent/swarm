...
func TestAutomationRealtimeAtomicRestart(t *testing.T) {
	...
	s.SetAutomationPublisher(func(record V3RealtimeOutboxRecord) {
		...
		if !s.automationsMu.TryLock() {
			t.Fatal("publisher holds automation lock")
		}
		s.automationsMu.Unlock()
		if _, fresh, err := s.ApplyAutomationMutation(m); err != nil || fresh {
			t.Fatalf("callback retry: %v %v", fresh, err)
		}
		if _, found, err := NewSessionStore(s).GetSession(record.SessionID); err != nil || found {
			t.Fatalf("synthetic session exposed: %v %v", found, err)
		}
	})
	check := func(want int) {
		t.Helper()
		...
		if err != nil || len(rows) != want {
			t.Fatalf("outbox count %d want %d: %v", len(rows), want, err)
		}
		for _, r := range rows {
			if r.AccountScopeID != m.Record.Scope.AccountID || r.Event.EventType != AutomationChangedEventType || string(r.Event.Payload) != "{}" {
				t.Fatalf("invalid signal: %+v", r)
			}
		}
	}
	...
	if _, _, err := s.ApplyAutomationMutation(stale); err == nil {
	...
	if participant.outbox != nil {
		t.Fatal("failed participant retained publication")
	}
	var foreign any
	if found, err := s.GetJSON("zz:foreign:key", &foreign); err != nil || found {
		t.Fatalf("foreign write persisted: %v %v", found, err)
	}
	got, found, err := s.GetAutomationRecord(m.Record.Scope, "check", "definition", "check", 0)
	if err != nil || !found || got.Revision != 1 || got.Definition == nil {
		t.Fatalf("partial head: %+v %v", got, err)
	}
	check(1)
	...
	if _, err := s.CreateAutomationApproval(g); err != nil {
		t.Fatal(err)
	}
	if _, err := s.RevokeAutomationApproval(g.Scope, g.ID, "writer", 1, 150); err != nil {
		t.Fatal(err)
	}
	if _, err := s.RevokeAutomationApproval(g.Scope, g.ID, "writer", 1, 150); err == nil {
	...
	if _, _, err := s.ApplyAutomationMutation(occurrence); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.FinishAutomationCancellation(admitted, "writer", 120); err != nil {
		t.Fatal(err)
	}
	check(6)
	if wakes != 6 {
		t.Fatalf("cancel wakes %d", wakes)
	}
	...
	check(6)
	if _, fresh, err := s.ApplyAutomationMutation(m); err != nil || fresh {
		t.Fatalf("restart replay: %v %v", fresh, err)
	}
	check(6)
	grant, ok, err := s.GetAutomationApproval(g.Scope, g.ID)
	if err != nil || !ok || grant.RevokedAt != 150 {
		t.Fatalf("revocation lost: %+v %v", grant, err)
	}
}
