...
- The Sessions API v3 shape is current: session create/lifecycle/messages/plans/permissions/run mutations go through the V3 event-sourced mutation boundary, with runtime placement and workspace binding preserved where target-aware execution requires them; do not retrace stored routes, path shapes, or legacy v1/v2 session handlers as authority.
...
- `swarmd/internal/api/sessions_v3_outbox.go`, `swarmd/internal/api/sessions_v3_realtime_contract.go`, and `swarmd/internal/api/sessions_v3_realtime_ws.go` — canonical V3 committed-event publication and realtime stream path (`/v3/realtime/stream`). Do not replace it with legacy run-stream or ad hoc websocket state for the same flow.
