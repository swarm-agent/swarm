...
- Run `./scripts/run-testbench-launch-prerun.sh` for the canonical launch pre-run. It checks connectivity once, then runs the independent onboarding, Desktop, TUI, Plan/Auto, task-routing, Task Program, and provider-backed sync/realtime suites with bounded parallelism and aggregate failure reporting. Use `--list-suites`, repeated `--suite`, and `--dry-run` to inspect or narrow the manifest; do not add a second launch-test manifest elsewhere.
