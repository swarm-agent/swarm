...
- The Sessions API v2 shape is stricter: session create/lifecycle authority is frozen execution + live runtime placement + live workspace binding, not stored session routes or target/path retracing.
...
- `swarmd/internal/api/sessions_v2_primary.go` — native Sessions API v2 create handlers: `POST /v2/sessions/primary` and `POST /v2/sessions/local-containers`. This is the create-time authority path for strict request decode, placement lookup, workspace binding validation, runtime-open dispatch, and frozen `SessionExecutionV2Record` persistence.
- `swarmd/internal/api/sessions_v2_lifecycle.go` — native `/v2/sessions/{id}` lifecycle, messages, metadata, modes, plans, permissions, usage, run, and run-stream handling. Lifecycle work must keep execution-class authority explicit instead of falling back to legacy route proxies.
- `swarmd/internal/api/runtime_sessions_v2.go` and `swarmd/internal/session/runtime_sessions_v2.go` — native runtime-session open/mirror authority for local-container execution. This path validates runtime authority, binding snapshots, source/runtime workspace facts, and mirror payloads without using legacy peer session wrappers.
...
- `swarmd/internal/api/topology.go` — account-scoped topology APIs for runtime owners, workspace bindings, and session-route inspection. Workspace binding lookups are product authority inputs; session-route inspection is not authority for native Sessions API v2 dispatch.
- `swarmd/internal/api/swarm_targets.go` — target catalog semantics for self, host-local container children, managed host peers, and mirrored/host-owned child routes. Swarm targets provide identity/selection and attached workspace-route projections; they do not replace v2 session authority from frozen execution, live placement, and live workspace binding. Legacy remote target references are not a current canonical plane for new Flow/workspace work.
...
- Native Sessions API v2 is the canonical path for session create/lifecycle work: use `/v2/sessions/primary`, `/v2/sessions/local-containers`, and `/v2/sessions/{id}...`; do not wrap, proxy, or fallback through `/v1/sessions`, `handleSessions`, `handleSessionByID`, `createSessionFromRequest`, `proxyRoutedSessionRequest`, or legacy peer session wrappers.
- The only authority sources for native v2 session dispatch/lifecycle are the frozen `SessionExecutionV2Record`, the live `TopologyRuntimePlacementRecord`, and the live `TopologyWorkspaceBindingRecord`. Do not use `SessionRouteRecord`, topology session routes, backend URLs, child URLs, next-hop fields, mirrored session snapshots, workspace names, filesystem paths, or client-supplied routing hints as v2 authority.
- Sessions v2 execution class must be explicit and enforced: `SessionExecutionClassPrimary` (`primary`) or `SessionExecutionClassLocalContainer` (`local_container`). There is no generic class, compatibility class, routed exception, or false retrace through old route state.
- Workspace bindings are required authority for desktop/web/local-container v2 sessions. The only no-binding exception is terminal/TUI primary CWD sessions with the expected TUI markers; do not expand that exception to desktop, web, local-container, managed-host, or ambiguous path-based cases.
