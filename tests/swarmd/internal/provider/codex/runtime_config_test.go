...
func TestEffectiveContextWindowSupportsGPT55(t *testing.T) {
	if got := EffectiveContextWindow("gpt-5.5", "", 123); got != 272000 {
		t.Fatalf("EffectiveContextWindow(gpt-5.5) = %d, want 272000", got)
	}
	if got := EffectiveContextWindow("gpt-5.5", "1m", 123); got != 272000 {
		t.Fatalf("EffectiveContextWindow(gpt-5.5, 1m) = %d, want 272000", got)
	}
}
