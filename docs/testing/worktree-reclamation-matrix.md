...
Final correction evidence: historical repository projections store runtime `WorkspacePath`; admission now matches their source metadata and retained source grant. Cross-workspace CAS/reselection and delegated target tests pass. Recovery-copy publication separately retains the immutable original source check. The five-package focused command above plus `TestTaskTargetRuntimePreflightAndRegularChildren` and `TestMultiWorkspaceIdentityTransitions` passed with `-count=2` on `320efe00f3975269d05c386444a4040b041b78f0` plus the copy-source guard. Atlas sync passes. This does not change unsupported R03/R12/R13 or external-writer/crash-repair gaps.

## Writer-fencing and interruption safety review (2026-09-10)

Reviewed candidate `f8dab3821fc2ca505623892565540b2b97837182` and corrected the following concrete gaps in the working tree:

- **R16/R19:** Task Program creation previously acquired the parent lock but did not check a retained recovery reservation; transitions did not share the ownership lock. Creation and active transitions now serialize with canonical ownership mutations and reject reserved current, explicit job and retained repository lanes. Terminal reconciliation remains possible. A store-reopen negative test asserts rejected program admission leaves no program record.
- **R22:** ordinary allocation could automatically remove partial worktree/branch resources after Git failure. Recovery now has a no-cleanup allocator. A journal callback commits exact planned destination path/branch/base before Git creation; copy publication requires that journal and matches its destination. Tests inject journal refusal and an external writer occupying the destination after journaling, asserting no premature allocation and no removal of external bytes.
- **R22:** retained reservations were not actionable through the tool. `cancel_worktree_recovery` now performs permission-gated, exact claimant/operation/revision/evidence cancellation through V3. It releases only the source reservation; it never deletes or imports retained resources, transfers ownership or switches the session. Discovery returns reserved-operation metadata without needing to read a possibly broken source index. A daemon-local producer guard prevents cancellation racing another recovery call. Reopen tests verify durable destination evidence; integration tests verify wrong-operation rejection and source/destination preservation after cancellation.

**Remaining deployment constraints — not passing matrix rows:** No advisory Git lock or fingerprint can fence arbitrary same-OS-principal processes, survive hostile path replacement as an atomic Git/Pebble transaction, or revoke already-spawned descendants of a deleted session. OS-enforced writer isolation/quiescence is required for that stronger contract; these corrections do not provide it. Cancellation is safe abandonment, not automatic resume/publication of an interrupted partial copy. Pre-journal legacy failed allocations still lack destination attribution. R03 external provenance and R12/R13 commit/hunk import remain unsupported; R14 tool replay still rejects stale requests rather than returning the original success. R18 live provider continuation and full process-kill/power-loss coverage remain unproved. Do not deploy on the strength of the focused tests alone or mark all 22 rows complete.

Executed focused command (working tree based on the candidate above): `GOMAXPROCS=2 go test ./internal/run ./internal/store/pebble ./internal/worktree ./internal/permission -run '^(TestRecovery.*|TestWorktreeRecovery.*|TestWorktreeAdmission.*|TestTaskProgramStore.*)$' -count=2 -parallel=1 -p=1 -timeout=120s`. Initial added-test compilation error was corrected before passing execution. No live deployment, tunnel change or host restart. Independent test audit remains pending.
