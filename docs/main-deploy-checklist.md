...
- Preferred user install path:

```bash
curl -fsSL https://raw.githubusercontent.com/swarm-agent/swarm/main/install.sh | sh
```

- If/when the website serves the same script at `https://swarmagent.dev/install.sh`, that becomes the public one-liner.

- Manual fallback after extraction:
...
- [ ] Verify the one-command installer works against the published GitHub release
- [ ] Verify the manual extracted-release fallback works with `swarmsetup --artifact-root ...`
