...
| Delegated Git integration | child base/head/branch/path and clean commit lineage are verified before integration; promotion may leave the active owned lane only for the source session's exact captured checkout after principal-backed account ownership and canonical-root equality are revalidated, then cherry-picks the captured-base commit range onto the exact current target branch/HEAD | `tool.Runtime.manageWorktreePromote`, `manageWorktreeResolvePromotionTarget`, `workspace.Service.ScopeForPathForPrincipal`, `worktree.Service` integration methods | `runtime_manage_worktree_test.go` exact captured/foreign-target promotion tests, worktree service/sparse tests, routed worktree integration tests |
