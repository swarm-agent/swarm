...
| HTML execution/render | declared animation HTML cannot bypass reviewed-profile preflight; `motion_ui` must self-start through artifact-owned RAF while deterministic ready/seek/stability, network/file isolation, viewport containment, and bounded frames/output remain enforced | `tool/runtime_manage_artifact_animation.go`, `htmlcapture/renderer.go`, `htmlcapture/animation.go` | `TestDeclaredAnimationHTMLRequiresReviewedProfileBeforePublication`, `TestAnimationPreflightRejectsMissingArtifactOwnedPlayback`, `TestAnimationPreflightAcceptsArtifactOwnedPlayback`, trusted export tool tests |
