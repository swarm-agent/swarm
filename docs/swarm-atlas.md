...
| Workspace containment | traversal, symlink, linked-root, and materialization paths cannot escape approved roots; principal-backed scope resolution validates the requested stored root before accepting its canonical target; delegated Coders cannot widen their worktree/read-only roots through user permission and fail before a pending permission is created | `storagecontract.Join`, `workspace.Service.ScopeForPathForPrincipal`, `run.resolveRunWorkspaceScope`, `tool.ScopeExpansionForCall`, `run.gateWorkspaceScopeCalls`, workspace/worktree/action/artifact path resolvers | `workspace/path_containment_security_test.go`, `tool/workspace_scope_request_test.go`, `run/service_workspace_scope_test.go`, discovery containment, artifact promotion security, sparse worktree tests |
