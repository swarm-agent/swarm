...
- `.github/workflows/critical-tests.yml`: runs repository policy/secret/path/atlas checks plus all three critical tiers on `dev`/`main` pull requests and `dev` pushes. `.github/workflows/dependency-vulnerability-scan.yml` independently runs Go/web dependency scanners on PRs, protected pushes, and a schedule. `.github/workflows/build-main.yml` reruns `all` before the release archive is built.
