...
- `scripts/run-critical-tests.sh {fast,deep,agents,all}`: the explicit atlas-driven executable manifest. `fast` covers deterministic listener, authentication/vault, authority, containment, media/image/video, FFmpeg-input, and critical Desktop V3 state/realtime failures; `deep` covers Pebble mutation/outbox/replay/restart/concurrency, executor idempotency, delegated Git integration, and artifact Git integrity; `agents` covers system-agent identity/tool authority, launch trust rejection, Task Program and Iteration Swarm scheduling, subagent capacity/approval, durable child lineage/recovery, and Desktop child/realtime presentation. Every invocation is package/function-bounded, verifies each declared Go package/pattern resolves at least one test before execution, and uses explicit Go/Node test timeouts.
...
9. Mint/activation reporting and authenticated feedback are current implementation and have focused transport/state/startup tests, but the inherited two-pass ledger inventory predates the feedback test file. The workspace-aware `/task` change also materially edited and added backend, TUI, and Desktop test symbols after that inventory; affected inherited rows are marked `stale-inventory`, while new symbols require a future bounded reinventory. Focused passing evidence does not make the wider audit complete.
