...
| Session-owned Git isolation | every routed or deployed Git-backed session allocates an owned managed worktree before durable run authority is published; `/task` resolves current/explicit saved workspace before Router, replay-checks before the one Router call, never gives Router workspace choice, and rolls back allocation on later mutation failure; provider start revalidation of exact owner, source/runtime path separation, branch, and effective CWD remains the runtime authority on the complete repository revision | `api.handleBackgroundRouterSessionStart`, `api.handleRoutedSessionStart`, `api.routeSessionOnce`, routed allocation/metadata helpers, and the runtime mandatory-worktree validation paths cited at revision 13 | PR `fast` tier: `TestBackgroundRouterSessionStart*`, TUI `TestTaskCommand*`/`TestParseTaskCommand*`, client `TestCreateRoutedTaskSession*`, and Desktop task/workspace/write API specs; additional routed/session Router/provider-start tests remain focused evidence; live `task-routing` checks current and explicit saved workspaces in Auto and Plan |
