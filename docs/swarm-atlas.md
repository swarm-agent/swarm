...
- Managed artifact authority and lineage are in `artifact/authority*.go`; bytes/manifests are in private `artifactgit` repositories. Workspace materialization must pass destination containment (`artifact/materialize.go`). Desktop projects each durable Iteration Swarm turn as one clickable sidebar group: initial waves drill into their collection overview, while later chain turns drill into Artifact Studio and retain an exact single-part target when present (`desktop-v3-artifact-sidebar.tsx`, `desktop-v3-existing-conversation-pane.tsx`, `artifact-studio-model.ts`).
...
| Task Programs, swarms, and child lineage | dependencies unlock only after integrated prerequisites; blockers terminalize; swarm modes cannot cross authority; child handoffs/leases survive restart with single-winner rotation | `run/service_task_program*.go`, `run/service_task_swarm*.go`, `store/pebble/task_program_store.go`, `delegated_child_rotation_store.go` | curated scheduler/blocker/handoff/swarm-mode tests plus Pebble reopen/concurrency lineage tests and Desktop bounded child-state tests |
