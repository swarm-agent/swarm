...
### L4 audited root; explicit launch approval still required
...
The first trial unexpectedly auto-launched under the lane's bounded subagent policy, with no pending permission record. It therefore does not satisfy this gate's explicit-approval requirement. The runner now checks `mode=ask` before new Designer authoring and never resolves its own launch permissions. The negative preflight rejects the current policy before any provider turn or model change. Further regular follow-up, two-candidate swarm, explicit selection/continuation and Designer video proof remain pending an operator-enabled approval path. No swarm, selection, proposal acceptance or final rendering was performed. Preserved login remained runnable after the same-lane rebuild; the known device-label continuity gap required restoring only the existing self label. Overall launch remains NOT READY.
