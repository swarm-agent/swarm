...
| HTML-to-MP4 | L4 repaired Designer source converted to four-second native MP4; three 1920×1080 frames inspected with readable, contained content and motion | Repaired source passed; earlier L1 flawed media unchanged; final mixed-media cut unproved |
...
| Regular Designer creation and iteration | L4 recovered native root/targeted revision/selected continuation; serialized-author repair inspected at full and narrow sizes | L4 passed; earlier failed and visually flawed candidates retained |
| Designer swarm sibling candidates | Two ready same-base revisions compared; cyan explicitly selected and continued; partial-wave failures preserved in task and Studio | L4 recovered journey passed; unselected violet quality defects remain explicit |
...
### L4 native Designer recovery and partial-wave evidence

On runtime `6cff8593`, the actual Designer contract exposes context-bound `artifact_v3_author`; producer/session grants and source/target parsing were inspected. Native task handoffs had incorrectly serialized Git identities as collection/variant fields. They now return native artifact/commit/projection and turn/candidate fields; pending slots omit a commit. Four focused run tests and the real Git/Pebble production/recovery test passed twice, including same-base sibling allocation, retained ready output after injected sibling failure, and unchanged selected head. This is deterministic failure evidence, not a live partial-wave PASS.

One regular Luna/medium Designer created a complete four-file, three-Part root with native inspect/build/finish output. The runner initially failed because it parsed the outer history wrapper, then the 280-character completed-output display excerpt rather than full output. Corrected exact-session inspection recovered the existing output without reauthoring. Two full-size frames and the settled 840×844 Studio preview were inspected: main content fits and is legible with preserved motion, but Pricing/Footer decorative markers cross text. The first Studio screenshot was blank before presentation despite substantive DOM text; bounded load/paint settlement is now required. No visual success is claimed for the overlapping markers. The next targeted brief includes their correction.

The test-only Ask-mode prerequisite was removed at user direction. Trials exercise the existing product policy without changing it; actual pending Designer permissions remain operator-owned. Regular targeted repair initially failed because literal edit strings did not match, not because selected-head CAS raced. A later native Designer produced a complete exact-root candidate; full-size and settled narrow pixels expose remaining full-width card-bottom clipping, so that candidate remains unselected. The runner now follows the actual restart checkpoint run rather than stopping when its submitting run completes, and retained-run inspection does not relaunch successful children.

A real two-child Designer swarm preserved one ready violet candidate and one failed cyan child, with unchanged selected root. The native task result reports both honestly, but Studio's Git-ref-only turn enumeration omitted the failed slot. The correction merges authenticated bounded durable candidate projections, preserving failed diagnostics without fabricated revisions. Native swarm target hints now follow regular Designer semantics: exact target IDs are authoritative; optional hints must agree but need no duplicate locator. Literal-edit errors now report match count and unchanged-file state. Focused target/mismatch/real-store partial-failure tests pass twice, including cross-account/user denial. Rebuild and retained-wave pixel inspection remain required; two ready siblings, explicit choice/continuation and Designer video proof are still open. No overall launch PASS.

**L4 subsequent evidence:** Runtime `c29b009b` preserved the first partial wave across rebuild and exposed its failed slot in Studio. A new CSS-override strategy produced two ready same-base Designer candidates; six inspected images showed the cyan option retained the Pricing label and fit both viewports, while the violet option omitted that label and overlapped markers. Explicit cyan selection and one direct-child Designer continuation passed, but the requested magenta stylesheet did not apply. Actual child history showed near-simultaneous successful edits losing the link/class and leaving trailing HTML: author operations lacked per-candidate serialization. The correction serializes read/write/build/finish/discard per candidate while siblings remain parallel, releases idle locks, and preserves literal content whitespace at the tool boundary. Focused concurrent-edit, byte-preservation, author lifecycle and real-store recovery tests pass twice. Prior candidates remain immutable; corrected runtime deployment and exact-source visual/video proof remain open.

**L4 native video proof:** On `6c69b6b4`, the serialized author path produced a visually inspected magenta repair with preserved labels/features/motion. Native video preflight then correctly rejected an off-viewport decorative `.stage::before` glow. A new exact-base Designer revision removed that decoration without weakening capture validation or hiding meaningful content. The selected repaired revision converted through `convert_artifact_v3` in the existing project/base into one pending four-second silent MP4 and fallback PNG. Three decoded 1920×1080 samples at 0/2000/3966ms were inspected: all requested labels, magenta Pricing, complete feature lines, Footer and changing motion are legible and contained. The source head/replay and confirmed empty cut remained unchanged by conversion; prior failed/successful candidates remain available. No new Designer ran during conversion and no video proposal was accepted or final render started.

The interrupted media-count review resumed from retained images without regeneration. Actual Studio first-frame/playback inspection passed, but a clip-status badge covered Footer content in the narrow player. The badge now sits below the preview; a live bounding-box assertion checks that separation. Final same-lane rebuild/reinspection on `41f6f5b4` passed: status lies below the preview, first-frame and playing canvas content are unobstructed, and all three decoded samples exactly match the inspected media. L4 is a recovered journey with recorded failures and source corrections, not twice-on-one-candidate release evidence; mixed-media user acceptance/render and shipped-runtime parity remain separate gates.
