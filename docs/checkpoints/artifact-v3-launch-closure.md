...
The interrupted media-count review resumed from retained images without regeneration. Actual Studio first-frame/playback inspection passed, but a clip-status badge covered Footer content in the narrow player. The badge now sits below the preview; a live bounding-box assertion checks that separation. Final rebuild/reinspection of this UI-only correction is pending. L4 is a recovered journey with recorded failures and source corrections, not twice-on-one-candidate release evidence; mixed-media user acceptance/render and shipped-runtime parity remain separate gates.
