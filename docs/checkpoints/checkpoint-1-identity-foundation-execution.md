...
- Fresh install / empty state:
  - TUI onboarding asks for exactly `username` and `swarmName`.
...
- Before product identity exists:
  ...
  - It may show the username+swarmName onboarding flow, or block protected actions with clear onboarding guidance.
...
- relevant desktop auth/session tests
...
- Save onboarding sends `username` and `swarmName` on first bootstrap.
...
- TUI before bootstrap must not create identity implicitly; it must show username+swarmName onboarding or block guarded actions with clear onboarding guidance.
