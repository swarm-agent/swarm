...
## Desktop proposal review and reserved Plan/AI sidechats

Approval-gated `exit_plan_mode` and plan proposal/revision/amendment/follow-up permissions render inline in the parent Desktop conversation. The pending `PermissionRecord` payload is authoritative before approval. It carries a positive monotonic `proposal_revision`; every saved edit increments that revision and emits a durable `permission.updated` event to the parent session so the inline structured-plan card updates live. An edit with a stale expected revision fails with a conflict and does not resolve the permission or wake its waiter.

Approval always reads the latest canonical backend proposal. Client-supplied approval arguments may overlay only supported execution-policy choices; they cannot replace the proposal title, plan ID, or structured document. Deny remains a direct rejection action and is not an edit transport.

`POST /v3/sessions/{parent_session_id}/sidechats/plan` and `POST /v3/sessions/{parent_session_id}/sidechats/ai` ensure two stable, parent-owned durable sessions. Their identities are derived only from the parent session and sidechat kind, never from a proposal revision. A new pending proposal rebinds the existing Plan session metadata to its permission, plan ID, and latest revision. Both sessions are marked `system_sidechat` and `navigation_hidden`: ordinary list, workset, search, and lineage/navigation surfaces must exclude them, while direct V3 hydration and realtime subscription by the parent-owned panel remain supported.

The reserved Plan sidechat selects the parent's plan model settings, falling back to the parent's single model. It receives plan-safe read/search/list/web capabilities plus the dedicated `edit_pending_plan` tool. That tool accepts only an expected revision and structured document; trusted session metadata supplies the parent session, permission, and plan binding. Calls from any session other than the bound reserved Plan sidechat fail. Plan cannot change mode/profile/settings, approve permissions, or mutate an approved/running plan.

The reserved AI sidechat selects the parent's auto model settings, with the same single-model fallback. It snapshots the parent's auto-capable tool/permission contract but is permanently auto-only: plan/mode/profile/settings transitions are disabled. Neither reserved profile is user-selectable or persisted as an ordinary agent.

Desktop keeps a single persistent **Plan & AI** right panel for plan-bearing parent sessions. Plan remains available from pending proposal through active execution; AI is a separate tab and durable conversation. The panel creates/hydrates the reserved sessions directly, subscribes to each session through V3 realtime, and does not create an ordinary helper session.
