...
Current state: **RED / UNVERIFIED**. One isolated journey proved a ready three-Part native V3 artifact, exact 1440×900 pixels, Desktop discovery, durable genesis replay, cursor-bearing realtime, zero Designer children, and native terminal handoff parsing, but its Fireworks continuation rejected an equivalent duplicate credential record because the media contract fingerprint included record ID. The source repair now binds equivalent records by normalized account/provider/type/API-key material while still rejecting a changed key; focused tests pass. The first exact-commit rerun crossed terminal completion and again passed artifact, Parts, build/preview, pixels, Desktop, replay, and zero-Designer gates, then exposed that the runner treated WebSocket `open` as subscription readiness and could miss genesis before `subscribe.session` completed. It now waits for the exact session's cursor-bearing `replay.complete`; the next exact commit passed that handshake and created three valid distinct selector Parts but exposed a harness-only literal-ID requirement absent from the ordinary user brief. The gate now follows exact manifest IDs/selectors and semantic content instead. Another clean-commit pooled rerun then found a genuine visual-overlap defect: primary Swarm attempted corrected HTML, but per-run idempotency silently replayed the old commit, so the model entered legacy list/read/select actions and timed out. Direct create now distinguishes exact-byte replay from changed corrected HTML: exact replay is explicitly labeled idempotent, while changed HTML with unchanged stable Part IDs becomes one exact-base native candidate that is selected as the corrected child head after build/preview. Focused exact-replay, Part-stability, candidate-before-selection, ancestry, and restart tests pass. The next exact commit passed creation, Parts, preview, pixels, Studio, replay, and zero-Designer checks, but the harness discarded earlier realtime records when it navigated to Studio. Its bounded same-origin diagnostic ring now survives document navigation. Another clean-commit pooled rerun remains required, so do not advance.
