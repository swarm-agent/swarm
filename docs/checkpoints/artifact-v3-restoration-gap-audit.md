...
Current state: **Gate 0A–0D GREEN for the bounded no-Designer product journeys; Gate 0E RED at trusted MP4 encoding.** Exact isolated commit `c95aed32` passed fresh account-backed Codex GPT-5.6 Luna/medium static HTML and targeted-Part journeys. On the same persistent lane, the bounded `animated-parts` stage created one three-Part native V3 artifact with zero Designers, exact 1440×900 layout, one running animation per Part, advancing timelines and changing computed visual signatures, two nonidentical inspected frame PNGs, a substantively rendered Desktop Studio iframe, durable replay, cursor-bearing realtime, and zero legacy writes. Native `convert_artifact_v3` now exists, authenticates an exact selected Git head, uses a cross-session Video Studio destination, injects deterministic CSS/WAAPI seek without mutating Git, and fails atomically before proposal/derivative publication. Live recovery crossed missing-auth routing, CSS authored-rAF mismatch, untrusted/missing encoder ownership, frame instability, and hidden-tab rAF timeout. The final bounded exact attempt reached `animation_encode_failed`: the checksum-pinned Playwright FFmpeg mounted into the isolated container supports WebM/VP8 but not the required libx264 MP4 pipeline (`animationEncoderArgs` uses `-c:v libx264`). No pending proposal or derivative was written. Resolution requires provisioning a separately checksum-pinned root-owned FFmpeg build with libx264/MP4 support through the fixed testbench base/helper contract, then one exact conversion rerun; no equivalent retry is authorized before that material change.
...
Gate 1 first passed on exact isolated-container commit `46fa0d95`: primary Swarm created one native Artifact with three stable selector Parts, valid 1440×900 pixels, visible Studio, durable replay, cursor-bearing realtime, zero Designer children, and zero legacy identity. The targeted-Part prerequisite then exposed a missing ordinary-Swarm authoring boundary: the Studio draft carried exact native identity, but only Designer task orchestration could consume it. Source now adds bounded authenticated `manage_artifact read_v3` and exact-base `revise_v3`; the latter preserves all stable Part IDs, publishes one complete build/preview-valid child candidate, and deliberately leaves head selection unchanged. Focused tests pass. The first exact targeted-Part run reached valid root pixels and zero Designers, then exposed a harness-only literal sibling-ID assertion; target identity now comes from the exact manifest label/ID. The next exact run durably created and pixel-inspected the no-Designer candidate, but the harness raced detail projection visibility immediately after terminal run intent; it now polls that canonical route for at most 30 seconds for `awaiting_selection`. A later exact run exposed a legitimate prior visual-repair turn, so the harness now records prior history and requires exactly one added targeted turn rather than two total. The next exact run reached valid root/candidate pixels, unchanged head, exact ancestry, and Studio candidate visibility; it exposed only a false harness expectation that the still-current root should show a historical warning after viewing an unselected candidate. The runner now asserts exact root preview identity instead. The next exact retry failed earlier because the provider exhausted its bounded root turn after two overflow attempts and produced zero Artifact records; the runner stopped correctly. One permitted same-source retry then reached every targeted product, pixel, ancestry, and Studio gate but its final replay assertion reused a bounded hydrate event slice. That check now uses canonical paginated replay and distinct root/candidate screenshot files. Exact isolated-container commit `28d0ef97` passed the no-Designer targeted-Part stage: primary Swarm read the exact root revision, created one build/preview-valid Pricing candidate, inspected its exact pixels, preserved all three Parts, recorded direct-child ancestry, left head unchanged, showed root/candidate/history in Studio, and delivered five durable plus five cursor-bearing realtime native events with no legacy identity. Gate 2 now advances to explicit selection, continued turns from the selected revision, and alternate whole-project choice. The runner's next bounded `selected-continuation` stage selects the exact candidate through Studio, then requires a new primary-Swarm Hero candidate to parent that selected commit and preserve the Pricing change; its first exact run stopped because implicit Part derivation accepted an extra semantic wrapper. Direct create now fails closed above three implicit Parts and tells primary Swarm to pass the exact three requested Parts. The next run exposed a declared footer clipped below an overflow-hidden 1440×900 viewport despite bounded global scroll size. Trusted capture now checks every server-authored selector Part for valid, visible, fully contained geometry before readiness; real-Chrome and focused runtime tests pass. Two later runs reached Studio selection and exposed a Desktop request ID containing a colon, rejected by the native Git transaction-ID contract. Desktop now uses a hyphen-safe ID and its API spec passes 4/4. Exact isolated-container commit `8262fdf1` then passed explicit Studio selection and continuation: the Pricing candidate became the exact selected head, a normal primary-Swarm Hero request created one unselected direct child of that selected commit, both labels survived in inspected 1440×900 pixels, ten durable and ten cursor-bearing realtime events agreed, and no Designer or legacy identity appeared. Alternate sibling choice remains the next sub-gate. Source now lets `revise_v3` calls share one stable turn key with distinct bounded candidate indexes, and the runner's `alternate-choice` stage requests two Footer siblings from one selected base, inspects both, requires unchanged head/sibling ancestry, and chooses one through Studio; focused tests pass. Its first run reached selected Pricing state but the harness failed to recognize exact manifest Part ID `hero` under label `Product introduction`; target resolution now prioritizes stable ID authority. The next exact run timed out while primary Swarm corrected a trusted-overflow failure in the already-proven Pricing step; no invalid candidate or head move occurred. The permitted exact-source retry failed before creation because Desktop realtime did not open within 30 seconds. A later recovery attempt identified and cleared host disk exhaustion, restored the exact-session realtime prerequisite, and added a bounded resume stage rather than replaying the full journey. That resume path corrected an overflowing continuation candidate before selection and then produced real sibling Footer candidates from one exact selected base. However, the provider-backed gate remains RED: repeated resume runs accumulated historical turns, and the runner selected a wrong same-base two-candidate turn or the provider completed after `read_v3` without emitting both requested revisions. A split one-candidate-per-run attempt created two new direct siblings from the exact base, but the runner first matched an earlier same-base two-candidate turn and failed label verification. A before/after turn-set filter fixed that ambiguity; the next exact run then showed one of the two provider runs completed without attaching its requested candidate, so no new two-candidate turn existed. Source now closes that provider-completion gap with one bounded `revise_v3.alternatives` array: a single tool call carries 2–16 complete sibling HTML trees with distinct contiguous candidate indexes, runs the existing build/preview/commit boundary for every item in their shared exact-base turn, and returns success only after every requested candidate is ready; head selection remains separate. The runner now requests both Footer alternatives through that one cardinality-enforcing call. Focused tool and harness syntax checks pass. The first exact isolated rerun deployed the repaired commit but could not resume because clean redeployment removed the prior session; a fresh full-stage run then created a valid one-Artifact/three-Part/1440×900 root but stopped because the harness required the literal contiguous substring `$29` while the visible pricing card rendered the same requested Team/$29 choice with DOM whitespace. That run exposed a harness-only recovery defect: the generic testbench wrapper rejected the runner's checked-in `--initial-run-id`, `--artifact-id`, and `--desktop-path` resume inputs. The wrapper now passes those bounded arguments so same-session recovery can continue without duplicating the root. The next exact isolated run reached and durably created both requested ready Footer siblings in one native call from the same base, but its provider run terminalized only because it attempted an unrelated follow-up `plan_manage transition_checkpoint_boundary` and then submitted an overlong handoff. The runner now treats canonical Artifact V3 two-candidate state as the authoritative gate result even if later provider text/handoff work fails, while still surfacing a terminal run when no two-candidate turn exists. An exact resume must inspect/select these siblings before animation; animation and storyboard/video work remain unstarted until that passes.
