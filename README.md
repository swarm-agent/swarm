...
That command fetches the latest stable GitHub release asset, extracts it, and runs the bundled installer. You do not need to clone or download this repository to install Swarm. The installer checks the mandatory Git and Bash runtime prerequisites and installs only missing packages through the detected supported Linux package manager; existing installations are left unchanged. Git is required because managed artifacts and delegated workspace operations are core agent-runtime capabilities. Deterministic video analysis requires `ffmpeg` and `ffprobe` on `PATH`; those remain feature-local prerequisites rather than startup requirements.
