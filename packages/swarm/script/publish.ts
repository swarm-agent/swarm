...
import { Script } from "@swarm-ai/script"
...
if (!Script.preview) {
  ...
  const sourcePkgbuild = [
    ...
    `  bun install`,
    "  cd ./packages/swarm",
    `  SWARM_CHANNEL=latest SWARM_VERSION=${pkgver} bun run ./script/build.ts --single`,
    "}",
    "",
    "package() {",
    `  cd "opencode-\${pkgver}/packages/swarm"`,
