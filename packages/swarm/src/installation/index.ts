...
declare global {
  const SWARM_VERSION: string
  const SWARM_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".swarm", "bin"))) return "curl"
    ...
    for (const check of checks) {
      ...
      if (output.includes(check.name === "brew" ? "swarm" : "opencode-ai")) {
        return check.name
      }
    }
  ...
  async function getBrewFormula() {
    ...
    if (tapFormula.includes("swarm")) return "sst/tap/opencode"
    const coreFormula = await $`brew list --formula opencode`.throws(false).text()
    if (coreFormula.includes("swarm")) return "swarm"
    return "swarm"
  }
  ...
  export const VERSION = typeof SWARM_VERSION === "string" ? SWARM_VERSION : "local"
  export const CHANNEL = typeof SWARM_CHANNEL === "string" ? SWARM_CHANNEL : "local"
