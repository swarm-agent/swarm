...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.SWARM_CONFIG) {
      result = mergeDeep(result, await loadFile(Flag.SWARM_CONFIG))
    ...
    // Check for .swarm/ directory with config files
    const swarmDirConfigs = await Array.fromAsync(
      Filesystem.up({
        targets: [".swarm/swarm.json", ".swarm/swarm.jsonc"],
        start: Instance.directory,
        stop: Instance.worktree,
      }),
    )
    const hasSwarmConfig = swarmDirConfigs.length > 0
    
    if (hasSwarmConfig) {
      // Use .swarm/ config files
      for (const file of [".swarm/swarm.jsonc", ".swarm/swarm.json"]) {
      ...
      // Fall back to legacy .opencode/ config files, then root-level opencode.*
    ...
    if (Flag.SWARM_CONFIG_CONTENT) {
      result = mergeDeep(result, JSON.parse(Flag.SWARM_CONFIG_CONTENT))
      log.debug("loaded custom config from SWARM_CONFIG_CONTENT")
    }
    ...
    if (Flag.SWARM_CONFIG_DIR) {
      directories.push(Flag.SWARM_CONFIG_DIR)
      log.debug("loading config from SWARM_CONFIG_DIR", { path: Flag.SWARM_CONFIG_DIR })
    }
    ...
    if (Flag.SWARM_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.SWARM_PERMISSION))
    }
    ...
    const sdkSandboxEnv = process.env["SWARM_SANDBOX"]
    if (sdkSandboxEnv) {
      log.debug("applying SDK sandbox override from SWARM_SANDBOX env var")
  ...
  export const Keybinds = z
    .object({
      ...
      thinking_toggle: z.string().optional().default("shift+tab").describe("Toggle thinking mode"),
  ...
  export const TUI = z.object({
    ...
    diff_style: z
      .enum(["auto", "split", "stacked"])
      .optional()
      .default("auto")
      .describe("Diff display style: auto (based on width), split (side-by-side), or stacked (unified)"),
  })
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://swarm.ai/docs/commands"),
      watcher: z
        .object({
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://swarm.ai/docs/agent"),
      provider: z
        .record(
          z.string(),
      ...
      permission: z
        .object({
          ...
          doom_loop: Permission.optional(),
          external_directory: Permission.optional(),
          background_agent: Permission.optional().describe("Permission for spawning background agents (default: ask)"),
        })
        .optional(),
      backgroundAgent: z
        .object({
          prompt: z.string().optional().describe("Additional instructions prepended to background agent prompts"),
          permission: z
            .object({
              edit: Permission.optional(),
              bash: z.union([Permission, z.record(z.string(), Permission)]).optional(),
            ...
            .optional()
            .describe("Override permissions specifically for background agents"),
        })
        .optional()
  ...
  export const global = lazy(async () => {
    // Check legacy opencode config directory for backwards compatibility
    const legacyConfigDir = path.join(os.homedir(), ".config", "opencode")
    let result: Info = pipe(
      {},
      mergeDeep(await loadFile(path.join(legacyConfigDir, "opencode.json"))), // legacy config
      mergeDeep(await loadFile(path.join(Global.Path.config, "config.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "opencode.json"))), // backwards compat
      mergeDeep(await loadFile(path.join(Global.Path.config, "swarm.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "swarm.jsonc"))),
    )
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://swarm.ai/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://swarm.ai/config.json"
