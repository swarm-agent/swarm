...
import AGENT_SWITCH from "../session/prompt/agent-switch.txt"
...
export namespace SessionPrompt {
  ...
  export async function prompt(input: PromptInput): Promise<MessageV2.WithParts> {
    ...
    const thinkingOptions =
      input.thinkingBudget && model.providerID === "anthropic"
        ? { thinking: { type: "enabled", budgetTokens: input.thinkingBudget } }
        : {}
    ...
    while (true) {
      const msgs: MessageV2.WithParts[] = pipe(
        ...
        (messages) => insertReminders({ messages, agent, agentSwitch: input.agentSwitch }),
      )
      step++
      ...
      // Handle mid-loop agent switch (user pressed tab while agent was running)
      if (result.info.error && MessageV2.AbortedError.isInstance(result.info.error)) {
        const switchTo = SessionLock.consumeAgentSwitch(input.sessionID)
        if (switchTo) {
          l.info("agent switch mid-loop", { from: agent.name, to: switchTo })
          ...
          // Queue continuation with new agent
          state().track(
            prompt({
              sessionID: input.sessionID,
              agent: switchTo,
              parts: [
                {
                  type: "text",
                  text: `Continue from where the previous agent (${agent.name}) left off.`,
                },
              ],
              agentSwitch: true,
            }).then(() => {}),
      ...
      // Handle agent switch from tool metadata (e.g., exit-plan-mode)
      if (result.switchToAgent && result.switchToAgent !== agent.name) {
        // Agent switch requested - end current prompt and start new one with switched agent
        // This ensures the next LLM message uses the correct agent mode
        for (const item of queued) {
          item.callback(result)
        }
        state().queued.delete(input.sessionID)
        ...
        // Build the continuation message, including user comment if provided
        const userMessage = result.userMessage as string | undefined
        const baseText = "Proceed with implementation based on the approved plan."
        const continuationText = userMessage ? `${baseText}\n\n${userMessage}` : baseText

        // Queue continuation with new agent after current prompt completes
        state().track(
          prompt({
            sessionID: input.sessionID,
            agent: result.switchToAgent,
            parts: [
              {
                type: "text",
      ...
      // Handle graceful agent switch (user pressed Tab, waited for stream to finish)
      const gracefulSwitchTo = SessionLock.consumeGracefulSwitch(input.sessionID)
      if (gracefulSwitchTo && gracefulSwitchTo !== agent.name) {
        l.info("graceful agent switch", { from: agent.name, to: gracefulSwitchTo })
        for (const item of queued) {
          item.callback(result)
        }
        state().queued.delete(input.sessionID)
  ...
  function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; agentSwitch?: boolean }) {
    const userMessage = input.messages.findLast((msg) => msg.info.role === "user")
    if (!userMessage) return input.messages
    if (input.agentSwitch) {
  ...
  async function createProcessor(input: {
    ...
    const result = {
      ...
      async process(stream: StreamTextResult<Record<string, AITool>, never>, retries: { count: number; max: number }) {
            ...
            switch (value.type) {
              ...
              case "tool-result": {
                ...
                if (match && match.state.status === "running") {
                  ...
                  // Check if tool wants to switch agent
                  if (value.output.metadata?.switchToAgent) {
                    switchToAgent = value.output.metadata.switchToAgent as string
                  }
                  // Check for user message (approve-with-comment from exit-plan-mode)
                  if (value.output.metadata?.userMessage) {
                    userMessage = value.output.metadata.userMessage as string
                  }
              ...
              case "tool-error": {
                const match = toolcalls[value.toolCallId]
                if (match && match.state.status === "running") {
                  // If this is a plan rejection, update the plan status
                  if (value.error instanceof Permission.RejectedError && value.error.metadata?.planID) {
                    await SessionPlan.update(input.sessionID, {
                      status: "rejected",
                      rejectionMessage: value.error.customMessage,
                    }).catch(() => {
                  ...
                  await Session.updatePart({
                    ...
                    state: {
                      status: "error",
                      input: value.input,
                      error: value.error instanceof Error ? value.error.message : (value.error as any).toString(),
        ...
        await Session.updateMessage(assistantMsg)
        return { info: assistantMsg, parts: p, blocked, shouldRetry, switchToAgent, userMessage }
      },
    }
    return result
  }
