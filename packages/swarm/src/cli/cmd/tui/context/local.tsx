...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const thinking = iife(() => {
      const [store, setStore] = createStore({ enabled: false, manual: false })
      return {
        get enabled() {
          return store.enabled
        },
        toggle() {
          // When manually toggled, mark it as manual so auto-detection doesn't override
          setStore("manual", true)
        ...
        resetManual() {
          setStore("manual", false)
        },
      }
    })

    // Auto-enable thinking for Anthropic models when model changes
    createEffect(() => {
      const current = model.current()
      if (current) {
        const isAnthropic = current.providerID === "anthropic"
        thinking.setAuto(isAnthropic)
      }
    })
