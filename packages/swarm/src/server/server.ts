    ...
    app
      ...
      .post(
        ...
        async (c) => {
          ...
          if (!body.containerProfile && process.env.SWARM_PROFILE) {
            body.containerProfile = process.env.SWARM_PROFILE
          }
      ...
      .post(
        ...
        describeRoute({
          description: "Switch agent mid-loop",
          operationId: "session.switchAgent",
          responses: {
            200: {
              description: "Agent switch initiated",
              content: {
                "application/json": {
                  schema: resolver(z.boolean()),
                },
      ...
      .post(
        ...
        async (c) => {
          ...
          log.info("permission response received", { sessionID: id, permissionID, response })
          Permission.respond({
            sessionID: id,
            permissionID,
            response,
      ...
      .get(
        ...
        describeRoute({
          ...
          operationId: "backgroundAgent.list",
          responses: {
            200: {
              description: "List of background agents",
              content: {
                "application/json": {
                  schema: resolver(BackgroundAgent.Info.array()),
                },
              },
            },
          },
        ...
        async (c) => {
          return c.json(BackgroundAgent.list())
        },
      )
      .get(
        "/background-agent/:id",
        describeRoute({
          description: "Get a specific background agent",
          operationId: "backgroundAgent.get",
          responses: {
            200: {
              description: "Background agent info",
              content: {
                "application/json": {
                  schema: resolver(BackgroundAgent.Info),
                },
              },
            },
            ...errors(404),
        ...
        async (c) => {
          const id = c.req.valid("param").id
          const info = BackgroundAgent.get(id)
          if (!info) {
            return c.json({ error: "Background agent not found" }, { status: 404 })
          }
          return c.json(info)
        },
      )
      .delete(
        "/background-agent/:id",
        describeRoute({
          description: "Abort a running background agent",
          operationId: "backgroundAgent.abort",
          responses: {
            200: {
              description: "Background agent aborted",
              content: {
                "application/json": {
                  schema: resolver(z.boolean()),
                },
        ...
        async (c) => {
          const id = c.req.valid("param").id
          const result = BackgroundAgent.abort(id)
          return c.json(result)
        },
      )
      .get(
