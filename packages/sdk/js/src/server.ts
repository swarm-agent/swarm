...
export async function createOpencodeServer(options?: ServerOptions) {
  ...
  // Extract sandbox config to pass via SWARM_SANDBOX (applied LAST, cannot be overridden by app config)
  const { sandbox, ...restConfig } = options.config ?? {}
  const env: Record<string, string> = {
    ...process.env as Record<string, string>,
    SWARM_CONFIG_CONTENT: JSON.stringify(restConfig),
  }
  
  // SDK sandbox config is passed separately and applied LAST (replaces, not merges)
  if (sandbox !== undefined) {
    env.SWARM_SANDBOX = JSON.stringify(sandbox)
  }
