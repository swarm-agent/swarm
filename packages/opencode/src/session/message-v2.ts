  ...
  export function toModelMessage(
    ...
    for (const msg of input) {
      ...
      if (msg.info.role === "user") {
        const parts = msg.parts.flatMap((part): UIMessage["parts"] => {
        ...
        // Skip messages with empty parts after conversion
      ...
      if (msg.info.role === "assistant") {
        ...
        // Skip messages with empty parts or only non-content parts (step-start, reasoning)
        // The AI SDK requires actual content (text or tool results)
        const hasContent = parts.some(
          (p) => p.type === "text" || (typeof p.type === "string" && p.type.startsWith("tool-")),
        )
        if (parts.length === 0 || !hasContent) continue
        result.push({
          id: msg.info.id,
          role: "assistant",
          parts,
