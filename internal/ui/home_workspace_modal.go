...
func (p *HomePage) submitWorkspaceModalEditor() {
	...
	// Repository readiness belongs to the authenticated daemon. The TUI may
	// run as a different OS user (for example root attaching to a service),
	// so local Git checks can reject repositories the daemon safely owns.
