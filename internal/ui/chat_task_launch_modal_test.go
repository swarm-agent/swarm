...
func TestTaskLaunchPermissionModalUsesDesktopLikeLayout(t *testing.T) {
	...
	for _, want := range []string{"[^ 2 launches]", "[! read-only tools]", "[# Router: explorer]", "SUBAGENTS", "(1) explorer · read-only tools", "backend/core service architecture", "(2) clone", "desktop permissions UI", "FULL PROMPT", "Prompt · 11 words", "Map files and summarize findings.", "[Ctrl+P] Show full prompt", "┌─ Prompt"} {
